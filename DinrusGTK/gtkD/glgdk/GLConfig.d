...
public class GLConfig : ObjectG
{
    ...
    this(GLConfigMode mode, GLConfigMode fallback)
    {
        ...
        if ( gdkGLConfig is null )
        {
            ...
            version(Dinsus)
            {
                Стдвыв("*** Не удаётся найти дважды буферированный вижуал.").нс;
                Стдвыв("*** Попытка использовать единожды буферированный.").нс;
            }
        ...
        if ( gdkGLConfig is null )
        {
            ...
            version(Dinsus)
            {
                Стдвыв("*** Не найдено соответствующего OpenGL-вижуала.").нс;
            }
            throw new ConstructionException("Конфигурирование GL не удалось");
        }
        this(gdkGLConfig);
    }
