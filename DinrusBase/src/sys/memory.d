...
проц проверь (ук процедура)
					...
					{
					if(ПлохойУкНаКод(процедура)) ошибка("Указатель на функцию неверен.\n" ~текстСисОшибки(ДайПоследнююОшибку()));
					}
...
проц проверь (усим текст)
					...
					{
					if(ПлохойСтрУк(текст, текст.sizeof)) ошибка("Указатель на текстовую константу/переменную типа усим неверен.\n");
					}
...
проц проверь (ушим текст)
					...
					{
					if( ПлохойШСтрУк(текст, текст.sizeof))ошибка("Указатель на текстовую константу/переменную типа ушим неверен.\n");
					}
...
проц проверьЧитается(ук первБайтБлока)
					...
					{
					if(ПлохойЧтенУк(cast(ук) первБайтБлока, первБайтБлока.sizeof))ошибка("Чтение указанного блока памяти запрещено.\n");
					}
...
проц проверьЗаписывается(ук первБайтБлока)
					...
					{
					if(ПлохойЗапУк(первБайтБлока, первБайтБлока.sizeof))ошибка("Запись в указанный блок памяти запрещена.\n");
					}
