...
import  sys.WinFuncs, std.string;
...
static	бул задержка = нет;
static	бул б_ртПущен = нет;	
...
/*private*/ бул стройНезависимые()
			{
				...
				debug(НА_КОНСОЛЬ) скажинс(std.string.format("%d цикл конструктора независимых (не импортирующих) модулей", raz));
				raz++;
				}
				return да;
			}
...
{
	debug(НА_КОНСОЛЬ) скажи(std.string.format("деструкторМодулей(): %d модулей\n", индекс));
...
export extern (C) бул ртПущен(){ return б_ртПущен;}
export extern (C) бул ртОстановлен(){ return !б_ртПущен;}	
...
////////////////////////////////////////////////////////	
	...
	бул смИниц(){return смИнициирован;}
