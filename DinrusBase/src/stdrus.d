...
private  ук м_укМодуль;
	...
	this(ткст имяМодуля)
		...
		in
		{
			assert(пусто !is имяМодуля);
		}
		body
		{
		м_укМодуль = cast(ук) ЗагрузиБиблиотекуА(имяМодуля);
			if (пусто is м_укМодуль)
			throw new ИсклВнешнМодуля(ДайПоследнююОшибку());
		}
...
export extern(D)
{
	проц инфо(ткст сооб)
			{
			ОкноСооб(пусто, toUTF16(сооб), "Сообщение Динрус", ПСооб.Инфо|ПСооб.Поверх);
			}
...
+/
	...
	цел числобукв(дим б){return std.ctype.isalnum(б);}
	цел буква(дим б){return  std.ctype.isalpha(б);}
	цел управ(дим б){return std.ctype.iscntrl(б);}
	цел цифра(дим б){return std.ctype.isdigit(б);}
	цел проп(дим б){return std.ctype.islower(б);}
	цел пунктзнак(дим б){return  std.ctype.ispunct(б);}
	цел межбукв(дим б){return std.ctype.isspace(б);}
	цел заг(дим б){return std.ctype.isupper(б);}
	цел цифраикс(дим б){return std.ctype.isxdigit(б);}
	цел граф(дим б){return  std.ctype.isgraph(б);}
	цел печат(дим б) {return  std.ctype.isprint(б);}
	цел аски(дим б){return  std.ctype.isascii(б);}
...
{	/////////////////////////////////
	...
	ткст сквиз(ткст т, ткст образец= пусто){return  std.string.squeeze(cast(сим[]) т, cast(сим[]) образец);}
	ткст следщ(ткст т){return std.string.succ(т);}
	
	ткст тз(ткст ткт, ткст из, ткст в, ткст модифф = пусто){return std.string.tr(ткт, из, в, модифф);}
	бул чис(in ткст т, in бул раздВкл = false){return cast(бул) std.string.isNumeric(т, раздВкл);}
	т_мера колном(ткст ткт, цел размтаб=8){return std.string.column(ткт, размтаб);}
	ткст параграф(ткст т, цел колонки = 80, ткст первотступ = пусто, ткст отступ = пусто, цел размтаб = 8){return std.string.wrap(т, колонки, первотступ, отступ, размтаб);}
	ткст эладр(ткст т){return  std.string.isEmail(т);}
	ткст урл(ткст т){return  std.string.isURL(т);}
...
export extern(D)
{
	ткст вТкст(бул с){return std.string.вТкст(с);}
	ткст вТкст(сим с)
	{
	...
	ткст вТкст(ббайт с){return std.string.вТкст(с);}
	ткст вТкст(бкрат с){return std.string.вТкст(с);}
	ткст вТкст(бцел с){return std.string.вТкст(с);}
	ткст вТкст(бдол с){return std.string.вТкст(с);}
	ткст вТкст(байт с){return std.string.вТкст(с);}
	ткст вТкст(крат с){return std.string.вТкст(с);}
	ткст вТкст(цел с){return std.string.вТкст(с);}
	ткст вТкст(дол с){return std.string.вТкст(с);}
	ткст вТкст(плав с){return std.string.вТкст(с);}
	ткст вТкст(дво с){return std.string.вТкст(с);}
	ткст вТкст(реал с){return std.string.вТкст(с);}
	ткст вТкст(вплав с){return std.string.вТкст(с);}
	ткст вТкст(вдво с){return std.string.вТкст(с);}
	ткст вТкст(вреал с){return std.string.вТкст(с);}
	ткст вТкст(кплав с){return std.string.вТкст(с);}
	ткст вТкст(кдво с){return std.string.вТкст(с);}
	ткст вТкст(креал с){return std.string.вТкст(с);}
	ткст вТкст(дол знач, бцел корень){return std.string.вТкст(знач, корень);}
	ткст вТкст(бдол знач, бцел корень){return std.string.вТкст(знач, корень);}
	ткст вТкст(сим *с){return std.string.вТкст(с);}
...
проц форматДелай(проц delegate(дим) putc, ИнфОТипе[] arguments, спис_ва argptr)
	...
	switch (m)
	{
		...
		if (tis.xtoString is пусто)
		    throw new ФорматИскл("Не удаётся преобразовать " ~ tis.вТкст() ~ " в ткст: функция \"ткст вТкст()\" не определена");
...
import std.file;
	...
	бул файл(ткст имяф){return cast(бул) std.file.isfile(имяф);}
	бул папка(ткст имяп){return cast(бул) std.file.isdir(имяп);}
...
ткст вТкст(т_время время){return std.date.вТкст(время);}
...
//getExt(r"d:\путь\foo.bat") // "bat"     getExt(r"d:\путь.two\bar") // пусто
ткст дайИмяПути(ткст полнимя){return std.path.getName(полнимя);}
//getName(r"d:\путь\foo.bat") => "d:\путь\foo"     getName(r"d:\путь.two\bar") => пусто
...
бул абсПуть(ткст путь){return cast(бул) std.path.isabs(путь);}
 ...
 export   проц пиши(бкрат w)		/// ditto
  ...
  export  проц пиши(реал f)		/// ditto
   ...
   export проц пиши(БуферВывода буф)		/// ditto
    ...
    /****************************************
     * Добавка члобайт of 0 to the internal буф.
     */
  ...
  export  проц расклад2()
   ...
   export проц расклад4()
    ...
    /**************************************
     * Convert internal буф to array of симs.
     */
   ...
   export ткст вТкст()
    ...
    /*****************************************
     * Добавка output of C'т vprintf() to internal буф.
     */
	...
	for (;;)
		{
			...
			if (count != -1)
				break;
			psize *= 2;
			p = cast(сим *) cidrus.alloca(psize);	// буф too small, try again with larger размер
		}
	пиши(p[0 .. count]);
    }

    /*****************************************
     * Добавка output of C'т эхо() to internal буф.
     */
  ...
  export  проц выводф(ткст формат, ...)
    ...
    /*****************************************
     * At смещение index целo буф, создай члобайт of space by shifting upwards
...
бул нч(реал x){return cast(бул) std.math.isnan(x);}
бул конечен(реал р){return cast(бул) std.math.isfinite(р);}

бул субнорм(плав п){return cast(бул) std.math.issubnormal(п);}
бул субнорм(дво п){return cast(бул) std.math.issubnormal(п);}
бул субнорм(реал п){return cast(бул) std.math.issubnormal(п);}
бул беск(реал р){return cast(бул) std.math.isinf(р);}
бул идентичен(реал р, реал д){return std.math.isIdentical(р, д);}
...
ткст подставь(ткст текст, ткст образец, ткст формат, ткст атрибуты = пусто)
	{
	return std.regexp.sub(текст, образец, формат, атрибуты);
	}
}//end of extern C

export extern(D) ткст подставь(ткст текст, ткст образец, ткст delegate(РегВыр) дг, ткст атрибуты = пусто)
	{
...
export extern(D) РегВыр ищи(ткст текст, ткст образец, ткст атрибуты = пусто)
	{
	auto r = РегВыр(образец, атрибуты);
...
export extern (D)
{
	цел найди(рткст текст, ткст образец, ткст атрибуты = пусто)//Возврат -1=совпадений нет, иначе=индекс совпадения
		{
...
бул юпроп(дим с){return cast(бул) std.uni.isUniLower(с);}
бул юзаг(дим с){return cast(бул) std.uni.isUniUpper(с);}
дим в_юпроп(дим с){return std.uni.toUniLower(с);}
дим в_юзаг(дим с){return std.uni.toUniUpper(с);}
бул юцб(дим с){return cast(бул) std.uni.isUniAlpha(с);}
...
export extern (D) class РегВыр
{
   ...
   export ~this(){};
    ...
    export this(рсим[] образец, рсим[] атрибуты = пусто)
    {
	псовп = (&гсовп)[0 .. 1];
	компилируй(образец, атрибуты);
    }

    export static РегВыр opCall(рсим[] образец, рсим[] атрибуты = пусто)
    {
...
private цел слово(дим c) { return числобукв(c) || c == '_'; }
...
export цел проверь(ткст текст, цел стартиндекс)
{
    ...
    {
	firstc = программа[1];
	if (атрибуты & РВА.любрег && буква(firstc))
	    firstc = 0;
    }
...
{   цел srcsave;
	...
	switch (программа[pc])
	{
		...
		if (истк == ввод.length)
		    goto Lnomatch;
		// BUG: РВА.любрег?
		if (memchr(cast(сим*)&программа[pc + 1 + бцел.sizeof], ввод[истк], длин) == пусто)
		    goto Lnomatch;
		истк++;
		pc += 1 + бцел.sizeof + длин;
		break;
		...
		if (истк > 0 && истк < ввод.length)
		{
		    ...
		    c2 = ввод[истк];
		    if (!(
			  (слово(cast(рсим)c1) && !слово(cast(рсим)c2)) ||
			  (!слово(cast(рсим)c1) && слово(cast(рсим)c2))
			 )
		       )
			goto Lnomatch;
...
////////////////
{
	...
	//согласовать ситуацию с родителем
	проц устОткрыт(бул дн){
		super.открыт(дн);
		this.открытый = super.открыт();
	}

	проц выяснитьРежим(ПРежимФайла режим,
...
//////////////////////
  ...
  this(ук флУк, ПРежимФайла режим)  {
    ...
    сканируемый(ДайТипФайла(файлУк) == 1);	
  }
  ...
  проц открой(ткст имяф, ПРежимФайла режим = cast(ПФРежим) 1) { 
    ...
    читаемый(cast(бул)(режим & ПРежимФайла.Ввод));
	записываемый(cast(бул)(режим & ПРежимФайла.Вывод));	
	файлУк = СоздайФайл(вЮ16(имяф), доступ, шара, пусто, режСозд, ПФайл.Нормальный, пусто);   
    устОткрыт(файлУк != cast(ук) НЕВЕРНХЭНДЛ);
    if (!super.открыт()&&!this.открытый)
  ...
  ~this() { закрой(); }
     ...
     бдол размер() {
      проверьСканируемость(this.вТкст(),__FILE__,__LINE__);
      бцел sizehi;
      бцел sizelow = ДайРазмерФайла(файлУк,&sizehi);
      return (cast(бдол)sizehi << 32) + sizelow;
    }

  override т_мера читайБлок(ук буф, т_мера размер) {
	  auto разм = размер;
	      проверьЧитаемость();
          ЧитайФайл(файлУк, адаптВхоУкз(буф), разм, &разм, cast(АСИНХРОН*) пусто);
		  читатьдоКФ(размер == 0);
    return разм;
  }
   override т_мера пишиБлок(ук буф, т_мера размер) {
    проверьЗаписываемость(this.вТкст());   
      ПишиФайл( файлУк, адаптВхоУкз(буф), размер, &размер, пусто);   
    return размер;
  }
  ...
  override бул открыт(){return this.открытый;}
...
override  бул открыт(){
	return this.открытый;}
	
override  проц открыт(бул дн){
	this.открытый = дн;}
	 ...
	 бул закрытьИсток(){return закрытьГнездо;}
	  проц закрытьИсток(бул б){закрытьГнездо = б;}
	  	...
	  	/***
	   * Указывает на изменение состояния исходного потока
	   * и необходимость переустановки свойств
	   * читаемый, записываемый, сканируемый, открыт
	   * и флагов буферирования
	   * для данного потока.
	   */
	  проц сбросьИсток() {
		if (п !is пусто) {
	  ...
	  /***
	  * Строит ФильтрПоток для данного источника.
	  */
		this(Поток исток) {
		this.п = исток;
		закрытьИсток(да);
	  ...
	  ~this(){}
	  /***
	   * Возвращает текущий исходный поток.
	   */
	   Поток исток(){return this.п;}

	  /***
	   * Устанавливает текущий исходный поток.
	   *
	   * Установка исходного порока закрывает данный до прикрепления нового
	   * источника. Прикрепление открытого источника переоткрывает этот поток
	   * и переустанавливает состояние потока. 
	   */
	  проц исток(Поток п) {
		закрой();
		this.п = п;
	 ...
	 /***
	   * читать из истока
	   */
	  т_мера читайБлок(ук буф, т_мера размер) {
		т_мера рез = п.читайБлок(адаптВхоУкз(буф),размер);
		читатьдоКФ(рез == 0);
		return рез;
	  }
	/***
	 ...
	 */
	 override т_мера пишиБлок(ук буф, т_мера размер) {
		return п.пишиБлок(адаптВхоУкз(буф),размер);
	  }
...
override Поток исток(){return this.п;}	
	  проц устБуфер(ббайт[] буф){this.буф = буф;}
	  ббайт[] дайБуфер(){return this.буф;}
  ...
  invariant() {
    assert(длинаБуф <= буф.length,вЮ8(cast(ткст)"Несоблюдение первого требования инварианта класса БУфПоток"));
  ...
  /***
   * Create a buffered stream for the stream исток with the буф размер
   * bufferSize.
   */
  this(Поток исток, бцел размБуф = дефРазмБуфера) {
   ...
   assert(super.исток() == исток);
		  ...
		  устОткрыт(super.открыт());
 
   if (размБуф)
    буф = new ббайт[размБуф];	 
	черновойБуф = нет;	  
  }
  ...
  override т_мера читайБлок(ук результат, т_мера длин) {
    ...
    проверьЧитаемость(this.вТкст());

    ббайт* буфвыв = cast(ббайт*)адаптВхоУкз(результат);
    т_мера размЧтен = 0;

    if (текБуфПоз + длин < длинаБуф) {
      // буф has all the данные so copy it
      буфвыв[0 .. длин] = буф[текБуфПоз .. текБуфПоз+длин];
      текБуфПоз += длин;
      размЧтен = длин;
      goto ВыходИзЧтения;
    }

    размЧтен = длинаБуф - текБуфПоз;
    if (размЧтен > 0) {
      // буф has some данные so copy what is left
      буфвыв[0 .. размЧтен] = буф[текБуфПоз .. длинаБуф];
      буфвыв += размЧтен;
      текБуфПоз += размЧтен;
      длин -= размЧтен;
    }
    ...
    if (длин >= буф.length) {
      // буф can't hold the данные so fill output буф directly
      т_мера siz = super.читайБлок(буфвыв, длин);
      размЧтен += siz;
      позПотока += siz;
    } else {
      // читай a new block целo буф
      длинаБуф = super.читайБлок(буф.ptr, буф.length);
      if (длинаБуф < длин) длин = длинаБуф;
      буфвыв[0 .. длин] = буф[0 .. длин];
  ...
  // запись блока данных определённого размера
  // возвращает реальное число записанных байтов
  override т_мера пишиБлок(ук результат, т_мера длин) {
    проверьЗаписываемость(this.вТкст());

    ббайт* буфер = cast(ббайт*)адаптВхоУкз(результат);
    т_мера размЗап = 0;

    if (длинаБуф == 0) {
      // буф пуст, заполняем по возможности
      if ((длин < буф.length) && (читаемый())) {
	// читаем из данных, если  буф на данный момент пуст
	длинаБуф = п.читайБлок(буф.ptr, буф.length);
	позИстокаБуф = длинаБуф;
	позПотока += длинаБуф;
	  
      } else if (длин >= буф.length) {
	// буф не может держать данные, поэтому делаем прямую запись и выход
	размЗап = п.пишиБлок(буфер, длин);
	позПотока += размЗап;
	goto ВыходИзЗаписи;
      }
    }

    if (текБуфПоз + длин <= буф.length) {
      // буф has space for all the данные so copy it and exit
      буфер[текБуфПоз .. текБуфПоз+длин] = буфер[0 .. длин];
      текБуфПоз += длин;
    ...
    размЗап = буф.length - текБуфПоз;
    if (размЗап > 0) { 
      // буф can take some данные
      буф[текБуфПоз .. буф.length] = буфер[0 .. размЗап];
      текБуфПоз = длинаБуф = буф.length;
      буфер += размЗап;
      длин -= размЗап;
      черновойБуф = да;
    }

    assert(текБуфПоз == буф.length);
    assert(длинаБуф == буф.length);

    слей();

    размЗап += пишиБлок(буфер,длин);
  ...
  // reusing the memory in буф if результат will fit, otherwise
  ...
  template TreadLine(T) {
	...
	L1:
	  foreach(ббайт b; буф[старт .. длинаБуф]) {
	    текБуфПоз++;
	    pc[инд] = b;
	    if(инд < T.sizeof - 1) {
	      инд++;
	  ...
	  слей();
	  т_мера рез = super.читайБлок(буф.ptr, буф.length);
	  if(!рез) break L0; // EOF
  ...
  body {
	...
	// move actual файл poцелer to front of буф
	позПотока = п.сместись(-позИстокаБуф, ППозКурсора.Тек);
      }
      // пиши буф out
      позИстокаБуф = п.пишиБлок(буф.ptr, длинаБуф);
  ...
  override бул кф() {
    if ((буф.length == 0) || !читаемый()) {
      return super.кф();
    }
    // some simple tests to avoid flushing
    if (верниЧтоЕсть() || текБуфПоз != длинаБуф)
      return нет;
    if (длинаБуф == буф.length)
      слей();
    т_мера рез = super.читайБлок(&буф[длинаБуф],буф.length-длинаБуф);
    позИстокаБуф +=  рез;
    длинаБуф += рез;
    позПотока += рез;
    return читатьдоКФ;
...
export:
 ...
 override Поток исток(){
	return this.п;}

  /// открывает файл с заданным режимом и размером буфера
  ...
  /// открывает файл для чтения с заданным размером буфера
  this(Файл файл, бцел размБуф = дефРазмБуфера) {
    super(файл,размБуф);
	//this.п = super.п;
  }

  /// открывает существующий хэндл
...
private Эндиан эндиан; 
export:

         /// Endianness property of the исток stream.
  ...
  /***
   * Correct the байт order of буф to match native endianness.
   * размер must be even.
   */
   проц фиксируйПБ(ук буф, бцел размер) {
    if (эндиан != _эндиан) {
      ббайт* startb = cast(ббайт*)адаптВхоУкз(буф);
      бцел* старт = cast(бцел*)адаптВхоУкз(буф);
      switch (размер) {
      case 0: break;
      case 2: {
	ббайт x = *startb;
	...
	*старт = развербит(*старт);
	break;
      ...
      default: {
	бцел* end = cast(бцел*)(буф + размер - бцел.sizeof);
	while (старт < end) {
  ...
  /***
   * Correct the байт order of the given буф in blocks of the given размер and
   * repeated the given number of times.
   * размер must be even.
   */
   проц фиксируйБлокПБ(ук буф, бцел размер, т_мера повтор) {
    while (повтор--) {
      фиксируйПБ(адаптВхоУкз(буф),размер);
      буф += размер;
    }
  }
  ...
  шим бериш() {
    шим c;
    if (super.возвратКаретки()) {
      возвратКаретки(нет);
      c = бериш();
      if (c != '\n') 
	return c;
    }
	auto вткст = возврТекст();
    if ( вткст.length > 1) {
      c = вткст[вткст.length - 1];
      вткст.length = вткст.length - 1;
    } else {
      ук буф = &c;
...
export:
  ...
  this(ббайт[] буф = пусто) {super (буф);  }
  ...
  override т_мера пишиБлок(ук буф, т_мера размер) {
    резервируй(размер);
    return super.пишиБлок(адаптВхоУкз(буф),размер);
  }
 
 override т_мера читайБлок(ук буф, т_мера размер) {  return super.читайБлок(адаптВхоУкз(буф), размер); }
...
override  т_мера читайБлок(ук буф, т_мера размер) {  return super.читайБлок(адаптВхоУкз(буф), размер); }
 ...
 override т_мера пишиБлок(ук буф, т_мера размер) { return super.пишиБлок(адаптВхоУкз(буф), размер);  }
...
///////
  ...
  override т_мера читайБлок (ук буф, т_мера размер) {
    ...
    if (сканируемый)
      п.позиция = низ + поз;
    т_мера возвр = super.читайБлок(адаптВхоУкз(буф), размер);
    if (сканируемый) {
      поз = п.позиция - низ;
      п.позиция = bp;
    }
  ...
  override т_мера пишиБлок (ук буф, т_мера размер) {
    ...
    if (сканируемый)
      п.позиция = низ + поз;
    т_мера возвр = п.пишиБлок(адаптВхоУкз(буф), размер);
    if (сканируемый) {
      поз = п.позиция - низ;
      п.позиция = bp;
    }
...
alias длина length;
    ...
    this(ткст имяф)
    {
		this(имяф, Режим.Чтение, 0, пусто);
    }
			...
			т_мера окно = 0)
    {
		...
		version (Win32)
		{
			...
			hFileMap = СоздайМаппингФайлаА(hFile, пусто, flProtect, hi, cast(бцел)размер, пусто);
			if (hFileMap == пусто)               // mapping failed
				goto err1;
...
version (linux)			флаги |= MAP_ANONYMOUS;
	...
	~this()
	{
		...
		version (Win32)
		{
			if (hFileMap != пусто && ЗакройДескр(hFileMap) != да)
				errNo();
			hFileMap = пусто;
	...
	*/
	проц слей()
	{
		...
		version (Win32)
		{
			СлейВидФайла(данные.ptr, данные.length);
		}
		else version (Posix)
		{
			цел i;

			i = msync(cast(проц*)данные, данные.length, MS_SYNC);	// sys/mman.h
			if (i != 0)
				errNo();
		}
		else
	...
	ббайт opIndexAssign(ббайт значение, бдол i)
	{
		...
		ensureMapped(i);
		т_мера off = cast(т_мера)(i-старт);
		return (cast(ббайт[])данные)[off] = значение;
	}
	...
	private цел mapped(бдол i) 
	{
		debug (РПФайл) скажифнс("РПФайл.mapped(%lld, %lld, %d)\n", i,старт, 
				данные.length);
		return i >= старт && i < старт+данные.length;
	}
	...
	private проц unmap() 
	{
		...
		version(Windows) {
			...
			*/
			if (данные && ВидФайлаИзКарты(данные.ptr) == нет &&
				(винВерсия & 0x80000000) == 0)
				errNo();
		} else {
			if (данные && munmap(cast(проц*)данные, данные.length) != 0)
				errNo();
		}
		данные = пусто;
	}
...
///////////
  ...
  override проц закрой() { 
    if (открыт())
      закройфл(файлси); 
    открыт(нет); читаемый(нет); записываемый(нет); сканируемый(нет); 
  }
  ...
  override т_мера читайБлок(ук буф, т_мера размер) {
    т_мера n = читайфл(адаптВхоУкз(буф),1,размер,файлси);
    читатьдоКФ(cast(бул)(n == 0));
    return n;
  }

  override т_мера пишиБлок(ук буф, т_мера размер) {
    return пишифл(адаптВхоУкз(буф),1,размер,файлси);
  }
...
body
{
    ...
    return пусто; // to shut the compiler up
}

export extern(D) проц выгрузиБиб(Биб биб)
{
    if(биб !is пусто && биб._укз !is пусто)
...
{
    assert(биб !is пусто);
    assert(имяПроц !is пусто);
}
body
{
    if(биб._укз is пусто)
...
export:
   ...
   проц заряжай(ткст винБибы, проц function(Биб) пользовательскийЗагр, ткст текстВерсии = "") {
        assert (пользовательскийЗагр !is пусто);
        this.винБибы = винБибы;
        this.пользовательскийЗагр = пользовательскийЗагр;
        this.текстВерсии = текстВерсии;
    }

    проц загружай(ткст текстНазвБиб = пусто)
    {
        if (мояБиб !is пусто) return;        
        зарегестрированныеЗагрузчики ~= this;
        if (текстНазвБиб is пусто) текстНазвБиб = винБибы;

            if(текстНазвБиб is пусто || текстНазвБиб == "")
            {
    ...
    проц загружай(ткст[] назвыБиб)
    {
        ...
        if(пользовательскийЗагр is пусто)
        {
    ...
    бул загружено()
    {
        return (мояБиб !is пусто);
    }

    ткст имяБиб()
    {
        return загружено ? мояБиб.имя : пусто;
    }
...
export extern(D) бул создайБибИзДлл(ткст имяБ, ткст имяД = пусто, ткст путь = пусто, ткст расшД = "dll")
{
...
export: 
	...
	тип = cast(ППротокол)прото.прот;
		имя = std.string.вТкст(прото.имя).dup;
		...
		if(i)
		{
			...
			{
			    алиасы[i] =
				std.string.вТкст(прото.алиасы[i]).dup;
			}
		}
		else
		{
...
export:
	...
	{
		имя = std.string.вТкст(служба.имя).dup;
		порт = с8хбк(cast(бкрат)служба.порт);
		имяПротокола = std.string.вТкст(служба.прот).dup;
		...
		if(i)
		{
			...
			{
                            алиасы[i] =
                                std.string.вТкст(служба.алиасы[i]).dup;
			}
		}
		else
		{
	...
	{
	служзап* serv;
		serv =cast(служзап*) дайслужбупоимени(имя , пусто);
		if(!serv)
			return нет;
		заполни(serv);
		return да;
	...
	{
	служзап* serv;
		serv =cast(служзап*) дайслужбупопорту(порт, пусто);
		if(!serv)
			return нет;
		заполни(serv);
		return да;
...
export:
	...
	int i;
		...
		имя = std.string.вТкст(хз.имя).dup;
		...
		if(i)
		{
			...
			{
                            алиасы[i] =
                                std.string.вТкст(хз.алиасы[i]).dup;
			}
		}
		else
		{
...
export:
	...
	override ткст вТкст()
	{
		return std.string.вТкст("Неизвестно");
	}
}
...
Сокет прими()
	{
		т_сокет newsock;
		//newsock = cast(socket_t).accept(sock, пусто, пусто); // DMD 0.101 error: found '(' when expecting ';' following 'statement
		alias win.пусти topaccept;
		newsock = cast(т_сокет)topaccept(cast(СОКЕТ) сок, пусто, пусто);
...
static ткст имяХоста() // getter
	{
		...
		return std.string.вТкст(cast(char*)результат).dup;
	}
...
Адрес локальныйАдрес()
	...
	{
                флаги |= ПФлагиСокета.БезСигнала;
		цел sent = win.шли_на(cast(СОКЕТ) сок, буф.ptr, буф.length, флаги, пусто, нет);
		return sent;
	}
	...
	цел получи_от(проц[] буф, ПФлагиСокета флаги)
	{
		if(!буф.length) //return 0 and don't think the connection closed
			return 0;
		цел read = прими_от(cast(СОКЕТ) сок, буф.ptr, буф.length, флаги, пусто, пусто);
		// if(!read) //connection closed
		return read;
	}
...
static цел выбери(НаборСокетов checkRead, НаборСокетов checkWrite, НаборСокетов checkError, win.значврем* tv)
	...
	body
	{
		...
		version(Win32)
		{
			// Windows has a problem with empty набор_уд`т that aren't пусто.
...
export extern (D) class СокетПоток: Поток
{
	...
	{
	  ббайт* буф = cast(ббайт*)адаптВхоУкз(_буфер);
	  проверьЧитаемость();
	  
	  if (размер == 0)
	    return размер;
	  
	  auto len = сок.получи(буф[0 .. размер]);
...
export:
   ...
   проц старт()
	...
	synchronized (Нить.classinfo)
	{
	    ...
	    состояние = СН.ПУЩЕНА;
	    hdl = начниНитьДоп(пусто, cast(бцел) размстека, &стартнити, cast(ук) this, 0, &id);
	    if (hdl == cast(ук) 0)
	    {
		allThreads[инд] = пусто;
		члонн--;
