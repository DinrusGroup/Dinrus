...
бул число(ИнфОТипе[] _arguments, спис_ва _argptr)
{
    ткст  s  = "";
    шткст ws = "";
    юткст ds = "";
    ...
    if (_arguments[0] == typeid(ткст))
        return чис(ва_арг!(ткст)(_argptr));
    else if (_arguments[0] == typeid(шткст))
        return чис(вЮ8(ва_арг!(шткст)(_argptr)));
    else if (_arguments[0] == typeid(юткст))
        return чис(вЮ8(ва_арг!(юткст)(_argptr)));
...
бул число(...)
{
    return cast(бул) число(_arguments, _argptr);
}
...
цел нормален(X)(X x)
{
    ...
    static if(real.mant_dig==106)   // doubledouble
    {
        ...
        return нормален((cast(дво*)&x)[МАНТИССА_МЗЧ]);
    }
    else
    {
...
extern (D) class РПФайл
{
    ...
    enum Режим
    {
        ...
        ЧтенЗапНов,	/// удалить существующий файл, записать новый
        ЧтенЗап,	/// чтен/зап в сущ. файл, создать, если не существует
        ЧтенКопирПриЗап, /// чтен/зап в сущ. файл, копирование по записи
...
/+ ПЕРЕЧЕНЬ ОБЪЯВЛЕН В МОДУЛЕ BASE. 
	...
	ЗДЕСЬ МОЖНО ИСПОЛЬЗОВАТЬ НЕПОСРЕДСТВЕННО +/
...
extern(D)
{
    ...
    бул юпроп(дим с); //alias _юпроп юпроп;
    бул юзаг(дим с); //alias _юзаг юзаг;
    дим в_юпроп(дим с); //alias _в_юпроп в_юпроп;
    дим в_юзаг(дим с);  //alias _в_юзаг в_юзаг;
    бул юцб(дим с);
    alias юцб юцифрабукв;
    ...
    цел числобукв(дим б); //alias _числобукв числобукв;
    цел буква(дим б); //alias _буква буква;
    цел управ(дим б); //alias _управ управ;
    цел цифра(дим б); //alias _цифра цифра;
    цел проп(дим б); //alias _проп проп;
    цел пунктзнак(дим б); //alias _пунктзнак пунктзнак;
    цел межбукв(дим б); //alias _межбукв межбукв;
    цел заг(дим б); //alias _заг заг;
    цел цифраикс(дим б); //alias _цифраикс цифраикс;
    цел граф(дим б); //alias _граф граф;
    цел печат(дим б) ; //alias _печат печат;
    цел аски(дим б); //alias _аски аски;
    дим впроп(дим б); //alias _впроп впроп;
    дим взаг(дим б); //alias _взаг взаг;
    цел руспроп(дим б); //alias _руспроп руспроп;
    цел русзаг(дим б); //alias _русзаг русзаг;

    бул пробел(дим s); //alias _пробел пробел;
    ...
    бул чис(in ткст т, in бул раздВкл = нет); //alias _чис чис;
    ...
    ткст эладр(ткст т); //alias _эладр эладр;
    ткст урл(ткст т); //alias _урл урл;
    ...
    бул файл(ткст имяф); //alias _файл файл;
    бул папка(ткст имяп); //alias _папка папка;
    ...
    бул абсПуть(ткст путь); //alias _абсПуть абсПуть;
    ...
    бул нч(реал х);  //alias _нч нч;
    бул конечен(реал р);  //alias _конечен конечен;
    бул субнорм(плав п);  //alias _субнорм субнорм;
    бул субнорм(дво п);
    бул субнорм(реал п);
    бул беск(реал р);  //alias _беск беск;
    бул идентичен(реал р, реал д);  //alias _идентичен идентичен;
