...
template статМас(T : U[N], U, т_мера N)
{
    const статМас = да;
}
...
template динМас(T, U = void)
{
    const динМас = нет;
}

template динМас(T : U[], U)
{
    const динМас = !статМас!(T);
}

template массив(T)
{
    const массив = статМас!(T) || динМас!(T);
}

template указатель(T)
{
    const указатель = is(T : ук);
}
...
template ТипВариант(T)
{
    ...
    else static if (массив!(T))
        const ТипВариант = ТипВариант!(typeof(*T)) | ПТипВарианта.Массив;
    else static if (указатель!(T)/* && !is(T == ук)*/)
...
template com_cast_impl(T, ППолитикаИсключений политика)
{
    ...
    T com_cast_impl(U)(U объ)
    {
        static if (is(U : Инкогнито))
        {
            ...
            else static if (is(typeof(объ.нулл)))
            {
                // com_ref
                if (объ.нулл)
                {
