...
/*export*/ abstract class ОсноваКнопки: СуперКлассУпрЭлта // docmain
{
/*export*/
	 ...
	 override проц предшОкПроц(inout Сообщение сооб)
	{
		...
		сооб.результат = вызовиОкПроц(первОкПроцКнопки, сооб.уок, сооб.сооб, сооб.парам1, сооб.парам2);
	}
...
/*export*/ class Кнопка: ОсноваКнопки, ИУпрЭлтКнопка // docmain
{
/*export*/
	this()
	{
	...
	проц выполниКлик()
	{
	 ...
	 override проц приКлике(АргиСоб ea)
	{
		super.приКлике(ea);
	...
	{
		if(txt.length)
			assert(!this.рисунок, "Кнопка с текстом-рисунком не поддерживается");
	}
	body
	{
		super.текст = txt;
	...
	{
		if(img)
			assert(!this.текст.length, "Кнопка с текстом-рисунком не поддерживается");
	}
	body
	{
		/+
		...
		if(img)
		{
			...
			switch(img._imgtype(пусто))
			{
				...
				default:
					throw new ВизИскл("Неподдерживаемый формат рисунка");
					not_unsupported: ;
			}
		}
	 ...
	 override проц поСозданиюУказателя(АргиСоб ea)
	{
	 ...
	 override проц поУдалениюУказателя(АргиСоб ea)
	{
...
/*export*/ class Флажок: ОсноваКнопки // docmain
{
/*export*/
...
/*export*/ class РадиоКнопка: ОсноваКнопки // docmain
{
/*export*/
		final проц наружность(ПНаружность ap) // setter
	{
	...
	/+ package +/ /+  +/ override цел _rtype() // package
	{
		if(автоУстанов)
