...
private import viz.x.utf, viz.x.winapi, viz.base, viz.drawing;
...
else
{
		class Ресурсы // docmain
	{
		...
		this(Ткст libName, WORD язык = 0)
		{
			...
			if(!экземп)
				throw new ВизИскл("Unable to загрузка ресурсы from '" ~ libName ~ "'");
		...
		final Пиктограмма дайПиктограмму(Ткст имя, бул дефРазм = да)
		{
			...
			+/
			HICON hi;
			hi = cast(HICON)viz.x.utf.загрузиРисунок(hinst, имя, IMAGE_ICON,
				0, 0, дефРазм ? (LR_DEFAULTSIZE | LR_SHARED) : 0);
			if(!hi)
				return пусто;
			return new Пиктограмма(hi, да); // Owned.
		...
		{
			КУРСОР h;
			h = cast(КУРСОР)LoadImageA(hinst, cast(LPCSTR)cast(WORD)id, IMAGE_CURSOR,
				0, 0, 0);
			if(!h)
				return пусто;
		...
		{
			КУРСОР h;
			h = cast(КУРСОР)загрузиРисунок(hinst, имя, IMAGE_CURSOR,
				0, 0, 0);
			if(!h)
				return пусто;
			return new Курсор(h, да); // Owned.
		...
		{
			throw new ВизИскл("Unable to загрузка " ~ тип ~ " resource");
		}
	}
}
