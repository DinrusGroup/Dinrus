...
import viz.x.winapi, viz.x.wincom;


private import viz.base, viz.form, viz.event;
private import viz.control, viz.drawing, viz.label;
private import viz.кнопка, viz.textbox, viz.x.wincom, viz.environment;
private import viz.x.utf;
...
debug(APP_PRINT)
{
	pragma(сооб, "viz: debug app print");
...
export extern(D) class КонтекстПриложения // docmain
{
export:
		this()
	{
...
export extern (D) class Приложение // docmain
{
			...
			`<assembly xmlns="urn:schemas-microsoft-com:asm.v1" manifestVersion="1.0">` \r\n
				...
				`<dependency>` \r\n
					`<dependentAssembly>` \r\n
						`<assemblyIdentity `
							`тип="win32" `
							`имя="Microsoft.Windows.Common-Controls" `
							`version="6.0.0.0" `
	...
	{
		return viz.x.utf.дайИмяФайлаМодуля(HMODULE.init);
	}
	...
	{
		return _извлекиПапку(viz.x.utf.дайИмяФайлаМодуля(HMODULE.init));
	}
	...
	package бул _doEvents(бул* keep)
	{
		...
		try
		{
			...
			while(viz.x.utf.peekMessage(&сооб._винСооб, HWND.init, 0, 0, PM_REMOVE))
			{
	...
	export:
		...
		protected override проц приЗагрузке(АргиСоб ea)
		{
			okBtn.фокус();
		}
		
		
		protected override проц приЗакрытии(АргиСобОтмены cea)
		{
			cea.отмена = !errdone;
		}
		...
		private цел inThread2()
		{
			try
			{
				...
				//while(viz.x.utf.getMessage(&сооб, HWND.init, 0, 0))
				{
					if(!IsDialogMessageA(указатель, &сооб))
					//if(!viz.x.utf.isDialogMessage(указатель, &сооб))
					{
						TranslateMessage(&сооб);
						DispatchMessageA(&сооб);
						//viz.x.utf.dispatchMessage(&сооб);
					}
	...
	проц полученоСообщение(inout Сообщение сооб)
	{
		...
		TranslateMessage(&сооб._винСооб);
		//DispatchMessageA(&сооб._винСооб);
		viz.x.utf.dispatchMessage(&сооб._винСооб);
	}
}
	...
	проц покажиИнфоСообщение(inout Сообщение m)
	{
		...
		switch(m.сооб)
		{
			...
			default:
				...
				{
					пишиСо("WM_USER+" ~ std.string.вТкст(m.сооб - WM_USER));
				}
		...
		writef("УОК=%d(0x%X) %s WPARAM=%d(0x%X) LPARAM=%d(0x%X)\n\n",
			cast(т_мера)m.уок, cast(т_мера)m.уок,
			упрэлм ? ("DFLname='" ~ упрэлм.имя ~ "'") : "<nonDFL>",
			m.парам1, m.парам1,
			m.парам2, m.парам2);
...
{
	//эхо("УОК %p; WM %d(0x%X); WPARAM %d(0x%X); LPARAM %d(0x%X);\n", уок, сооб, сооб, wparam, wparam, lparam, lparam);
...
проц _initCommonControls(DWORD dwControls)
{
	...
	{
		pragma(сооб, "viz: extended common упрэлты supported at compile time");
...
static this()
{
	...
	wmViz = RegisterWindowMessageA("WM_DFL");
	if(!wmViz)
		wmViz = WM_USER + 0x7CD;
	...
	{
		wmGetControlName = RegisterWindowMessageA("WM_GETКОНТРОЛNAME");
	}
	...
	{
		pragma(сооб, "viz: непрозрачность supported at compile time");
...
body
{
	...
	if(!viz.x.utf.зарегистрируйКласс(ко))
...
export extern(Windows)
{
	...
	проц _initButton()
	{
		...
		{
			viz.x.utf.КлассОкна инфо;
			первОкПроцКнопки = суперКласс(экз.init, "КНОПКА", BUTTON_CLASSNAME, инфо);
			if(!первОкПроцКнопки)
...
// Returns the old окПроц.
WNDPROC суперКласс(экз hinst, Ткст имяКласса, Ткст newClassName, out viz.x.utf.КлассОкна getInfo) // package
{
	WNDPROC окПроц;
	
	if(!viz.x.utf.дайИнфОКлассе(hinst, имяКласса, getInfo))
		throw new ВизИскл("Не удалось получить информацию об оконном классе '" ~ имяКласса ~ "'");
