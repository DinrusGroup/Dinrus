...
extern(Windows)
{
	...
	enum: UINT
	{
		...
		CBEM_GETCOMBOКОНТРОЛ 	= WM_USER + 6,
		CBEM_GETEDITКОНТРОЛ		= WM_USER + 7,
	...
	extern(C) COLORREF RGB(int к, int з, int с)
	{
		return cast(COLORREF)(cast(ббайт)к | (cast(ббайт)з << 8) | (cast(ббайт)с << 16));
	}
	...
	alias int function(HWND уок, UINT uMsg, LPARAM парам2, LPARAM lpData) BFFCALLBACK;
	...
	BOOL SHGetPathFromIDListA(ITEMIDLIST* pidl, ткст0 pszPath);
	...
	/* *** User32.dll *** */	
	HDWP DeferWindowPos(HDWP hWinPosInfo, HWND уок, HWND hWndInsertAfter, int ш, int в, int cx, int cy, UINT uFlags);
	...
	BOOL SetWindowPos(HWND уок, HWND hWndInsertAfter, int X, int Y, int cx, int cy, UINT uFlags);
	BOOL GetTextExtentPoint32A(HDC hdc, ткст0 lpString, int ктрл, РАЗМЕР* lpSize);
	...
	UINT SetTimer(HWND уок, UINT nIDEvent, UINT uElapse, TIMERPROC lpТаймерFunc);
	...
	int SetScrollInfo(HWND уок, int fnBar, SCROLLINFO* lpsi, BOOL fRedraw);
	BOOL EnableMenuItem(HMENU hMenu, UINT uIDEnableItem, UINT uEnable);
	int GetWindowTextA(HWND уок, LPSTR lpString, int nMaxCount);
	BOOL GetScrollInfo(HWND уок, int fnBar, SCROLLINFO* lpsi);
	LONG SetWindowLongA(HWND уок, int nIndex, LONG dwNewLong);
	DWORD SetClassLongA(HWND уок, int nIndex, LONG lNewLong);
	...
	/* *** Comctl32.dll *** */
	BOOL ImageList_DrawEx(HIMAGELIST himl, int i, HDC hdcDst, int ш, int в, int dx, int dy, COLORREF rgbBk, COLORREF rgbFg, UINT fStyle);
	BOOL ImageList_Draw(HIMAGELIST himl, int i, HDC hdcDst, int ш, int в, UINT fStyle);
	...
	BOOL ExtTextOutA(HDC hdc, int ш, int в, UINT fuOptions, RECT* lprc, LPCTSTR lpString, uint cbCount, int* lpDx);
	...
	/* *** Ole32.dll *** */
	HRESULT CLSIDFromString(шткст0 lpsz, CLSID* pclsid);
}

/* *** Win32 Macros *** */

BYTE GetRValue(COLORREF ктрл) 
{
	return cast(BYTE)ктрл;
}

BYTE GetGValue(COLORREF ктрл) 
{
	return cast(BYTE)(ктрл >> 8);
}

BYTE GetBValue(COLORREF ктрл) 
{
	return cast(BYTE)(ктрл >> 16);
}

short GetWheelDelta(WPARAM парам1)
{
	return cast(short)HIWORD(парам1);
}