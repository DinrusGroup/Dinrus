...
private import viz.control, viz.base, viz.x.winapi, viz.application;
private import viz.drawing, viz.event, viz.x.utf;
...
abstract class ОсноваТекстБокса: СуперКлассУпрЭлта // docmain
{
		...
		проц выделенныйТекст(Ткст sel) // setter
	{
		...
		if(создан)
		{
			//viz.x.utf.шлиСооб(указатель, EM_REPLACESEL, FALSE, sel);
			viz.x.utf.шлиСообНебезоп(указатель, EM_REPLACESEL, FALSE, sel);
		}
	}
	...
	Ткст выделенныйТекст() // getter
	{
		...
		if(создан)
			return viz.x.utf.дайВыделенныйТекст(указатель);
		return пусто;
	}
	...
	override проц предшОкПроц(inout Сообщение сооб)
	{
		...
		сооб.результат = viz.x.utf.вызовиОкПроц(первОкПроцТексБокса, сооб.уок, сооб.сооб, сооб.парам1, сооб.парам2);
	}
	
	
	protected override бул обработайАргиСобКлавиш(inout Сообщение сооб) // package
	{
		switch(сооб.сооб)
		{
			case WM_KEYDOWN:
...
class ТекстБокс: ОсноваТекстБокса // docmain
{
	...
	final проц passwordChar(дим pwc) // setter
	{
		if(pwc)
		{
			...
			if(создан)
				...
				viz.x.utf.emSetPasswordChar(указатель, pwc);
			else
		...
		else
		{
			...
			if(создан)
				...
				viz.x.utf.emSetPasswordChar(указатель, 0);
			else
				_style(_style() & ~ES_PASSWORD);
		}
	...
	final дим passwordChar() // getter
	{
		if(создан)
			...
			passchar = viz.x.utf.emGetPasswordChar(указатель);
		return passchar;
	}
