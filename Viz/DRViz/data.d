...
private import viz.base, viz.x.winapi, viz.x.wincom, viz.app,
	viz.x.utf, viz.x.com;


class ФорматыДанных // docmain
{
	...
	Ткст rtf() // getter
	{
		return дайФормат("Rich Text Формат").имя;
	}
	...
	Формат дайФормат(Ткст имя)
	{
		...
		// Didn't найди it.
		return _didntFindId(viz.x.utf.регистрируйФорматБуфОбмена(имя));
	}
	...
	{
		Ткст результат;
		результат = viz.x.utf.дайИмяФорматаБуфОбмена(id);
		if(!результат.length)
			throw new ВизИскл("Unable to get format");
		return результат;
	}
	...
	проц[] getCbFileDrop(Ткст[] фимена)
	{
		...
		foreach(fn; фимена)
		{
			разм += (viz.x.utf.вДлинуЮникода(fn) + 1) << 1;
		}
		разм += 2;
...
struct Данные // docmain
{
	...
	deprecated alias дайЮ8 getUTF8;
...
class ОбъектДанных: viz.data.ИОбъектДанных // docmain
{
		Данные получитьДанные(Ткст фмт)
	{
	...
	Данные получитьДанные(Ткст фмт, бул doConvert)
	{
		...
		//эхо("Looking for format '%.*s'.\n", фмт);
		цел i;
		i = найди(фмт);
		if(i == -1)
			throw new ВизИскл("Данные format not present");
		return все[i].объ;
	}
...
package Данные _doConvertFormat(Данные dat, Ткст toFmt)
{
	...
	if(!сравнлюб(toFmt, "UTF-8"))
	{
		...
		else if(typeid(ббайт[]) == dat.инфо)
		{
			ббайт[] ubs;
			ubs = dat.дайТекст();
			результат = Данные(viz.x.utf.изАнзи(cast(Ткст0)ubs.ptr, ubs.length));
		}
	}
	else if(!сравнлюб(toFmt, ФорматыДанных.текстЮникод))
	{
		...
		{
			результат = Данные(вЮ16(dat.дайЮ8()));
		}
		else if(typeid(ббайт[]) == dat.инфо)
		{
			ббайт[] ubs;
			ubs = dat.дайТекст();
			результат = Данные(viz.x.utf.анзиВЮникод(cast(Ткст0)ubs.ptr, ubs.length));
		}
	}
	else if(!сравнлюб(toFmt, ФорматыДанных.текст))
	{
		if(typeid(Ткст) == dat.инфо)
		{
			результат = Данные(cast(ббайт[])viz.x.utf.вАнзи(dat.дайЮ8()));
		}
...
class КомВОбъектДанных: viz.data.ИОбъектДанных // package
{
	this(viz.x.wincom.ИОбъектДанных объДанных)
	{
	...
	private Данные _getData(цел id)
	{
		...
		if(S_OK != объДанных.GetData(&fmte, &stgm))
			throw new ВизИскл("Unable to get данные");
		...
		{
			release();
			throw new ВизИскл("Error obtaining данные");
		}
	...
	Ткст[] дайФорматы()
	{
		...
		if(S_OK != объДанных.EnumFormatEtc(1, &fenum))
			throw new ВизИскл("Unable to get formats");
	...
	private проц _setData(цел id, Данные объ)
	{
		...
		if(S_OK != hr)
		{
			//эхо("Unable to ИОбъектДанных::SetData() = %d (0x%X).\n", hr, hr);
	...
	final бул такойЖеОбъектДанных_ли(viz.x.wincom.ИОбъектДанных объДанных)
	{
	...
	private:
	viz.x.wincom.ИОбъектДанных объДанных;
}


package class EnumDataObjectFORMATETC: ВизКомОбъект, winapi.IEnumFORMATETC
{
	this(viz.data.ИОбъектДанных объДанных, Ткст[] fmts, ULONG старт)
	{
		this.объДанных = объДанных;
		this.fmts = fmts;
		idx = старт;
	...
	this(viz.data.ИОбъектДанных объДанных)
	{
...
class DtoComDataObject: ВизКомОбъект, viz.x.wincom.ИОбъектДанных // package
{
	this(viz.data.ИОбъектДанных объДанных)
	{
		this.объДанных = объДанных;
	...
	override HRESULT QueryInterface(IID* riid, проц** ppv)
	{
		...
		{
			*ppv = cast(проц*)cast(viz.x.wincom.ИОбъектДанных)this;
			AddRef();
			return S_OK;
		}
		else if(*riid == _IID_IUnknown)
