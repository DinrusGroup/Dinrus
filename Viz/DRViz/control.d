...
private import viz.x.dlib, stdrus, cidrus: malloc;
	
private import viz.base, viz.form, viz.drawing;
private import viz.x.winapi, viz.app, viz.event, viz.label;
private import viz.x.wincom, viz.x.utf, viz.collections, viz.x.com;
...
/// Flags for setting упрэлт границы.
...
export extern(Windows) BOOL обрвызовЗэтИндексГет(УОК уок, LPARAM lparam)
	{
...
{
	ПРОДОЛЖЕНИЕ = S_OK,
 	ОТМЕНА = DRAGDROP_S_CANCEL, 
	БРОС = DRAGDROP_S_DROP, 
}


// Flags.
deprecated enum UICues: бцел
{
	НЕУК = 0,
	SHOW_FOCUS = 1,
	SHOW_KEYBOARD = 2,
	...
	CHANGE_FOCUS = 4,
	CHANGE_KEYBOARD = 8, // Key мнемоника underline cues are on.
...
deprecated class UICuesEventArgs: АргиСоб
{
	deprecated:
	...
	final UICues изменено() // getter
	{
		return chg;
	}
	
	
	final бул changeFocus()
	{
		return (chg & UICues.CHANGE_FOCUS) != 0;
	}
	
	
	final бул changeKeyboard()
	{
...
export extern(D) class АргиСобУпрЭлта: АргиСоб
{
export:
		this(УпрЭлт упрэлм)
	{
...
export extern(D)  class АргиСобСправка: АргиСоб
{
export:
		this(Точка позМыши)
	{
...
export extern(D)  class АргиСобИнвалидировать: АргиСоб
{
export:
...
deprecated class BeforeResizeEventArgs: АргиСоб
{
	deprecated:
...
export extern(D) class АргиСобРасположение: АргиСоб
{
export:
...
{
export extern(D) class АргиСобДрэг: АргиСоб
	{
	export:
			this(viz.data.ИОбъектДанных объДанных, цел состКл, цел ш, цел в,	ПЭффектыДД разрешённыйЭффект, ПЭффектыДД эффект)
		{
			_dobj = объДанных;
			_состКл = состКл;
			_x = ш;
		...
		final ПЭффектыДД эффект() // getter
		{
			return _эффект;
				...
				final viz.data.ИОбъектДанных данные() // getter
		{
			return _dobj;
		}
		
		
				// State of упрэлм, альт, шифт, and mouse buttons.
...
export extern(D) class АргиСобФидбэк: АргиСоб
	{
	export:
...
export extern (D) class АргиСобДрэгОпросПродолжить: АргиСоб
	{
	export:
...
package alias BOOL delegate(УОК) EnumWindowsCallback;


// Callback for EnumWindows() and EnumChildWindows().
export extern(Windows) BOOL перечисляемОкна(УОК уок, LPARAM lparam)
{
...
export extern(Windows) BOOL перечисляемПервыеОкна(УОК уок, LPARAM lparam)
{
...
package BOOL перечислиОкнаОтпрыски(УОК hwParent, EnumWindowsCallback дг)
{
	static assert((&дг).sizeof <= LPARAM.sizeof);
...
package BOOL перечислиПервыеОтпрыскиОкна(УОК hwParent, EnumWindowsCallback дг)
{
	Efi efi;
...
{
export:
		static class КоллекцияУпрЭлтов
	{
		protected this(УпрЭлт хозяин)
		{
			_владелец = хозяин;
		}
		...
		private проц _removeCreated(УОК уок)
		{
			DestroyWindow(уок); // ?
		}
		...
		protected final УпрЭлт хозяин() // getter
		{
			return _владелец;
		}
	...
	private проц _ctrladded(АргиСобУпрЭлта cea)
	{
		...
		{
			assert(дайСтиль(ПСтилиУпрЭлта.КОНТЕЙНЕР), "УпрЭлт добавлен в родитель-неконтейнер");
		}
	...
	final УОкно указатель() // ИОкно getter
	{
		if(!созданУказатель_ли)
		{
			debug(APP_PRINT)
				скажиф("УпрЭлт создан due to указатель request.\n");
				...
				проц разрешиБрос(бул подтвержд) // setter
		{
			...
			if(подтвержд)
			{
				if(!цельброса)
				{
					...
					if(созданУказатель_ли)
					{
						switch(RegisterDragDrop(уок, цельброса))
						{
							case S_OK:
		...
		final бул фокусируемый() // getter
	{
		...
		// Don't need to check -созданУказатель_ли- because IsWindowVisible() will fail from а пусто УОК.
		return /+ созданУказатель_ли && +/ IsWindowVisible(уок) && IsWindowEnabled(уок);
	}
	...
	body
	{
		...
		return /+ созданУказатель_ли && +/ (ктрлСтиль & ПСтилиУпрЭлта.ВЫДЕЛЕНИЕ) &&
			IsWindowVisible(уок) && IsWindowEnabled(уок);
	}
	...
	// When да, validating and validated события are fired when the упрэлт
	// receives фокус. Typically установи to нет for упрэлты such as а Help кнопка.
	// Default is да.
	deprecated final бул вызываетВалидацию() // getter
	{
		//return cvalidation;
		return нет;
	}
	...
	deprecated protected проц приИзмененииВызоваВалидации(АргиСоб ea)
	{
		//causesValidationChanged(this, ea);
	}
	
	
	deprecated final проц вызываетВалидацию(бул vyes) // setter
	{
		/+
		if(cvalidation == vyes)
			return;
		...
		приИзмененииВызоваВалидации(АргиСоб.пуст);
		+/
	}
	...
	protected проц установиЯдроКлиентскогоРазмера(цел ширина, цел высота)
	{
		...
		AdjustWindowRectEx(&к, _style(), FALSE, _exStyle());
		
		установиЯдроГраниц(0, 0, к.right - к.лево, к.bottom - к.top, ПЗаданныеПределы.РАЗМЕР);
	}
	...
	final бул содержитФокус() // getter
	{
		...
		return hwfocus == уок || IsChild(уок, hwfocus);
	}
		...
		protected проц приИзмененииКурсора(АргиСоб ea)
	{
		...
		if(созданУказатель_ли)
		{
			if(виден && включен)
			{
				...
				if(уок == WindowFromPoint(curpt.точка))
				{
	...
	private static Шрифт _createCompatibleFont()
	{
		...
		try
		{
			...
			if(GetVersionExA(&osi) && osi.dwMajorVersion >= 5)
			{
				...
				результат = new Шрифт("MS Shell Dlg 2", результат.дайРазмер(ЕдиницаГрафики.POINT), ЕдиницаГрафики.POINT);
			}
		}
		catch
		{
	...
	deprecated alias дефШрифт controlFont;
		...
		Прям выведиПрямоугольник() // getter
	{
		...
		//protected проц onDockChanged(АргиСоб ea)
	...
	{
		throw new ВизИскл("УпрЭлт already has а разметка");
	}
		...
		проц шрифт(Шрифт f) // setter
	{
		...
		окШрифт = f;
		if(созданУказатель_ли)
			SendMessageA(уок, WM_SETFONT, cast(WPARAM)окШрифт.указатель, MAKELPARAM(да, 0));
	...
	final бул естьОтпрыски() // getter
	{
		...
		if(созданУказатель_ли)
		{
			return GetWindow(уок, GW_CHILD) != HWND.init;
		}
		else
		{
		...
		final проц высота(цел h) // setter
	{
		...
		SetWindowPos(уок, HWND.init, 0, 0, rect.right - rect.лево, h, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOMOVE);
		*/
		...
		final проц лево(цел l) // setter
	{
		...
		RECT rect;
		GetWindowRect(уок, &rect);
		SetWindowPos(уок, HWND.init, l, rect.top, 0, 0, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE);
		*/
	...
	static ПКлавиши клавишиМодификаторы() // getter
	{
		...
		if(GetAsyncKeyState(VK_ШИФТ) & 0x8000)
			ks |= ПКлавиши.ШИФТ;
	...
	protected проц onParentChanging(АргиСобУпрЭлта ea)
	{
	}
	+/
		...
		final проц родитель(УпрЭлт ктрл) // setter
	{
		...
		if(!(_style() & WS_CHILD) || (_exStyle() & WS_EX_MDICHILD))
			throw new ВизИскл("Cannot добавь а top level упрэлт to а упрэлт");
		...
		final проц регион(Регион rgn) // setter
	{
		if(созданУказатель_ли)
		{
			...
			SetWindowRgn(уок, dupHrgn(rgn.указатель), да);
		}
	...
	body
	{
		...
		if(созданУказатель_ли)
		{
			ЗэтИндексГет gzi;
			gzi.найди = this;
			EnumChildWindows(родитель.уок, &обрвызовЗэтИндексГет, cast(LPARAM)&gzi);
			return gzi.индекс;
		}
		else
		{
	...
	Ткст текст() // getter
	{
		...
		final проц top(цел t) // setter
	{
		установиЯдроГраниц(0, t, 0, 0, ПЗаданныеПределы.Y);
	}
	
	
	final цел top() // getter
	{
		return окПрям.в;
	}
		...
		final проц поместиНазад()
	{
		...
		SetWindowPos(уок, HWND_BOTTOM, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
	}
		...
		final проц выведиВперёд()
	{
		...
		SetWindowPos(уок, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
	...
	final проц выведиВышеНаОдно()
	{
		...
		SetWindowPos(уок, hw, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
	}
	...
	version(VIZ_NO_DRAG_DROP) {} else
	{
		...
		export:
			...
			проц проверьОбъДанных(ИОбъектДанных pDataObject)
			{
		...
		final ПЭффектыДД выполниДД(viz.data.ИОбъектДанных объДанных, ПЭффектыДД разрешённыеЭффекты)
		{
			Объект foo = cast(Объект)объДанных; // Hold а reference to the Объект...
			
			DWORD эффект;
			ИстокБроска dropsrc;
			ИОбъектДанных dropdata;
			
			dropsrc = new ИстокБроска(this);
			dropdata = new DtoComDataObject(объДанных);
			
			// объДанных seems to be killed too early.
			switch(DoDragDrop(dropdata, dropsrc, cast(DWORD)разрешённыеЭффекты, &эффект))
			{
	...
	// provided.
	static УпрЭлт поУказателюОтпрыска(УОК hwChild)
	{
		УпрЭлт результат;
		for(;;)
		{
	...
	static УпрЭлт поУказателю(УОК hw)
	{
	...
	{
		УОК hwChild;
		hwChild = ChildWindowFromPoint(уок, тчк.точка);
		if(!hwChild)
			return пусто;
	...
	package final проц перерисуйПолностью()
	{
		if(уок)
		{
			SetWindowPos(уок, HWND.init, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_DRAWFRAME | SWP_NOMOVE
		...
		final проц инвалидируй()
	{
		...
		RedrawWindow(уок, пусто, HRGN.init, RDW_ERASE | RDW_INVALIDATE | RDW_NOCHILDREN);
	}
	...
	final проц инвалидируй(бул andChildren)
	{
		...
		RedrawWindow(уок, пусто, HRGN.init, RDW_ERASE | RDW_INVALIDATE | (andChildren ? RDW_ALLCHILDREN : RDW_NOCHILDREN));
	}
	...
	final проц инвалидируй(Прям к)
	{
		...
		RedrawWindow(уок, &rect, HRGN.init, RDW_ERASE | RDW_INVALIDATE | RDW_NOCHILDREN);
	}
	...
	final проц инвалидируй(Прям к, бул andChildren)
	{
		...
		RedrawWindow(уок, &rect, HRGN.init, RDW_ERASE | RDW_INVALIDATE | (andChildren ? RDW_ALLCHILDREN : RDW_NOCHILDREN));
	}
	...
	final проц инвалидируй(Регион rgn)
	{
		...
		RedrawWindow(уок, пусто, rgn.указатель, RDW_ERASE | RDW_INVALIDATE | RDW_NOCHILDREN);
	}
	...
	final проц инвалидируй(Регион rgn, бул andChildren)
	{
		...
		RedrawWindow(уок, пусто, rgn.указатель, RDW_ERASE | RDW_INVALIDATE | (andChildren ? RDW_ALLCHILDREN : RDW_NOCHILDREN));
	}
	...
	final проц перерисуй()
	{
		...
		RedrawWindow(уок, пусто, HRGN.init, RDW_ERASE | RDW_INVALIDATE | RDW_FRAME);
	}
	...
	{
		DWORD tid = GetWindowThreadProcessId(уок, пусто);
	...
	private static проц плохойУказательВызова()
	{
		...
		throw new ВизИскл("Must вызови with создан указатель");
	}
	...
	final Объект вызови(Объект delegate(Объект[]) дг, Объект[] арги ...)
	{
		...
		if(LRESULT_VIZ_INVOKE != SendMessageA(уок, wmViz, WPARAM_VIZ_INVOKE, cast(LRESULT)&inv))
			throw new ВизИскл("Invoke failure");
		if(inv.исключение)
	...
	final проц вызови(проц delegate() дг)
	{
		...
		if(LRESULT_VIZ_INVOKE != SendMessageA(уок, wmViz, WPARAM_VIZ_INVOKE_SIMPLE, cast(LRESULT)&inv))
			throw new ВизИскл("Invoke failure");
		if(inv.исключение)
			throw inv.исключение;
	}
	...
	**/
	...
	final проц задержиВызов(проц function() fn)
	{
		...
		PostMessageA(уок, wmViz, WPARAM_VIZ_DELAY_INVOKE, cast(LPARAM)fn);
	}
	...
	deprecated alias задержиВызов beginInvoke;
	...
	final Прям прямоугольникКЭкрану(Прям к)
	{
		...
		MapWindowPoints(уок, HWND.init, cast(POINT*)&rect, 2);
		return Прям(&rect);
	}
		...
		проц сбросьТекст()
	{
		...
		// Just allow разметка recalc, but don't do it right now.
	...
	protected проц scaleCore(float ширина, float высота)
	{
		заморозьРазметку();
	...
	private static бул _eachild(УОК hw, бул delegate(УОК hw) обрвыз, inout т_мера xiter, бул nested)
	{
	...
	package static проц eachGoodChildHandle(УОК hwparent, бул delegate(УОК hw) обрвыз, бул nested = да)
	{
		УОК hw = GetWindow(hwparent, GW_CHILD);
		т_мера xiter = 2000;
		_eachild(hw, обрвыз, xiter, nested);
	}
	
	
	private static бул _isHwndControlSel(УОК hw)
	{
	...
	package static проц _dlgselnext(Форма dlg, УОК hwcursel, бул forward,
		бул tabStopOnly = да, бул selectableOnly = нет,
		бул nested = да, бул wrap = да,
		УОК hwchildrenof = пусто)
	{
	...
	package final Размер _fetchClientSize()
	{
		RECT к;
		GetClientRect(уок, &к);
		return Размер(к.right, к.bottom);
	}
	
	
	deprecated protected проц приИнвалидировании(АргиСобИнвалидировать iea)
	{
	...
	/+
	protected проц приИзмененииПоложения(АргиСоб ea)
	{
	...
	/+
	protected проц onSizeChanged(АргиСоб ea)
	{
	...
	deprecated protected проц onBeforeResize(BeforeResizeEventArgs ea)
	{
	}
	+/
	...
	/+
	deprecated protected проц onEnter(АргиСоб ea)
	{
		//enter(this, ea);
	}
	
	
	deprecated protected проц onLeave(АргиСоб ea)
	{
		//leave(this, ea);
	}
	
	
	deprecated protected проц onValidated(АргиСоб ea)
	{
		//validated(this, ea);
	}
	
	
	deprecated protected проц onValidating(АргиСобОтмены cea)
	{
		/+
		...
		protected проц поСозданиюУказателя(АргиСоб ea)
	{
		...
		fon = шрифт;
		if(fon)
			SendMessageA(уок, WM_SETFONT, cast(WPARAM)fon.указатель, 0);
		...
		version(VIZ_NO_DRAG_DROP) {} else
		{
			if(цельброса)
			{
				if(S_OK != RegisterDragDrop(уок, цельброса))
				{
					цельброса = пусто;
					throw new ВизИскл("Unable to register drag-drop");
				}
			}
		}
		...
		protected проц приОтрисовкеФона(АргиСобРис pea)
	{
		...
		FillRect(pea.графика.указатель, &rect, hbrBg);
	}
	...
	deprecated protected проц onNotifyMessage(Сообщение сооб)
	{
	}
	...
	final УОК тчкНадВидимымОтпрыском(Точка тчк) // package
	{
		if(тчк.ш < 0 || тчк.в < 0)
			return HWND.init;
		...
		protected проц окПроц(inout Сообщение сооб)
	{
		...
		switch(сооб.сооб)
		{
			case WM_PAINT:
				{
					...
					PAINTSTRUCT ps;
					BeginPaint(сооб.уок, &ps);
					try
					{
						//приИнвалидировании(new АргиСобИнвалидировать(Прям(&uprect)));
						
						scope АргиСобРис pea = new АргиСобРис(new Графика(ps.hdc, нет), Прям(&ps.rcPaint));
			...
			case WM_ERASEBKGND:
				...
				else if(!(ктрлСтиль & ПСтилиУпрЭлта.ALL_PAINTING_IN_WM_PAINT))
				{
					...
					uprect.лево = 0;
					uprect.top = 0;
					uprect.right = клиентРазм.ширина;
					uprect.bottom = клиентРазм.высота;
			...
			//case 0x0019: //WM_CTLCOLOR; obsolete.
				{
					УпрЭлт упрэлм = поУказателюОтпрыска(cast(УОК)сооб.парам2);
					if(упрэлм)
					{
			...
			case WM_MOUSEMOVE:
				...
				if(trackMouseEvent) // Requires Windows 95 with IE 5.5, 98 or NT4.
				{
					if(!menter)
					{
						...
						POINT тчк;
						GetCursorPos(&тчк);
						MapWindowPoints(HWND.init, cast(HWND) уок, &тчк, 1);
			...
			case WM_SETCURSOR:
				...
				if(cast(УОК)сооб.парам1 == уок)
				{
					Курсор cur;
					cur = курсор;
					if(cur)
					{
						if(cast(КУРСОР)GetClassLongA(уок, GCL_HCURSOR) != cur.указатель)
							SetClassLongA(уок, GCL_HCURSOR, cast(LONG)cur.указатель);
					}
					else
					{
						if(cast(КУРСОР)GetClassLongA(уок, GCL_HCURSOR) != КУРСОР.init)
							SetClassLongA(уок, GCL_HCURSOR, cast(LONG)cast(КУРСОР)пусто);
					}
			...
			case WM_MOUSELEAVE: // Requires Windows 95 with IE 5.5, 98 or NT4.
				{
					...
					POINT тчк;
					GetCursorPos(&тчк);
					MapWindowPoints(HWND.init, уок, &тчк, 1);
					scope АргиСобМыши mea = new АргиСобМыши(wparamMouseButtons(сооб.парам1), 0, тчк.ш, тчк.в, 0);
					приВыходеМыши(mea);
				}
				break;
				...
				case WM_MEASUREITEM:
					{
						...
						{
							упрэлм = УпрЭлт.поУказателюОтпрыска(cast(УОК)mis.CtlID);
							if(упрэлм)
							{
								//сооб.результат = упрэлм.customMsg(*(cast(ОсобоеСооб*)&сооб));
								упрэлм.поОбратномуСообщению(сооб);
				...
				case WM_COMMAND:
					{
						...
						упрэлм = УпрЭлт.поУказателюОтпрыска(cast(УОК)сооб.парам2);
						if(упрэлм)
						{
							//сооб.результат = упрэлм.customMsg(*(cast(ОсобоеСооб*)&сооб));
							упрэлм.поОбратномуСообщению(сооб);
		...
		if(сооб.сооб == WM_CREATE)
		{
			...
			debug
			{
				assert(_handlecreated, "If overriding поСозданиюУказателя(), be sure to call super.поСозданиюУказателя()!");
			}
	...
	package final проц _wmKillFocus()
	{
		...
		protected проц дефОкПроц(inout Сообщение сооб)
	{
		//сооб.результат = DefWindowProcA(сооб.уок, сооб.сооб, сооб.парам1, сооб.парам2);
		сооб.результат = дефОкПроц(сооб.уок, сооб.сооб, сооб.парам1, сооб.парам2);
	}
	...
	package final проц mustWndProc(inout Сообщение сооб)
	{
		if(needCalcSize)
		{
			needCalcSize = нет;
			RECT crect;
			GetClientRect(сооб.уок, &crect);
			клиентОкРазм.ширина = crect.right;
			клиентОкРазм.высота = crect.bottom;
		}
	...
	/+
	deprecated СобОбработчик causesValidationChanged;
	deprecated InvalidateEventHandler invalidated;
	deprecated СобОбработчик validated;
	deprecated ОбработчикСобытияОтмены validating; // Once отмена is да, remaining события are supнажато (including validated).
	deprecated СобОбработчик enter; // Cascades up. TODO: fix implementation.
	deprecated СобОбработчик leave; // Cascades down. TODO: fix implementation.
	...
	this(Ткст текст, цел лево, цел top, цел ширина, цел высота)
	{
		this();
		окТекст = текст;
		окПрям = Прям(лево, top, ширина, высота);
		
		коллекция = создайЭкземплярУпрЭлтов();
	}
	
	
	this(УпрЭлт ктрлРодитель, Ткст текст, цел лево, цел top, цел ширина, цел высота)
	{
		this();
		окТекст = текст;
		окПрям = Прям(лево, top, ширина, высота);
	...
	{
		бул результат = нет;
		УОК hw = указатель; // Always reference указатель.
		...
		protected проц создайУказатель()
	{
		...
		if(удаляется)
		{
			debug
			{
				er = "the упрэлт is being вымещен";
			}
			
			debug(APP_PRINT)
			{
				скажиф("Creating УпрЭлт указатель while вымещается.\n");
			}
			
			create_err:
			Ткст kmsg = "УпрЭлт creation failure";
			if(имя.length)
				kmsg ~= " (" ~ имя ~ ")";
			debug
			{
		...
		with(cp)
		{
			...
			if(!уок)
			{
				...
				debug
				{
					...
					{
						er =std.string.format("CreateWindowEx failed {имяКласса=%s;допСтиль=0x%X;стиль=0x%X;родитель=0x%X;меню=0x%X;экземп=0x%X;}",
			...
			if(vis)
				doShow(); // Properly fires приИзмененииВидимости.
		}
	...
	body
	{
		...
		бул hfocus = вФокусе;
		УОК prevHwnd = GetWindow(уок, GW_HWNDPREV);
	...
	protected бул processDialogChar(дим кодСим)
	{
		if(окРодитель)
	...
	public deprecated проц setDFL095()
	{
		version(SET_VIZ_095)
		{
			pragma(сооб, "viz: DFL 0.9.5 compatibility установи at compile time");
		}
		else
		{
			//_compat = CCompat.DFL095;
			Приложение.setCompat(DflCompat.КОНТРОЛ_RECREATE_095);
		}
	}
	...
	version(SET_VIZ_095)
		package const ббайт _compat = CCompat.DFL095;
	else version(VIZ_NO_COMPAT)
		package const ббайт _compat = CCompat.НЕУК;
	else
		package CCompat _compat() // getter
			{ if(Приложение._compat & DflCompat.КОНТРОЛ_RECREATE_095) return CCompat.DFL095; return CCompat.НЕУК; }
	...
	LRESULT defwproc(UINT сооб, WPARAM wparam, LPARAM lparam)
	{
		...
		return дефОкПроц(уок, сооб, wparam, lparam);
	}
			...
			case WM_PAINT:
				{
					...
					//if(!сооб.парам1)
						GetUpdateRect(уок, &uprect, нет); // Preserve.
...
export:
	...
	final проц _scale(Размер toScale) // package
	{
		...
		проц xscale(УпрЭлт ктрл, Размер fromScale)
		{
			...
			else
			{
				Точка тчк;
				Размер разм;
				разм = вычислиМасштаб(Размер(ктрл.лево, ктрл.top), toScale, fromScale);
	...
	override Прям выведиПрямоугольник() // getter
	{
		...
		результат.ш = результат.ш + dpad.лево;
		результат.ширина = результат.ширина - dpad.right - dpad.лево;
		результат.в = результат.в + dpad.top;
		результат.высота = результат.высота - dpad.bottom - dpad.top;
	...
	class КраяДокПаддинга
	{
		...
		цел _all;
		//package проц delegate() изменено;
	...
	deprecated final проц setAutoScrollMargin(цел ш, цел в)
	{
		//
	}
	...
	override проц приВращенииМыши(АргиСобМыши ea)
	{
		...
		if(ea.дельта < 0)
		{
			if(yspos < maxp)
			{
				...
				yspos += дельта;
				SetScrollPos(уок, SB_VERT, yspos, TRUE);
				ScrollWindow(уок, 0, -дельта, пусто, пусто);
			}
		}
		else
		{
	...
	проц _adjustScrollSize(BOOL fRedraw = TRUE)
	{
		...
		//if(прокруткаГ)
		{
			...
			if(si.nMax)
				si.nMax--;
			SetScrollInfo(уок, SB_HORZ, &si, fRedraw);
		}
	}
}
...
export:
	...
	проц активныйУпрЭлт(УпрЭлт упрэлм) // setter
	{
		if(!активируйУпрЭлт(упрэлм))
			throw new ВизИскл("Unable to активируй упрэлт");
	}
	...
	/+
	deprecated protected override бул обработайМнемонику(дим кодСим)
	{
		return нет;
	}
