...
{   // Properly prototyped versions
	 alias BOOL function(УОК, UINT, WPARAM, LPARAM) DLGPROC;
	 alias VOID function(УОК, UINT, UINT, DWORD) TIMERPROC;
	 alias BOOL function(HDC, LPARAM, int) GRAYSTRINGPROC;
	 alias BOOL function(УОК, LPARAM) WNDENUMPROC;
	 alias LRESULT function(int code, WPARAM парам1, LPARAM парам2) HOOKPROC;
	 alias VOID function(УОК, UINT, DWORD, LRESULT) SENDASYNCPROC;
	 alias BOOL function(УОК, LPCSTR, HANDLE) PROPENUMPROCA;
	 alias BOOL function(УОК, LPCWSTR, HANDLE) PROPENUMPROCW;
	 alias BOOL function(УОК, LPSTR, HANDLE, DWORD) PROPENUMPROCEXA;
	 alias BOOL function(УОК, LPWSTR, HANDLE, DWORD) PROPENUMPROCEXW;
...
{
	 DELETE =                           0x00010000,
	 READ_КОНТРОЛ =                     0x00020000,
	 WRITE_DAC =                        0x00040000,
	 WRITE_OWNER =                      0x00080000,
	 SYNCHRONIZE =                      0x00100000,

	 STANDARD_RIGHTS_REQUIRED =         0x000F0000,
	 STANDARD_RIGHTS_READ =             READ_КОНТРОЛ,
	 STANDARD_RIGHTS_WRITE =            READ_КОНТРОЛ,
	 STANDARD_RIGHTS_EXECUTE =          READ_КОНТРОЛ,
...
struct WIN32_FIND_DATA {
	 ...
	 DWORD dwReserved0;
	 DWORD dwReserved1;
	 сим   cFileName[MAX_PATH];
	 сим   cAlternateFileName[ 14 ];
}
...
BOOL   ЗакройДескр(HANDLE hObject);

HANDLE CreateFileA(сим *lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
...
BOOL   DeleteFileA(сим *lpFileName);
BOOL   DeleteFileW(LPCWSTR lpFileName);

BOOL   FindClose(HANDLE hFindFile);
HANDLE FindFirstFileA(сим *lpFileName, WIN32_FIND_DATA* lpFindFileData);
...
DWORD  GetLastError();
DWORD  GetFileAttributesA(сим *lpFileName);
DWORD  GetFileAttributesW(wchar *lpFileName);
DWORD  GetFileSize(HANDLE hFile, DWORD *lpFileSizeHigh);
BOOL   CopyFileA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName, BOOL bFailIfExists);
BOOL   CopyFileW(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName, BOOL bFailIfExists);
BOOL   MoveFileA(сим *from, сим *to);
BOOL   MoveFileW(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName);
BOOL   ReadFile(HANDLE hFile, проц *lpBuffer, DWORD nNumberOfBytesToRead,
	DWORD *lpNumberOfBytesRead, OVERLAPPED *lpOverlapped);
DWORD  SetFilePointer(HANDLE hFile, LONG lDistanceToMove,
	LONG *lpDistanceToMoveHigh, DWORD dwMoveMethod);
BOOL   WriteFile(HANDLE hFile, проц *lpBuffer, DWORD nNumberOfBytesToWrite,
...
/*export*/
{
 ...
 LONG  InterlockedExchange(LPLONG Target, LONG значение);
 LONG  InterlockedExchangeAdd(LPLONG Addend, LONG значение);
...
проц FreeLibraryAndExitThread(HMODULE hLibModule, DWORD dwExitCode);
...
extern(D)
{
	const int REG_CREATED_NEW_KEY =         0x00000001;   // New Registry Key создан
...
//
//
// Predefined значение Types.
//
enum
{
	REG_NONE =                    0,   // No значение тип
...
enum
{
	...
	MB_HELP =                     0x00004000, // Help Кнопка
...
int MessageBoxA(УОК уок, LPCSTR lpText, LPCSTR lpCaption, UINT uType);
int MessageBoxExA(УОК уок, LPCSTR lpText, LPCSTR lpCaption, UINT uType, WORD wLanguageId);
...
enum
{
	...
	REG_OPTION_CREATE_LINK =      (0x00000002),   // Created key is а
																	 // symbolic link
...
// devioctl.h as FILE_READ_ACCESS and FILE_WRITE_ACCESS. The значения for these
// constants *MUST* always be in sync.
// The значения are redefined in devioctl.h because they must be available to
// both DOS and NT.
//
...
enum
{
	...
	FILE_READ_ATTRIBUTES =      ( 0x0080 ),    // все

	FILE_WRITE_ATTRIBUTES =     ( 0x0100 ),    // все
...
/*export*/
{
 ...
 проц GlobalFix(HGLOBAL hMem);
 проц GlobalUnfix(HGLOBAL hMem);
 LPVOID GlobalWire(HGLOBAL hMem);
 BOOL GlobalUnWire(HGLOBAL hMem);
 проц GlobalMemoryStatus(LPMEMORYSTATUS lpBuffer);
...
/*export*/ проц GetSystemTime(SYSTEMTIME* lpSystemTime);
/*export*/ проц GetSystemTimeAsFileTime(FILETIME* lpSystemTimeAsFileTime);
/*export*/ BOOL SetSystemTime(SYSTEMTIME* lpSystemTime);
/*export*/ проц GetLocalTime(SYSTEMTIME* lpSystemTime);
...
/*export*/ DWORD FormatMessageA(DWORD dwFlags, LPCVOID lpSource, DWORD dwMessageId, DWORD dwLanguageId, LPSTR lpBuffer, DWORD nSize, ук *Arguments);/*export*/ DWORD FormatMessageW(DWORD dwFlags, LPCVOID lpSource, DWORD dwMessageId, DWORD dwLanguageId, LPWSTR lpBuffer, DWORD nSize, ук *Arguments);
...
//  Language IDs.
//
//  The following two combinations of primary язык ID and
//  sublanguage ID have special semantics:
//
//    Primary Language ID   Sublanguage ID      Result
//    -------------------   ---------------     ------------------------
//    LANG_NEUTRAL          SUBLANG_NEUTRAL     Language neutral
//    LANG_NEUTRAL          SUBLANG_DEFAULT     User default язык
//    LANG_NEUTRAL          SUBLANG_SYS_DEFAULT System default язык
//
...
//  Sublanguage IDs.
//
//  The имя immediately following SUBLANG_ dictates which primary
//  язык ID that sublanguage ID can be combined with to form а
//  valid язык ID.
//
enum
{
	SUBLANG_NEUTRAL =                  0x00,    // язык neutral
...
//
//  A язык ID is а 16 bit значение which is the combination of а
//  primary язык ID and а secondary язык ID.  The bits are
...
//
//    MAKELANGID    - construct язык id from а primary язык id and
//                    а sublanguage id.
//    PRIMARYLANGID - extract primary язык id from а язык id.
//    SUBLANGID     - extract sublanguage id from а язык id.
//
...
//
// The following флаги упрэлт the contents of the CONTEXT structure.
//
	CONTEXT_i386 =    0x00010000,    // this assumes that i386 and
	CONTEXT_i486 =    0x00010000,    // i486 have identical контекст records

	CONTEXT_КОНТРОЛ =         (CONTEXT_i386 | 0x00000001), // SS:SP, CS:IP, FLAGS, BP
	CONTEXT_INTEGER =         (CONTEXT_i386 | 0x00000002), // AX, BX, CX, DX, SI, DI
	CONTEXT_SEGMENTS =        (CONTEXT_i386 | 0x00000004), // DS, ES, FS, GS
	CONTEXT_FLOATING_POINT =  (CONTEXT_i386 | 0x00000008), // 387 состояние
	CONTEXT_DEBUG_REGISTERS = (CONTEXT_i386 | 0x00000010), // DB 0-3,6,7

	CONTEXT_FULL = (CONTEXT_КОНТРОЛ | CONTEXT_INTEGER | CONTEXT_SEGMENTS),
}
...
struct CONTEXT
{
	 ...
	 //
	 // The флаги значения within this флаг упрэлт the contents of
	 // а CONTEXT record.
	 //
	 // If the контекст record is used as an input parameter, then
	 // for each portion of the контекст record controlled by а флаг
	 // whose значение is установи, it is assumed that that portion of the
	 // контекст record содержит valid контекст. If the контекст record
	 // is being used to modify а threads контекст, then only that
	 // portion of the threads контекст will be изменён.
	 //
	 // If the контекст record is used as an IN OUT parameter to захвати
	 // the контекст of а thread, then only those portions of the thread's
	 // контекст corresponding to установи флаги will be returned.
	 //
	 // The контекст record is never used as an OUT only parameter.
	 //

	 DWORD ConтекстFlags;

	 //
	 // This section is задано/returned if CONTEXT_DEBUG_REGISTERS is
	 // установи in ConтекстFlags.  Note that CONTEXT_DEBUG_REGISTERS is NOT
	 ...
	 //
	 // This section is задано/returned if the
	 // ConтекстFlags word contians the флаг CONTEXT_FLOATING_POINT.
	 //
	 ...
	 //
	 // This section is задано/returned if the
	 // ConтекстFlags word contians the флаг CONTEXT_SEGMENTS.
	 //
	 ...
	 //
	 // This section is задано/returned if the
	 // ConтекстFlags word contians the флаг CONTEXT_INTEGER.
	 //
	 ...
	 //
	 // This section is задано/returned if the
	 // ConтекстFlags word contians the флаг CONTEXT_КОНТРОЛ.
	 //
...
enum
{
	THREAD_BASE_PRIORITY_LOWRT =  15,  // значение that gets а thread to LowRealtime-1
	THREAD_BASE_PRIORITY_MAX =    2,   // maximum thread base priority boost
	THREAD_BASE_PRIORITY_MIN =    -2,  // minimum thread base priority boost
	THREAD_BASE_PRIORITY_IDLE =   -15, // значение that gets а thread to вБездействии
...
/*export*/ BOOL GetThreadConтекст(HANDLE hThread, CONTEXT* lpConтекст);
/*export*/ BOOL SetThreadConтекст(HANDLE hThread, CONTEXT* lpConтекст);
/*export*/ DWORD SuspendThread(HANDLE hThread);
/*export*/ DWORD ResumeThread(HANDLE hThread);
/*export*/ DWORD ЖдиОдинОбъект(HANDLE hHandle, DWORD dwMilliseconds);
/*export*/ DWORD ЖдиНесколькоОбъектов(DWORD nCount, HANDLE *lpHandles, BOOL bWaitAll, DWORD dwMilliseconds);
/*export*/ проц Sleep(DWORD dwMilliseconds);
...
/*
 * УпрЭлт Manager Structures and Definitions
 */
...
/*
 * Edit УпрЭлт Notification Codes
 */
	EN_SETFOCUS =         0x0100,
	EN_KILLFOCUS =        0x0200,
	EN_CHANGE =           0x0300,
...
/* Edit упрэлт EM_SETMARGIN parameters */
...
/*
 * User Кнопка Notification Codes
 */
	BN_CLICKED =          0,
	BN_PAINT =            1,
	BN_HILITE =           2,
 ...
 */
	...
	WM_ACTIVATE =                     0x0006,
/*
 * WM_ACTIVATE состояние значения
 */
	WA_INACTIVE =     0,
	WA_ACTIVE =       1,
	WA_CLICKACTIVE =  2,
...
/*export*/
{
 BOOL UpdateWindow(УОК уок);
 УОК SetActiveWindow(УОК уок);
 УОК GetForegroundWindow();
 BOOL PaintDesktop(HDC hdc);
 BOOL SetForegroundWindow(УОК уок);
 УОК WindowFromDC(HDC hDC);
 HDC GetDC(УОК уок);
 HDC GetDCEx(УОК уок, HRGN hrgnClip, DWORD флаги);
 HDC GetWindowDC(УОК уок);
 int ReleaseDC(УОК уок, HDC hDC);
 HDC BeginPaint(УОК уок, LPPAINTSTRUCT lpPaint);
 BOOL EndPaint(УОК уок, PAINTSTRUCT *lpPaint);
 BOOL GetUpdateRect(УОК уок, LPRECT lpRect, BOOL bErase);
 int GetUpdateRgn(УОК уок, HRGN hRgn, BOOL bErase);
 int SetWindowRgn(УОК уок, HRGN hRgn, BOOL bRedraw);
 int GetWindowRgn(УОК уок, HRGN hRgn);
 int ExcludeUpdateRgn(HDC hDC, УОК уок);
 BOOL InvalidateRect(УОК уок, RECT *lpRect, BOOL bErase);
 BOOL ValidateRect(УОК уок, RECT *lpRect);
 BOOL InvalidateRgn(УОК уок, HRGN hRgn, BOOL bErase);
 BOOL ValidateRgn(УОК уок, HRGN hRgn);
 BOOL RedrawWindow(УОК уок, RECT *lprcUpdate, HRGN hrgnUpdate, UINT флаги);
}
...
/*export*/
{
 BOOL GetClientRect(УОК уок, LPRECT lpRect);
 BOOL GetWindowRect(УОК уок, LPRECT lpRect);
...
/*export*/
{
 ...
 LONG    SetBitmapBits(HBITMAP, DWORD, проц *);
 UINT    SetBoundsRect(HDC,   RECT *, UINT);
 int     SetDIBits(HDC, HBITMAP, UINT, UINT, проц *, BITMAPINFO *, UINT);
 int     SetDIBitsToDevice(HDC, int, int, DWORD, DWORD, int,
		  int, UINT, UINT, проц *, BITMAPINFO *, UINT);
...
/*export*/ проц PostQuitMessage(int nExitCode);
/*export*/ LRESULT DefWindowProcA(УОК уок, UINT Msg, WPARAM парам1, LPARAM парам2);
/*export*/ HMODULE GetModuleHandleA(LPCSTR lpModuleName);

alias LRESULT (* WNDPROC)(УОК, UINT, WPARAM, LPARAM);
...
/*export*/
{
 HICON LoadIconA(экз hInstance, LPCSTR lpIconName);
 HICON LoadIconW(экз hInstance, LPCWSTR lpIconName);
 КУРСОР LoadCursorA(экз hInstance, LPCSTR lpCursorName);
 КУРСОР LoadCursorW(экз hInstance, LPCWSTR lpCursorName);
}
...
extern(D)
{
	...
	/*
	 * Special значение for CreateWindow, et al.
	 */
	const УОК HWND_DESKTOP = (cast(УОК)0);
}


/*export*/ ATOM RegisterClassA(WNDCLASSA *lpWndClass);

/*export*/ УОК CreateWindowExA(
	 DWORD dwExStyle,
	 LPCSTR lpClassName,
	 LPCSTR lpWindowName,
	 DWORD dwStyle,
...
/*export*/
{
 BOOL GetMessageA(LPMSG lpMsg, УОК уок, UINT wMsgFilterMin, UINT wMsgFilterMax);
 BOOL TranslateMessage(MSG *lpMsg);
 LONG DispatchMessageA(MSG *lpMsg);
 BOOL PeekMessageA(MSG *lpMsg, УОК уок, UINT wMsgFilterMin, UINT wMsgFilterMax, UINT wRemoveMsg);
 УОК GetFocus();
}
...
/*export*/ BOOL ShowWindow(УОК уок, int nCmdShow);
...
/*export*/ int SetScrollPos(УОК уок, int nBar, int nPos, BOOL bRedraw);
/*export*/ int GetScrollPos(УОК уок, int nBar);
/*export*/ BOOL SetScrollRange(УОК уок, int nBar, int nMinPos, int nMaxPos, BOOL bRedraw);
/*export*/ BOOL GetScrollRange(УОК уок, int nBar, LPINT lpMinPos, LPINT lpMaxPos);
/*export*/ BOOL ShowScrollBar(УОК уок, int wBar, BOOL bShow);
/*export*/ BOOL EnableScrollBar(УОК уок, UINT wSBflags, UINT wArrows);

/*
 * LockWindowUpdate API
 */

/*export*/ BOOL LockWindowUpdate(УОК hWndLock);
/*export*/ BOOL ScrollWindow(УОК уок, int XAmount, int YAmount, RECT* lpRect, RECT* lpClipRect);
/*export*/ BOOL ScrollDC(HDC hDC, int dx, int dy, RECT* lprcScroll, RECT* lprcClip, HRGN hrgnUpdate, LPRECT lprcUpdate);
/*export*/ int ScrollWindowEx(УОК уок, int dx, int dy, RECT* prcScroll, RECT* prcClip, HRGN hrgnUpdate, LPRECT prcUpdate, UINT флаги);

/*
 * Virtual ПКлавиши, Standard Set
 */
enum
{	VK_LBUTTON =        0x01,
	VK_RBUTTON =        0x02,
...
/*
 * VK_L* & VK_R* - лево and право Alt, Ctrl and Shift virtual ПКлавиши.
 * Used only as parameters to GetAsyncKeyState() and GetKeyState().
 * No other API or сообщение will distinguish лево and право ПКлавиши in this way.
 */
	VK_LШИФТ =         0xA0,
	VK_RШИФТ =         0xA1,
	VK_LКОНТРОЛ =       0xA2,
...
/*export*/ LRESULT SendMessageA(УОК уок, UINT Msg, WPARAM парам1, LPARAM парам2);

alias UINT (*LPOFNHOOKPROC) (УОК, UINT, WPARAM, LPARAM);
...
/*export*/ HMENU LoadMenuA(экз hInstance, LPCSTR lpMenuName);
/*export*/ HMENU LoadMenuW(экз hInstance, LPCWSTR lpMenuName);

/*export*/ HMENU GetSubMenu(HMENU hMenu, int nPos);

/*export*/ HBITMAP LoadBitmapA(экз hInstance, LPCSTR lpBitmapName);
/*export*/ HBITMAP LoadBitmapW(экз hInstance, LPCWSTR lpBitmapName);

extern(D) // This is а macro anyway
...
/*export*/ КУРСОР SetCursor(КУРСОР hCursor);
/*export*/ BOOL GetCursorPos(LPPOINT lpPoint);
/*export*/ BOOL ClipCursor( RECT *lpRect);
/*export*/ BOOL GetClipCursor(LPRECT lpRect);
/*export*/ КУРСОР GetCursor();
/*export*/ BOOL CreateCaret(УОК уок, HBITMAP hBitmap , int nWidth, int nHeight);
/*export*/ UINT GetCaretBlinkTime();
/*export*/ BOOL SetCaretBlinkTime(UINT uMSeconds);
/*export*/ BOOL DestroyCaret();
/*export*/ BOOL HideCaret(УОК уок);
/*export*/ BOOL ShowCaret(УОК уок);
/*export*/ BOOL SetCaretPos(int X, int Y);
/*export*/ BOOL GetCaretPos(LPPOINT lpPoint);
/*export*/ BOOL ClientToScreen(УОК уок, LPPOINT lpPoint);
/*export*/ BOOL ScreenToClient(УОК уок, LPPOINT lpPoint);
/*export*/ int MapWindowPoints(УОК hWndFrom, УОК hWndTo, LPPOINT lpPoints, UINT cPoints);
/*export*/ УОК WindowFromPoint(Point Point);
/*export*/ УОК ChildWindowFromPoint(УОК hWndParent, Point Point);


/*export*/ BOOL TrackPopupMenu(HMENU hMenu, UINT uFlags, int ш, int в,
	int nReserved, УОК уок, RECT *prcRect);
...
/*export*/ int DialogBoxParamA(экз hInstance, LPCSTR lpTemplateName,
	УОК hWndParent, DLGPROC lpDialogFunc, LPARAM dwInitParam);
/*export*/ int DialogBoxIndirectParamA(экз hInstance,
...
enum
{
	...
	SND_MEMORY =          0x0004, /* pszSound точки to а memory file */
	SND_LOOP =            0x0008, /* loop the sound until next sndPlaySound */
	SND_NOSTOP =          0x0010, /* don't стоп any currently playing sound */

	SND_NOWAIT =	0x00002000, /* don't wait if the driver is busy */
	SND_ALIAS =       0x00010000, /* имя is а registry alias */
	SND_ALIAS_ID =	0x00110000, /* alias is а predefined ID */
	SND_FILENAME =    0x00020000, /* имя is file имя */
	SND_RESOURCE =    0x00040004, /* имя is resource имя or atom */

	SND_PURGE =           0x0040, /* purge non-static события for task */
...
/*export*/  ПЕРО      CreatePen(int, int, COLORREF);
/*export*/  ПЕРО      CreatePenIndirect(LOGPEN *);
...
extern(D) // This is а macro anyway.
COLORREF RGB(int к, int з, int с)
{
	 return cast(COLORREF)
	((cast(BYTE)к|(cast(WORD)(cast(BYTE)з)<<8))|((cast(DWORD)cast(BYTE)с)<<16));
}
...
/*export*/ BOOL EndDialog(УОК hDlg, int nResult);
/*export*/ УОК GetDlgItem(УОК hDlg, int nIDDlgItem);

/*export*/ BOOL SetDlgItemInt(УОК hDlg, int nIDDlgItem, UINT uValue, BOOL bSigned);
/*export*/ UINT GetDlgItemInt(УОК hDlg, int nIDDlgItem, BOOL *lpTranslated,
	 BOOL bSigned);

/*export*/ BOOL SetDlgItemTextA(УОК hDlg, int nIDDlgItem, LPCSTR lpString);
/*export*/ BOOL SetDlgItemTextW(УОК hDlg, int nIDDlgItem, LPCWSTR lpString);

/*export*/ UINT GetDlgItemTextA(УОК hDlg, int nIDDlgItem, LPSTR lpString, int nMaxCount);
/*export*/ UINT GetDlgItemTextW(УОК hDlg, int nIDDlgItem, LPWSTR lpString, int nMaxCount);

/*export*/ BOOL CheckDlgButton(УОК hDlg, int nIDButton, UINT uCheck);
/*export*/ BOOL CheckRadioButton(УОК hDlg, int nIDFirstButton, int nIDLastButton,
	 int nIDCheckButton);

/*export*/ UINT IsDlgButtonChecked(УОК hDlg, int nIDButton);

/*export*/ УОК SetFocus(УОК уок);
...
extern(D)
{
	const бцел INFINITE = бцел.max;
	const бцел WAIT_OBJECT_0 = 0;
}
