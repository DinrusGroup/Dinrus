...
private import viz.x.winapi, viz.control, viz.base, viz.event;
private import viz.x.utf, viz.drawing, viz.app, viz.collections;
...
else
{
		...
		abstract class Меню: Объект // docmain
	{
		...
		/+ package +/ protected проц _setInfo(UINT uItem, BOOL fByPosition, LPMENUITEMINFOA lpmii, Ткст typeData = пусто) // package
		{
			if(typeData.length)
			{
				if(viz.x.utf.использоватьЮникод)
				{
					...
					lpmii.dwTypeData = cast(typeof(lpmii.dwTypeData))viz.x.utf.вЮни0(typeData);
					_setMenuItemInfoW(hmenu, uItem, fByPosition, cast(MENUITEMINFOW*)lpmii);
				}
				else
				{
					lpmii.dwTypeData = cast(typeof(lpmii.dwTypeData))viz.x.utf.небезопАнзи0(typeData);
		...
		/+ package +/ protected проц _insert(UINT uItem, BOOL fByPosition, LPMENUITEMINFOA lpmii, Ткст typeData = пусто) // package
		{
			if(typeData.length)
			{
				if(viz.x.utf.использоватьЮникод)
		...
		/+ package +/ protected проц _remove(UINT uPosition, UINT uFlags) // package
		{
		...
		class ГлавноеМеню: Меню // docmain
	{
		// Used internally.
		this(HMENU hmenu, бул owned = да)
		{
		...
		/+ package +/ protected override проц _setInfo(UINT uItem, BOOL fByPosition, LPMENUITEMINFOA lpmii, Ткст typeData = пусто) // package
		{
		...
		/+ package +/ protected override проц _insert(UINT uItem, BOOL fByPosition, LPMENUITEMINFOA lpmii, Ткст typeData = пусто) // package
		{
		...
		/+ package +/ protected override проц _remove(UINT uPosition, UINT uFlags) // package
		{
