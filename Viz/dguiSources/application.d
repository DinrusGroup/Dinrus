...
private alias extern(Windows) бул function(INITCOMMONCONTROLSEX*) InitCommonControlsExProc;
...
class Application
{
	...
	{		
		public this(Object e)
		{
			...
			this.боксУпрЭлта = нет;
			this.стартPosition = FormStartPosition.CENTER_PARENT;
			this.formBorderStyle = FormBorderStyle.FIXED_DIALOG;

			this._lblHead = new Label();
			this._lblHead.расположение = РасположениеТекста.MIDDLE | РасположениеТекста.LEFT;
			this._lblHead.foreColor = Color(0xB4, 0x00, 0x00);
			this._lblHead.док = DockStyle.TOP;
			this._lblHead.height = 50;
			this._lblHead.text = ERR_MSG;
			this._lblHead.parent = this;
			
			this._lblInfo = new Label();
			this._lblInfo.расположение = РасположениеТекста.MIDDLE | РасположениеТекста.LEFT;
			this._lblInfo.док = DockStyle.TOP;
			this._lblInfo.height = 50;
			this._lblInfo.text = INFO;
			this._lblInfo.parent = this;
			
			this._rtfText = new RichTextBox();
			this._rtfText.док = DockStyle.TOP;
			this._rtfText.height = 90;
			this._rtfText.backColor = СистемныеЦвета.colorBtnFace;
			this._rtfText.полосыПрокрутки = да;
			this._rtfText.readOnly = да;
	...
	public static string стартupPath()
	{
	...
	private static int doRun(Form mainForm, бул cont)
	{
		...
		try
		{
			...
			for(;;)
			{
				...
				{
					if(m.сообщение == WM_QUIT) //Esci dal programma
					{
						break;
					}
		...
		catch(Object e)
		{
			...
			switch(ef.showDialog())
			{
				...
				case DialogResult.IGNORE:
					return Application.doRun(mainForm, да);
