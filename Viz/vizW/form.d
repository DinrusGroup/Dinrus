...
export:
	...
	{
		return добавьСокращение(быстрыйЗапуск, cast(проц delegate(Объект отправитель, АргиСобБыстрЗапускаФормы ea))нажато);
	}
	...
	final проц удалиСокращение(ПКлавиши быстрыйЗапуск)
	{
		...
		_shortcuts.remove(быстрыйЗапуск);
	}
	...
	final Точка центрЭкрана() // getter
	{
		...
		тчк.в = area.верх + (((area.верх - area.верх) - this.высота) / 2);
		return тчк;
	}
			...
			switch(стартпоз)
			{
				case ПНачПоложениеФормы.ЦЕНТР_РОДИТЕЛЯ:
					if(cwparent && cwparent.виден)
					{
						...
						if(cp.в < area.верх)
							cp.в = area.верх;
						else if(cp.в + cp.высота > area.верх)
							cp.в = area.верх - cp.высота;
						break;
					}
				...
				case ПНачПоложениеФормы.ЦЕНТР_ЭКРАНА:
					{
						...
						cp.в = area.верх + (((area.верх - area.верх) - cp.высота) / 2);
					}
					break;
	 ...
	 override проц создайУказатель()
	{
		...
		if(удаляется)
		{
			...
			debug(APP_PRINT)
			{
				скажиф("Creating Форма указатель while удаляется.\n");
			}
		...
		with(cp)
		{
			...
			уок = создайОкноДоп(допСтиль, имяКласса, заглавие, окСтиль & ~WS_VISIBLE,
			...
			if(!уок)
			{
				debug
				{
					version(Dinrus)
					{
						ош = "CreateWindowEx failed";
					}
					else
					{
						ош = фм("CreateWindowEx failed {имяКласса=%s;допСтиль=0x%X;стиль=0x%X;родитель=0x%X;меню=0x%X;экземп=0x%X;}",
		...
		if(vis & CBits.FVISIBLE)
		{
			...
			switch(состояниеОкна)
			{
				case ПСостОкнаФормы.НОРМА: show_normal:
					...
					// Possible to-do: see if non-MDI is "main form" and use SHOWNORMAL or Покажи.
					break;
	...
	final проц границыРабСтола(Прям к) // setter
	{
		...
		установиЯдроГраниц(rect.лево, rect.верх, rect.право - rect.лево, rect.верх - rect.верх, ПЗаданныеПределы.ВСЕ);
	}
	...
	{
		ПРЯМ к;
		GetWindowRect(указатель, cast(RECT*) &к);
		return Прям(&к);
	}
	...
	override проц приИзмененииРазмера(АргиСоб ea)
	{
		...
		if(_isPaintingSizeGrip)
		{
			...
			InvalidateRect(уок, cast(RECT*) &rect, TRUE);
		}
	}
	
	
	private проц _getSizeGripArea(ПРЯМ* rect)
	{
		rect.право = клиентРазм.ширина;
		rect.верх = клиентРазм.высота;
		rect.лево = rect.право - GetSystemMetrics(SM_CXVSCROLL);
		rect.верх = rect.верх - GetSystemMetrics(SM_CYHSCROLL);
	}
	...
	override проц установиЯдроВидимого(бул подтвержд)
	{
		if(созданУказатель_ли)
		{
			if(виден == подтвержд)
	...
	проц поАктивации(АргиСоб ea)
	{
		активировано(this, ea);
	}
	
	
	проц поДезактивации(АргиСоб ea)
	{
	...
	/+
		проц onInputLanguageChanged(InputLanguageChangedEventArgs ilcea)
	{
		inputLanguageChanged(this, ilcea);
	}
	
	
		проц onInputLanguageChanging(InputLanguageChangingEventArgs ilcea)
	{
			...
			case WM_GETMINMAXINFO:
				{
					...
					if(minsz.ширина && minsz.высота)
					{
						if(mmi.ptMinTrackSize.x < minsz.ширина)
							mmi.ptMinTrackSize.x = minsz.ширина;
						if(mmi.ptMinTrackSize.y < minsz.высота)
							mmi.ptMinTrackSize.y = minsz.высота;
					}
					if(maxsz.ширина && maxsz.высота)
					{
						if(mmi.ptMaxTrackSize.x > maxsz.ширина)
							mmi.ptMaxTrackSize.x = maxsz.ширина;
						if(mmi.ptMaxTrackSize.y > maxsz.высота)
							mmi.ptMaxTrackSize.y = maxsz.высота;
					}
	...
	package alias дефДлгПроц _defFormProc;
	
	override проц дефОкПроц(inout Сообщение сооб)
	{
		switch(сооб.сооб)
		{
			/+
			...
			case WM_SETFOCUS:
				...
				else
				{
					if(мдиОтпрыск_ли)
					{
						...
						сооб.результат = дефМДИОтпрыскПроц(сооб.уок, сооб.сооб, сооб.парам1, сооб.парам2);
						return;
					}
				}
			...
			default: def_def:
				...
				else
				{
					if(клиентМди && клиентМди.созданУказатель_ли && сооб.сооб != WM_SIZE)
						...
						сооб.результат = дефФреймПроц(сооб.уок, клиентМди.указатель, сооб.сооб, сооб.парам1, сооб.парам2);
   ...
   проц приЗакрытии(АргиСобОтмены cea)
	...
	override проц установиЯдроКлиентскогоРазмера(цел ширина, цел высота)
	{
		...
		AdjustWindowRectEx(cast(RECT*)&к, wl, !(wl & WS_CHILD) && hasmenu, _exStyle());
		
		установиЯдроГраниц(0, 0, к.право - к.лево, к.верх - к.верх, ПЗаданныеПределы.РАЗМЕР);
	}
	
	
	override проц установиЯдроГраниц(цел ш, цел в, цел ширина, цел высота, ПЗаданныеПределы задано)
	{
		if(созданУказатель_ли)
		{
	...
	final проц безФильтраСообщений() // package
	{
		nofilter = да;
	}
	...
	version(NO_MDI) {} else
	{
		final КлиентМди клиентМди() // getter
		{ return _mdiClient; }
	}
					...
					case WM_CHAR:
						switch(cast(ПКлавиши)m.парам1)
						{
							case ПКлавиши.ENTER:
								...
								{
									isDialogMessage(form.указатель, &m._винСооб);
									return да; // Prevent.
								}
								return нет;
							...
							case ПКлавиши.ESCAPE:
								...
								{
									//isDialogMessage(form.указатель, &m._винСооб); // Closes the родитель; bad for nested упрэлты.
							...
							case ПКлавиши.ПРАВ, ПКлавиши.ЛЕВ:
								//if(isDialogMessage(form.указатель, &m._винСооб)) // Stopped working after removing controlparent.
								//	return да; // Prevent.
								{
									LRESULT dlgc;
					...
					case WM_SYSCHAR:
						{
							...
							if(!foundmhw)
							{
								// Didn't найди текущий упрэлт, so go from верх-to-верх.
								eachGoodChildHandle(form.указатель,
									(УОК hw)
									{
										if(pmnemonic(hw))
	...
	final проц _recalcClientSize()
	{
		...
		AdjustWindowRectEx(cast(RECT*)&к, wl, hasmenu && !(wl & WS_CHILD), _exStyle());
		
		// Subtract the difference.
		клиентОкРазм = Размер(окПрям.ширина - ((к.право - к.лево) - окПрям.ширина), окПрям.высота - ((к.верх - к.верх) - окПрям.высота));
	}
}
...
export extern(D) class КлиентМди: СуперКлассУпрЭлта
	...
	export:	
		...
		override проц создайУказатель()
		{
			...
			Приложение.созданиеУпрЭлта(this);
			уок = создайОкноДоп(окДопСтиль, MDICLIENT_CLASSNAME, окТекст, окСтиль, окПрям.ш, окПрям.в,
		...
		override проц предшОкПроц(inout Сообщение сооб)
		{
			...
			сооб.результат = вызовиОкПроц(первОкПроцМдиКлиента, сооб.уок, сооб.сооб, сооб.парам1, сооб.парам2);
		}
...
version(ВИЗ_ПАРК_ОКНО)
{
	...
	проц _makePark()
	{
		...
		{
			debug(APP_PRINT)
				скажиф("RegisterClassEx() failed for park class.\n");
		...
		{
			debug(APP_PRINT)
				скажиф("CreateWindowEx() failed for park окно.\n");
