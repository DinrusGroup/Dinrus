...
export extern(Windows) BOOL обрвызовЗэтИндексГет(HWND уок, LPARAM lparam)
	{
		...
		if(упрэлм && упрэлм.родитель is gzi.найден.родитель)
		{
			gzi._tmp++;
		}
...
deprecated alias выведиВышеНаОдно zIndexUp;
	...
	version(VIZ_NO_DRAG_DROP) {} else
	{
		...
		export:
			...
			{
				if(*riid == cast(IID) IID_IDropTarget)
				{
			...
			HRESULT DragEnter(winapi.IDataObject pDataObject, DWORD grfKeyState, POINTL тчк, DWORD *pdwEffect)
			{
				...
				try
				{
					...
					scope АргиСобДрэг ea = new АргиСобДрэг(объДанных, cast(цел)grfKeyState, тчк.x, тчк.y, 
		...
		export:
			...
			override HRESULT QueryInterface(IID* riid, проц** ppv)
			{
				if(*riid ==cast(IID) IID_IDropSource)
				{
	...
	**/
			...
			case WM_HELP:
				{
					...
					scope АргиСобСправка hea = new АргиСобСправка(Точка(hi.MousePos.x, hi.MousePos.y));
			...
			case WM_WINDOWPOSCHANGED:
				{
					...
					{
						//окПрям = _fetchBounds();
						окПрям = Прям(wp.x, wp.y, wp.cx, wp.cy);
	...
	package final проц Покажи()
	{
		...
		{
			SetWindowPos(cast(HWND) уок, HWND.init, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOZORDER);
		}
		else
		{
			SetWindowPos(cast(HWND) уок, HWND_TOP, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_SHOWWINDOW);
		}
	}
	...
	{
		SetWindowPos(cast(HWND) уок, HWND.init, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE | SWP_HIDEWINDOW | SWP_NOZORDER);
	}
...
{	
	...
	{
		return viz.structs.Размер(0, 0);
	}
	
	/// Construct а new УпрЭлт instance.
	this()
	{
	debug(APP_PRINT) скажифнс("зис контрола");
		имя = Объект.вТкст(); // ?
		
		окПрям.размер = дефРазм();
		debug(APP_PRINT) скажифнс("зис контрола установил дефолтный размер");
		//oldwrect = окПрям;
		...
		цвфона = Цвет.пуст;
		цвпп = Цвет.пуст;
		debug(APP_PRINT) скажифнс("зис контрола установил дефолтный цвет");
		окШрифт = пусто;
		окКурс = пусто;
	...
	this(Ткст текст)
	{
		...
		this.коллекция = new КоллекцияУпрЭлтов(this);
	}
	...
	package final проц _disableVisualStyle()
	{
		...
		if(hmuxt)
		{
			...
			if(setWinTheme)
			{
				setWinTheme(cast(HWND) уок, " "w.ptr, " "w.ptr); // Clear the theme.
			}
		}
	}
		 ...
		 проц создайУказатель()
	{
		...
		if(удаляется)
		{
			...
			create_err:
			Ткст kmsg = "Неудачное создание управляющего элемента";
			if(имя.length)
				kmsg ~= " (" ~ имя ~ ")";
			debug
			{
	...
	body
	{
		...
		бул hfocus = вФокусе;
		HWND prevHwnd = GetWindow(cast(HANDLE) уок, GW_HWNDPREV);
		...
		if(prevHwnd)
			SetWindowPos(cast(HANDLE) уок, prevHwnd, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
		else
			SetWindowPos(cast(HANDLE) уок, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
		if(hfocus)
			выдели();
	}	
