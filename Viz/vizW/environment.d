...
export  class Среда // docmain
{
	...
	{
		return дайКоманднуюСтроку();
	}
	
	
		проц текущаяПапка(Ткст cd) // setter
	{
		if(!установиТекущуюПапку(cd))
			throw new ВизИскл("Не удаётся установить текущую папку");
	}
	
	
	Ткст текущаяПапка() // getter
	{
		return дайТекущуюПапку();
	}
	...
	{
		Ткст результат;
		результат = дайИмяКомпьютера();
		if(!результат.length)
			throw new ВизИскл("Не удаётся получить имя машины");
		return результат;
	}
	...
	ОперационнаяСистема версияОС() // getter
	{
		...
		if(!GetVersionExA(&osi))
			throw new ВизИскл("Не удаётся получить информацию о версии операционной системы");
	...
	{
		Ткст результат;
		результат = дайСистПапку();
		if(!результат.length)
			throw new ВизИскл("Не удаётся определить системную папку");
		return результат;
	}
	...
	{
		Ткст результат;
		результат = дайИмяПользователя();
		if(!результат.length)
			throw new ВизИскл("Не удаётся определить имя пользователя");
		return результат;
	}
	...
	{
		Ткст результат;
		if(!разверниСтрокиСреды(str, результат))
			throw new ВизИскл("Не удаётся развернуть переменные среды");
		return результат;
	}
	...
	{
		Ткст результат;
		результат = дайПеременнуюСреды(имя);
		if(!результат.length)
			throw new ВизИскл("Не удаётся получить переменную среды");
		return результат;
	}
...
export class Версия // docmain ?
{
	...
	this(Ткст str)
	{
		...
		switch(stuff.length)
		{
			...
			case 2:
				...
				_major = вЦел(stuff[0]);
			default:
				throw new ВизИскл("Неверный параметр версии");
		}
	}
		...
		Ткст вТкст()
	{
		...
		результат = stdrus.вТкст(_major) ~ "." ~ stdrus.вТкст(_minor);
		if(_build != -1)
			результат ~= "." ~ stdrus.вТкст(_build);
		if(_revision != -1)
			результат ~= "." ~ stdrus.вТкст(_revision);
...
export class ОперационнаяСистема // docmain
{
	export
	{
		...
		Ткст вТкст()
		{
			...
			switch(cast(DWORD)platId)
			{
				...
				default:
					throw new ВизИскл("Неизвестен ИД Платформы");
			}
