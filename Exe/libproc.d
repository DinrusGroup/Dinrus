...
проц обработай(ткст имяБ,ткст расшД = "dll", ткст путь = пусто, ткст имяД = пусто )
{
ткст[] список;
try{
...
if(создайБибИзДлл(имяБ, имяД, путь,расшД))
{
   ...
   else ошибка("Листинг файла не найден");
   
     if(естьФайл(имяБ~".lst"))список = обработатьЛистинг(имяБ);
	 else ошибка("Листинг файла не обработан");	
 }
 else ошибка("Не удалось создать библиотеку импорта");
if(список != пусто) генМакетИмпорта(имяБ, список); 
//_удалиФайл(имяБ~".lst"); 
}
catch(Исключение искл){искл.выведи();}
}	
