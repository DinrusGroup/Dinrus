...
private import gc, thread, exception, stdrus, sync, sys.WinConsts, cidrus, std.intrinsic;
...
///////////////////////////////////////////
    ...
    проц устФинализатор(ук p, gc.ФИНАЛИЗАТОР_СМ pFn)
    {
        synchronized (смЗамок)
        {
            gcx.финализатор = pFn;
...
struct Gcx
{
    ...
    gc.ФИНАЛИЗАТОР_СМ финализатор;	// финализатор function (one per СМ)
    ...
    проц устМинАдр(байт *новАдр)
    {
        минАдр = новАдр;
    }

    проц устМаксАдр(байт *новАдр)
    {
        максАдр = новАдр;
    }
...
struct Пул
{
    ...
    ббайт *дайТаблицуСтраниц()
    {
        return таблица_страниц;
    }
...
version (SENTINEL)
{
    ...
    т_мера* сентинель_размер(ук p)
    {
        return &(cast(т_мера *)p)[-2];
    }
    т_мера* sentinel_pre(ук p)
    {
        return &(cast(т_мера *)p)[-1];
    }
    ббайт* sentinel_post(ук p)
    {
        return &(cast(ббайт *)p)[*сентинель_размер(p)];
    }
...
version (Win32)
{
    ...
    т_нук эта_нить()
    {
        return cast(т_нук) cast(ук) thread.Нить.дайЭту();//ДескрТекущейНити();//stdrus.Нить.дайУкНаТекНить();
    }
