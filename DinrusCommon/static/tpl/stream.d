...
abstract class Поток :  win.ПотокВвода, win.ПотокВывода
 {
	...
	// debug скажинс(фм("вход в пишиРовно: буфер=%s; размер=%s", буфер, размер));
		for(;;) {
		  if (!размер) return;
	 ...
	 // скажинс(форматируй("направление в пишиРовно из пишиТкст: текст =%s", s));
	  ...
	  win.ПотокВывода пишификс(ИнфОТипе[] аргументы, ук argptr, цел newline=0) {
		форматДелай(&doFormatCallback,аргументы,argptr);
		if (newline) 
		  пишиСтр("");
		return this;
	  }
