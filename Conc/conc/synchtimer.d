...
enum ѕ–ежимы—инх { Ѕлок–ежим, –ежим“аймаут, };

class “естированный ласс(Cls,BuffCls)
 {
    final ткст name; 
    final Cls cls; 
    ...
    бул enabled_ = да;
    synchronized проц активен(Ѕул b) { enabled_ = b; }
...
class “аймер—инхронизации 
{


  static “естированный ласс[] классы;

  static this {
    классы ~= new “естированный ласс!(NoSynchRNG)("NoSynchronization", нет, да);
    классы ~= new “естированный ласс!(PublicSynchRNG)("PublicSynchronization", да, да);
    классы ~= new “естированный ласс!(SemRNG)("—емафор", да, да);
  }
  ...
  static ткст форматируй¬рем€(дол ns, бул showDecimal) ;
    
			  static class »нфќЌити
			  {
				final ткст name;
				final цел число;
				Ѕул активен;
				»нфќЌити(цел nthr) ;
				synchronized Ѕул дайјктив();
				synchronized проц установијктив(Ѕул v);
				synchronized проц переключијктив();
			  }

  final »нфќЌити[] инфќЌити = new »нфќЌити[nthreadsChoices.length];
  ...
  final цел tableRows = “естированный ласс.классы.length + headerRows;
  ...
  JPanel resultPanel() {
    ...
    for (цел col = 1; col < tableColumns; ++col) {
      ...
      JCheckBox tcb = new JCheckBox(инфќЌити[члонитей].name, да);
      tcb.addActionListener(new ActionListener() {
        проц actionPerformed(ActionEvent evt) {
          инфќЌити[члонитей].переключијктив();
        }});
    ...
    for (цел row = 1; row < tableRows; ++row) {
      ...
      JCheckBox cb = new JCheckBox(“естированный ласс.классы[cls].name, да); 
      cb.addActionListener(new ActionListener() {
        проц actionPerformed(ActionEvent evt) {
          “естированный ласс.классы[cls].переключијктив();
        }});
  ...
  проц setTime(final дол ns, цел clsIdx, цел nthrIdx) {
    ...
    SwingUtilities.invokeLater(new ѕускаемый() {
      проц пуск() { 
        cell.setText(форматируй¬рем€(ns, да)); 
      } 
    });
  }
  ...
  проц setChecks(final бул setting) {
    for (цел i = 0; i < “естированный ласс.классы.length; ++i) {
      “естированный ласс.классы[i].установијктив(new Ѕул(setting));
  ...
  “аймер—инхронизации() { 
    for (цел i = 0; i < инфќЌити.length; ++i) 
      инфќЌити[i] = new »нфќЌити(nthreadsChoices[i]);

  }
  
  final —инхрон÷ел nextClassIdx_ = new —инхрон÷ел(0);
  final —инхрон÷ел nextThreadIdx_ = new —инхрон÷ел(0);
  ...
  JComboBox syncModePanel() {
    ...
    for (цел j = 0; j < ѕ–ежимы—инх.length; ++j) {
      ткст lab = "Locks: " + режим¬“кст(ѕ–ежимы—инх[j]);
    ...
    syncModeComboBox.addItemListener(new ItemListener() {
      проц itemStateChanged(ItemEvent evt) {
        ...
        RNG.syncMode.установи(ѕ–ежимы—инх[инд]);
      }
    });
    
    RNG.syncMode.установи(ѕ–ежимы—инх[0]);
  ...
  JComboBox producerSyncModePanel() {
    ...
    for (цел j = 0; j < ѕ–ежимы—инх.length; ++j) {
      ткст lab = "Producers: " + режим¬“кст(ѕ–ежимы—инх[j]);
    ...
    producerSyncModeComboBox.addItemListener(new ItemListener() {
      проц itemStateChanged(ItemEvent evt) {
        JComboBox src = (JComboBox)(evt.getItemSelectable());
        цел инд = src.getSelectedIndex();
        RNG.producerMode.установи(ѕ–ежимы—инх[инд]);
      }
    });
    
    RNG.producerMode.установи(ѕ–ежимы—инх[0]);
  ...
  JComboBox consumerSyncModePanel() {
    ...
    for (цел j = 0; j < ѕ–ежимы—инх.length; ++j) {
      ткст lab = "Consumers: " + режим¬“кст(ѕ–ежимы—инх[j]);
    ...
    consumerSyncModeComboBox.addItemListener(new ItemListener() {
      проц itemStateChanged(ItemEvent evt) {
        JComboBox src = (JComboBox)(evt.getItemSelectable());
        цел инд = src.getSelectedIndex();
        RNG.consumerMode.установи(ѕ–ежимы—инх[инд]);
      }
    });
    
    RNG.consumerMode.установи(ѕ–ежимы—инх[0]);
  ...
  JComboBox capacityBox() {
    ...
    bcapComboBox.addItemListener(new ItemListener() {
      проц itemStateChanged(ItemEvent evt) {
        JComboBox src = (JComboBox)(evt.getItemSelectable());
        цел инд = src.getSelectedIndex();
        ƒефолтна€®мкость анала.установи(bufferCapacityChoices[инд]);
      }
    });
    
    
    ƒефолтна€®мкость анала.установи(bufferCapacityChoices[3]);
  ...
  private final —инхрон÷ел loopsPerTest_ = new —инхрон÷ел(0);
  ...
  */
  ...
  class TestSeries : ѕускаемый {
    ...
    проц пуск() {
      Ќить.дайЁту().setPriority(Ќить.NORM_PRIORITY);
      ...
      try {
        ...
        if (t < nthreadsChoices.length &&
            c < “естированный ласс.классы.length) {
          ...
          for (;;) {
            ...
            if (нитьјктивирована(t)) {
              ...
              “естированный ласс entry = “естированный ласс.классы[c];
            ...
            if (++c >= “естированный ласс.классы.length) {
              c = 0;
              if (++t >= nthreadsChoices.length) 
                break;
            }
  ...
  class OneTest : ѕускаемый {
    ...
    проц пуск() {
      Ќить.дайЁту().setPriority(Ќить.NORM_PRIORITY-3);

      бул wasInterrupted = нет;

      final “естированный ласс entry = “естированный ласс.классы[clsIdx];
      ...
      try {
        ...
        if (echoToSystemOut.дай()) {
          System.out.println(форматируй¬рем€(ns, да));
        }
...
class Threads : ‘абрикаЌитей {

  static final —инхрон÷ел activeThreads = new —инхрон÷ел(0);
  ...
  static { 
    pool.установи¬рем€јктивности(10000); 
    pool.установи‘абрикуЌитей(фабрика);
  }

  static class MyThread : Ќить {
    ...
    return new ѕускаемый() {
      проц пуск() {
        ...
        try {
          ...
          Ќить.дайЁту().setPriority(Ќить.NORM_PRIORITY+delta);
          ...
          for (цел i = iters; i > 0; --i) {
            ...
            if (i % itersPerBarrier == 0)
              ...
              дол rnd = r.следщ();
              if (rnd % 2 == 0 && Ќить.дайЁту().isInterrupted()) 
                break;
            }
          }
        }
...
abstract class RNG {
  ...
  const ќбъект constructionLock = RNG.class;

  // Use construction замок for all params to disable
  // changes in midst of construction of test объекты.

  const —инхрон÷ел computeLoops = 
    new —инхрон÷ел(16, constructionLock);
  const —инхрон÷ел syncMode = 
    new —инхрон÷ел(0, constructionLock);
  const —инхрон÷ел producerMode = 
    new —инхрон÷ел(0, constructionLock);
  const —инхрон÷ел consumerMode = 
    new —инхрон÷ел(0, constructionLock);
  const —инхрон÷ел bias = 
    new —инхрон÷ел(0, constructionLock);
  const SynchronizedLong таймаут = 
    new SynchronizedLong(100, constructionLock);
  const —инхрон÷ел exchangeParties = 
    new —инхрон÷ел(1, constructionLock);
  const —инхрон÷ел sequenceNumber = 
    new —инхрон÷ел(0, constructionLock);
  const —инхрон÷ел itersPerBarrier = 
    new —инхрон÷ел(0, constructionLock);
  ...
  final дол врем€ќжидани€ = таймаут.дай();
...
class GetFunction : Callable {
  ...
  ќбъект call() { return new Long(obj_.дай()); } 
}

class NextFunction : Callable {
  private final RNG obj_;
  NextFunction(RNG o) { obj_ = o;  }
  ќбъект call() { return new Long(obj_.следщ()); } 
}
...
class SyncDelegatedRNG : DelegatedRNG {
  protected final —инх cond_;
  SyncDelegatedRNG(—инх c) { 
    cond_ = c; 
  ...
  protected final проц обрети() {
    if (smode == 0) {
      ...
      while (!cond_.пытайс€(врем€ќжидани€)) {}
    }
  }
...
class FifoRNG : SyncDelegatedRNG {
  FifoRNG() { super(new —емафорѕ¬ѕ¬(1)); }
}

class PrioritySemRNG : SyncDelegatedRNG {
  PrioritySemRNG() { super(new —емафорѕриоритета(1)); }
}

class RlockRNG : SyncDelegatedRNG {
  RlockRNG() { super(new ¬озобновл€емый«амок()); }
}


class RWLockRNG : NoSynchRNG {
  protected final „««амок замок_;
  RWLockRNG(„««амок l) { 
    замок_ = l; 
  }
  ...
  protected final проц acquireR() {
    if (smode == 0) {
      замок_.замок„тени€().обрети();
    }
    else {
      while (!замок_.замок„тени€().пытайс€(врем€ќжидани€)) {}
    }
  }

  protected final проц acquireW() {
    if (smode == 0) {
      замок_.замок«аписи().обрети();
    }
    else {
      while (!замок_.замок«аписи().пытайс€(врем€ќжидани€)) {}
    }
  }
...
class WpRWlockRNG : RWLockRNG {
  WpRWlockRNG() { super(new „««амокѕредпочтени€ѕисател€()); }
}

class ReaderPrefRWlockRNG : RWLockRNG {
  ReaderPrefRWlockRNG() { 
    super(new „««амокѕредпочтени€„итател€()); 
  }
...
class FIFORWlockRNG : RWLockRNG {
  FIFORWlockRNG() { super(new „««амокѕ¬ѕ¬()); }
}


class ReentrantRWlockRNG : RWLockRNG {
  ReentrantRWlockRNG() { 
    super(new ¬озобновл€емый„««амокѕредпочтени€ѕисател€()); 
  }
...
abstract class ExecutorRNG : DelegatedRNG {
  ...
  synchronized дол следщ() { 
    дол res = 0;
    try {
      if (nextResult_ == пусто) { // direct call первое врем€ through
...
class DirectExecutorRNG : ExecutorRNG {
  DirectExecutorRNG() { 
    ...
    setExecutor(new ѕр€мой»сполнитель()); 
  }
}
...
class LockedSemRNG : ExecutorRNG {
  LockedSemRNG() { 
    setDelegate(new NoSynchRNG()); 
    setExecutor(new Ѕлокированный»сполнитель(new —емафор(1))); 
  }
}

class QueuedExecutorRNG : ExecutorRNG {
  const ќчередной»сполнитель exec = new ќчередной»сполнитель();
  static { exec.установи‘абрикуЌитей(Threads.фабрика); }
  QueuedExecutorRNG() { 
    setDelegate(new PublicSynchRNG()); 
    setExecutor(exec); 
  }
...
class ChanRNG : DelegatedRNG {
  ...
  дол producerNext( анал c) {
    ...
    if (isSingle()) {
      ...
      if (pmode == 0) {
        ...
        while (!(c.предложи(r, врем€ќжидани€))) {}
      }
    }
    return r.дай();
  }
  ...
  дол consumerNext( анал c) {
    ...
    if (cmode == 0) {
      ...
      while (r == пусто) r = (RNG)(c.запроси(врем€ќжидани€));
    }
