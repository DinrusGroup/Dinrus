...
import std.x.base64, std.x.bitarray, std.x.path, std.x.string /*rt.console*/;
...
export extern(D)
{
	...
	проц суммаМД5(ббайт[16] дайджест, проц[] данные){std.x.md5.sum(дайджест, данные);}
	проц выведиМД5Дайджест(ббайт дайджест[16]){std.x.md5.printDigest(дайджест);}
	ткст дайджестМД5вТкст(ббайт[16] дайджест){return std.x.md5.digestToString(дайджест);}
...
///////////////////////////////////////////////////////
ткст0 ю8_в_Вин16н(ткст с, бцел кодСтр = 0)
{
return cast(усим) rt.charset.toMBSz(cast(char[]) с, cast(uint) кодСтр);
}
////////////////////////////////////////////////////////////
ткст вин16н_в_Ю8(ткст0 с, цел кодСтр = 0)
{
...
export extern(D) struct МассивБит
{
  ...
  export  бул opIndex(т_мера i)
   ...
   export цел opCmp(МассивБит a2)
    ...
    проц init(проц[] v, т_мера numбиты)
    in
    {
	assert(numбиты <= v.length * 8);
	assert((v.length & 3) == 0);
    }
    body
    {
...
проц форматДелай(проц delegate(дим) putc, ИнфОТипе[] arguments, спис_ва argptr)
	...
	L1:
	     ...
	     * format specifieрс are always ASCII. This means we only need
		...
		if (c > 0x7F)		// if UTF sequence
		    goto Lошибка;	// format specifieрс can't be UTF
...
проц  вГодНедИСО8601(т_время t, out цел год, out цел неделя){ std.x.date.toISO8601YearWeek(t, год, неделя);}

цел День(т_время t)	{return cast(цел)std.x.date.floor(t, 86400000);	}
...
цел днейВГоду(цел y)	{		return 365 + std.x.date.LeapYear(y);	}

цел деньИзГода(цел y)	{		return std.x.date.DayFromYear(y);	}

т_время времяИзГода(цел y)	{		return cast(т_время) (msPerDay * std.x.date.DayFromYear(y));	}

цел годИзВрем(т_время t)	{return std.x.date.YearFromTime(cast(d_time) t);}
...
цел месИзВрем(т_время t)	{return std.x.date.MonthFromTime(cast(d_time) t);	}

цел датаИзВрем(т_время t)	{return std.x.date.DateFromTime(cast(d_time) t);	}

т_время нокругли(т_время d, цел делитель)	{	return cast(т_время) std.x.date.floor(cast(d_time) d, делитель);		}

цел дмод(т_время n, т_время d)	{   return std.x.date.dmod(n,d);	}

цел часИзВрем(т_время t)	{		return std.x.date.dmod(std.x.date.floor(t, msPerHour), HoursPerDay);	}

цел минИзВрем(т_время t)	{		return std.x.date.dmod(std.x.date.floor(t, msPerMinute), MinutesPerHour);	}

цел секИзВрем(т_время t)	{		return std.x.date.dmod(std.x.date.floor(t, TicksPerSecond), 60);	}

цел мсекИзВрем(т_время t)	{		return std.x.date.dmod(t / (TicksPerSecond / 1000), 1000);	}

цел времениВДне(т_время t)	{		return std.x.date.dmod(t, msPerDay);	}

цел ДеньНедели(т_время вр){return std.x.date.WeekDay(вр);}
т_время МВ8Местное(т_время вр){return cast(т_время) std.x.date.UTCtoLocalTime(вр);}
т_время местное8МВ(т_время вр){return cast(т_время) std.x.date.LocalTimetoUTC(вр);}
т_время сделайВремя(т_время час, т_время мин, т_время сек, т_время мс){return cast(т_время) std.x.date.MakeTime(час, мин, сек, мс);}
т_время сделайДень(т_время год, т_время месяц, т_время дата){return cast(т_время) std.x.date.MakeDay(год, месяц, дата);}
т_время сделайДату(т_время день, т_время вр){return cast(т_время) std.x.date.MakeDate(день, вр);}
//d_time TimeClip(d_time время)
цел датаОтДняНеделиМесяца(цел год, цел месяц, цел день_недели, цел ч){return  std.x.date.DateFromNthWeekdayOfMonth(год, месяц, день_недели, ч);}
цел днейВМесяце(цел год, цел месяц){return std.x.date.DaysInMonth(год, месяц);}
ткст вТкст(т_время время){return std.x.date.toString(время);}
ткст вТкстМВ(т_время время){return std.x.date.toUTCString(время);}
ткст вТкстДаты(т_время время){return std.x.date.toDateString(время);}
ткст вТкстВремени(т_время время){return std.x.date.toTimeString(время);}
т_время разборВремени(ткст т){return cast(т_время) std.x.date.parse(т);}
т_время дайВремяМВ(){return cast(т_время) std.x.date.getUTCtime();}
т_время ФВРЕМЯ8т_время(ФВРЕМЯ *фв){return cast(т_время) std.x.date.FILETIME2d_time(фв);}
т_время СИСТВРЕМЯ8т_время(СИСТВРЕМЯ *св, т_время вр){return cast(т_время) std.x.date.SYSTEMTIME2d_time(св,cast(дол) вр);}
т_время дайМестнуюЗЧП(){return cast(т_время) std.x.date.дайЛокTZA();}
цел дневноеСохранениеЧО(т_время вр){return std.x.date.DaylightSavingTA(вр);}
т_время вДвремя(ФВремяДос вр){return cast(т_время) std.x.date.toDtime(cast(DosFileTime) вр);}
ФВремяДос вФВремяДос(т_время вр){return cast(ФВремяДос) std.x.date.toDosFileTime(вр);}
...
ткст подставь(ткст текст, ткст образец, ткст формат, ткст атрибуты = null)
	{
	return std.x.regexp.sub(текст, образец, формат, атрибуты);
	}
}//end of extern C
...
export extern(D) ткст подставь(ткст текст, ткст образец, ткст delegate(РегВыр) дг, ткст атрибуты = null)
	...
	// Optimize by using std.x.string.replace if possible - Dave Fladebo
...
export extern (D)
{
    ...
    return i;
		...
		//return std.x.regexp.find(текст, образец, атрибуты);
		}

	цел найдирек(рткст текст, ткст образец, ткст атрибуты = null)
		{
		return std.x.regexp.rfind(текст, образец, атрибуты);
		}
    ...
    return результат;
		...
		//return std.x.regexp.split(текст, образец, атрибуты);
		}
...
бцел аски8гекс(дим с){return std.x.uri.ascii2hex(с);}
ткст раскодируйУИР(ткст кодирУИР){return std.x.uri.decode(кодирУИР);}
ткст раскодируйКомпонентУИР(ткст кодирКомпонУИР){return std.x.uri.decodeComponent(кодирКомпонУИР);}
ткст кодируйУИР(ткст уир){return std.x.uri.encode(уир);}
ткст кодируйКомпонентУИР(ткст уирКомпон){return std.x.uri.encodeComponent(уирКомпон);}
 ...
 export рсим[] замени(рсим[] текст, рсим[] формат)
{
	...
	// Optimize by using std.x.текст.замени if possible - Dave Fladebo
	рсим[] срез = результат[смещение + so .. смещение + eo];
	   ...
	   формат == замена)		// simple формат, not $ formats
	{
	    ...
	    результат = std.x.string.replace(результат,срез,замена);
	    break;
	}
...
{   цел srcsave;
	...
	switch (программа[pc])
	{
		...
		if (c1 != c2)
		{
			...
			c2 = std.x.ctype.toupper(cast(рсим)c2);
		    else
			goto Lnomatch;
		    if (c1 != c2)
			goto Lnomatch;
		...
		if (!(атрибуты & РВА.тчксовплф) && ввод[истк] == cast(рсим)'\n')
		    goto Lnomatch;
		истк += std.x.utf.stride(ввод, истк);
		//истк++;
		pc++;
		break;
		...
		if (истк == ввод.length)
		    goto Lnomatch;
		if (!std.x.ctype.isdigit(ввод[истк]))
		    goto Lnomatch;
		истк++;
		pc++;
		break;
...
{   бцел смещение;
	...
	case '{':	// {n} {n,} {n,m}
	    p++;
	    if (p == plength || !std.x.ctype.isdigit(образец[p]))
		goto Lerr;
	    n = 0;
	    do
	    {
		...
		if (p == plength)
		    goto Lerr;
	    } while (std.x.ctype.isdigit(образец[p]));
	    if (образец[p] == '}')		// {n}
	    {	m = n;
		goto Lnm;
	    }
	    ...
	    if (!std.x.ctype.isdigit(образец[p]))
		goto Lerr;
	    m = 0;			// {n,m}
	    do
	    {
	...
	switch (c)
	{
		...
		*cast(бцел *)&буф.данные[смещение] =
		...
		if (атрибуты & РВА.любрег)
		{
		    ...
		    {
			op = РВлсим;
			c = cast(сим)std.x.ctype.toupper(c);
		    }
		}
		if (op == РВсим && c <= 0xFF)
		{
...
export цел starrchars(Диапазон r, ббайт[] прог)
{   рсим c;
    бцел maxc;
    бцел maxb;
    бцел длин;
  ...
  //  debug(РегВыр) скажифнс("РегВыр.starrchars(прог = %p, progend = %p)\n", прог, progend);
	...
	switch (прог[i])
	{
		...
		if (c <= 0x7F)
		{   r.устбит2(c);
		    r.устбит2(std.x.ctype.tolower(cast(рсим)c));
		}
		return 1;
		...
		if (c <= 0x7F)
		{   r.устбит2(std.x.ctype.toupper(cast(рсим)c));
		    r.устбит2(std.x.ctype.tolower(cast(рсим)c));
		}
		return 1;
...
export:
   ...
   проц старт()
     ...
     * elapsed, whichever occuрс fiрсt.
     * Simply returns if thread has already terminated.
     * Выводит исключение: $(B ОшибкаНити) if the thread hasn't begun yet or
     * is called on itself.
     */
...
private
{
 ...
 /* ***********************************************
   ...
   export extern (Windows) static бцел стартнити(ук p)
     ...
     * closed.
     * Many thanks to Justin (jhenzie@mastd.x.c.com) for figuring this out
	...
	ук currentProcess = ДайТекущийПроцесс(); // http://www.digitalmaрс.com/drn-bin/wwwnews?D/21217
...
проц удалиРек(ткст имя)
{
    ...
    if (std.x.file.isdir(имя)) {
        foreach (элем; std.x.file.listdir(имя)) {
            // don't delete . or ..
            if (элем == "." ||
                элем == "..") continue;
        ...
        std.x.file.rmdir(имя);
    } else {
        std.x.file.remove(имя);
    }
}
