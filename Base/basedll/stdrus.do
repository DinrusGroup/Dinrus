...
export extern(D)
{
проц копируйФайл(ткст из, ткст в){std.file.copy(из, в);}
...
т_время времяИзГода(цел y)	{		return cast(т_время) (МсекВДень * std.date.DayFromYear(y));	}

цел годИзВрем(т_время t)	{return std.date.YearFromTime(cast(т_время) t);}

бул високосный_ли(т_время t)
	{
		if(std.date.LeapYear(std.date.YearFromTime(cast(т_время) t)) != 0)
		return да;
		else return нет;
	}

цел месИзВрем(т_время t)	{return std.date.MonthFromTime(cast(т_время) t);	}

цел датаИзВрем(т_время t)	{return std.date.DateFromTime(cast(т_время) t);	}

т_время нокругли(т_время d, цел делитель)	{	return cast(т_время) std.date.floor(cast(т_время) d, делитель);		}

цел дмод(т_время n, т_время d)	{   return std.date.dmod(n,d);	}

цел часИзВрем(т_время t)	{		return std.date.dmod(std.date.floor(t, МсекВЧасе), cast(т_время) ЧасовВДне);	}

цел минИзВрем(т_время t)	{		return std.date.dmod(std.date.floor(t, МсекВМинуте), cast(т_время) МинутВЧасе);	}

цел секИзВрем(т_время t)	{		return std.date.dmod(std.date.floor(t, ТиковВСекунду),cast(т_время) 60);	}

цел мсекИзВрем(т_время t)	{		return std.date.dmod(cast(т_время)(t / (ТиковВСекунду / 1000)),cast(т_время) 1000);	}

цел времениВДне(т_время t)	{		return std.date.dmod(t, cast(т_время) МсекВДень);	}
...
т_время разборВремени(ткст т){return cast(т_время) std.date.разбор(т);}
...
т_время СИСТВРЕМЯ8т_время(СИСТВРЕМЯ *св, т_время вр){return cast(т_время) std.date.SYSTEMTIME2d_time(св,cast(т_время) вр);}
...
{
	return cast(Дата)std.date.разборДаты(т);
}
