...
private import std.x.io, sys.WinFuncs;
private import cidrus;//, rt.console;
private import std.x.utf;
private import std.x.uni;
private import std.x.format;
private import std.x.ctype;
...
int ifind(char[] s, dchar c)
{
    ...
    if (c <= 0x7F)
    {	// Plain old ASCII
	char c1 = cast(char) std.x.ctype.tolower(c);

	foreach (int i, char c2; s)
	{
	    c2 = cast(char)std.x.ctype.tolower(c2);
	    if (c1 == c2)
		return i;
	}
    }
    ...
    {	// c is a universal character
	dchar c1 = std.x.uni.toUniLower(c);

	foreach (int i, dchar c2; s)
	{
	    c2 = std.x.uni.toUniLower(c2);
	    if (c1 == c2)
		return i;
	}
    }
...
int irfind(char[] s, char[] sub)
	...
	if (c <= 0x7F)
	{
	    c = std.x.ctype.tolower(c);
	    for (int i = s.length - sub.length; i >= 0; i--)
	    {
		if (std.x.ctype.tolower(s[i]) == c)
		{
...
string tolower(string s)
{
	...
	else if (c > 0x7F)
	{
	    ...
	    {
		if (std.x.uni.isUniUpper(dc))
		{
		    dc = std.x.uni.toUniLower(dc);
		    if (!changed)
		    {
			r = s[0 .. i + j].dup;
			changed = 2;
...
string toupper(string s)
{
	...
	else if (c > 0x7F)
	{
	    foreach (size_t j, dchar dc; s[i .. length])
	    {
		if (std.x.uni.isUniLower(dc))
		{
		    dc = std.x.uni.toUniUpper(dc);
		    if (!changed)
		    {
			r = s[0 .. i + j].dup;
			changed = 2;
...
{   auto len = s.length;
	...
	while ((s[len - 1] & 0xC0) == 0x80)
	{
	    len--;
	    if (len == 0)
		throw new std.x.utf.ИсклКодировки("неверная последовательность UTF", 0);
	}
...
{   size_t i;
	...
	{
	    if (i >= s.length)
		onArrayBoundsError("std.x.string.sformat", cast(ulong) 0);
	    s[i] = cast(char)c;
	    ++i;
	}
	else
...
unittest
{
    debug(string) printf("std.x.string.format.unittest\n");
...
unittest
{
    debug(string) printf("std.x.string.inPattern.unittest\n");
...
unittest
{
    debug(string) printf("std.x.string.count.unittest\n");
...
unittest
{
    debug(string) printf("std.x.string.remove.unittest\n");
...
unittest
{
    debug(string) printf("std.x.string.squeeze.unittest\n");
    char[] s,r;
...
unittest
{
    debug(string) printf("std.x.string.succ.unittest\n");
...
char[] tr(char[] str, char[] from, char[] to, char[] modifiers = null)
{
	...
	{   dchar t = std.x.utf.decode(to, i);
	    if (t == '-' && lastt != dchar.init && i < to.length)
	    {
		nextt = std.x.utf.decode(to, i);
	...
	if (mod_s && m && newc == lastc)
	    continue;
	std.x.utf.encode(результат, newc);
	m = 1;
	lastc = newc;
	continue;
...
unittest
{
    debug(string) printf("std.x.string.tr.unittest\n");
...
final bool isNumeric(in char[] s, in bool bAllowSep = false)
{
    ...
    bool   bComplex = false;
    char[] sx = std.x.string.tolower(s); 
    int    j  = 0;
    char   c;
...
bool isNumeric(TypeInfo[] _arguments, va_list _argptr)
{
    ...
    if (_arguments[0] == typeid(char[]))
        ...
        return isNumeric(std.x.utf.toUTF8(va_arg!(wchar[])(_argptr)));
    else if (_arguments[0] == typeid(dchar[]))
        return isNumeric(std.x.utf.toUTF8(va_arg!(dchar[])(_argptr)));
    ...
    else if (_arguments[0] == typeid(wchar))
    {
        ...
        return isNumeric(std.x.utf.toUTF8(ws));
    }
    else if (_arguments[0] == typeid(dchar))
    { 
        ds.length =  1;
        ds[0] = va_arg!(dchar)(_argptr);
        return isNumeric(std.x.utf.toUTF8(ds));
    }
...
unittest
{
    ...
    assert(isNumeric(std.x.string.toString(real.nan)) == true);
    assert(isNumeric(std.x.string.toString(-real.infinity)) == true);
    assert(isNumeric(std.x.string.toString(123e+2+1234.78Li)) == true);
 ...
 * 
 *    auto abbrevs = std.x.string.abbrev(list);
 * 
...
char[][char[]] abbrev(char[][] values)
{
	...
	for (size_t j = 0; j < value.length; j += std.x.utf.stride(value, j))
	{   char[] v = value[0 .. j];
...
char[] isURL(char[] s)
{
    ...
    //writefln("isURL(%s)", s);
    if (s.length > 7 && std.x.string.icmp(s[0 .. 7], "http://") == 0)
	i = 7;
    else if (s.length > 8 && std.x.string.icmp(s[0 .. 8], "https://") == 0)
	i = 8;
//    if (icmp(s[0 .. 4], "www.") == 0)
//	i = 4;
    else
