...
private import std.x.path;
private import std.x.string;
private import std.x.regexp;
...
void[] read(char[] name)
{
    ...
    if (useWfuncs)
    {
	wchar* namez = std.x.utf.toUTF16z(name);
...
void rename(char[] from, char[] to)
{
    ...
    if (useWfuncs)
	результат = MoveFileW(std.x.utf.toUTF16z(from), std.x.utf.toUTF16z(to));
    else
...
ulong getSize(char[] name)
{
	...
	findhndl = FindFirstFileW(std.x.utf.toUTF16z(name), &filefindbuf);
...
void getTimes(char[] name, out d_time ftc, out d_time fta, out d_time ftm)
{
	...
	findhndl = НайдиПервыйФайл(std.x.utf.toUTF16(name), &filefindbuf);
	ftc = std.x.date.FILETIME2d_time(&filefindbuf.времяСоздания);
	fta = std.x.date.FILETIME2d_time(&filefindbuf.времяПоследнегоДоступа);
	ftm = std.x.date.FILETIME2d_time(&filefindbuf.времяПоследнейЗаписи);
    }
    else
    {
	ПДАН_А filefindbuf;
...
int exists(char[] name)
{
	...
	результат = GetFileAttributesW(std.x.utf.toUTF16z(name));
    else
...
{   BOOL результат;
    ...
    if (useWfuncs)
	результат = SetCurrentDirectoryW(std.x.utf.toUTF16z(pathname));
    else
...
{   BOOL результат;
    ...
    if (useWfuncs)
	результат = CreateDirectoryW(std.x.utf.toUTF16z(pathname), null);
    else
...
{   BOOL результат;
    ...
    if (useWfuncs)
	результат = RemoveDirectoryW(std.x.utf.toUTF16z(pathname));
    else
...
char[] getcwd()
{
	...
	if (!len)
	    goto Lerr;
	return std.x.utf.toUTF8(dir[0 .. len]); // leave off terminating 0
    }
    else
    {
	char c;
...
struct DirEntry
{
	...
	name = std.x.path.join(path, std.x.utf.toUTF8(wbuf[0 .. wlength]));

	size = (cast(ulong)fd.размерФайлаВ << 32) | fd.размерФайлаН;
	creationTime = std.x.date.FILETIME2d_time(&fd.времяСоздания);
	lastAccessTime = std.x.date.FILETIME2d_time(&fd.времяПоследнегоДоступа);
	lastWriteTime = std.x.date.FILETIME2d_time(&fd.времяПоследнейЗаписи);
	...
	name = std.x.path.join(path, std.x.utf.toUTF8(fd.имяФайла[0 .. clength]));
	size = (cast(ulong)fd.размерФайлаВ << 32) | fd.размерФайлаН;
	creationTime = std.x.date.FILETIME2d_time(&fd.времяСоздания);
	lastAccessTime = std.x.date.FILETIME2d_time(&fd.времяПоследнегоДоступа);
	lastWriteTime = std.x.date.FILETIME2d_time(&fd.времяПоследнейЗаписи);
 ...
 *		$(LINK2 std_path.html, std.x.path).
...
{   string[] результат;
	...
	{   if (std.x.path.fnmatch(de.name, pattern))
		результат ~= de.name;
	}
	return true; // continue
    }
 ...
 *    bool listing(string filename)
 *    {
 *      результат ~= std.x.path.join(pathname, filename);
 *      return true; // continue
 *    }
 *
 *    listdir(pathname, &listing);
...
void listdir(string pathname, bool delegate(string filename) callback)
{
    ...
    {
	return callback(std.x.path.getBaseName(de.name));
    }
...
void listdir(string pathname, bool delegate(DirEntry* de) callback)
{
    ...
    c = std.x.path.join(pathname, "*.*");
    if (useWfuncs)
    {
	ПДАН fileinfo;

	h = НайдиПервыйФайл(std.x.utf.toUTF16(c), &fileinfo);
...
void copy(string from, string to)
{
    ...
    if (useWfuncs)
	результат = CopyFileW(std.x.utf.toUTF16z(from), std.x.utf.toUTF16z(to), false);
    else
...
class ФайлИскл : Exception
{
	...
	this(name, std.x.string.toString(s).dup);
	this.errno = errno;
    }
}
...
void getTimes(string name, out d_time ftc, out d_time fta, out d_time ftm)
{
    ...
    version (linux)
    {
	ftc = cast(d_time)statbuf.st_ctime * std.x.date.TicksPerSecond;
	fta = cast(d_time)statbuf.st_atime * std.x.date.TicksPerSecond;
	ftm = cast(d_time)statbuf.st_mtime * std.x.date.TicksPerSecond;
    }
    else version (OSX)
    {	// BUG: should add in tv_nsec field
	ftc = cast(d_time)statbuf.st_ctimespec.tv_sec * std.x.date.TicksPerSecond;
	fta = cast(d_time)statbuf.st_atimespec.tv_sec * std.x.date.TicksPerSecond;
	ftm = cast(d_time)statbuf.st_mtimespec.tv_sec * std.x.date.TicksPerSecond;
    }
...
alias name имя;
	...
	name = std.x.path.join(path, fd.d_name[0 .. len]);
	d_type = fd.d_type;
	...
	version (linux)
	{
	    _creationTime = cast(d_time)statbuf.st_ctime * std.x.date.TicksPerSecond;
	    _lastAccessTime = cast(d_time)statbuf.st_atime * std.x.date.TicksPerSecond;
	    _lastWriteTime = cast(d_time)statbuf.st_mtime * std.x.date.TicksPerSecond;
	}
	else version (OSX)
	{
	    _creationTime =   cast(d_time)statbuf.st_ctimespec.tv_sec * std.x.date.TicksPerSecond;
	    _lastAccessTime = cast(d_time)statbuf.st_atimespec.tv_sec * std.x.date.TicksPerSecond;
	    _lastWriteTime =  cast(d_time)statbuf.st_mtimespec.tv_sec * std.x.date.TicksPerSecond;
	}
	else version (FreeBSD)
	{
	    _creationTime =   cast(d_time)statbuf.st_ctimespec.tv_sec * std.x.date.TicksPerSecond;
...
unittest
{
    ...
    {
	auto s = std.x.string.format("%s : c %s, w %s, a %s", de.name,
