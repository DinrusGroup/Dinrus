 ...
 *
 * std.x.regexp is designed to work only with valid UTF strings as input.
 * To validate untrusted input, use std.x.utf.validate().
 *
...
char[] sub(char[] string, char[] pattern, char[] delegate(RegExp) дг, char[] attributes = null)
{
	    ...
	    pattern == slice)				// simple pattern (exact match, no special characters) 
	{
	    ...
	    результат = std.x.string.replace(результат,slice,replacement);
	    break;
	}
...
unittest
{
    ...
    auto s = sub("Strap a rocket engine on a chicken.",
		 "[ar]",
	         delegate char[] (RegExp m)
	         {
		    return std.x.string.toupper(m.match(0));
	         },
	         "g");
 ...
 * ---
 * auto s = "abcabcabab";
 * std.x.regexp.find(s, "b");    // match, returns 1
 * std.x.regexp.find(s, "f");    // no match, returns -1
 * ---
 */
 ...
 * ---
 * auto s = "abcabcabab";
 * std.x.regexp.find(s, "b");    // match, returns 9
 * std.x.regexp.find(s, "f");    // no match, returns -1
 * ---
 */
 ...
 * void main()
 * {
 *     if (auto m = std.x.regexp.search("abcdef", "c"))
 *     {
...
unittest
{
    ...
    if (auto m = std.x.regexp.search("abcdef", "c()"))
    {
	auto результат = std.x.string.format("%s[%s]%s", m.pre, m.match(0), m.post);
    ...
    if (auto n = std.x.regexp.search("abcdef", "g"))
    {
	assert(0);
    }
}
...
alias replaceOld замениСтарый;
	...
	foreach(m; RegExp("ab").search("abcabcabab"))
	{
	    auto s = std.x.string.format("%s[%s]%s", m.pre, m.match(0), m.post);
...
unittest
{
    ...
    assert(результат.length == 2);
    i = std.x.string.cmp(результат[0], "a");
    assert(i == 0);
    i = std.x.string.cmp(результат[1], "b");
    assert(i == 0);

    r = new RegExp("a*", null);
    результат = r.split("ab");
    assert(результат.length == 2);
    i = std.x.string.cmp(результат[0], "");
    assert(i == 0);
    i = std.x.string.cmp(результат[1], "b");
    assert(i == 0);
    ...
    j = join(результат, ",");
    //эхо("j = '%.*s'\n", j);
    i = std.x.string.cmp(j, "a,,b,font,/,b,bar,,TAG,hello,/,TAG,");
    assert(i == 0);

    r = new RegExp("a[bc]", null);
    результат = r.match("123ab");
    j = join(результат, ",");
    i = std.x.string.cmp(j, "ab");
    assert(i == 0);
    
    результат = r.match("ac");
    j = join(результат, ",");
    i = std.x.string.cmp(j, "ac");
    assert(i == 0);
}
...
unittest
{
    ...
    i = std.x.string.cmp(результат, "1xaby2xacy3");
    assert(i == 0);

    r = new RegExp("ab", "g");
    результат = r.replace("1ab2ac3", "xy");
    i = std.x.string.cmp(результат, "1xy2ac3");
    assert(i == 0);
}
...
void main()
{
  auto x = grep(&RegExp("[Ff]oo").test,
                std.x.string.split("mary had a foo lamb"));
  writefln(x);
}
---
 * which prints: 1
...
{   int srcsave;
	...
	switch (program[pc])
	{
		...
		if (c1 != c2)
		{
			...
			c2 = std.x.ctype.toupper(cast(rchar)c2);
		    else
			goto Lnomatch;
		    if (c1 != c2)
			goto Lnomatch;
		...
		if (!(attributes & REA.dotmatchlf) && input[src] == cast(rchar)'\n')
		    goto Lnomatch;
		src += std.x.utf.stride(input, src);
		//src++;
		pc++;
		break;
	...
	switch (c)
	{
		...
		*cast(uint *)&buf.data[offset] =
		...
		if (attributes & REA.ignoreCase)
		{
		    ...
		    {
			op = REichar;
			c = cast(char)std.x.ctype.toupper(c);
		    }
		}
		if (op == REchar && c <= 0xFF)
		{
...
{   rchar c;
	...
	switch (prog[i])
	{
		...
		if (c <= 0x7F)
		{   r.setbit2(c);
		    r.setbit2(std.x.ctype.tolower(cast(rchar)c));
		}
		return 1;
		...
		if (c <= 0x7F)
		{   r.setbit2(std.x.ctype.toupper(cast(rchar)c));
		    r.setbit2(std.x.ctype.tolower(cast(rchar)c));
		}
		return 1;
