...
module std.x.demangle;

//debug=demangle;		// uncomment to turn on debugging эхо's

private import std.x.ctype, sys.WinFuncs;
...
string demangle(string name)
{
    ...
    {
	if (!std.x.ctype.isxdigit(c)){номош =1;
	    error();}
	return cast(ubyte)
	      ( (c >= 'a') ? c - 'a' + 10 :
		(c >= 'A') ? c - 'A' + 10 :
	...
	while (ni < name.length && std.x.ctype.isdigit(name[ni]))
	{   int i = name[ni] - '0';
