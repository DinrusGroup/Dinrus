...
private import std.x.io, cidrus;
private import std.x.dateparse;
...
цел День(д_время t)
	{
		return cast(int)std.x.date.floor(t, msPerDay);
	}
 ...
 * ------------------------------------
  ...
  lNow = std.x.date.getUTCtime();
  // Convert this into the local дата and time for display.
  lNowString = std.x.date.toString(lNow);
 * ------------------------------------
 */
...
d_time toDtime(DosFileTime time)
{
    ...
    t = std.x.date.MakeDate(std.x.date.MakeDay(year, month, dayofmonth),
	    std.x.date.MakeTime(hour, minute, second, 0));
