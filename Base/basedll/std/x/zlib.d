...
unittest
{
    ...
    debug(zlib) эхо("D.zlib.std.x.crc32.unittest\n");
...
private import std.x.io;
private import std.x.random;

unittest // by Dave
{
    debug(zlib) эхо("std.x.zlib.unittest\n");

    bool CompressThenUncompress (ubyte[] src)
    {
      try {
	ubyte[] dst = cast(ubyte[])std.x.zlib.compress(cast(void[])src);
	...
	uncompressedBuf = cast(ubyte[])std.x.zlib.uncompress(cast(void[])dst);
    ...
    debug(zlib) эхо("PASSED std.x.zlib.unittest\n");
}
