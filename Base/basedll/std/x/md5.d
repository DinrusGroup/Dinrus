 ...
 * это 16-байтные количества, подобные контрольным суммам (checksum)
 * либо crc, но более громоздкие
 *
 * Есть два способа. Первый всё выполяет вызовом одной функции
 * sum(). Второй используется для буферированных данных.
 *
 * Bugs:
...
void MDFile(char[] filename)
{
    ...
    if ((file = fopen(std.x.string.toStringz(filename), "rb")) == null)
...
unittest
{
    debug(md5) эхо("std.x.md5.unittest\n");
