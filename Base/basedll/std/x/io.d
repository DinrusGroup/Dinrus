...
void writefx(фук fp, TypeInfo[] arguments, void* argptr, int newline=false)
{
	...
	void putc(dchar c)
	{
	    ...
	    {   char[4] buf;
		auto b = std.x.utf.toUTF8(buf, c);
		for (size_t i = 0; i < b.length; i++)
		    FPUTC(b[i], fp);
	    }
	}

	std.x.format.doFormat(&putc, arguments, argptr);
	if (newline)
	    FPUTC('\n', fp);
    }
	...
	std.x.format.doFormat(&putcw, arguments, argptr);
	if (newline)
	    FPUTWC('\n', fp);
    }
}
...
size_t readln(фук fp, inout ткст buf)
{
	...
	{   /* Stream is in wide characters.
		...
		for (int c; (c = FGETWC(fp)) != -1; )
		{
			...
			std.x.utf.encode(buf, cast(dchar)c);
		    if (c == '\n')
			break;
		}
		if (ошфл(fp))
