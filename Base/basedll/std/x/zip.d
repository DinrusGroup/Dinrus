...
alias expandedData расжатыеДанные;
    ...
    std.x.date.DosFileTime time;	/// Read/Write: Last modified time of the member. It's in the DOS дата/time format.
...
alias expand развернуть;
     ...
     * For each ArchiveMember, fills in properties std.x.crc32, compressedSize,
     * compressedData[].
     *
	...
	foreach (ArchiveMember de; directory)
	{
		...
		case 8:
		    de.compressedData = cast(ubyte[])std.x.zlib.compress(cast(void[])de.expandedData);
	    ...
	    de.crc32 = std.x.zlib.crc32(0, cast(void[])de.expandedData);
     ...
     * std.x.crc32, compressedSize, expandedSize, compressedData[], diskNumber,
     ...
     * std.x.crc32, compressedSize, expandedSize, expandedData[], name[], extra[].
     */
	...
	switch (de.compressionMethod)
	{
		...
		// and 4 byte trailer.
		de.expandedData = cast(ubyte[])std.x.zlib.uncompress(cast(void[])de.compressedData, de.expandedSize, -15);
		return de.expandedData;
