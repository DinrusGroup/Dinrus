...
//конкретные указатели на конкретную область памяти, в которой располагается наша консоль).			
      ...
      НАЧАТРЫКОНСОЛИ = инфОбКонсБуф.атрибуты;
//============================================================
...
/*******************
         * Включить/выключить интенсивность.
         * Параметры:
         *      яркий = включить
         */
     export extern(C)   проц устЯркостьЦветаКонсоли(бул яркий)
        {
            УстановиАтрибутыТекстаКонсоли(КОНСВЫВОД, cast(ПТекстКонсоли)( инфОбКонсБуф.атрибуты | (яркий ? ПТекстКонсоли.ИнтПП : 0)));
        }
      ...
      export extern(C)  проц устЦветКонсоли(Цвет цвет)
         ...
         */
        export extern(C) проц сбросьЦветКонсоли()
        {
          установиАтрыКонсоли(НАЧАТРЫКОНСОЛИ);
        }
