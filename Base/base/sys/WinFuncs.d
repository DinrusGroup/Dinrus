...
бул УстановиТекущуюПапку(ткст путь)
	{
	return cast(бул) SetCurrentDirectoryW(toUTF16z(путь));
	}
////////////////////
...
бцел ДайТекущуюПапку(бцел длинаБуфера, ткст буф)
	{
...
ук НайдиПервыйФайл(in ткст ф, sys.WinStructs.ПДАН* данныеПоискаФайла)
	{
...
бцел ДайАтрибутыФайла(in ткст ф)
	{
...
бцел ДайИмяФайлаМодуля(экз модуль, ткст фимя, бцел размер)
	{
	return cast(бцел)  GetModuleFileNameW(cast(HMODULE) модуль, cast(LPWSTR) toUTF16z(фимя), cast(DWORD) размер);
	}
/////////////////
...
{ 
return cast(экз)GetModuleHandleW(cast(LPCWSTR) toUTF16z(имя));
}
///////////////////////
...
ук ЗагрузиБиблиотеку(ткст фимя){ return cast(ук) LoadLibraryW(cast(LPCWSTR) toUTF16z(фимя));}
//////////////////
...
ук ЗагрузиБиблиотекуДоп(ткст фимя, ук файл, ПЗагрФлаг флаги){ return cast(ук) LoadLibraryExW(cast(LPCWSTR) toUTF16z(фимя), cast(HANDLE)файл, cast(DWORD) флаги);}
///////////////////
...
цел ОкноСооб(ук окно, ткст текст, ткст заголовок, ПСооб тип)
	{
	return cast(цел)  MessageBoxW(cast(HWND) окно, toUTF16z(текст), cast(LPCWSTR) toUTF16z(заголовок), cast(UINT) тип);
	}
...
цел ОкноСообДоп(ук окно, ткст текст, ткст заголовок, ПСооб тип, бкрат идЯзыка)
	{
	return cast(цел)  MessageBoxExW(cast(HWND) окно, toUTF16z(текст), cast(LPCWSTR) toUTF16z(заголовок), cast(UINT) тип, cast(WORD) идЯзыка);
	}
...
бцел ФорматируйСооб(ПФорматСооб флаги, ук исток, бцел идСооб, бцел идЯз, ткст буф, бцел разм, ук* арги)
	{
	return cast(бцел) FormatMessageW(cast(DWORD) флаги, cast(LPCVOID) исток, cast(DWORD) идСооб, cast(DWORD) идЯз, toUTF16z(буф), cast(DWORD) разм, cast(ук*) арги);
	}
...
ук ОткройМаппингФайла(ППамять желДоступ, бул наследовать, ткст имяМаппинга){return cast(ук) OpenFileMappingW(cast(DWORD) желДоступ, cast(BOOL) наследовать, cast(LPCWSTR) toUTF16z(имяМаппинга));}
...
бцел ДайТипФайла(ук файлУк){return cast(бцел)  GetFileType(cast(HANDLE) файлУк);}
 ...
 бул ЗавершиРис(ук ок, РИССТРУКТ * рис){return cast(бул) EndPaint(cast(HWND) ок, cast(LPPAINTSTRUCT) рис);} 
  ...
  бул ПерерисуйОкно(ук ок, ПРЯМ *обн, ук ргнОб, бцел фОкПерерис){return cast(бул) RedrawWindow(cast(HWND) ок, cast(LPRECT) обн, cast(HRGN) ргнОб, cast(UINT) фОкПерерис);}
	...
	бул СоздайПроцессПользователя(ук токен, ткст назвПрил, ткст комСтр, БЕЗАТРЫ* атрыПроц, БЕЗАТРЫ* атрыНити, бул наследоватьДескрипторы, ПФлагСоздПроц создПроцФлаги, ук блокСреды, ткст текПап, ИНФОСТАРТА* стартИнф, ИНФОПРОЦ* инфОПроц)
	{
	return cast(бул) CreateProcessAsUserW(cast(HANDLE) токен, cast(LPCWSTR) toUTF16z(назвПрил), cast(LPWSTR) toUTF16z(комСтр), cast(LPSECURITY_ATTRIBUTES) атрыПроц, cast(LPSECURITY_ATTRIBUTES) атрыНити, cast(BOOL) наследоватьДескрипторы, cast(DWORD) создПроцФлаги, cast(LPVOID) блокСреды, cast(LPCWSTR) toUTF16z(текПап), cast(LPSTARTUPINFO) стартИнф, cast(LPPROCESS_INFORMATION) инфОПроц);
	}
	
	бцел ВерсияПостройкиКо()	{	return cast(бцел) CoBuildVersion();	}
	
	цел ТкстИзГУИД2(sys.WinStructs.ГУИД *удгуид, ткст уш, цел кбМакс)
	{
	return cast(цел) StringFromGUID2(cast(GUID*) удгуид, cast(LPOLESTR) toUTF16z(уш), cast(цел) кбМакс);
	}
...
бул УстановиЗагКонсоли(ткст загКонсоли){return cast(бул) SetConsoleTitleW(cast(LPCWSTR) toUTF16z(загКонсоли));}
