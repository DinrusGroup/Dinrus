...
import /*rt.console*/ std.string: вТкст0, вТкст;
...
т_мера читайфл(ук указат, т_мера размер, т_мера nmemb, фук поток){return fread(указат, размер, nmemb, cast(FILE*)поток);}
т_мера пишифл(in ук указат, т_мера размер, т_мера nmemb, фук поток){return fwrite(указат, размер, nmemb, cast(FILE*)поток);}
...
ук   бпоиск(in ук key, in ук ова, т_мера члочленов, т_мера разм, цел function(in проц*, in проц*) compar){return bsearch(key, ова, члочленов, разм, compar);}
проц    бсорт(ук ова, т_мера члочленов, т_мера разм, цел function(in проц*, in проц*) compar){ qsort(ова, члочленов, разм, compar);}
...
version( DigitalMars )
{
    ук разместа(т_мера разм){return alloca(разм);} // non-standard
}
...
ук ищисим(in ук строка, цел симв, т_мера члобайт)
    ...
    ббайт *cs_ = cast(ббайт*) строка;
    ббайт c_ = cast(ббайт) симв;
    т_мера i;

    for (i = 0; i < члобайт; ++i)
    {
	if (cs_[i] == c_) return cast(ук) (cs_ + i);
    }
...
{
	//version(useSNN)
	return cast(ук) memcpy(приёмник, источник, cast(size_t) члобайт);
...
ук перембуф(ук куда, in ук откуда, т_мера сколько)
	{
	return cast(ук) memmove(куда, откуда, cast(size_t) сколько);
	/+try
	{
...
{
return cast(ук) _beginthread(адр, размстэка, аргспис);
}
...
{
return cast(ук) _beginthreadex(безоп, размстэка, cast(stdfp) адр, аргспис, иницфлаг, адрнити);
}
