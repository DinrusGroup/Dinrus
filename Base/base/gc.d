...
//////////////////////////
     ...
     */
//extern (C) проц rt_attachDisposeEvent(Объект h, ДСобыт e);
//extern (C) проц rt_detachDisposeEvent(Объект h, ДСобыт e);
    ...
    export extern (D) проц удалиСлабУк( ук p )
    {
        if (p)
           ...
           locked!(проц)({
                   if (wp.ссылка)
                       rt_detachDisposeEvent(wp.ссылка, &wp.ondestroy);
                  });
           cidrus.освободи(wp);
           }
    }
   ...
   static Бины найдиБин(т_мера размер)
    {
	  static байт[2049] binTable =cast(байт[2049]) ctfeBins();
        ...
        return   cast(Бины)( размер <= 2048) ? cast(Бины) binTable[размер] :  cast(Бины) Б_СТР;
    }
