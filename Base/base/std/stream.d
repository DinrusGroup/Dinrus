module std.stream;
import win, exception, std.string, std.intrinsic, std.utf,tpl.stream: Поток, ТПотокМассив;
import sys.WinStructs, sys.WinConsts, sys.WinFuncs, cidrus;
...
export:
  ...
  override т_мера читайБлок(ук буфер, т_мера размер) {
    auto разм = размер;
        проверьЧитаемость();
          ЧитайФайл(файлУк, буфер, разм, &разм, cast(АСИНХРОН*) null);
      читатьдоКФ(размер == 0);
    return разм;
  }
   override т_мера пишиБлок(ук буфер, т_мера размер) {
    проверьЗаписываемость(this.toString());
      ПишиФайл( файлУк, буфер, размер, &размер, null);
    return размер;
  }
...
export extern (D) class ФильтрПоток : Поток
 {
  ...
  export:
   ...
   бул закрытьИсток(){return закрытьГнездо;}
    ...
    т_мера рез = п.читайБлок(буфер,размер);
    читатьдоКФ(рез == 0);
    return рез;
    }
    ...
    // пиши to исток
    override т_мера пишиБлок(ук буфер, т_мера размер) {
    return п.пишиБлок(буфер,размер);
    }
...
export:
  ...
  override т_мера читайБлок(ук результат, т_мера длин) {
    ...
    ббайт* outbuf = cast(ббайт*)результат;
    т_мера readsize = 0;
  ...
  override т_мера пишиБлок(ук результат, т_мера длин) {
    ...
    ббайт* буф = cast(ббайт*)результат;
    т_мера writesize = 0;
...
//  win.скажинс("Вход в конструктор БуфФайла");
 ...
 // this.п = super.п;
  ...
  this(ук  файлУк, ПРежимФайла режим, бцел размбуфа) {
    super(new Файл(файлУк ,режим),размбуфа);
  //this.п = super.п;
  }
...
export:
  ...
  скажинс(фм("Установленная эндианность потока: "~эн));
  }
   ...
   */
   проц фиксируйПБ(ук буфер, бцел размер) {
    if (эндиан != _эндиан) {
      ббайт* startb = cast(ббайт*)буфер;
      бцел* старт = cast(бцел*)буфер;
      switch (размер) {
      case 0: break;
      case 2: {
  ббайт x = *startb;
