module std.socket;
import sys.WinStructs, sys.WinFuncs, cidrus, sys.WinConsts, tpl.stream, exception;
...
проц слушай(цел backlog)
	{
		if(-1 == sys.WinFuncs.слушай(cast(СОКЕТ) сок, backlog))
...
Сокет прими()
	{
		...
		alias sys.WinFuncs.пусти topaccept;
	...
	{
		sys.WinFuncs.экстрзак(cast(СОКЕТ) сок,  how);
	}
...
Адрес локальныйАдрес()
	...
	цел шли(проц[] буф, ПФлагиСокета флаги)
	{
		...
		цел sent = sys.WinFuncs.шли(сок, буф.ptr, буф.length, флаги);
		return sent;
	}
	...
	{
                флаги |= ПФлагиСокета.БезСигнала;
		цел sent = sys.WinFuncs.шли_на(cast(СОКЕТ) сок, буф.ptr, буф.length, флаги,cast(адрессок*) to.имя(), to.длинаИм());
		return sent;
	}
	...
	{
                флаги |= ПФлагиСокета.БезСигнала;
		цел sent = sys.WinFuncs.шли_на(cast(СОКЕТ) сок, буф.ptr, буф.length, флаги, null, нет);
		return sent;
	}
	...
	цел получи(проц[] буф, ПФлагиСокета флаги)
	{
		if(!буф.length) //return 0 and don't think the connection closed
			return 0;
		цел read = sys.WinFuncs.прими(cast(СОКЕТ) сок, буф.ptr, буф.length, флаги);
	...
	цел получи_от(проц[] буф, ПФлагиСокета флаги, out Адрес от)
	{
		...
		цел read = sys.WinFuncs.прими_от(cast(СОКЕТ) сок, буф.ptr, буф.length, флаги, cast(адрессок*) от.имя(), &длинаИм);
...
static цел выбери(НаборСокетов checkRead, НаборСокетов checkWrite, НаборСокетов checkError, значврем* tv)
	...
	body
	{
		...
		цел результат = sys.WinFuncs.выбери(n, cast(набор_уд*) fr, cast(набор_уд*) fw, cast(набор_уд*) fe, cast(значврем*)tv);
