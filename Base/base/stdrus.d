...
цел секИзВрем(т_время t)	{		return std.date.dmod(std.date.floor(t,cast(т_время) ТиковВСекунду), cast(т_время)60);	}

цел мсекИзВрем(т_время t)	{		return std.date.dmod(t / (cast(т_время)(ТиковВСекунду / 1000)),cast(т_время) 1000);	}
...
body
{
    ...
    auto и = new ИсключениеЗагрузкиБиблиотеки("Ошибка: ");
    и.выведи(незагрБибы, причины);
    assert(0); // to shut the compiler up
}
...
export extern (D)  class ИнтернетАдрес: Адрес
{
	...
	this(ткст адр, бкрат порт)
	{
		...
		иас.адр.с_адр =cast(крат) х8сбц(cast(бцел) uiaddr);
		иас.порт = cast(крат) х8сбк(cast(uint16_t) порт);
	}

	this(бцел адр, бкрат порт)
	{
		иас.адр.с_адр =cast(крат) х8сбц(адр);
		иас.порт =cast(крат) х8сбк(порт);
	}
