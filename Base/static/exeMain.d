	...
	//	static экз g_hInst;
		...
		extern (C) цел main(цел аргчло, ткст0 *аргткст)
		{
			...
			static ткст[] арги; 
				...
				проц обрИскл2(Исключение и)
				{
				throw new Исключение("Проблема с остановкой\nрантайма в функции (ртСтоп):\n"~и.сооб, и.файл, и.строка, и.следщ, и.инфо);
				}
				...
				debug(НА_КОНСОЛЬ) скажинс("Начинается работа ртСтоп");
				ртСтоп();//&передВыходом);, &обрИскл2); //При использовании консоли из io.Console возникает ошибка доступа,
				//решение этой проблемы необходимо, но отнимает массу времени, поэтому пока этот обработчик не используется.
				}
