...
BOOL DllMain(HINSTANCE экземп, бдол резон, ук резерв)
{
	...
	ук вДанные; 
		...
		case ДЛЛ_ПРИКРЕПИ_ПРОЦЕСС:
				...
				картОбъект = СоздайМаппингФайлаА( НЕВЕРНХЭНДЛ, пусто, ППамять.СтрЗапЧтен, 0, РАЗМСОВМПАМ, "commonmem"); 
				if (картОбъект == пусто)
					{
					throw new Ошибка("Не удалось создание маппинга");				
						//return 1;
					}
				...
				if ((индексНЛХ = РазместиНлх()) == ПОшибка.НЛХВнеИндексов) 
					{
					throw new Ошибка("Выход индекса НЛХ за пределы диапазона");				
						//return 1;
					} 
