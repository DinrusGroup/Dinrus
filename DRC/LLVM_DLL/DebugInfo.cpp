...
LLEXPORT ЛЛМетаданные ЛЛПостроительОИ_СоздайВыражение(ЛЛПостроительОИ Builder,
  ...
  return LLVMDIBuilderCreateВыражение(Builder, Addr,  Length);
}
...
ЛЛПостроительОИ_СОздайВыражениеКонстЗначения(ЛЛПостроительОИ Builder,
                                           int64_t Value){
  return LLVMDIBuilderCreateConstantValueВыражение( Builder, Value);
}
/**
...
LLEXPORT ЛЛМетаданные ЛЛПостроительОИ_СоздайВыражениеГлобПеременной(
  ...
  return LLVMDIBuilderCreateGlobalVariableВыражение(Builder,  Scope, Name,
     NameLen, Linkage,  LinkLen,  File, LineNo,  Ty,  LocalToUnit,
     Expr,  Decl,  AlignInBits);
}
/**
 * Retrieves the \c DIVariable associated with this global variable Выражение.
 * \param GVE    The global variable Выражение.
 *
...
LLEXPORT ЛЛМетаданные ЛЛВыражениеГлобПеременной_ДайПеременную(ЛЛМетаданные GVE){
  return LLVMDIGlobalVariableВыражениеGetVariable(GVE);
}

/**
 * Retrieves the \c DIВыражение associated with this global variable Выражение.
 * \param GVE    The global variable Выражение.
 *
...
LLEXPORT ЛЛМетаданные ЛЛВыражениеГлобПеременной_ДайВыражение(
    ЛЛМетаданные GVE){
  return LLVMDIGlobalVariableВыражениеGetВыражение(GVE);
}
/**
 ...
 * \param Expr        A complex location Выражение for the variable.
