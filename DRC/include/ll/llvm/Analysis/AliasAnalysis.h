...
//===----------------------------------------------------------------------===//
//
// Этот файл определяет генерный интерфейс AliasAnalysis, используемый в качестве
// общего интерфейса для всех клиентов информации анализа псевдонимов, и
// реализуемый всеми имплементациями анализа псевдонимов. Информация о  Mod/Ref
// также охватывается этим интерфейсом.
//
// Имплементация этого интерфейса должна реализовывать различные виртуальные
// методы, что автоматически обеспечивает функциональность для всего набора
// клиентских API.
//
// Этот API идентифицирует регионы памяти с помощью класса MemoryLocation.
// Компонент-указатель опередляет базовый адрес памяти региона. Size определяет
// максимальный размер (в адресных единицах) региона памяти, или
// MemoryLocation::UnknownSize, если размер неизвестен. Тег TBAA
// идентифицирует "тип" ссылки на память; подробности смотрите
// в справке к классу TypeBasedAliasAnalysis.
//
