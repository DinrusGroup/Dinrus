...
byte[] NPTRSIZE() { return _tysize[TYnptr]; }
...
Symbol *except_gentables()
{
    ...
    if (config.ehmethod == EHmethod.EH_DM && !(funcsym_p.Sfunc.Fflags3 & Feh_none))
    {
        ...
        if (Alloca.size)
            выведиОшибку(null, 0, 0, "cannot mix `core.std.stdlib.alloca()` and exception handling in `%s()`", &funcsym_p.Sident[0]);

        char[13+5+1] имя = void;
        int tmpnum;
        sprintf(имя.ptr,"_HandlerTable%d",tmpnum++);

        Symbol *s = symbol_name(имя.ptr,SCstatic,tstypes[TYint]);
        symbol_keep(s);
        //symbol_debug(s);
        ...
        objmod.ehtables(funcsym_p,cast(бцел)funcsym_p.Ssize,s);
    }
    return null;
}
...
/* This is what the тип should be on the target machine, not the host compiler
 *
 * struct Guard
 * {
 *    if (EHmethod.EH_DM)
 *    {
 *        бцел смещение;            // смещение of start of guarded section (Linux)
 *        бцел endoffset;         // ending смещение of guarded section (Linux)
 *    }
 *    int last_index;             // previous index (enclosing guarded section)
 *    бцел catchoffset;           // смещение to catch block from Symbol
...
{
    бцел fsize = NPTRSIZE;             // target size of function pointer
    auto dtb = DtBuilder(0);

    /*
        void*           pointer to start of function (Windows)
        бцел            смещение of ESP from EBP
        бцел            смещение from start of function to return code
        бцел nguards;           // dimension of guard[] (Linux)
        Guard guard[];          // sorted such that the enclosing guarded sections come first
      catchoffset:
        бцел ncatches;          // number of catch blocks
        {   void *тип;         // Symbol representing тип
            бцел bpoffset;      // EBP смещение of catch variable
 ...
 */
    ...
    if (config.ehmethod == EHmethod.EH_DM)
        GUARD_SIZE = (глоб2.парамы.is64bit ? 3*8 : 5*4);
    ...
    //printf("ehtables: func = %s, смещение = x%x, startblock.Boffset = x%x\n", funcsym_p.Sident, funcsym_p.Soffset, startblock.Boffset);

    // Get смещение of ESP from EBP
    ...
    // First, calculate starting catch смещение
...
//      printf("b.BC = %2d, Bscope_index = %2d, last_index = %2d, смещение = x%x\n",
//              b.BC, b.Bscope_index, b.Blast_index, b.Boffset);
    ...
    бцел catchoffset = sz + (guarddim + ndctors) * GUARD_SIZE;

    // Generate guard[]
    int i = 0;
    foreach (b; BlockRange(startblock))
    {
        //printf("b = %p, b.Btry = %p, b.смещение = %x\n", b, b.Btry, b.Boffset);
        if (b.BC == BC_try)
        {
            ...
            if (config.ehmethod == EHmethod.EH_DM)
            {
            //printf("DHandlerInfo: смещение = %x", (int)(b.Boffset - startblock.Boffset));
            dtb.dword(cast(int)(b.Boffset - startblock.Boffset));    // смещение to start of block

            // Compute ending смещение
            бцел endoffset;
            for (block *bn = b.Bnext; 1; bn = bn.Bnext)
            {
                //printf("\tbn = %p, bn.Btry = %p, bn.смещение = %x\n", bn, bn.Btry, bn.Boffset);
                assert(bn);
                if (bn.Btry == b.Btry)
                {    endoffset = cast(бцел)(bn.Boffset - startblock.Boffset);
                     break;
                }
            }
            //printf(" endoffset = %x, prev_index = %d\n", endoffset, b.Blast_index);
            dtb.dword(endoffset);               // смещение past end of guarded block
            }
            ...
            else                                        // else try-finally
            {
                ...
                if (config.ehmethod == EHmethod.EH_DM)
                {
                    ...
                    dtb.size(bhandler.Boffset - startblock.Boffset);    // finally handler смещение
                }
                else
                    dtb.coff(cast(бцел)bhandler.Boffset);
            }
            sz += GUARD_SIZE;
        }
    }
     ...
     * within a single Выражение. These are marked by the special instruction pairs
    ...
    foreach (b; BlockRange(startblock))
    {
        ...
        бцел boffset = cast(бцел)b.Boffset;
        for (code *c = b.Bcode; c; c = code_next(c))
        {
            if (c.Iop == (ESCAPE | ESCdctor))
            {
                ...
                if (config.ehmethod == EHmethod.EH_DM)
                    dtb.dword(cast(int)(boffset - startblock.Boffset)); // guard смещение
                // Find corresponding ddtor instruction
                int n = 0;
                бцел eoffset = boffset;
                бцел foffset;
                for (; 1; c2 = code_next(c2))
                {
                    // https://issues.dlang.org/show_bug.cgi?ид=13720
                    ...
                    if (c2.Iop == (ESCAPE | ESCddtor))
                    {
                        ...
                        else
                        {
                            ...
                            //foffset += calccodsize(cf);
                            if (config.ehmethod == EHmethod.EH_DM)
                                dtb.dword(cast(int)(eoffset - startblock.Boffset)); // guard смещение
                            break;
                        }
                    }
                ...
                dtb.dword(stack[stacki - 1]);   // родитель index
                dtb.dword(0);           // no catch смещение
                if (config.ehmethod == EHmethod.EH_DM)
                {
                    assert(foffset > startblock.Boffset);
                    dtb.size(foffset - startblock.Boffset);    // finally handler смещение
                }
                else
                    dtb.coff(foffset);  // finally handler address
                if (stacki == stackmax)
                {   // stack[] is out of space; enlarge it
                    int *pi = cast(int *)Пам.check(malloc((stackmax + STACKINC) * int.sizeof));
    ...
    foreach (b; BlockRange(startblock))
    {
        if (b.BC == BC_try && b.jcatchvar)         // if try-catch
        {
            ...
            for (int j = 1; j < nsucc; ++j)
            {
                ...
                dtb.size(cod3_bpoffset(b.jcatchvar));     // EBP смещение

                // catch handler address
                if (config.ehmethod == EHmethod.EH_DM)
                {
                    assert(bcatch.Boffset > startblock.Boffset);
                    dtb.size(bcatch.Boffset - startblock.Boffset);  // catch handler смещение
                }
                else
                    dtb.coff(cast(бцел)bcatch.Boffset);
