/**
 * Compiler implementation of the
 * $(LINK2 http://www.dlang.org, D programming language).
 *
 * Copyright:   Copyright (C) 1999-2020 by The D Language Foundation, All Rights Reserved
 * Authors:     Martin Kinkelin
 * License:     $(LINK2 http://www.boost.org/LICENSE_1_0.txt, Boost License 1.0)
 * Source:      $(LINK2 https://github.com/dlang/dmd/blob/master/src/dmd/argtypes_sysv_x64.d, _argtypes_sysv_x64.d)
 * Documentation:  https://dlang.org/phobos/dmd_argtypes_sysv_x64.html
 * Coverage:    https://codecov.io/gh/dlang/dmd/src/master/src/dmd/argtypes_sysv_x64.d
 */

module dmd.argtypes_sysv_x64;

import dmd.declaration;
import dmd.globals;
import dmd.mtype;
import drc.ast.Visitor;

/****************************************************
 * This breaks a тип down into 'simpler' types that can be passed to a function
 * in registers, and returned in registers.
 * This is the implementation for the x86_64 System V ABI (not используется for Win64),
 * based on https://www.uclibc.org/docs/psABI-x86_64.pdf.
 * Параметры:
 *      t = тип to break down
 * Возвращает:
 *      кортеж of types, each element can be passed in a register.
 *      A кортеж of нуль length means the тип cannot be passed/returned in registers.
 *      null indicates a `проц`.
 */
 КортежТипов toArgTypes_sysv_x64(Тип t)
{
    if (t == Тип.terror)
        return new КортежТипов(t);

    const size = cast(т_мера) t.size();
    if (size == 0)
        return null;
    if (size > 32)
        return new КортежТипов();

    const classification = classify(t, size);
    const classes = classification.slice();
    const N = classes.length;
    const c0 = classes[0];

    switch (c0)
    {
    case Class.memory:
         return new КортежТипов();
    case Class.x87:
        return new КортежТипов(Тип.tfloat80);
    case Class.complexX87:
        return new КортежТипов(Тип.tfloat80, Тип.tfloat80);
    default:
        break;
    }

    if (N > 2 || (N == 2 && classes[1] == Class.sseUp))
    {
        assert(c0 == Class.sse);
        foreach (c; classes[1 .. $])
            assert(c == Class.sseUp);

        assert(size % 8 == 0);
        return new КортежТипов(new TypeVector(Тип.tfloat64.sarrayOf(N)));
    }

    assert(N >= 1 && N <= 2);
    Тип[2] argtypes;
    foreach (i, c; classes)
    {
        // the last eightbyte may be filled partially only
        auto sizeInEightbyte = (i < N - 1) ? 8 : size % 8;
        if (sizeInEightbyte == 0)
            sizeInEightbyte = 8;

        if (c == Class.integer)
        {
            argtypes[i] =
                sizeInEightbyte > 4 ? Тип.tint64 :
                sizeInEightbyte > 2 ? Тип.tint32 :
                sizeInEightbyte > 1 ? Тип.tint16 :
                                      Тип.tint8;
        }
        else if (c == Class.sse)
        {
            argtypes[i] =
                sizeInEightbyte > 4 ? Тип.tfloat64 :
                                      Тип.tfloat32;
        }
        else
            assert(0, "Unexpected class");
    }

    return N == 1
        ? new КортежТипов(argtypes[0])
        : new КортежТипов(argtypes[0], argtypes[1]);
}


private:

// classification per eightbyte (64-bit chunk)
enum Class : ббайт
{
    integer,
    sse,
    sseUp,
    x87,
    x87Up,
    complexX87,
    noClass,
    memory
}

Class merge(Class a, Class b)
{
    бул any(Class значение) { return a == значение || b == значение; }

    if (a == b)
        return a;
    if (a == Class.noClass)
        return b;
    if (b == Class.noClass)
        return a;
    if (any(Class.memory))
        return Class.memory;
    if (any(Class.integer))
        return Class.integer;
    if (any(Class.x87) || any(Class.x87Up) || any(Class.complexX87))
        return Class.memory;
    return Class.sse;
}

struct Classification
{
    Class[4] classes;
    цел numEightbytes;

    Class[] slice() { return classes[0 .. numEightbytes]; }
}

Classification classify(Тип t, т_мера size)
{
    scope v = new ToClassesVisitor(size);
    t.прими(v);
    return Classification(v.результат, v.numEightbytes);
}

 final class ToClassesVisitor : Визитор2
{
    const т_мера size;
    цел numEightbytes;
    Class[4] результат = Class.noClass;

    this(т_мера size)
    {
        assert(size > 0);
        this.size = size;
        this.numEightbytes = cast(цел) ((size + 7) / 8);
    }

    проц memory()
    {
        результат[0 .. numEightbytes] = Class.memory;
    }

    проц one(Class a)
    {
        результат[0] = a;
    }

    проц two(Class a, Class b)
    {
        результат[0] = a;
        результат[1] = b;
    }

    alias Визитор2.посети посети;

    override проц посети(Тип)
    {
        assert(0, "Unexpected тип");
    }

    override проц посети(TypeEnum t)
    {
        t.toBasetype().прими(this);
    }

    override проц посети(TypeBasic t)
    {
        switch (t.ty)
        {
        case Tvoid:
        case Tbool:
        case Tint8:
        case Tuns8:
        case Tint16:
        case Tuns16:
        case Tint32:
        case Tuns32:
        case Tint64:
        case Tuns64:
        case Tchar:
        case Twchar:
        case Tdchar:
            return one(Class.integer);

        case Tint128:
        case Tuns128:
            return two(Class.integer, Class.integer);

        case Tfloat80:
        case Timaginary80:
            return two(Class.x87, Class.x87Up);

        case Tfloat32:
        case Tfloat64:
        case Timaginary32:
        case Timaginary64:
        case Tcomplex32: // struct { float a, b; }
            return one(Class.sse);

        case Tcomplex64: // struct { double a, b; }
            return two(Class.sse, Class.sse);

        case Tcomplex80: // struct { real a, b; }
            результат[0 .. 4] = Class.complexX87;
            return;

        default:
            assert(0, "Unexpected basic тип");
        }
    }

    override проц посети(TypeVector t)
    {
        результат[0] = Class.sse;
        результат[1 .. numEightbytes] = Class.sseUp;
    }

    override проц посети(TypeAArray)
    {
        return one(Class.integer);
    }

    override проц посети(TypePointer)
    {
        return one(Class.integer);
    }

    override проц посети(TypeNull)
    {
        return one(Class.integer);
    }

    override проц посети(TypeClass)
    {
        return one(Class.integer);
    }

    override проц посети(TypeDArray)
    {
        if (!глоб2.парамы.isLP64)
            return one(Class.integer);
        return two(Class.integer, Class.integer);
    }

    override проц посети(TypeDelegate)
    {
        if (!глоб2.парамы.isLP64)
            return one(Class.integer);
        return two(Class.integer, Class.integer);
    }

    override проц посети(TypeSArray t)
    {
        // treat as struct with N fields

        Тип baseElemType = t.следщ.toBasetype();
        if (baseElemType.ty == Tstruct && !(cast(TypeStruct) baseElemType).sym.isPOD())
            return memory();

        classifyStaticArrayElements(0, t);
        finalizeAggregate();
    }

    override проц посети(TypeStruct t)
    {
        if (!t.sym.isPOD())
            return memory();

        classifyStructFields(0, t);
        finalizeAggregate();
    }

    проц classifyStructFields(бцел baseOffset, TypeStruct t)
    {
        extern(D) Тип getNthField(т_мера n, out бцел смещение, out бцел typeAlignment)
        {
            auto field = t.sym.fields[n];
            смещение = field.смещение;
            typeAlignment = field.тип.alignsize();
            return field.тип;
        }

        classifyFields(baseOffset, t.sym.fields.dim, &getNthField);
    }

    проц classifyStaticArrayElements(бцел baseOffset, TypeSArray t)
    {
        Тип elemType = t.следщ;
        const elemSize = elemType.size();
        const elemTypeAlignment = elemType.alignsize();

        extern(D) Тип getNthElement(т_мера n, out бцел смещение, out бцел typeAlignment)
        {
            смещение = cast(бцел)(n * elemSize);
            typeAlignment = elemTypeAlignment;
            return elemType;
        }

        classifyFields(baseOffset, cast(т_мера) t.dim.toInteger(), &getNthElement);
    }

    extern(D) проц classifyFields(бцел baseOffset, т_мера nfields, Тип delegate(т_мера, out бцел, out бцел) getFieldInfo)
    {
        if (nfields == 0)
            return memory();

        // classify each field (recursively for aggregates) and merge all classes per eightbyte
        foreach (n; new бцел[0 .. nfields])
        {
            бцел foffset_relative;
            бцел ftypeAlignment;
            Тип ftype = getFieldInfo(n, foffset_relative, ftypeAlignment);
            const fsize = cast(т_мера) ftype.size();

            const foffset = baseOffset + foffset_relative;
            if (foffset & (ftypeAlignment - 1)) // not aligned
                return memory();

            if (ftype.ty == Tstruct)
                classifyStructFields(foffset, cast(TypeStruct) ftype);
            else if (ftype.ty == Tsarray)
                classifyStaticArrayElements(foffset, cast(TypeSArray) ftype);
            else
            {
                const fEightbyteStart = foffset / 8;
                const fEightbyteEnd = (foffset + fsize + 7) / 8;
                if (ftype.ty == Tcomplex32) // may lie in 2 eightbytes
                {
                    assert(foffset % 4 == 0);
                    foreach (ref existingClass; результат[fEightbyteStart .. fEightbyteEnd])
                        existingClass = merge(existingClass, Class.sse);
                }
                else
                {
                    assert(foffset % 8 == 0 ||
                        fEightbyteEnd - fEightbyteStart <= 1,
                        "Field not aligned at eightbyte boundary but contributing to multiple eightbytes?"
                    );
                    foreach (i, fclass; classify(ftype, fsize).slice())
                    {
                        Class* existingClass = &результат[fEightbyteStart + i];
                        *existingClass = merge(*existingClass, fclass);
                    }
                }
            }
        }
    }

    проц finalizeAggregate()
    {
        foreach (i, ref c; результат)
        {
            if (c == Class.memory ||
                (c == Class.x87Up && !(i > 0 && результат[i - 1] == Class.x87)))
                return memory();

            if (c == Class.sseUp && !(i > 0 &&
                (результат[i - 1] == Class.sse || результат[i - 1] == Class.sseUp)))
                c = Class.sse;
        }

        if (numEightbytes > 2)
        {
            if (результат[0] != Class.sse)
                return memory();

            foreach (c; результат[1 .. numEightbytes])
                if (c != Class.sseUp)
                    return memory();
        }

        // Undocumented special case for aggregates with the 2nd eightbyte
        // consisting of padding only (`struct S { align(16) цел a; }`).
        // clang only passes the first eightbyte in that case, so let's do the
        // same.
        if (numEightbytes == 2 && результат[1] == Class.noClass)
            numEightbytes = 1;
    }
}
