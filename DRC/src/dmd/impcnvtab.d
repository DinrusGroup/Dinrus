...
const ENUMTY[TMAX][TMAX] impcnvрезультат = impCnvTab.impcnvрезультатTab;
const ENUMTY[TMAX][TMAX] impcnvType1 = impCnvTab.impcnvType1Tab;
const ENUMTY[TMAX][TMAX] impcnvType2 = impCnvTab.impcnvType2Tab;

private:

struct ImpCnvTab
{
    ENUMTY[TMAX][TMAX] impcnvрезультатTab;
    ENUMTY[TMAX][TMAX] impcnvType1Tab;
    ENUMTY[TMAX][TMAX] impcnvType2Tab;
}

const ImpCnvTab impCnvTab = generateImpCnvTab();
...
ImpCnvTab generateImpCnvTab()
{
    ...
    foreach (i; new бцел[0 .. cast(т_мера)TMAX])
    {
        foreach (j; new бцел[0 .. cast(т_мера)TMAX])
        {
            impCnvTab.impcnvрезультатTab[i][j] = Terror;
            impCnvTab.impcnvType1Tab[i][j] = Terror;
            impCnvTab.impcnvType2Tab[i][j] = Terror;
        }
    }

    проц X(ENUMTY t1, ENUMTY t2, ENUMTY nt1, ENUMTY nt2, ENUMTY rt)
    {
        impCnvTab.impcnvрезультатTab[t1][t2] = rt;
    ...
    foreach (i; new бцел[0 .. cast(т_мера)TMAX])
    {
        foreach (j; new бцел[0 .. cast(т_мера)TMAX])
        {
            if (impCnvTab.impcnvрезультатTab[i][j] == Terror)
            {
                impCnvTab.impcnvрезультатTab[i][j] = impCnvTab.impcnvрезультатTab[j][i];
