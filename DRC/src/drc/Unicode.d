...
/// Возвращает: "да", если этот символ не суррогатная
/// точка кода и не выше, чем 0x10FFFF.
бул верноСимвол(дим d)
{
  return d < 0xD800 || d > 0xDFFF && d <= 0x10FFFF;
}

/// Всего существует 66 несимволов.
/// Возвращает: "да", если это один из них.
/// See_also: Глава 16.7 Несимволы в Юникод 5.0
бул неСимвол(дим d)
{
...
/// Возвращает: "да", если это трейл-байт последовательности UTF-8.
бул ведомыйБайт(ббайт b)
{
  return (b & 0xC0) == 0x80; // 10xx_xxxx
}

/// Возвращает: "да", если это лид-байт последовательности UTF-8.
бул ведущийБайт(ббайт b)
{
  return (b & 0xC0) == 0xC0; // 11xx_xxxx
}

/// Передвигает ссыл_ук только тогда, если это валидный алфавитный символ Юникода.
/// Параметры:
///   ссыл_ук = установлен на последний трейл-байт валидной последовательности UTF-8.
бул юАльфа(ref ткст0 ссыл_ук, ткст0 конец)
...
/// Раскодирует символ из ткт по индексу.
/// Параметры:
///   индекс = установлен на один после ASCII сим или один после последнего трейл-байта
///           валидной последовательности UTF-8.
дим раскодируй(ткст ткт, ref т_мера индекс)
in { assert(ткт.length && индекс < ткт.length); }
out { assert(индекс <= ткт.length); }
body
{
  ткст0 у = ткт.ptr + индекс;
  ткст0 конец = ткт.ptr + ткт.length;
...
/// Раскодирует символ, начиная с ссыл_ук.
/// Параметры:
///   ссыл_ук = установлен на один после ASCII сим или один после последнего трейл-байта
///           валидной последовательности UTF-8.
дим раскодируй(ref ткст0 ссыл_ук, ткст0 конец)
in { assert(ссыл_ук && ссыл_ук < конец); }
...
body
{
  ...
  // Ошибка, если второй байт не трейл-байт.
  if (!ведомыйБайт(*у))
    return СИМ_ОШИБКИ;
  ...
  else if ((с & 0b1111_1000) == 0b1111_0000)
  {
    ...
    // 5 и 6 байты последовательности UTF-8 пока недопустимы.
...
/// Раскодирует символ из последовательности UTF-16.
/// Параметры:
///   ткт = последовательности UTF-16.
///   индекс = откуда начинать.
/// Возвращает: СИМ_ОШИБКИ в случае ошибки в последовательности.
...
/// Раскодирует символ из последовательности UTF-16.
/// Параметры:
///   у = старт последовательности UTF-16.
///   конец = один после конца последовательности.
/// Возвращает: СИМ_ОШИБКИ в случае ошибки в последовательности.
...
/// Раскодирует символ из ткст UTF-16 с нулевым окончанием.
/// Параметры:
///   у = старт последовательностиe UTF-16.
/// Возвращает: СИМ_ОШИБКИ в случае ошибки в последовательности.
