...
class Парсер
{
  ...
  Диагностика диаг;
  ОшибкаПарсера[] ошибки; /// МассивДРК сообщений об ошибках парсера.
  ...
  private alias ТОК T; /// Часто используется данным классом.
  ...
  /// Возвращает "да", если установи() было вызвано над узлом.
  ...
  бул проверено()(ТОК k) // В виде шаблона, поэтому инлайнится.
  {
                                      ...
                                      бул списокНеобязатПарамов = да)
  {
    ...
    if (проверятьДекларациюАвто && сема.вид == T.Идентификатор)
    {
      ...
      { // Проверим на шаблонную функцию с автоматическим типом возврата.
        // КлассыСохранения Имя ( СписокПараметровШаблона ) ( СписокПараметров )
        имя = сема.идент;
        auto следщ = сема;
        возьмиПосле(следщ);
        if (семаПослеСкобкиЯвляется(T.ЛСкобка, следщ))
        {
          ...
          goto LparseTPList; // Продолжить разбор шаблонной функци.
        }
      }
    }

    тип = разборТипа(); // ТипПеременная или ТипИтога
    ...
    { // Указатели на функции в стиле Си усложняют грамматику.
      ...
      // Пример:
      //   проц  foo() {
      //     // Указатель на функцию, принимающий целое число и возвращающий 'некоторый_тип'.
      //     некоторый_тип (*p_func)(цел);
      //     // В следующем случае преимущество отдаётся ВыражениеВызов.
      //     что_то(*у); // 'что_то' может быть функцией/методом или объектом,
      //  у которого есть перегруженный opCall.
      тип = разборТипаУказательНаФункциюСи(тип, имя, списокНеобязатПарамов);
    }
    else if (возьмиСледщ() == T.ЛСкобка)
    { // Тип ИмяФункции ( СписокПараметров ) ТелоФункции
      ...
      LparseTPList:
        // ( СписокПараметровШаблона ) ( СписокПараметров )
        шпарамы = разборСпискаПараметровШаблона();

      auto парамы = разборСпискаПараметров();
    version(D2)
    {
      if (шпарамы) // Если ( ВыражениеКонстрейнт )
  ...
  LparseVariables:
    // Это декларация переменных.
    Идентификатор*[] имена = [имя]; // Один идентификатор уже разобран.
    Выражение[] значения;
    goto LenterLoop; // Вход в цикл и проверка инициализатора.
  ...
  Выражение разборНеПроцИнициализатора()
  {
    ...
    case T.ЛКвСкобка:
      ...
      break;
    case T.ЛФСкобка:
      // ИнициализаторСтруктуры:
      //         { }
      //         { ИнициализацииЧленовСтруктуры }
      Выражение разборИнициализатораСтрукт()
      {
        ...
        while (сема.вид != T.ПФСкобка)
        {
          if (сема.вид == T.Идентификатор &&
              // Peek for colon, чтобы узнать, является ли он  идентификатором члена.
  ...
  ТипКомпоновки разборТипаКомпоновки()
  {
    ...
    auto семаИдент = требуетсяИд();

    ВИД видИд = семаИдент ? семаИдент.идент.видИд : ВИД.Нуль;
  ...
  Декларация разборАтрибутаСохранения()
  {
    ...
    auto сохр_классХранения = this.классХранения; // Сохранить.
    // Гнездовая функция.
    Декларация разбор()
    {
      Декларация декл;
      ...
      case T.Инвариант: // D 2.0
        ...
        if (возьмиПосле(следщ) == T.ЛСкобка)
        {
          ...
          // NB: должно быть подобно коду в конце
      ...
      case T.Масштаб:
        ...
        goto Lcommon;
      Lcommon:
        // Выдать ошибку, если повторяется.
        if (кхр & stc_tmp)
      ...
      случай_Декларация:
        // Этот может быть нормальной Декларация или ДекларацияАвто
      ...
      default:
        ...
        this.классХранения = сохр_классХранения; // Reset.
      }
      assert(узелУстановлен(декл));
      return декл;
    }
  ...
  {
    пропусти(T.Расклад);
    бцел размер = РАЗМЕР_РАСКЛАДКИ_ПО_УМОЛЧАНИЮ; // глоб дефолт.
    if (проверено(T.ЛСкобка))
    {
      if (сема.вид == T.Цел32)
...
version(D2)
{
  /// Возвращает да, если является манифестом перечня или
  /// нет, если это нормальная декларация перечня.
  ...
  Инструкция разборИнструкцииДляВсех()
  {
    ...
    Выражение в; // Агрегат или LwrВыражение
    ...
    в = разборВыражения();
  version(D2)
  { //ДляВсех (ForeachType; LwrВыражение .. UprВыражение ) ИнструкцияМасштаб
    if (проверено(T.Срез))
    {
      // if (парамы.length != 1)
  ...
  /// Вспомогательная функция для разбора инструкции default или case.
  Инструкция разборТелаКейсИлиДефолт()
  {
    // Эта функция подобна разборИнструкцииБезМасштаба()
    auto начало = сема;
  ...
  Инструкция разборИнструкцииПробуй()
  {
    ...
    if (телаЛови.length == 0 && finBody is пусто)
      assert(начало.вид == T.Пробуй), ошибка(начало, сооб.НеДостаетКэтчИлиВИтоге);
  ...
  Инструкция разборИнструкцииАсемблера()
  {
    ...
    case T.Расклад:
      ...
      if (сема.вид == T.Цел32)
        ...
        ошибка2(сооб.ОжидалсяЦЧПослеАлайн, сема);
      требуется(T.ТочкаЗапятая);
  ...
  Выражение разборВыраженияАсемблера()
  {
    ...
    // TODO: создай AsmВыражение that содержит в?
    return в;
  }
  ...
  Выражение разборУнарногоВыражения()
  {
    ...
    case T.ЛСкобка:
      ...
      if (успех)
      {
        auto идент = требуетсяИдентификатор(сооб.ОжидалсяИдПослеТипаТчк);
  ...
  Выражение разборПримарногоВыражения()
  {
    ...
    case T.Ткст:
      ...
      while (сема.вид == T.Ткст)
      {
        ...
        if (сема.pf && сема.pf != postfix)
          ошибка(сема, сооб.НесоответствиеТекстаПостфикса);
    ...
    LparseAssocArray:
      ...
      break;
    case T.ЛФСкобка:
      // ЛитералДелегата := { Инструкции }
      auto телоФунк = разборТелаФункции();
      в = new ВыражениеЛитералФункции(телоФунк);
      break;
    case T.Функция, T.Делегат:
      // ЛитералФункции := ("function"|"delegate") Тип? "(" СписокАргументов ")" ТелоФункции
      далее(); // Пропустим ключевое слово function или delegate.
      Тип типВозврата;
      Параметры параметры;
      if (сема.вид != T.ЛФСкобка)
      {
    ...
    version(D2)
    {
      // is ( Тип Идентификатор : СпециализацияТипа , СписокПараметровШаблона )
      // is ( Тип Идентификатор == СпециализацияТипа , СписокПараметровШаблона )
  ...
  Выражение разборВыраженияНов(/*Выражение в*/)
  {
    ...
    //         new (СписокАргументов)opt class (СписокАргументов)opt SuperClassopt InterfaceClassesopt ТелоКласса
    ...
    // ВыражениеНов:
    //         АргументыНов Тип [ ВыражениеПрисвой ]
    //         АргументыНов Тип ( СписокАргументов )
    //         АргументыНов Тип
    auto тип = разборТипа();
  ...
  Тип разборБазовогоТипа()
  {
    ...
    default:
      ошибка2(ИДС.НайденоИноеЧемОжидалось, "БазовыйТип", сема);
  ...
  /// Возвращает "да", если сема после закрывающий скобки
  /// соответствует искомому виду.
  бул семаПослеСкобкиЯвляется(ТОК2 вид)
  {
    assert(сема.вид == T.ЛСкобка);
    auto следщ = сема;
  ...
  /// определено
  бул семаПослеСкобкиЯвляется(ТОК2 вид, ref Сема* следщ)
  {
    assert(следщ !is пусто && следщ.вид == T.ЛСкобка);
    return пропустиСкобку(следщ) == вид;
  }

  /// Пропуск до сему за закрывающей скобкой.
  /// Принимает во внимание гнездовые скобки.
  ТОК2 пропустиСкобку(ref Сема* следщ)
  {
    assert(следщ !is пусто && следщ.вид == T.ЛСкобка);
    // Подсчитываем семы гнездовых скобок, так как шаблонные типы, typeof и т.д.
    // могут появлять внутри списка параметров. Напр.: (цел x, Foo!(цел) y)
    бцел уровень = 1;
  Loop:
    while (1)
      ...
      case T.ПСкобка:
        if (--уровень == 0)
          return возьмиПосле(следщ); // Найдена закрывающая скобка.
        break;
      case T.КФ:
        return T.КФ;
      default:
  ...
  /// Разбор типов массивов после декларатора (си-стиль.) Напр.: цел a[]
  Тип разборСуффиксаДекларатора(Тип lhsType)
  {
    ...
    //   <- <-             ->  -.
    //       ^-----------------´
    // Итоговая цепочка: [][32]*[3]цел
    Тип разборСледующего() //Требуется выполнение этого для гнездовой функции.
    {
      if (сема.вид != T.ЛКвСкобка)
        return lhsType; // Выход из рекурсии; возвращаем Тип по левую сторону от Идентификатора.
  ...
  Тип разборТипаУказательНаФункциюСи(Тип тип, ref Идентификатор* идент, бул опцСписокПарам)
  {
    ...
    тип = new ТУказательНаФункцСи(тип, парамы);
  ...
  /// Разбирает список of AssignВыражения.
  /// $(PRE
  /// СписокВыражений :=
  ///   ВыражениеПрисвой
  ///   ВыражениеПрисвой , СписокВыражений
  /// )
  ...
  body
  {
    ...
    do
    {
      ...
      if (проверено(T.Эллипсис))
      {
        ...
        суньПараметр(); // тип, идент и дефЗначение будут равны пусто.
        break;
      }

      while (1)
      { // Разбор классов хранения.
        switch (сема.вид)
        {
      version(D2)
      {
        ...
        version(D2)
          ...
          break; // В D1.0 грамматика допускает только один класс хранения.
        default:
        }
...
version(D2)
{
  ...
  проц  разборСпискаПараметровШаблона_(ПараметрыШаблона шпарамы)
  {
    do
    {
      ...
      // Сунуть параметр шаблона.
      шпарамы ~= установи(tp, началоПарама);

    } while (проверено(T.Запятая));
  }

  /// Возвращает ткст семы, печатаемой для клиента.
  ткст дайПечатный(Сема* сема)
  { // TODO: требуется обработка других сем, напр. ткстов.
    return сема.вид == T.КФ ? "КФ" : сема.исхТекст;
  }

  alias требуется ожидаемое;

  /// Требуется сема вида лекс.
  проц  требуется(ТОК2 лекс)
  {
    if (сема.вид == лекс)
      далее();
    else
  ...
  /// Требуется следщ сема вида лекс.
  проц  требуетсяСледующий(ТОК2 лекс)
  {
    далее();
    требуется(лекс);
  }

  /// Опционально разбирает идентификатор.
  /// Возвращает: пусто или идентификатор.
  ...
  /// Отчёт об ошибке, если текущая сема не идентификатор.
  /// Параметры:
  ///   ошСооб = используемое сообщение об ошибке.
  /// Возвращает: пусто или идентификатор.
  ...
  /// Отчёт об ошибке, если текущая сема не идентификатор.
  /// Параметры:
  ///   идс = используемое сообщение об ошибке.
  /// Возвращает: пусто или идентификатор.
  ...
  /// Отчёт об ошибке, если закрывающая противочасть семы не найдена.
  проц  требуетсяЗакрыв(ТОК2 закрывающий, Сема* открывающий)
  {
  ...
  /// Возвращает "да", если ткст ткт имеет неверную последовательность UTF-8.
