...
struct ПарсерМакросов
{
  ...
  static ткст0 сканируйМакрос(ткст0 у, ткст0 конецТекста)
  {
    assert(*у == '$');
...
struct РаскрывательМакросов
{
  ...
  ткст раскройМакрос(ткст текст, ткст предшArg0 = пусто/+, бцел depth = 1000+/)
  {
    ...
    ткст результат;
    ткст0 у = текст.ptr;
    ткст0 конецТекста = у + текст.length;
    ткст0 конецМакроса = у;
          ...
          { // Ignилиe recursive macro if:
            ...
            if (macro_.callLevel != 0 &&
                (аргиМакроса.length == 0/+ || // У макроса нет аргументов.
                 предшArg0 == macroArg0+/)) // macroArg0 равен предшious арг0.
            { continue; }
            macro_.callLevel++;
  ...
  static сим[][] сканируйАргументы(ref ткст0 у, ткст0 конецТекста)
  ...
  body
  {
    ...
    ткст0 началоАрг0 = у; // Весь список аргументов.
    ткст0 началоАрга = у;
  ГлавныйЦикл:
    while (у < конецТекста)
    {
  ...
  /// Разворачивает "&#36;+", "&#36;0" - "&#36;9" с арги[n] в тексте.
  /// Параметры:
  ///   текст = текст, сканируемый на наличие местодержателей аргументов.
  ///   арги = первый элемент, арги[0], - это весь текст аргумента,
  ///          а следующие элементы - срезы в нём.$(BR)
  ///          Этот массив пуст, если в нём нет аргументов.
  ...
  body
  {
    ...
    if (конецМестодержателя == текст.ptr)
      return текст; // Местодержатель не найден. Итог - оригинальный текст.
