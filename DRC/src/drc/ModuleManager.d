...
class МодульМенеджер
{
  ...
  Пакет корневойПакет;
  /// Мипирует полные имена пакетов в пакеты. Напр.: drc.ast
  Пакет[ткст] таблицаПакетов;
  /// Мапирует ПКИ пути в модули. Напр.: dil/ast/Узел
  ...
  static ткст найдиПутьКФайлуМодуля(ткст путьПоПКНМодуля, ткст[] путиИмпорта)
  {
    ...
    {
      путьКФайлу.установи(путьИмпорта); // Напр.: ист/
      путьКФайлу.добавь(путьПоПКНМодуля); // Напр.: dil/ast/Узел
      foreach (суффиксМодуля; [".d", ".di"/*interface file*/])
      {
        путьКФайлу.суффикс(суффиксМодуля);
        if (путьКФайлу.есть_ли()) // Напр.: ист/dil/ast/Узел.d
