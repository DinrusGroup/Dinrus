...
private проц pe_add(block *b);
private цел need_prolog(block *b);
...
else
{
    ...
    if (!(config.flags4 & CFG4optimized) ||
        ...
        tym == TYifunc ||
        tym == TYmfunc ||       // can't yet handle ECX passed as параметр
        tym == TYjfunc ||       // can't yet handle EAX passed as параметр
...
private цел need_prolog(block *b)
{
    if (b.Bregcon.используется & fregsaved)
        goto Lneed;
