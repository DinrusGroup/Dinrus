...
const CHARMASK  = 0xFF;
const SHORTMASK = 0xFFFF;
const INTMASK   = SHORTMASK;
const LONGMASK  = 0xFFFFFFFF;

/* Common constants often checked for */
const LLONGMASK = 0xFFFFFFFFFFFFFFFFL;
const ZEROLL    = 0L;
...
private бул cnst(elem* e) { return e.Eoper == OPconst; }
цел REGSIZE();

version (Dinrus)
{
    import drc.backend.errors;
}
...
/*************************
 * Возвращает:
 *    да if elem is the constant 1.
 */

цел elemisone(elem *e)
{
    if (e.Eoper == OPconst)
    {
...
/*************************
 * Возвращает: да if elem is the constant -1.
 */

цел elemisnegone(elem *e)
{
    if (e.Eoper == OPconst)
    {
        switch (tybasic(e.Ety))
...
private проц fixside(elem **pe1,elem **pe2)
{
    const tym = (*pe1).Ety;
...
private цел cost(elem* n) { return opcost[n.Eoper]; }

/*******************************
 * For floating point Выражения, the cost would be the number
 * of registers in the FPU stack needed.
 */

private цел fcost(elem *e)
{
    цел cost;
    ...
    switch (e.Eoper)
    {
        ...
        case OPdiv:
        {
            const цел cost1 = fcost(e.EV.E1);
            const цел cost2 = fcost(e.EV.E2);
            cost = cost2 + 1;
            if (cost1 > cost)
                cost = cost1;
            break;
 ...
 * Since e1 can only be evaluated once, if it is an Выражение we
 * must use a temporary.
 */

private elem *fixconvop(elem *e)
{
    static const ббайт[CNVOPMAX - CNVOPMIN + 1] invconvtab =
    [
        OPбул,         // OPb_8
        OPs32_d,        // OPd_s32
        OPd_s32,        // OPs32_d
    ...
    // (such as (intdbl 8int сим += double))
    elem *ex = e;
    elem **pe = &e;
    while (OTconv(ed.Eoper))
    {
        const бцел copx = ed.Eoper;
        const бцел icopx = invconvtab[convidx(copx)];
    ...
    /* If we have a *, must assign a temporary to the Выражение
...
private elem * elstrcpy(elem *e, goal_t goal)
{
    ...
    switch (e.EV.E2.Eoper)
    {
        ...
        case OPstring:
            /* Replace strcpy(e1,"ткст") with memcpy(e1,"ткст",sizeof("ткст")) */
            // As streq
            e.Eoper = OPstreq;
            тип *t = type_allocn(TYarray, tstypes[TYchar]);
...
private elem * elstrcmp(elem *e, goal_t goal)
{
    ...
    if (OPTIMIZER)
    {
        ...
        switch (e.EV.E2.Eoper)
        {
            ...
            case OPstring:
                // Replace strcmp(e1,"ткст") with memcmp(e1,"ткст",sizeof("ткст"))
                e.Eoper = OPparam;
...
/****************************
 * For OPmemcmp
 * memcmp(a, b, члобайт) => ((a param b) OPmemcmp члобайт)
 */
...
private elem * elmemcmp(elem *e, goal_t goal)
{
     ...
     */
    {
        ...
        if (ec.Eoper == OPcomma)
        {
            /* Rewrite: (((a,b) param c) OPmemcmp члобайт)
             * As: a,((b param c) OPmemcmp члобайт)
             */
     ...
     */
    {
        ...
        if (ec.Eoper == OPcomma)
        {
            /* Have: ((a param (b,c)) OPmemcmp члобайт)
             */
            elem* a = e.EV.E1.EV.E1;
            elem* b = ec.EV.E1;
            if (a.canHappenAfter(b))
            {
                /* Rewrite: ((a param (b,c)) OPmemcmp члобайт)
                 * As: b,((a param c) OPmemcmp члобайт)
                 */
...
private elem * elmemset(elem *e, goal_t goal)
{
    ...
    if (OPTIMIZER)
    {
        ...
        {
            // lvalue OPmemset (члобайт param значение)
            ...
            {
                цел члобайт = cast(цел)el_tolong(enbytes);
                targ_llong значение = el_tolong(evalue);
                elem *e1 = e.EV.E1;
                ...
                switch (члобайт)
                {
                    ...
                    {
                        tym_t ety = e.Ety;
                        memset(&значение, значение & 0xFF, значение.sizeof);
                        evalue.EV.Vullong = значение;
                        evalue.Ety = tym;
                        e.Eoper = OPeq;
...
private elem * elmemcpy(elem *e, goal_t goal)
{
    ...
    if (OPTIMIZER)
    {
        ...
        {
            if (!boolres(ex.EV.E2))
            {   // Copying 0 bytes, so удали memcpy
                e.EV.E2 = e.EV.E1;
                e.EV.E1 = ex.EV.E1;
                ex.EV.E1 = null;
                e.Eoper = OPcomma;
            ...
            e.Eoper = OPstreq;
            тип *t = type_allocn(TYarray, tstypes[TYchar]);
            t.Tdim = cast(бцел)el_tolong(ex.EV.E2);
            e.ET = t;
            t.Tcount++;
            e.EV.E1 = el_una(OPind,TYstruct,e.EV.E1);
...
{
    //printf("eladd(%p)\n",e);
    targ_т_мера ptrmask = ~cast(targ_т_мера)0;
    if (_tysize[TYnptr] <= 4)
        ptrmask = 0xFFFFFFFF;
L1:
    elem *e1 = e.EV.E1;
             ...
             e1.Eoper == OPadd && cnst(e1.EV.E2))
    {
        ...
        goto L1;                        /* try and найди some more       */
    }
...
private elem * elmul(elem *e, goal_t goal)
{
    ...
    if (e2.Eoper == OPconst)           // try to replace multiplies with shifts
    {
        ...
        {
            цел i = ispow2(el_tolong(e2));      // check for power of 2
...
/*****************************
 * OPand,OPor,OPxor
 * This should be expanded to include long тип stuff.
 */
...
private elem * elbitwise(elem *e, goal_t goal)
{
    ...
    if (e2.Eoper == OPconst)
    {
        switch (sz)
        {
            ...
            case SHORTSIZE:
            {
                ...
                if (
                    /* OK for бцел if AND or high bits of i are 0   */
        ...
        (sz == 4 || sz == 8))
    {
         ...
         * цел bt32(бцел *p, бцел b) { return ((p[b >> 5] & (1 << (b & 0x1F)))) != 0; }
         * цел bt64a(бдол *p, бцел b) { return ((p[b >> 6] & (1L << (b & 63)))) != 0; }
         * цел bt64b(бдол *p, т_мера b) { return ((p[b >> 6] & (1L << (b & 63)))) != 0; }
         */

        static бул ELCONST(elem* e, long c) { return e.Eoper == OPconst && el_tolong(e) == c; }
        цел pow2sz = ispow2(sz);
        ...
        {
            const цел sz1 = tysize(e.EV.E1.Ety);
            e.Eoper = OPbtst;
            e.Ety = TYбул;
            e.EV.E1 = e2;
            e.EV.E2 = e1.EV.E2;
            //e.EV.E2.Ety = e.EV.E1.Ety; // leave тип as цел
            e1.EV.E2 = null;
            el_free(e1);
 ...
 * Возвращает:
 *      да    didn't fail
 *      нет   more than maxops operands
 */

бул fillinops(elem **ops, цел *opsi, цел maxops, цел oper, elem *e)
{
    if (e.Eoper == oper)
    {
...
private elem *elor(elem *e, goal_t goal)
{
    ...
    /* BSWAP: (данные[0]<< 24) | (данные[1]<< 16) | (данные[2]<< 8) | (данные[3]<< 0)
     */
    if (sz == 4 && OPTIMIZER)
    {
        elem*[4] ops;
            ...
            for (цел i = 0; i < 4; i++)
            {
                ...
                if (eo111.Eoper == OPadd)
                {
                    ...
                    if (eo111.EV.E2.Eoper != OPconst)
                        goto L1;
                    off = cast(бцел)el_tolong(eo111.EV.E2);
                    if (off < 1 || off > 3)
                        goto L1;
                }
                else
 ...
 *      ! ! e => бул e
 *      ! бул e => ! e
 *      ! OTrel => !OTrel       (invert the условие)
 *      ! OTconv => !
 */
...
private elem * elnot(elem *e, goal_t goal)
{
    ...
    switch (op)
    {
        case OPnot:                     // ! ! e => бул e
        case OPбул:                    // ! бул e => ! e
            e1.Eoper = cast(ббайт)(op ^ (OPбул ^ OPnot));
            /* That was a clever substitute for the following:  */
            /* e.Eoper = (op == OPnot) ? OPбул : OPnot;               */
        ...
        default:
            if (OTrel(op))                      /* ! OTrel => !OTrel            */
            {
                  ...
                  e1.Eoper = cast(ббайт)op2;
                  e = optelem(el_selecte1(e), goal);
            }
            else if (tybasic(e1.Ety) == TYбул && tysize(e.Ety) == 1)
            {
                // !e1 => (e1 ^ 1)
                e.Eoper = OPxor;
    ...
    switch (e1.Eoper)
    {
...

        {
            ...
            if (tyintegral(ty) && ec1.Eoper == OPconst && ec2.Eoper == OPconst)
            {
                ...
                if ((ty1 == TYбул && !OTlogical(e1.Eoper) || e1.Eoper == OPand && e1.EV.E2.Eoper == OPconst) &&
                    tysize(ty) == tysize(ec1.Ety))
                {
                    targ_llong b = ty1 == TYбул ? 1 : el_tolong(e1.EV.E2);

                    if (b == 1 && ispow2(i1 - i2) != -1)
                    {
                        // replace (e1 ? i1 : i2) with (i1 + (e1 ^ 1) * (i2 - i1))
                        // replace (e1 ? i2 : i1) with (i1 + e1 * (i2 - i1))
                        цел sz = tysize(e1.Ety);
                ...
                // The следщ two optimizations attempt to replace with an
                // бцел compare, which the code generator can generate
            ...
            // Try to detect absolute значение Выражение
            // (a < 0) -a : a
             ...
             * with:
             *    (a && noreturn), c
             * because that means fewer noreturn cases for the данные flow analysis to deal with
             */
        ...
        else
        {
          static if (1) // This optimization is undone in eleq().
          {
                ...
                el_match(e1.EV.E1,e2.EV.E1))
            {
                ...
                e1.Eoper = cast(ббайт)opeqtoop(e1op);
...
{
    static if (0) version (Dinrus)
    if (cnst(e.EV.E2) && !boolres(e.EV.E2))
        выведиОшибку(e.Esrcpos.Sfilename, e.Esrcpos.Slinnum, e.Esrcpos.Scharnum, "divide by нуль\n");
...
private elem * eldiv(elem *e, goal_t goal)
{
    ...
    elem *e2 = e.EV.E2;
    tym_t tym = e.EV.E1.Ety;
    цел uns = tyuns(tym) | tyuns(e2.Ety);
    if (cnst(e2))
    {
        static if (0) version (Dinrus)
        if (!boolres(e2))
    ...
    if (OPTIMIZER)
    {
        const цел SQRT_INT_MAX = 0xB504;
        const бцел SQRT_UINT_MAX = 0x10000;
        elem *e1 = e.EV.E1;
            ...
            e1.Eoper == OPdiv && e1.EV.E2.Eoper == OPconst)
        {
            ...
            бул uns1 = tyuns(e1.EV.E1.Ety) || tyuns(e1.EV.E2.Ety);
            бул uns2 = tyuns(e1.Ety) || tyuns(e2.Ety);
            ...
            e1.EV.E1.Eoper == OPremquo && e1.EV.E1.EV.E2.Eoper == OPconst)
        {
            ...
            targ_llong c2 = el_tolong(e2);
            бул uns1 = tyuns(erq.EV.E1.Ety) || tyuns(erq.EV.E2.Ety);
            бул uns2 = tyuns(e1.Ety) || tyuns(e2.Ety);
            if (uns1 == uns2)   // identity doesn't hold for mixed sign case
            {
                // The transformation will fail if c1*c2 overflows. This substitutes
        ...
        /* Convert if(e1/e2) to if(e1>=e2) iff бцел division.
         */
...
private elem * eloror(elem *e, goal_t goal)
{
    ...
    if (OPTIMIZER)
    {
        ...
        if (e1.Eoper == OPoror)
        {   /* convert (a||b)||c to a||(b||c). This will найди more CSEs.    */
    ...
    if (e1.Eoper == OPconst || e1.Eoper == OPrelconst || e1.Eoper == OPstring)
    {
        ...
        else                            /* (x,0) || e2  =>  (x,0),(бул e2) */
        {
            e.Eoper = OPcomma;
            if (tybasic(e.EV.E2.Ety) != TYvoid)
                e.EV.E2 = el_una(OPбул,t,e.EV.E2);
        }
  }
  else if (OPTIMIZER &&
        e.EV.E2.Eoper == OPvar &&
...
private бул optim_loglog(elem **pe)
{
    if (I16)
        return нет;
    elem *e = *pe;
    ...
    т_мера n = el_opN(e, op);
    if (n <= 3)
        return нет;
    бцел ty = e.Ety;
    elem **массив = cast(elem **)malloc(n * (elem *).sizeof);
    assert(массив);
    elem **p = массив;
    el_opArray(&p, e, op);
    ...
    цел cmpop = op == OPandand ? OPne : OPeqeq;
    for (т_мера i = 0; i < n; ++i)
    {
            ...
            !el_sideeffect(eq.EV.E1))
        {
            ...
            if (any)
            {
                if (el_match(массив[first].EV.E1, eq.EV.E1))
                {
                    last = i;
                    if (m < emin)
                        emin = m;
    ...
    //printf("n = %d, count = %d, min = %d, max = %d\n", (цел)n, last - first + 1, (цел)emin, (цел)emax);
    if (any && last - first > 2 && emax - emin < REGSIZE * 8)
    {
        /**
         * Transforms Выражения of the form x==c1 || x==c2 || x==c3 || ... into a single
         * comparison by using a bitmapped representation of данные, as follows. First, the
         ...
         * да for the Выражение.
         *
         * I.e. replace:
         *   e==c1 || e==c2 || e==c3 ...
         * with:
         *   (e - emin) <= (emax - emin) && (1 << (цел)(e - emin)) & bits
         * where bits is:
        ...
        // Compute bit mask
        targ_ullong bits = 0;
        for (т_мера i = first; i <= last; ++i)
        {
            elem *eq = массив[i];
            if (0 && eq.EV.E2.Eoper != OPconst)
            {
                printf("eq = %p, eq.EV.E2 = %p\n", eq, eq.EV.E2);
                printf("first = %d, i = %d, last = %d, Eoper = %d\n", cast(цел)first, cast(цел)i, cast(цел)last, eq.EV.E2.Eoper);
                printf("any = %d, n = %d, count = %d, min = %d, max = %d\n", any, cast(цел)n, cast(цел)(last - first + 1), cast(цел)emin, cast(цел)emax);
            }
            assert(eq.EV.E2.Eoper == OPconst);
            bits |= cast(targ_ullong)1 << (el_tolong(eq.EV.E2) - emin);
        }
        //printf("n = %d, count = %d, min = %d, max = %d\n", cast(цел)n, last - first + 1, cast(цел)emin, cast(цел)emax);
        //printf("bits = x%llx\n", bits);

        if (op == OPandand)
            bits = ~bits;

        бцел tyc = массив[first].EV.E1.Ety;

        elem *ex = el_bin(OPmin, tyc, массив[first].EV.E1, el_long(tyc,emin));
        ex = el_bin(op == OPandand ? OPgt : OPle, TYбул, ex, el_long(touns(tyc), emax - emin));
        elem *ey = el_bin(OPmin, tyc, массив[first + 1].EV.E1, el_long(tyc,emin));
        ...
        ey = el_bin(OPbtst,TYбул,el_long(tybits,bits),ey);
        ex = el_bin(op == OPandand ? OPoror : OPandand, ty, ex, ey);

        /* Free unneeded nodes
         */
        массив[first].EV.E1 = null;
        el_free(массив[first]);
        массив[first + 1].EV.E1 = null;
        el_free(массив[first + 1]);
        for (т_мера i = first + 2; i <= last; ++i)
            el_free(массив[i]);

        массив[first] = ex;

        for (т_мера i = first + 1; i + (last - first) < n; ++i)
            массив[i] = массив[i + (last - first)];
        n -= last - first;
        (*pe) = el_opCombine(массив, n, op, ty);
...
private elem * elandand(elem *e, goal_t goal)
{
    ...
    if (e2.Eoper == OPconst || e2.Eoper == OPrelconst || e2.Eoper == OPstring)
    {
        ...
        {
            if (e2 == e.EV.E2)    // if no x, replace e with (бул e1)
            {
                el_free(e2);
                e.EV.E2 = null;
                e.Eoper = OPбул;
  ...
  {
        e.Eoper = OPcomma;
        if (boolres(e1))                // (x,1) && e2  =>  (x,1),бул e2
        {
            if (tybasic(e.EV.E2.Ety) != TYvoid)
                e.EV.E2 = el_una(OPбул,e.Ety,e.EV.E2);
        }
        else                            // (x,0) && e2  =>  (x,0),0
        {
            if (tybasic(e.EV.E2.Ety) == TYvoid)
...
private elem * elbit(elem *e, goal_t goal)
{
    ...
    бцел sz = tysize(tym1) * 8;
    elem *e2 = e.EV.E2;
    бцел wb = e2.EV.Vuns;

    бцел w = (wb >> 8) & 0xFF;               // width in bits of field
    targ_ullong m = (cast(targ_ullong)1 << w) - 1;   // mask w bits wide
    бцел b = wb & 0xFF;                      // bits to right of field
    бцел c = 0;
    assert(w + b <= sz);

    if (tyuns(tym1))                      // if бцел bit field
    {
    ...
    e.EV.E1 = el_bin(OPshr,tym1,
                ...
                el_long(TYint,b));
L1:
    return optelem(e,GOALvalue);         // optimize результат
}
...
private elem * elind(elem *e, goal_t goal)
{
    ...
    switch (e1.Eoper)
    {
        case OPrelconst:
            ...
            e = el_selecte1(e);
            e.Eoper = OPvar;
            e.Ety = tym;               /* preserve original тип       */
            break;
...
private проц elstructwalk(elem *e,tym_t tym)
{
    tym_t ety;
...
elem * elstruct(elem *e, goal_t goal)
{
    ...
    бцел sz = (e.Eoper == OPstrpar && type_zeroSize(t, global_tyf)) ? 0 : cast(бцел)type_size(t);
    //printf("\tsz = %d\n", (цел)sz);
    if (sz == 16)
    {
    ...
    тип *targ2 = null;
    if (ty == TYstruct)
    {   // If a struct is a wrapper for another тип, prefer that other тип
 ...
 *          l  ~(m<<b)
 * Note:
 *      This depends on the Выражение (r&m)<<b before l. This is because
 *      of Выражения like (l.a = l.b = n). It is an artifact of the way
...
static if (0)  // Doesn't work too well, removed
{
    ...
    if (OPTIMIZER)
    {
           ...
           )
        {
           ...
           ei.EV.E1 = el_copytree(ei.EV.E1.EV.E1);            // копируй p
           return optelem(e,GOALvalue);
        }
           ...
           )
        {
            ...
            L2:
                e.EV.E2.Ety = ty;
                e.Eoper = cast(ббайт)optoopeq(op2);
                goto L1;
            }
            if (OTcommut(op2))
            {
...
static if (0)
{
// Note that this optimization is undone in elcomma(), this результатs in an
...
// this clause needs investigation because the code doesn't match the коммент
            // Disable this rewrite if we're using x87 and `e1` is a FP-значение
            // but `e2` is not, or vice versa
            // https://issues.dlang.org/show_bug.cgi?ид=18197
            (config.fpxmmregs ||
...
else
{
  ...
  if (e1.EV.E1.Eoper == OPcomma || OTassign(e1.EV.E1.Eoper))
    ...
    elem *r = e.EV.E2;
    tym_t tyl = l.Ety;
    бцел sz = tysize(tyl) * 8;
    бцел w = (e1.EV.E2.EV.Vuns >> 8);        // width in bits of field
    targ_ullong m = (cast(targ_ullong)1 << w) - 1;  // mask w bits wide
    бцел b = e1.EV.E2.EV.Vuns & 0xFF;        // bits to shift
...
version (SCPP)   // have bit fields to worry about?
{
    ...
    if (e1.Eoper == OPbit)
    {
        ...
        tym_t tyl = l.Ety;
        elem *r = e.EV.E2;
        бцел w = (e1.EV.E2.EV.Vuns >> 8) & 0xFF; // width in bits of field
...
private elem * elpost(elem *e, goal_t goal)
{
    ...
    if (tyuns(e1.EV.E1.Ety))             /* if бцел bit field        */
...
private elem * elcmp(elem *e, goal_t goal)
{
  ...
  //printf("elcmp(%p)\n",e); elem_print(e);
    ...
    if (OPTIMIZER)
    {
        ...
        if (op == OPeqeq && e2.Eoper == OPconst && e1.Eoper == OPand)
        {
            ...
            if (e12.Eoper == OPconst && el_tolong(e2) == 1 && el_tolong(e12) == 1)
            {
                ...
                e.Ety = ty1;
                цел sz = tysize(ty);
                for (цел sz1 = tysize(ty1); sz1 != sz; sz1 = tysize(e.Ety))
                {
                    switch (sz1)
                    {
                        case 1:
    ...
    цел uns = tyuns(e1.Ety) | tyuns(e2.Ety);
    if (cnst(e2))
    {
        tym_t tym;
        цел sz = tysize(e2.Ety);
        ...
        /* Try to convert to byte/word comparison for ((x & c)==d)
           when mask c essentially casts x to a smaller тип
         */
        if (OPTIMIZER &&
            e1.Eoper == OPand &&
            e1.EV.E2.Eoper == OPconst &&
        ...
        /* Convert (бдол > бцел.max) to (msw(бдол) != 0)
         */
...
private elem * elбул(elem *e, goal_t goal)
{
    //printf("elбул()\n");
    if (OTlogical(e.EV.E1.Eoper) ||
        // бул бул => бул
        (tybasic(e.EV.E1.Ety) == TYбул && tysize(e.Ety) == 1)
       )
        return el_selecte1(e);
    ...
    if (OPTIMIZER)
    {
        ...
        // Replace бул(e % 2) with (бцел сим)(e & 1)
        ...
        // Replace бул((1<<c)&b) with -(b btst c)
        else if ((I32 || I64) &&
                 e.EV.E1.Eoper == OPand &&
        ...
        // Replace бул(a & c) when c is a power of 2 with ((a >> shift) & 1)
 ...
 * Also используется for (OPoffset) (TYfptr|TYvptr).
 * Also используется for conversions of ints to bytes.
 */
...
private elem * ellngsht(elem *e, goal_t goal)
{
    ...
    case OPs8_16:
        ...
        // one is бцел and the other isn't.
    ...
    case OPvar:                 // simply paint тип of variable
        // Do not paint тип of ints into bytes, as this causes
        // many CSEs to be missed, результатing in bad code.
        // Loading a word anyway is just as fast as loading a byte.
        // for 68000 byte is swapped, load byte != load word
        if (e.Eoper == OP16_8)
        {
            // Mark symbol as being используется sometimes as a byte to
    ...
    case OPnp_fp:
        if (e.Eoper != OPoffset)
            goto case_default;
        // Replace (смещение)(ptrlptr)e11 with e11
        e = el_selecte1(el_selecte1(e));
        e.Ety = ty;                    // retain original тип
        break;
    ...
    case_default:
        ...
        if (OTwid(e1.Eoper) && !OTassign(e1.Eoper))
        {
            ...
            {
                case OP16_8:
                    // Make sure e1.EV.E1 is of the тип we're converting from
                    ...
                    // Rvalue may be an цел if it is a shift operator
...
elem *elmsw(elem *e, goal_t goal)
{
    ...
    if (OPTIMIZER &&
        ...
        tysize(ty) == LONGSIZE)
    {
        // Replace (цел)(msw (long)x) with (цел)*(&x+4)
        if (e1.Eoper == OPvar)
        {
            e1.EV.Voffset += LONGSIZE;      // address high dword in longlong
            if (I64)
...
static if (TARGET_LINUX || TARGET_OSX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
{
    ...
    // (OPva_start &va)
    // (OPeq (OPind E1) __va_argsave+смещение)
    //elem_print(e);
...
private проц elparamx(elem *e)
{
    ...
    else
    {
        static if (0)
        {
            // Unfortunately, these don't work because if the last параметр
            // is a pair, and it is a D function, the last параметр will get
            // passed in EAX.
 ...
 * there is more than one родитель for an elem).
 * If (goal)
 *      we care about the результат.
 */
...
beg:
    ...
    auto op = e.Eoper;
    if (OTleaf(op))                     // if not an operator узел
    {
...
//                  e1 = e.EV.E1 = optelem(e1, GOALnone);
                   ...
                   )
                {
                  ...
                  if (op == OPadd)  /* fix types                    */
                  {
                      ...
                      if (typtr(e1.EV.E2.Ety))
                          ...
                          /* then this will fix the тип of op2 to be цел   */
                          e1.Ety = e1.EV.E1.Ety;
                  }
                  goto beg;
                }
        ...
        if (OTcommut(op))                // if commutative
        {
              ...
              version (Dinrus) { const Dinrus = да; } else { const Dinrus = нет; }
              if (
                Dinrus ? (
                cost(e2) > cost(e1)
                 ...
                 )
              {
                    ...
                    op = e.Eoper = cast(ббайт)swaprel(op);
              }
           ...
           )
        {
            ...
            if (OTeop0e(op))            /* if e1 op 0 => e1             */
            {
                if (!boolres(e2))       /* if e2 is 0                   */
                {
                    ...
                    { }
                    // Don't do it if we're assembling a complex значение
  ...
  else /* unary operator */
  {
        ...
        elem *e1 = e.EV.E1 = optelem(e.EV.E1, (op == OPddtor)
                                                 ? GOALnone
                                                 : (op == OPбул || op == OPnot) ? GOALflags : GOALvalue);
        if (!e1)
            goto retnull;
        if (e1.Eoper == OPconst)
        {
...
/********************************
 * Optimize and canonicalize an Выражение tree.
...
elem *doptelem(elem *e, goal_t goal)
{
    ...
    /* If entire Выражение is a struct, and we can replace it with     */
...
/****************************************
 * Do optimizations after bltailrecursion() and before common subВыражения.
 */

проц postoptelem(elem *e)
{
    Srcpos pos = {0};
    ...
    while (1)
    {
        if (OTunary(e.Eoper))
        {
            ...
            if (e.Eoper == OPind)
            {
                ...
                {
                    выведиОшибку(pos.Sfilename, pos.Slinnum, pos.Scharnum, "null dereference in function %s", funcsym_p.Sident.ptr);
...
private бул canHappenAfter(elem* a, elem* b)
{
...
private  alias elem *function(elem *, goal_t) elfp_t;

private extern (D) const elfp_t[OPMAX] elxxx =
[
    OPunde:    &elerr,
    OPadd:     &eladd,
    OPmul:     &elmul,
