...
extern  ббайт[FLMAX] segfl;
extern  бул[FLMAX] stackfl, flinsymtab;

private extern (D) бцел mask(бцел m) { return 1 << m; }

//private проц genorreg(ref CodeBuilder c, бцел t, бцел f) { genregs(c, 0x09, f, t); }

extern  targ_т_мера retsize;

const JMPJMPTABLE = нет;               // benchmarking shows it's slower

const MINLL =           0x8000_0000_0000_0000L;
const MAXLL =           0x7FFF_FFFF_FFFF_FFFFL;
...
private  const ббайт[256] inssize32 =
[       2,2,2,2,        2,5,1,1,                /* 00 */
        2,2,2,2,        2,5,1,1,                /* 08 */
        2,2,2,2,        2,5,1,1,                /* 10 */
        2,2,2,2,        2,5,1,1,                /* 18 */
 ...
 *      cdb = where to пиши generated code
...
ббайт vex_inssize(code *c)
{
    ...
    if (c.Iflags & CFvex3)
    {
        ...
        case 0: // no префикс
        case 1: // 0F
            ins = cast(ббайт)(inssize2[c.Ivex.op] + 2);
            break;
        case 2: // 0F 38
            ins = cast(ббайт)(inssize2[0x38] + 1);
            break;
        case 3: // 0F 3A
            ins = cast(ббайт)(inssize2[0x3A] + 1);
            break;
        default:
            printf("Iop = %x mmmm = %x\n", c.Iop, c.Ivex.mmmm);
            assert(0);
    ...
    {
        ins = cast(ббайт)(inssize2[c.Ivex.op] + 1);
    }
    return ins;
}
...
/********************************
 * set initial глоб2 variable values
 */
 ...
 * Параметры:
 *      seg = segment to пиши alignment bytes to
 *      члобайт = number of alignment bytes to пиши
 */
проц cod3_align_bytes(цел seg, т_мера члобайт)
{
    ...
    {   т_мера n = члобайт;
        ...
        if (члобайт > 1 && (I64 || config.fpxmmregs))
        {
            switch (n)
            {
        ...
        objmod.write_bytes(SegData[seg],cast(бцел)n,cast(сим*)p);
        члобайт -= n;
    }
}
...
static if (TARGET_WINDOS)
{
    if (config.flags4 & CFG4speed)      // if optimized for speed
    {
        ...
        {   // 486 does reads on 16 byte boundaries, so if we are near
            ...
            члобайт = -Offset(seg) & 15;
            if (члобайт < 8)
                cod3_align_bytes(seg, члобайт);
        }
    }
}
else
{
    члобайт = -Offset(seg) & 7;
    cod3_align_bytes(seg, члобайт);
}
}
...
/**********************************
 * Generate code to adjust the stack pointer by `члобайт`
 * Параметры:
 *      cdb = code builder
 *      члобайт = number of bytes to adjust stack pointer
 */
проц cod3_stackadj(ref CodeBuilder cdb, цел члобайт)
{
    //printf("cod3_stackadj(%d)\n", члобайт);
    бцел grex = I64 ? REX_W << 16 : 0;
    бцел rm;
    if (члобайт > 0)
        rm = modregrm(3,5,SP); // SUB ESP,члобайт
    else
    {
        члобайт = -члобайт;
        rm = modregrm(3,0,SP); // ADD ESP,члобайт
    }
    cdb.genc2(0x81, grex | rm, члобайт);
}

/**********************************
 * Generate code to align the stack pointer at `члобайт`
 * Параметры:
 *      cdb = code builder
 *      члобайт = number of bytes to align stack pointer
 */
проц cod3_stackalign(ref CodeBuilder cdb, цел члобайт)
{
    //printf("cod3_stackalign(%d)\n", члобайт);
    const grex = I64 ? REX_W << 16 : 0;
    const rm = modregrm(3, 4, SP);             // AND ESP,-члобайт
    cdb.genc2(0x81, grex | rm, -члобайт);
}
...
проц cod3_buildmodulector(Outbuffer* буф, цел codeOffset, цел refOffset)
{
    ...
    else
    {
        ...
        буф.пишиБайт(0xB8);
        codeOffset += 1;
        const бцел reltype = I64 ? R_X86_64_32 : R_386_32;
    ...
    if (I64)
        буф.пишиБайт(REX | REX_W);
    буф.пишиБайт(0x8B); буф.пишиБайт(0x11); /* movl (%ecx), %edx */
    if (I64)
        буф.пишиБайт(REX | REX_W);
    буф.пишиБайт(0x89); буф.пишиБайт(0x10); /* movl %edx, (%eax) */
    if (I64)
        буф.пишиБайт(REX | REX_W);
    буф.пишиБайт(0x89); буф.пишиБайт(0x01); /* movl %eax, (%ecx) */

    буф.пишиБайт(0xC3); /* ret */
}
...
/*****************************
 * Given a тип, return a mask of
 * registers to hold that тип.
 * Input:
 *      tyf     function тип
 */
...
/*******************************
 * setup register allocator parameters with platform specific данные
 */
...
проц cgreg_set_priorities(tym_t ty, reg_t** pseq, reg_t** pseqmsw)
{
    ...
    if (tyxmmreg(ty))
    {
        static const ббайт[9] sequence = [XMM0,XMM1,XMM2,XMM3,XMM4,XMM5,XMM6,XMM7,NOREG];
        *pseq = sequence.ptr;
    }
    else if (I64)
    {
        if (sz == REGSIZE * 2)
        {
            static const ббайт[3] seqmsw1 = [CX,DX,NOREG];
            static const ббайт[5] seqlsw1 = [AX,BX,SI,DI,NOREG];
            *pseq = seqlsw1.ptr;
            *pseqmsw = seqmsw1.ptr;
        }
        else
        {   // R10 is reserved for the static link
            static const ббайт[15] sequence2 = [AX,CX,DX,SI,DI,R8,R9,R11,BX,R12,R13,R14,R15,BP,NOREG];
            *pseq = cast(ббайт*)sequence2.ptr;
        }
    }
    else if (I32)
    {
        if (sz == REGSIZE * 2)
        {
            static const ббайт[5] seqlsw3 = [AX,BX,SI,DI,NOREG];
            static const ббайт[3] seqmsw3 = [CX,DX,NOREG];
            *pseq = seqlsw3.ptr;
            *pseqmsw = seqmsw3.ptr;
        }
        else
        {
            static const ббайт[8] sequence4 = [AX,CX,DX,BX,SI,DI,BP,NOREG];
            *pseq = sequence4.ptr;
        }
    }
    else
    {   assert(I16);
        if (typtr(ty))
        {
            ...
            static const ббайт[8] seqidx5 = [BX,SI,DI,AX,CX,DX,BP,NOREG];
            *pseq = seqidx5.ptr;
        }
        else
        {
            // Otherwise, try to pick index registers last
            static const ббайт[8] sequence6 = [AX,CX,DX,BX,SI,DI,BP,NOREG];
            *pseq = sequence6.ptr;
        }
    }
}
...
private code *callFinallyBlock(block *bf, regm_t retregs)
{
    ...
    calledFinally = да;
    бцел npush = gensaverestore(retregs,cdbs,cdbr);
    ...
    cdbs.genc(0xE8,0,0,0,FLblock,cast(targ_т_мера)bf);
    regcon.immed.mval = 0;
    if (nalign)
        cod3_stackadj(cdbs, -nalign);
    cdbs.приставь(cdbr);
...
проц outblkexitcode(ref CodeBuilder cdb, block *bl, ref цел anyspill, ткст0 sflsave, Symbol** retsym,  regm_t mfuncregsave)
{
    ...
    switch (bl.BC)                     /* block exit условие         */
    {
        case BCiftrue:
        {
            бул jcond = да;
...
version (Dinrus)
{
        case BCjcatch:          // D catch clause of try-catch
            ...
            // register assignments to variables используется in catch blocks.
            getregs(cdb,lpadregs());

            if (config.ehmethod == EHmethod.EH_DWARF)
            {
                /* Each block must have ESP set to the same значение it was at the end
                 * of the prolog. But the unwinder calls catch blocks with ESP set
                 * at the значение it was when the throwing function was called, which
                 * may have arguments pushed on the stack.
                 * This instruction will сбрось ESP to the correct смещение from EBP.
                 */
...
version (SCPP)
{
        ...
        case BCgoto:
            ...
            {
                regm_t retregsx = 0;
                gencodelem(cdb,e,&retregsx,да);
                цел toindex = nextb.Btry ? nextb.Btry.Bscope_index : -1;
                assert(bl.Btry);
                цел fromindex = bl.Btry.Bscope_index;
version (Dinrus)
{
...
version (Dinrus)
{
                if (toindex + 1 <= fromindex)
                {
                    ...
                    {   block *bf;
                        ...
                        // call __finally
                        cdb.приставь(callFinallyBlock(bf.nthSucc(0), retregsx));
                    }
                }
}
                }
        ...
        case BC_finally:
            if (ehmethod(funcsym_p) == EHmethod.EH_DWARF)
            {
                ...
                regm_t retregsx = 0;
                gencodelem(cdb,bl.Belem,&retregsx,да);
            ...
            else
            {
                ...
                {
                    // Mark all registers as destroyed. This will prevent
                    // register assignments to variables используется in finally blocks.
                    getregsNoSave(lpadregs());
                }

                assert(!e);
                // Generate CALL to finalizer code
                cdb.приставь(callFinallyBlock(bl.nthSucc(0), 0));
...
static if (NTEXCEPTIONS)
{
        ...
        case BC_filter:
        {
            nteh_filter(cdb, bl);
            // Mark all registers as destroyed. This will prevent
            // register assignments to variables используется in filter blocks.
            getregsNoSave(allregs);
        ...
        case BCretexp:
            ...
            // For the final load into the return regs, don't set regcon.используется,
            ...
            {   regm_t usedsave;
                ...
                docommas(cdb,&e);
                usedsave = regcon.используется;
                if (!OTleaf(e.Eoper))
                    gencodelem(cdb,e,&retregs,да);
                else
                {
                    if (e.Eoper == OPconst)
                        regcon.mvar = 0;
                    gencodelem(cdb,e,&retregs,да);
                    regcon.используется = usedsave;
                    if (e.Eoper == OPvar)
                    {   Symbol *s = e.EV.Vsym;
        ...
        L4:
            ...
            {   assert(global87.stackused == 1);
                pop87();                // account for return значение
            }
            ...
            else if (Dinrus || usednteh & NTEH_try)
            {
                block *bt = bl;
...
version (Dinrus)
{
                        ...
                        config.ehmethod == EHmethod.EH_SEH)
                    {
                        if (bt.Bscope_index == 0)
                        {
                            ...
                            cdb.приставь(cdbs);
                            cdb.genc(0xE8,0,0,0,FLblock,cast(targ_т_мера)bf.nthSucc(0));
                            regcon.immed.mval = 0;
                            cdb.приставь(cdbr);
                        }
                        else
                    ...
                    {
                        // call __finally
                        cdb.приставь(callFinallyBlock(bf.nthSucc(0), retregs));
                    }
                }
            }
            break;
...
struct CaseVal
{
    ...
    extern (C) static  цел cmp(ук p,  ук q)
    {
        CaseVal* c1 = cast(CaseVal*)p;
        CaseVal* c2 = cast(CaseVal*)q;
 ...
 */
private проц cmpval(ref CodeBuilder cdb, targ_llong val, бцел sz, reg_t reg, reg_t reg2, reg_t sreg)
{
    if (I64 && sz == 8)
    {
        assert(reg2 == NOREG);
        if (val == cast(цел)val)    // if val is a 64 bit значение sign-extended from 32 bits
        {
            cdb.genc2(0x81,modregrmx(3,7,reg),cast(targ_т_мера)val);     // CMP reg,value32
            cdb.last().Irex |= REX_W;                  // 64 bit operand
        }
        else
        {
            assert(sreg != NOREG);
            movregconst(cdb,sreg,cast(targ_т_мера)val,64);  // MOV sreg,val64
        ...
        cdb.genc2(0x81,modregrmx(3,7,reg),cast(targ_т_мера)val);         // CMP reg,casevalue
    else
    {
        cdb.genc2(0x81,modregrm(3,7,reg2),cast(targ_т_мера)MSREG(val));  // CMP reg2,MSREG(casevalue)
        code *cnext = gennop(null);
        genjmp(cdb,JNE,FLcode,cast(block *) cnext);  // JNE cnext
        cdb.genc2(0x81,modregrm(3,7,reg),cast(targ_т_мера)val);          // CMP reg,casevalue
        cdb.приставь(cnext);
    }
}

private проц ifthen(ref CodeBuilder cdb, CaseVal *casevals, т_мера ncases,
        бцел sz, reg_t reg, reg_t reg2, reg_t sreg, block *bdefault, бул last)
{
    if (ncases >= 4 && config.flags4 & CFG4speed)
    {
        т_мера pivot = ncases >> 1;

        // Compares for casevals[0..pivot]
        CodeBuilder cdb1; cdb1.ctor();
        ifthen(cdb1, casevals, pivot, sz, reg, reg2, sreg, bdefault, да);
        ...
        // Note бцел jump here, as cases were sorted using бцел comparisons
    ...
    {   // Not worth doing a binary search, just do a sequence of CMP/JE
        ...
        if (last)       // if default is not следщ block
            genjmp(cdb,JMP,FLblock,bdefault);
    }
}

/*******************************
 * Generate code for blocks ending in a switch инструкция.
...
проц doswitch(ref CodeBuilder cdb, block *b)
{
    targ_ulong msw;

    // If switch tables are in code segment and we need a CS: override to get at them
    бул csseg = cast(бул)(config.flags & CFGromable);
    ...
    цел sz = _tysize[tys];
    бул dword = (sz == 2 * REGSIZE);
    бул mswsame = да;                // assume all msw's are the same
    targ_llong *p = b.Bswitch;          // pointer to case данные
    assert(p);
    бцел ncases = cast(бцел)*p++;       // number of cases

    targ_llong vmax = MINLL;            // smallest possible llong
    targ_llong vmin = MAXLL;            // largest possible llong
    for (бцел n = 0; n < ncases; n++)   // найди max and min case values
    {
        targ_llong val = *p++;
    ...
    Lifthen:
        ...
        {
            reg = findreg(retregs);     // reg that результат is in
            reg2 = NOREG;
        }
        list_t bl = b.Bsucc;
        ...
        for (бцел n = 0; n < ncases; n++)
        {
            ...
            if (sreg == NOREG && I64 && sz == 8 && p[n] != cast(цел)p[n])
    ...
    /*************************************************************************/
    {
        // Use switch значение to index into jump table
    Ljmptab:
        //printf("Ljmptab:\n");
         ...
         * Must be same computation as используется in outjmptab().
         */
        if (vmin > 0 && vmin <= _tysize[TYint])
            vmin = 0;

        b.Btablesize = cast(цел) (vmax - vmin + 1) * tysize(TYnptr);
...
static if (TARGET_LINUX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
{
        ...
        бул modify = (I16 || I64 || vmin);
        scodelem(cdb,e,&retregs,0,!modify);
        reg_t reg = findreg(retregs & IDXREGS); // reg that результат is in
        reg_t reg2;
        if (dword)
            reg2 = findregmsw(retregs);
        if (modify)
        ...
        {
            cdb.genc2(0x81,modregrm(3,5,reg),cast(targ_т_мера)vmin); // SUB reg,vmin
            if (dword)
            {   cdb.genc2(0x81,modregrm(3,3,reg2),cast(targ_т_мера)MSREG(vmin)); // SBB reg2,vmin
        ...
        {                               // CMP reg,vmax-vmin
            cdb.genc2(0x81,modregrm(3,7,reg),cast(targ_т_мера)(vmax-vmin));
            if (I64 && sz == 8)
                code_orrex(cdb.last(), REX_W);
        ...
        if (I64)
        {
            ...
            if (config.flags3 & CFG3pic || config.exe == EX_WIN64)
            {
                ...
                b.Btablesize = cast(цел) (vmax - vmin + 1) * 4;
                code *ce = cdbe.finish();
                pinholeopt(ce, null);
...
{
            /* LEA jreg,смещение ctable[reg][reg * 4]
               JMP jreg
              ctable:
               JMP case0
               JMP case1
...
else
{
            ...
            {
                /* MOV  R1,EBX
                 * SUB  R1,funcsym_p@GOTOFF[смещение][reg*4][EBX]
                 * JMP  R1
                 */
        ...
        else if (I16)
        {
            ...
            бцел rm = getaddrmode(retregs) | modregrm(0,4,0);
...
static if (TARGET_LINUX || TARGET_OSX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
{
        ...
        {   // Add in GOT
            ...
            genmovreg(cdb, DX, DI);    // MOV EDX, EDI
                                        // ADD EDI,смещение of switch table
        ...
        if (!(config.flags3 & CFG3pic))
        {
                                        // MOV DI,смещение of switch table
         ...
         * Therefore, load ES with proper segment значение.
         */
        if (config.flags3 & CFG3eseqds)
        {
            assert(!csseg);
            getregs(cdb,mCX);           // размести CX
        }
        else
        {
            getregs(cdb,mES|mCX);       // размести ES and CX
            cdb.gen1(csseg ? 0x0E : 0x1E);      // PUSH CS/DS
            cdb.gen1(0x07);                     // POP  ES
        }

        targ_т_мера disp = (ncases - 1) * _tysize[TYint];  // displacement to jump table
        if (dword && !mswsame)
        {
            ...
            const цел mod = (disp > 127) ? 2 : 1;         // displacement size
        ...
        genjmp(cdb,JNE,FLblock,b.nthSucc(0)); // JNE default
        const цел mod = (disp > 127) ? 2 : 1;     // 1 or 2 byte displacement
        if (csseg)
...
/******************************
 * Output данные block for a jump table (BCjmptab).
...
проц outjmptab(block *b)
{
    ...
    targ_llong *p = b.Bswitch;               // pointer to case данные
    т_мера ncases = cast(т_мера)*p++;        // number of cases

    /* Find vmin and vmax, the range of the table will be [vmin .. vmax + 1]
     * Must be same computation as используется in doswitch().
     */
    targ_llong vmax = MINLL;                 // smallest possible llong
    targ_llong vmin = MAXLL;                 // largest possible llong
    for (т_мера n = 0; n < ncases; n++)      // найди min case значение
    {   targ_llong val = p[n];
        if (val > vmax) vmax = val;
        if (val < vmin) vmin = val;
    }
    ...
    /* Segment and смещение into which the jump table will be emitted
     */
    цел jmpseg = objmod.jmpTableSegment(funcsym_p);
    targ_т_мера *poffset = &Offset(jmpseg);

    /* Align start of jump table
     */
    targ_т_мера alignbytes = _align(0,*poffset) - *poffset;
    objmod.lidata(jmpseg,*poffset,alignbytes);
    assert(*poffset == b.Btableoffset);        // should match precomputed значение

    Symbol *gotsym = null;
    targ_т_мера def = b.nthSucc(0).Boffset;  // default address
    for (targ_llong u = vmin; ; u++)
    {   targ_т_мера targ = def;                     // default
        for (т_мера n = 0; n < ncases; n++)
        {       if (p[n] == u)
                {       targ = b.nthSucc(cast(цел)(n + 1)).Boffset;
                        break;
                }
        }
static if (TARGET_LINUX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
...
else static if (TARGET_WINDOS)
{
        if (I64)
        {
            targ_т_мера val = targ - b.Btableoffset;
            objmod.write_bytes(SegData[jmpseg],4,&val);
        }
        else
        {
...
/******************************
 * Output данные block for a switch table.
 * Two consecutive tables, the first is the case значение table, the
...
проц outswitab(block *b)
{
    ...
    const цел seg = objmod.jmpTableSegment(funcsym_p);
    targ_т_мера *poffset = &Offset(seg);
    targ_т_мера смещение = *poffset;
    targ_т_мера alignbytes = _align(0,*poffset) - *poffset;
    objmod.lidata(seg,*poffset,alignbytes);  // any alignment bytes necessary
    assert(*poffset == смещение + alignbytes);

    бцел sz = _tysize[TYint];
    assert(SegData[seg].SDseg == seg);
    for (бцел n = 0; n < ncases; n++)          // send out значение table
    {
        //printf("\tcase %d, смещение = x%x\n", n, *poffset);
        objmod.write_bytes(SegData[seg],sz,p);
        p++;
    }
    смещение += alignbytes + sz * ncases;
    assert(*poffset == смещение);
    ...
    if (b.Btablesize == ncases * (REGSIZE * 2 + tysize(TYnptr)))
    {
        ...
        for (бцел n = 0; n < ncases; n++)
        {
            targ_т_мера val = cast(targ_т_мера)MSREG(*p);
            p++;
            objmod.write_bytes(SegData[seg],REGSIZE,&val);
        }
        смещение += REGSIZE * ncases;
        assert(*poffset == смещение);
    }

    list_t bl = b.Bsucc;
    for (бцел n = 0; n < ncases; n++)          // send out address table
    {
        bl = list_next(bl);
        objmod.reftocodeseg(seg,*poffset,list_block(bl).Boffset);
        *poffset += tysize(TYnptr);
    }
    assert(*poffset == смещение + ncases * tysize(TYnptr));
}

/*****************************
 * Return a jump opcode relevant to the elem for a JMP да.
 */
...
цел jmpopcode(elem *e)
{
    tym_t tym;
    цел нуль,i,jp,op;
    static const ббайт[6][2][2] jops =
    [   /* <=  >   <   >=  ==  !=    <=0 >0  <0  >=0 ==0 !=0    */
       [ [JLE,JG ,JL ,JGE,JE ,JNE],[JLE,JG ,JS ,JNS,JE ,JNE] ], /* signed   */
       [ [JBE,JA ,JB ,JAE,JE ,JNE],[JE ,JNE,JB ,JAE,JE ,JNE] ], /* бцел */
/+
...
+/
    ...
    бул needsNanCheck = tyfloating(tymx) && config.inline8087 &&
...
static if (1)
{
            ...
            {
                const бул NOSAHF = (I64 || config.fpxmmregs);
                if (нуль)
                {
                    if (NOSAHF)
                        op = swaprel(op);
            ...
            {
                if (нуль && config.target_cpu < TARGET_80386)
                { }
                else
                    op = swaprel(op);
            }
...
{
            if (нуль && !rel_exception(op) && config.target_cpu >= TARGET_80386)
                op = swaprel(op);
            else if (!нуль &&
...
else
{
    ...
    jp = jops[i][нуль][op - OPle];        /* table starts with OPle       */

    /* Try to rewrite бцел comparisons so they rely on just the Carry флаг
     */
    if (i == 1 && (jp == JA || jp == JBE) &&
...
L1:
    ...
    {
        WROP(op);
        printf("i %d нуль %d op x%x jp x%x\n",i,нуль,op,jp);
    }
 ...
 * Параметры:
 *    cdb = приставь generated code to this
 *    pcs = original addressing mode to be updated
 *    keepmsk = mask of registers we must not разрушь or use
...
проц cod3_ptrchk(ref CodeBuilder cdb,code *pcs,regm_t keepmsk)
{
    ...
    reg = rm & 7;
    if (I16)
    {   static const ббайт[8] imode = [ BP,BP,BP,BP,SI,DI,BP,BX ];
    ...
    {
        // Load the смещение into a register, so we can сунь the address
        ...
        pcs.Iflags &= ~(CFopsize | CFss | CFes | CFcs);        // no префикс bytes needed
    ...
    //используется = (mBP | ALLREGS | mES) & ~fregsaved;
    regm_t используется = 0;                           // much less code generated this way

    code *cs2 = null;
    regm_t tosave = используется & (keepmsk | idxregs);
    for (цел i = 0; tosave; i++)
    {
        regm_t mi = mask(i);
        ...
        if (mi & tosave)        /* i = register to save                 */
        {
            ...
            cdb.gen1(сунь);                     // PUSH i
            cs2 = cat(gen1(null,вынь),cs2);      // POP i
            tosave &= ~mi;
        }
    }

    // For 16 bit models, сунь a far pointer
    if (I16)
    {
        цел segreg;
    ...
    // Call the validation function
    {
        ...
        используется &= ~(keepmsk | idxregs);           // regs destroyed by this exercise
        getregs(cdb,используется);
                                                // CALL __ptrchk
...
/***********************************
 * Determine if BP can be используется as a general purpose register.
 * Note parallels between this routine and prolog().
 * Возвращает:
 *      0       can't be используется, needed for frame
 *      mBP     can be используется
 */
...
regm_t cod3_useBP()
{
    ...
    stackoffsets(0);
    localsize = Auto.смещение + Fast.смещение;                // an estimate only
//    if (localsize)
    {
        if (!(config.flags4 & CFG4speed) ||
            ...
            localsize >= 0x100 ||       // arbitrary значение < 0x1000
...
/*************************************************
 * Generate code segment to be используется later to restore a cse
 */
...
/**************************
 * Store `reg` to the common subВыражение save area in index `slot`.
 * Параметры:
 *      cdb = where to пиши code to
 *      tym = тип of значение that's in `reg`
 *      reg = register to save
 *      slot = index into common subВыражение save area
 */
проц gen_storecse(ref CodeBuilder cdb, tym_t tym, reg_t reg, т_мера slot)
{
    ...
    {
        const aligned = tyvector(tym) ? STACKALIGN >= 16 : да;
        const op = xmmstore(tym, aligned);
        cdb.genc1(op,modregxrm(2, reg - XMM0, BPRM),FLcs,cast(targ_т_мера)slot);
        return;
    }
...
проц gen_testcse(ref CodeBuilder cdb, tym_t tym, бцел sz, т_мера slot)
{
    // CMP slot[BP],0
 ...
 * This значение gets cached in the local variable 'localgot'.
 */
...
проц cdgot(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    ...
    else static if (TARGET_LINUX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
    {
        ...
        /* Because the 2:3 смещение from L1: is hardcoded,
...
private цел obj_namestring(сим *p,ткст0 имя)
{
    ...
    if (len > 255)
    {
        ...
        ps[1] = cast(short)len;
        memcpy(p + 4,имя,len);
        const цел ONS_OHD = 4;           // max # of extra bytes added by obj_namestring()
        len += ONS_OHD;
    }
    else
    {
...
проц gentstreg(ref CodeBuilder cdb, бцел t)
{
...
{
    // register вид. ex: GPR,XMM,SEG
    static бцел _K(бцел reg)
    {
        switch (reg)
        {
        case ES:                   return ES;
    ...
    // вид combination (order kept)
    static бцел _X(бцел to, бцел from) { return (_K(to) << 8) + _K(from); }

    if (to != from)
    {
        if (tym == TYMAX) tym = TYт_мера; // avoid register slicing
        switch (_X(to, from))
        {
            case _X(AX, AX):
...
/******************************
 * Load CX with the значение of _AHSHIFT.
 */
...
/******************************
 * Move constant значение into reg.
 * Take advantage of existing values in registers.
 * If flags & mPSW
 *      set flags based on результат
 * Else if flags & 8
...
проц movregconst(ref CodeBuilder cdb,reg_t reg,targ_т_мера значение,regm_t flags)
{
    ...
    //printf("movregconst(reg=%s, значение= %lld (%llx), flags=%x)\n", regm_str(mask(reg)), значение, значение, flags);

    regm_t regm = regcon.immed.mval & mask(reg);
    targ_т_мера regv = regcon.immed.значение[reg];
    ...
    if (flags & 1)      // 8 bits
    {
        ...
        // If we already have the right значение in the right register
        if (regm && (regv & 0xFF) == значение)
            goto L2;

        if (flags & 16 && reg & 4 &&    // if an H byte register
            regcon.immed.mval & mask(reg & 3) &&
            (((regv = regcon.immed.значение[reg & 3]) >> 8) & 0xFF) == значение)
            goto L2;
        ...
        for (mreg = (regcon.immed.mval & BYTEREGS); mreg; mreg >>= 1)
        {
            if (mreg & 1)
            {
                if ((regcon.immed.значение[r] & 0xFF) == значение)
                {
                ...
                if (!(I64 && reg >= 4) &&
                    r < 4 && ((regcon.immed.значение[r] >> 8) & 0xFF) == значение)
                {
        ...
        {
            cdb.genc2(0xC6,modregrmx(3,0,reg),значение);  // MOV regL,значение
            if (reg >= 4 && I64)
            {
                code_orrex(cdb.last(), REX);
            }
    ...
    L2:
        ...
        if (regm)
            // Set just the 'L' part of the register значение
            regimmed_set(reg,(regv & ~cast(targ_т_мера)0xFF) | значение);
        else if (flags & 16 && reg & 4 && regcon.immed.mval & mask(reg & 3))
            // Set just the 'H' part of the register значение
            regimmed_set((reg & 3),(regv & ~cast(targ_т_мера)0xFF00) | (значение << 8));
        return;
    }
L3:
    if (I16)
        значение = cast(targ_short) значение;             // sign-extend MSW
    else if (I32)
        значение = cast(targ_int) значение;

    if (!I16 && flags & 2)                      // load 16 bit значение
    {
        значение &= 0xFFFF;
        if (значение == 0)
            goto L1;
        else
        {
            if (flags & mPSW)
                goto L1;
            cdb.genc2(0xC7,modregrmx(3,0,reg),значение); // MOV reg,значение
            cdb.last().Iflags |= CFopsize;           // yes, even for I64
            if (regm)
                // High bits of register are not affected by 16 bit load
                regimmed_set(reg,(regv & ~cast(targ_т_мера)0xFFFF) | значение);
        }
        return;
    }
L1:
    ...
    // If we already have the right значение in the right register
    if (regm && (regv & 0xFFFFFFFF) == (значение & 0xFFFFFFFF) && !(flags & 64))
    {
        if (flags & mPSW)
            gentstreg(cdb,reg);
    }
    else if (flags & 64 && regm && regv == значение)
    ...
    else
    {
        if (flags & mPSW)
        {
            switch (значение)
            {
                ...
                L4:
                    if (flags & 64)
                    {
                        cdb.genc2(0xB8 + (reg&7),REX_W << 16 | (reg&8) << 13,значение); // MOV reg,value64
                        gentstreg(cdb,reg);
                        code_orrex(cdb.last(), REX_W);
                    }
                    else
                    {
                        значение &= 0xFFFFFFFF;
                        cdb.genc2(0xB8 + (reg&7),(reg&8) << 13,значение); // MOV reg,значение
                        gentstreg(cdb,reg);
                    }
                    break;
            }
        ...
        else
        {
            ...
            if (regcon.immed.mval & mAX)
            {
                if (I32)
                {
                    if (reg == AX && значение == cast(targ_short) regv)
                    {
                        cdb.gen1(0x98);               // CWDE
                        goto done;
                    }
                    if (reg == DX &&
                        значение == (regcon.immed.значение[AX] & 0x80000000 ? 0xFFFFFFFF : 0) &&
                ...
                {
                    if (reg == AX &&
                        cast(targ_short) значение == cast(byte) regv)
                    {
                        cdb.gen1(0x98);               // CBW
                        goto done;
                    }
                    ...
                    if (reg == DX &&
                        cast(targ_short) значение == (regcon.immed.значение[AX] & 0x8000 ? cast(targ_short) 0xFFFF : cast(targ_short) 0) &&
                        !(config.flags4 & CFG4speed && config.target_cpu >= TARGET_Pentium)
                       )
                    {
                        cdb.gen1(0x99);               // CWD
            ...
            if (значение == 0 && !(flags & 8) && config.target_cpu >= TARGET_80486)
            {
                genregs(cdb,0x31,reg,reg);              // XOR reg,reg
                goto done;
            }
            ...
            {
                if (regv + 1 == значение ||
                    // Уловитель case of (0xFFFF+1 == 0) for 16 bit compiles
                    (I16 && cast(targ_short)(regv + 1) == cast(targ_short)значение))
                {
                inc:
            ...
            {
                debug
                assert(!I16 || regcon.immed.значение[r] == cast(targ_short)regcon.immed.значение[r]);

                if (mreg & 1 && regcon.immed.значение[r] == значение)
                {
                    genmovreg(cdb,reg,r);
                    goto done;
                }
            ...
            {   // See if we can just load a byte
                ...
                {
                    if ((regv & ~cast(targ_т_мера)0xFF) == (значение & ~cast(targ_т_мера)0xFF))
                    {
                        movregconst(cdb,reg,значение,(flags & 8) |4|1);  // load regL
                        return;
                    }
                    if (regm & (mAX|mBX|mCX|mDX) &&
                        (regv & ~cast(targ_т_мера)0xFF00) == (значение & ~cast(targ_т_мера)0xFF00) &&
                        !I64)
                    {
                        movregconst(cdb,4|reg,значение >> 8,(flags & 8) |4|1|16); // load regH
                        return;
                    }
                }
                if (flags & 64)
...
проц genjmp(ref CodeBuilder cdb,opcode_t op,бцел fltarg,block *targ)
{
    ...
    cs.IFL2 = cast(ббайт)fltarg;        // FLblock (or FLcode)
...
{
    static const ббайт[4] ops2 = [ 0x60,0x1E,0x06,0 ];
    static const ббайт[11] ops0 = [ 0x50,0x51,0x52,0x53,
                                    0x54,0x55,0x56,0x57,
                                    0x1E,0x06,0 ];

    ббайт* p = (config.target_cpu >= TARGET_80286) ? ops2.ptr : ops0.ptr;
    do
        cdb.gen1(*p);
    while (*++p);
    ...
    if (localsize)
        cod3_stackadj(cdb, cast(цел)localsize);
...
проц prolog_16bit_windows_farfunc(ref CodeBuilder cdb, tym_t* tyf, бул* pushds)
{
    ...
    switch (wflags & (WFdgroup | WFds | WFss))
    {
        ...
        {
            if (wflags & WFreduced)
                *tyf &= ~mTYloadds;          // удали redundancy
 ...
 * Output:
 *      *enter          set to да if ENTER instruction can be используется, нет otherwise
 *      *xlocalsize     amount to be subtracted from stack pointer
 *      *cfa_offset     the frame pointer's смещение from the CFA
 * Возвращает:
 *      generated code
 */
...
static if (NTEXCEPTIONS == 2)
{
        ...
        {
            цел off = 2 * REGSIZE;      // 1 for the return address + 1 for the PUSH EBP
             ...
             * which is why the смещение of BP is set to 8
             */
        ...
        *enter = нет;              /* do not use ENTER instruction */
    }
    else
        *enter = да;
}
...
проц prolog_stackalign(ref CodeBuilder cdb)
{
    ...
    const смещение = (hasframe ? 2 : 1) * REGSIZE;   // 1 for the return address + 1 for the PUSH EBP
    if (смещение & (STACKALIGN - 1) || TARGET_STACKALIGN < STACKALIGN)
        cod3_stackalign(cdb, STACKALIGN);
}

проц prolog_frameadj(ref CodeBuilder cdb, tym_t tyf, бцел xlocalsize, бул enter, бул* pushalloc)
{
    бцел pushallocreg = (tyf == TYmfunc) ? CX : AX;
static if (TARGET_LINUX)
{
    бул check = нет;               // seems that Linux doesn't need to fault in stack pages
}
else
{
    бул check = (config.flags & CFGstack && !(I32 && xlocalsize < 0x1000)) // if stack overflow check
    ...
    if (check)
    {
        if (I16)
        {
            // BUG: Won't work if параметр is passed in AX
            movregconst(cdb,AX,xlocalsize,нет); // MOV AX,localsize
        ...
        {
            /* Watch out for 64 bit code where EDX is passed as a register параметр
             */
             ...
             */
            movregconst(cdb, reg, xlocalsize / 0x1000, нет);
    ...
    else
    {
        ...
        {   // ENTER xlocalsize,0
            ...
            assert(!(config.fulltypes == CVDWARF_C || config.fulltypes == CVDWARF_D)); // didn't emit Dwarf данные
        }
...
{
    //printf("prolog_setupalloca() смещение x%x size x%x alignment x%x\n",
        //cast(цел)Alloca.смещение, cast(цел)Alloca.size, cast(цел)Alloca.alignment);
    // Set up magic параметр for alloca()
    // MOV -REGSIZE[BP],localsize - BPoff
    cdb.genc(0xC7,modregrm(2,0,BPRM),
            FLconst,Alloca.смещение + BPoff,
 ...
 * Параметры:
 *      cdb = приставь generated instructions to this
 *      topush = mask of registers to сунь
 *      cfa_offset = смещение of frame pointer from CFA
 */
...
проц prolog_saveregs(ref CodeBuilder cdb, regm_t topush, цел cfa_offset)
{
    if (pushoffuse)
    {
        ...
        targ_т_мера xmmoffset = pushoff + BPoff;
        if (!hasframe || enforcealign)
            xmmoffset += EBPtoESP;
        targ_т_мера gpoffset = xmmoffset + xmmtopush * 16;
        while (topush)
        {
            ...
            else
            {
                ...
                if (config.fulltypes == CVDWARF_C || config.fulltypes == CVDWARF_D ||
                    config.ehmethod == EHmethod.EH_DWARF)
                {   // Emit debug_frame данные giving location of saved register
                    code *c = cdb.finish();
                    pinholeopt(c, null);
                    dwarf_CFA_set_loc(calcblksize(c));  // address after save
                    dwarf_CFA_offset(reg, cast(цел)(gpoffset - cfa_offset));
                    cdb.сбрось();
                    cdb.приставь(c);
                }
                gpoffset += REGSIZE;
    ...
    {
        while (topush)                      /* while registers to сунь      */
        {
            reg_t reg = findreg(topush);
            topush &= ~mask(reg);
            if (isXMMreg(reg))
...
private проц epilog_restoreregs(ref CodeBuilder cdb, regm_t topop)
{
    debug
    ...
    {
        // Save to preallocated section in the stack frame
        цел xmmtopop = numbitsset(topop & XMMREGS);   // XMM regs take 16 bytes
        цел gptopop = numbitsset(topop) - xmmtopop;   // general purpose registers to save
        targ_т_мера xmmoffset = pushoff + BPoff;
        if (!hasframe || enforcealign)
            xmmoffset += EBPtoESP;
        targ_т_мера gpoffset = xmmoffset + xmmtopop * 16;
        while (topop)
        {
...
проц prolog_trace(ref CodeBuilder cdb, бул farfunc, бцел* regsaved)
{
    ...
    /* Embedding the function имя inline after the call works, but it
     * makes disassembling the code annoying.
     */
    static if (ELFOBJ || MACHOBJ)
    {
        // Generate length prefixed имя that is recognized by profiler
        т_мера len = strlen(funcsym_p.Sident);
        сим *буфер = cast(сим *)malloc(len + 4);
        assert(буфер);
        if (len <= 254)
        {
            буфер[0] = len;
            memcpy(буфер + 1, funcsym_p.Sident, len);
            len++;
        }
        else
        {
            ...
            буфер[2] = len & 0xFF;
            буфер[3] = len >> 8;
            memcpy(буфер + 4, funcsym_p.Sident, len);
            len += 4;
        }
        cdb.genasm(буфер, len);         // приставь func имя
        free(буфер);
    }
    else
    {
        сим [IDMAX+IDOHD+1] имя = проц;
        т_мера len = objmod.mangle(funcsym_p,имя.ptr);
        assert(len < имя.length);
        cdb.genasm(имя.ptr,len);             // приставь func имя
    }
...
проц prolog_genvarargs(ref CodeBuilder cdb, Symbol* sv, regm_t namedargs)
{
     ...
     *     ук reg_args;
     *   }
     * The MOVAPS instructions seg fault if данные is not aligned on
     * 16 bytes, so this gives us a nice check to ensure no mistakes.
        ...
        MOV     voff+4*8[RBP],R8
        MOV     voff+5*8[RBP],R9
        MOVZX   EAX,AL                      // AL = 0..8, # of XMM registers используется
        SHL     EAX,2                       // 4 bytes for each MOVAPS
        LEA     R11,смещение L2[RIP]
        SUB     R11,RAX
        LEA     RAX,voff+6*8+0x7F[RBP]
        JMP     R11d
        MOVAPS  -0x0F[RAX],XMM7             // only save XMM registers if actually используется
      ...
      L2:
        ...
        LEA     R11, Para.size+Para.смещение[RBP]
    ...
    */
     ...
     */
    targ_т_мера voff = Auto.size + BPoff + sv.Soffset;  // EBP смещение of start of sv
    const цел vregnum = 6;
    const бцел vsize = vregnum * 8 + 8 * 16;

    static const ббайт[vregnum] regs = [ DI,SI,DX,CX,R8,R9 ];

    if (!hasframe || enforcealign)
        voff += EBPtoESP;

    for (цел i = 0; i < vregnum; i++)
    {
        бцел r = regs[i];
        if (!(mask(r) & namedargs))  // unnamed arguments would be the ... ones
        {
            бцел ea = (REX_W << 16) | modregxrm(2,r,BPRM);
            if (!hasframe || enforcealign)
    ...
    цел raxoff = cast(цел)(voff+6*8+0x7F);
    бцел L2offset = (raxoff < -0x7F) ? 0x2D : 0x2A;
    if (!hasframe || enforcealign)
    ...
    бцел ea = (REX_W << 16) | modregrm(2,AX,BPRM);
    if (!hasframe || enforcealign)
     ...
     */
    бцел offset_regs = 0;
    бцел offset_fpregs = vregnum * 8;
    for (цел i = AX; i <= XMM7; i++)
    {
        regm_t m = mask(i);
        if (m & namedargs)
        {
    ...
    Para.смещение = (Para.смещение + (REGSIZE - 1)) & ~(REGSIZE - 1);
    cdb.genc1(LEA,(REX_W << 16) | ea,FLconst,Para.size + Para.смещение);
 ...
 *      tf = what's the тип of the function
 *      pushalloc = use PUSH to размести on the stack rather than subtracting from SP
...
проц prolog_loadparams(ref CodeBuilder cdb, tym_t tyf, бул pushalloc, out regm_t namedargs)
{
    ...
    {
        Symbol *s = globsym.tab[si];
        бцел sz = cast(бцел)type_size(s.Stype);
        ...
        {   // Argument is passed in a register
            ...
            //
            // Treat массив of 1 the same as its element тип
            ...
            // If struct just wraps another тип
            if (tyb == TYstruct)
            {
                ...
                {
                    тип *targ1 = t.Ttag.Sstruct.Sarg1type;
            ...
            {
                targ_т_мера смещение = Fast.size + BPoff;
                ...
                reg_t preg = s.Spreg;
                for (цел i = 0; i < 2; ++i)     // twice, once for each possible параметр register
                {
                    ...
                    if (!(pushalloc && preg == pushallocreg) || s.Sclass == SCshadowreg)
                    {
                        ...
                        {
                            // MOV x[EBP],preg
                            cdb.genc1(op,modregxrm(2,preg,BPRM),FLconst,смещение);
                            if (isXMMreg(preg))
                            {
                            ...
                            else
                            {
                                ...
                                //         s.Sident, (цел)Fast.size, (цел)BPoff, (цел)s.Soffset, (цел)sz);
                                if (I64 && sz > 4)
                                    code_orrex(cdb.last(), REX_W);
                            }
                        }
                        ...
                        {
                            // MOV смещение[ESP],preg
                            // BUG: byte size?
                            cdb.genc1(op,
                                      (modregrm(0,4,SP) << 8) |
                                       modregxrm(2,preg,4),FLconst,смещение);
                            if (isXMMreg(preg))
                            {
                                checkSetVex(cdb.last(), t.Tty);
                            }
    ...
    if (config.exe == EX_WIN64 && variadic(funcsym_p.Stype))
    {
         ...
         */
        static const reg_t[4] vregs = [ CX,DX,R8,R9 ];
        for (цел i = 0; i < vregs.length; ++i)
        {
            бцел preg = vregs[i];
            бцел смещение = cast(бцел)(Para.size + i * REGSIZE);
            if (!(shadowregm & (mask(preg) | mask(XMM0 + i))))
            {
                if (hasframe)
                {
                    // MOV x[EBP],preg
                    cdb.genc1(0x89,
                                     modregxrm(2,preg,BPRM),FLconst, смещение);
                    code_orrex(cdb.last(), REX_W);
                }
                else
                {
                    ...
                    cdb.genc1(0x89,
                                     (modregrm(0,4,SP) << 8) |
                                     modregxrm(2,preg,4),FLconst,смещение + EBPtoESP);
                }
                cdb.last().Irex |= REX_W;
            }
        }
     ...
     * initialize the registers with the параметр stack values.
    ...
    for (SYMIDX si = 0; si < globsym.top; si++)
    {
        ...
        if ((s.Sclass == SCregpar || s.Sclass == SCparameter) &&
            ...
            (refparam
                ...
                || Dinrus && s.Stype.Tty & mTYvolatile
                ))
        {
            // MOV reg,param[BP]
            //assert(refparam);
            ...
            else
            {
                ...
                {   // Convert to ESP relative address rather than EBP
                    assert(!I16);
                    c.Irm = cast(ббайт)modregxrm(2,s.Sreglsw,4);
                    c.Isib = modregrm(0,4,SP);
                    c.IEV1.Vpointer += EBPtoESP;
                }
                if (sz > REGSIZE)
                {
                    ...
                    {   // Convert to ESP relative address rather than EBP
                        assert(!I16);
                        cx.Irm = cast(ббайт)modregxrm(2,s.Sregmsw,4);
...
проц epilog(block *b)
{
    ...
    regm_t topop,regm;
    targ_т_мера xlocalsize = localsize;

    CodeBuilder cdbx; cdbx.ctor();
    tym_t tyf = funcsym_p.ty();
    tym_t tym = tybasic(tyf);
    бул farfunc = tyfarfunc(tym) != 0;
    ...
    if (tym == TYifunc)
    {
        static const ббайт[5] ops2 = [ 0x07,0x1F,0x61,0xCF,0 ];
        static const ббайт[12] ops0 = [ 0x07,0x1F,0x5F,0x5E,
                                        0x5D,0x5B,0x5B,0x5A,
                                        0x59,0x58,0xCF,0 ];
    ...
    if (usednteh & (NTEH_try | NTEH_except | NTEHcpp | EHcleanup | EHtry | NTEHpassthru) && (config.exe == EX_WIN32 || Dinrus))
    {
        nteh_epilog(cdbx);
    }
    ...
    else
    {
        ...
        L4:
            ...
            if (xlocalsize || enforcealign)
            {
                ...
                {   /*   MOV  ECX,0xBEAF
                     ...
                     * 4. be a значение that stands out in the debugger
                     */
                    assert(I32 || I64);
                    targ_т_мера значение = 0x0000BEAF;
                    reg_t regcx = CX;
                    mfuncreg &= ~mask(regcx);
                    бцел grex = I64 ? REX_W << 16 : 0;
                    cdbx.genc2(0xC7,grex | modregrmx(3,0,regcx),значение);   // MOV regcx,значение
        ...
        else if (xlocalsize == REGSIZE && (!I16 || b.BC == BCret))
        {
            ...
            cod3_stackadj(cdbx, cast(цел)-xlocalsize);
    }
...
Lret:
        ...
        if (tym == TYhfunc)
        {
            cdbx.genc2(0xC2,0,4);                       // RET 4
                 ...
                 config.exe == EX_WIN64 ||
                 Para.смещение == 0)                      // or nothing pushed on the stack anyway
        {
            op++;                                       // to a regular RET
            cdbx.gen1(op);
        }
        ...
        {   // Stack is always aligned on register size boundary
            Para.смещение = (Para.смещение + (REGSIZE - 1)) & ~(REGSIZE - 1);
            if (Para.смещение >= 0x10000)
            {
                /*
                    POP REG
                    ADD ESP, Para.смещение
                    JMP REG
                */
                cdbx.gen1(0x58+regx);
                cdbx.genc2(0x81, modregrm(3,0,SP), Para.смещение);
                if (I64)
                    code_orrex(cdbx.last(), REX_W);
                ...
                cdbx.genc2(op,0,Para.смещение);          // RET Para.смещение
        }
    }
...
/*******************************
 * Return смещение of SP from BP.
 */

targ_т_мера cod3_spoff()
{
    //printf("spoff = x%x, localsize = x%x\n", (цел)spoff, (цел)localsize);
...
проц gen_spill_reg(ref CodeBuilder cdb, Symbol* s, бул toreg)
{
    ...
    {
        const цел sz = cast(цел)type_size(s.Stype);
 ...
 *      thisty =  Тип of this pointer
 *      p =       ESP параметр смещение to this pointer
 *      d =       смещение to add to 'this' pointer
 *      d2 =      смещение from 'this' to vptr
 *      i =       смещение into vtbl[]
 */

проц cod3_thunk(Symbol *sthunk,Symbol *sfunc,бцел p,tym_t thisty,
        бцел d,цел i,бцел d2)
{
    targ_т_мера thunkoffset;
    ...
    {
        const бул FARTHIS = (tysize(thisty) > REGSIZE);
        const бул FARVPTR = FARTHIS;
 ...
 * referenced.
 * BFLjmpoptdone is set to да if nothing more can be done
 * with this block.
 * Input:
 *      флаг    !=0 means don't have correct Boffsets yet
 * Возвращает:
...
цел branch(block *bl,цел флаг)
{
    ...
    if (!c)
        return 0;
    bytesaved = 0;
    смещение = bl.Boffset;                 /* смещение of start of block     */
    while (1)
    {
        ббайт op;
            ...
            (op == JMP && !(c.Iflags & CFjmp5)))
        {
          L1:
            ...
            {
                case FLblock:
                    if (флаг)           // no offsets yet, don't optimize
                        goto L3;
                    disp = c.IEV2.Vblock.Boffset - смещение - csize;
                     ...
                     * closer.
                     */
                    if (disp >= 0x7F-4 && c.IEV2.Vblock.Boffset > смещение)
                ...
                case FLcode:
                {
                    ...
                    if (!cr)
                    {   // Didn't найди it in forward search. Try backwards jump
                        цел s = 0;
                        disp = 0;
                    ...
                    if (config.flags4 & CFG4optimized && !флаг)
                    {
            ...
            if (disp == 0)                      // bra to следщ instruction
            {
                bytesaved += csize;
            ...
            else if (cast(targ_т_мера)cast(targ_schar)(disp - 2) == (disp - 2) &&
                     cast(targ_т_мера)cast(targ_schar)disp == disp)
            {
                if (op == JMP)
                {
                ...
                else                            // else Jcond
                {
                       ...
                       )
                    {
                        ...
                        // If nobody else points to ct, we can удали the CFtarg
                        if (флаг && ct)
                        {
                            code *cx;
...
//printf("s = '%s', Soffset = x%x, Para.size = x%x, EBPtoESP = x%x\n", s.Sident, s.Soffset, Para.size, EBPtoESP);
            ...
            case SCfastpar:
//printf("\tfastpar %s %p Soffset %x Fast.size %x BPoff %x\n", s.Sident, s, (цел)s.Soffset, (цел)Fast.size, (цел)BPoff);
...
//printf("s = '%s', Soffset = x%x, Auto.size = x%x, BPoff = x%x EBPtoESP = x%x\n", s.Sident, (цел)s.Soffset, (цел)Auto.size, (цел)BPoff, (цел)EBPtoESP);
//              if (!(funcsym_p.Sfunc.Fflags3 & Fnested))
...
{
    цел EBPtoESPsave = EBPtoESP;
    цел hasframesave = hasframe;
    ...
    for (; c; c = code_next(c))
    {
        ...
        switch (c.IFL1)
        {
            ...
            case FLstack:
                ...
                c.IEV1.Vpointer += s.Soffset + EBPtoESP - base - EEStack.смещение;
                break;
            ...
            L1:
                if (Symbol_Sisdead(s, anyiasm))
                {
                    c.Iop = NOP;               // удали references to it
                    continue;
                }
            ...
            case FLallocatmp:
                c.IEV1.Vpointer += Alloca.смещение + BPoff;
                goto L2;

            case FLfuncarg:
                c.IEV1.Vpointer += cgstate.funcarg.смещение + BPoff;
                goto L2;
            ...
            case FLcs:
                ...
                c.IEV1.Vpointer = CSE.смещение(sn) + CSoff + BPoff;
                c.Iflags |= CFunambig;
                goto L2;
        ...
        switch (c.IFL2)
        {
            case FLdata:
                ...
                else
                {
                    ...
                    c.IEV2.Vseg = Dinrus ? s.Sseg : DATA;
            ...
            case FLudata:
                ...
                {
                    c.IEV2.Vseg = Dinrus ? s.Sseg : UDATA;
                    c.IEV2.Vpointer += s.Soffset;
                    c.IFL2 = FLdatseg;
                    goto done;
                }
            ...
            case FLallocatmp:
                c.IEV2.Vpointer += Alloca.смещение + BPoff;
                goto L3;

            case FLfuncarg:
                c.IEV2.Vpointer += cgstate.funcarg.смещение + BPoff;
                goto L3;
...
/*******************************
 * Return смещение from BP of symbol s.
 */
...
targ_т_мера cod3_bpoffset(Symbol *s)
{
    ...
    switch (s.Sfl)
    {
        ...
        case FLfast:
            смещение += Fast.size + BPoff;
            break;

        case FLauto:
            смещение += Auto.size + BPoff;
            break;
 ...
 *      replaces jmps to the следщ instruction with NOPs
 *      sign extension of modregrm displacement
 *      sign extension of immediate данные (can't do it for OR, AND, XOR
...
проц pinholeopt(code *c,block *b)
{
    targ_т_мера a;
    бцел mod;
    ббайт ins;
    ...
    debug
    {
         цел tested; if (!tested) { tested++; pinholeopt_unittest(); }
    }
    ...
    L1:
        ...
        {
            цел shortop = (c.Iflags & CFopsize) ? !I16 : I16;
            цел local_BPRM = BPRM;
            ...
            if ((ins & T ||
                ...
                c.IFL2 == FLconst)
            {
                цел flags = c.Iflags & CFpsw;      /* if want результат in flags */
                ...
                if (op == 0xF7 && rm >= modregrm(3,0,AX) && rm <= modregrm(3,0,7) && (I64 || ereg < 4))
                {
                    if (u == 0xFF)
                    {
                        if (ereg & 4) // SIL,DIL,BPL,SPL need REX префикс
                            c.Irex |= REX;
                        goto L4;
                    }
                ...
                /* Look for sign extended immediate данные */
                if (cast(byte) u == u)
                {
                    if (op == 0x81)
                    {
            ...
            // Look to удали redundant REX префикс on XOR
            ...
            {
                c.Iop &= 1;
                c.Irm = cast(ббайт)((rm & modregrm(3,0,7)) | (ereg << 3));
                if (c.Irex & REX_B)
                    c.Irex |= REX_R;
                ...
                c.IFL1 == FLconst)      // and it's a constant
            {
                ...
                {
                    if (cast(targ_т_мера)cast(targ_schar)a == a)
                        c.Irm ^= 0xC0;                 /* do 8 sx      */
                }
                else if ((cast(targ_т_мера)cast(targ_schar)a & 0xFFFF) == (a & 0xFFFF))
                    c.Irm ^= 0xC0;                     /* do 8 sx      */
            }
            ...
            if (op == LEA)
            {
                        ...
                        c.IFL1 == FLconst && c.IEV1.Vpointer == 0)
                {
                    ...
                    c.Irm = cast(ббайт)(modregrm(3,0,BP) + reg);
                }
            }
        ...
        else if (!(c.Iflags & CFvex))
        {
            switch (op)
            {
                default:
                    ...
                    {
                        /* Look for нуль extended immediate данные */
                        if (c.IEV2.Vт_мера == c.IEV2.Vuns)
                        {
                            c.Irex &= ~REX_W;
                        }
                        /* Look for sign extended immediate данные */
                        else if (c.IEV2.Vт_мера == c.IEV2.Vint)
                        {
                            c.Irm = modregrm(3,0,op & 7);
                            c.Iop = op = 0xC7;
                            c.IEV2.Vт_мера = c.IEV2.Vuns;
                        }
                    }
                    if ((op & ~0x0F) != 0x70)
                        break;
...
debug
{
private проц pinholeopt_unittest()
{
...
+/
        ...
        [ { 64,0xB8,0x80000,0,cast(targ_т_мера)0x1FFFFFFFF,0 }, { 0,0xB8,0x80000,0,cast(targ_т_мера)0x1FFFFFFFF,0 }],
        [ { 64,0xB8,0x80000,0,cast(targ_т_мера)0xFFFFFFFFFFFFFFFF,0 }, { 0,0xC7,0x800C0,0,cast(targ_т_мера)0xFFFFFFFF,0}],
    ];

    //config.flags4 |= CFG4space;
    for (цел i = 0; i < tests.length; i++)
    {   CS *pin  = &tests[i][0];
        CS *pout = &tests[i][1];
        code cs = проц;
        ...
        cs.IFL1 = FLconst;
        cs.IFL2 = FLconst;
        cs.IEV1.Vт_мера = pin.ev1;
        cs.IEV2.Vт_мера = pin.ev2;
        cs.Iflags = pin.flags;
        pinholeopt(&cs, null);
        if (cs.Iop != pout.op)
        ...
        assert(cs.IEV1.Vт_мера == pout.ev1);
        assert(cs.IEV2.Vт_мера == pout.ev2);
...
проц simplify_code(code* c)
{
    ...
    if (config.flags4 & CFG4optimized &&
        ...
        c.IFL2 == FLconst &&
        reghasvalue((c.Iop == 0x80) ? BYTEREGS : ALLREGS,I64 ? c.IEV2.Vт_мера : c.IEV2.Vlong,&reg) &&
        !(I16 && c.Iflags & CFopsize)
       )
    {
        // See if we can replace immediate instruction with register instruction
        static const ббайт[8] regop =
...
проц jmpaddr(code *c)
{
    ...
    while (c)
    {
            ...
            ((op & ~0x0F) == 0x70 || op == JMP || op == JMPS || op == JCXZ || op == CALL))
        {
            ...
            if (!ci)
                goto Lbackjmp;      // couldn't найди it
 ...
 * Note that NOPs are sometimes используется as markers, but are
...
бцел calccodsize(code *c)
{
    ...
    switch (op)
    {
        ...
        case 0xA3:
            if (c.Irex)
            {
                size = 9;               // 64 bit immediate значение for MOV to/from RAX
                goto Lret;
            }
            goto Ldefault;
...
/**************************
 * Write code to intermediate файл.
 * Code starts at смещение.
 * Возвращает:
 *      addr of end of code
 */
...
struct MiniCodeBuf
{
    ...
    {
        index = 0;
        this.смещение = cast(т_мера)Offset(seg);
        this.seg = seg;
    }
    ...
    проц flushx()
    {
        ...
        смещение += objmod.bytes(seg, смещение, cast(бцел)index, bytes.ptr);
        index = 0;
    }

    проц gen(сим c) { bytes[index++] = c; }

    проц genp(т_мера n, проц *p) { memcpy(&bytes[index], p, n); index += n; }

    проц flush() { if (index) flushx(); }

    бцел getOffset() { return cast(бцел)(смещение + index); }

    бцел доступно() { return cast(бцел)(bytes.sizeof - index); }
}

private проц do8bit(MiniCodeBuf *pbuf, FL, evc *);
private проц do16bit(MiniCodeBuf *pbuf, FL, evc *,цел);
private проц do32bit(MiniCodeBuf *pbuf, FL, evc *,цел,цел = 0);
private проц do64bit(MiniCodeBuf *pbuf, FL, evc *,цел);
...
бцел codout(цел seg, code *c)
{
    ...
    debug
    if (debugc) printf("codout(%p), Coffset = x%llx\n",c,cast(бдол)Offset(seg));

    MiniCodeBuf ggen = проц;
    ggen.index = 0;
    ggen.смещение = cast(т_мера)Offset(seg);
    ggen.seg = seg;
    ...
    for (; c; c = code_next(c))
    {
        debug
        {
        if (debugc) { printf("off=%02lx, sz=%ld, ",cast(цел)ggen.getOffset(),cast(цел)calccodsize(c)); code_print(c); }
        бцел startoffset = ggen.getOffset();
        }
        ...
        switch (op & 0xFF)
        {
            case ESCAPE:
                ...
                {   case ESClinnum:
                        ...
                        objmod.номстр(c.IEV1.Vsrcpos,seg,ggen.getOffset());
                        break;
version (SCPP)
{
static if (1)
...
else
{
            ...
            case ASM:
                ...
                {
                    ggen.смещение += objmod.bytes(seg,ggen.смещение,cast(бцел)c.IEV1.len,c.IEV1.bytes);
                }
                debug
        ...
        if (ggen.доступно() < (1+4+4+8+8))
            ggen.flush();

        // see if we need to put out префикс bytes
        if (flags & (CFwait | CFPREFIX | CFjmp16))
        {
            ...
            {   case CFes:      override_ = SEGES;       goto segover;
                ...
                segover:        ggen.gen(cast(ббайт)override_);
                                break;
            ...
            if ((op & ~0x0F) == 0x70 && flags & CFjmp16) /* long condit jmp */
            {
                ...
                else
                {
                    cn = code_calloc();
                    /*cxcalloc++;*/
                    cn.следщ = code_next(c);
                    c.следщ= cn;          // link into code
                    cn.Iop = JMP;              // JMP block
                    cn.IFL2 = c.IFL2;
                    cn.IEV2.Vblock = c.IEV2.Vblock;
                    c.Iop = op ^= 1;           // toggle условие
                    c.IFL2 = FLconst;
        ...
        if (flags & CFvex)
        {
            ...
            {
                ggen.gen(0xC4);
                ggen.gen(cast(ббайт)VEX3_B1(c.Ivex));
                ggen.gen(cast(ббайт)VEX3_B2(c.Ivex));
                ggen.gen(c.Ivex.op);
            }
            else
            {
                ggen.gen(0xC5);
                ggen.gen(cast(ббайт)VEX2_B1(c.Ivex));
                ggen.gen(c.Ivex.op);
            }
            ins = vex_inssize(c);
            goto Lmodrm;
...
static if (TARGET_OSX || TARGET_WINDOS)
{
        ...
        `                        */
                        ...
                        do32bit(&ggen, cast(FL)c.IFL1,&c.IEV1,cfflags,cast(цел)val);
                        break;
                    }
            ...
            else
            {
                switch (op)
                {
                    ...
                    ptr1632:
                        ...
                        if (c.IFL2 == FLdatseg)
                        {
                            objmod.reftodatseg(seg,ggen.смещение,c.IEV2.Vpointer,
                                    c.IEV2.Vseg,flags);
                            ggen.смещение += 4;
                        }
                        else
                        {
                            s = c.IEV2.Vsym;
                            ggen.смещение += objmod.reftoident(seg,ggen.смещение,s,0,flags);
                        }
                        break;
        ...
        {
            printf("actual: %d, calc: %d\n", cast(цел)(ggen.getOffset() - startoffset), cast(цел)calccodsize(c));
            code_print(c);
            assert(0);
        }
    }
    ggen.flush();
    Offset(seg) = ggen.смещение;
    //printf("-codout(), Coffset = x%x\n", Offset(seg));
    return cast(бцел)ggen.смещение;                      /* ending address               */
}


private проц do64bit(MiniCodeBuf *pbuf, FL fl, evc *uev,цел flags)
{
    сим *p;
    Symbol *s;
    targ_т_мера ad;
    ...
    switch (fl)
    {
        ...
        case FLdatseg:
            pbuf.flush();
            objmod.reftodatseg(pbuf.seg,pbuf.смещение,uev.Vpointer,uev.Vseg,CFoffset64 | flags);
            break;
        ...
        case FLswitch:
            ...
            if (config.flags & CFGromable)
                    objmod.reftocodeseg(pbuf.seg,pbuf.смещение,ad);
            else
                    objmod.reftodatseg(pbuf.seg,pbuf.смещение,ad,objmod.jmpTableSegment(funcsym_p),CFoff);
            break;
        ...
        case FLfardata:
            ...
            // un-named external with is the start of .rodata or .данные
        case FLextern:                      /* external данные symbol         */
        case FLtlsdata:
static if (TARGET_LINUX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
{
        case FLgot:
        case FLgotoff:
            ...
            objmod.reftoident(pbuf.seg,pbuf.смещение,s,uev.Voffset,CFoffset64 | flags);
            break;
...
static if (TARGET_OSX)
{
        ...
        case FLfunc:                        /* function call                */
            ...
            pbuf.flush();
            objmod.reftoident(pbuf.seg,pbuf.смещение,s,0,CFoffset64 | flags);
            break;
        ...
        case FLblockoff:
            pbuf.flush();
            assert(uev.Vblock);
            //printf("FLblockoff: смещение = %x, Boffset = %x, funcoffset = %x\n", pbuf.смещение, uev.Vblock.Boffset, funcoffset);
            objmod.reftocodeseg(pbuf.seg,pbuf.смещение,uev.Vblock.Boffset);
            break;
...
private проц do32bit(MiniCodeBuf *pbuf, FL fl, evc *uev,цел flags, цел val)
{
    сим *p;
    Symbol *s;
    targ_т_мера ad;
    ...
    {
        case FLconst:
            assert(targ_т_мера.sizeof == 4 || targ_т_мера.sizeof == 8);
            ad = * cast(targ_т_мера *) uev;
        L1:
            pbuf.genp(4,&ad);
            return;
        ...
        case FLdatseg:
            pbuf.flush();
            objmod.reftodatseg(pbuf.seg,pbuf.смещение,uev.Vpointer,uev.Vseg,flags);
            break;
        ...
        case FLcode:
            ...
            pbuf.flush();
            ad = *cast(targ_т_мера *) uev + pbuf.getOffset();
            objmod.reftocodeseg(pbuf.seg,pbuf.смещение,ad);
            break;
    ...
    static if (TARGET_LINUX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
    {
        ...
        case FLgotoff:
            ...
            if (TARGET_WINDOS && I64 && (flags & CFpc32))
            {
                 ...
                 */
                assert(val >= -5 && val <= 0);
                flags |= (-val & 7) << 24;          // set CFREL значение
                assert(CFREL == (7 << 24));
                objmod.reftoident(pbuf.seg,pbuf.смещение,s,uev.Voffset,flags);
            }
            else
                objmod.reftoident(pbuf.seg,pbuf.смещение,s,uev.Voffset + val,flags);
            break;
    ...
    static if (TARGET_OSX)
    {
        ...
        case FLfunc:                        /* function call                */
            ...
            {   /* Large code references are always absolute    */
                pbuf.flush();
                pbuf.смещение += objmod.reftoident(pbuf.seg,pbuf.смещение,s,0,flags) - 4;
            }
            ...
            else
            {
                assert(TARGET_SEGMENTED || !tyfarfunc(s.ty()));
                pbuf.flush();
                objmod.reftoident(pbuf.seg,pbuf.смещение,s,val,flags);
            }
            break;
...
private проц do16bit(MiniCodeBuf *pbuf, FL fl, evc *uev,цел flags)
{
    сим *p;
    Symbol *s;
    targ_т_мера ad;
    ...
    switch (fl)
    {
        ...
        case FLfunc:                        /* function call                */
            ...
            {   /* Large code references are always absolute    */
                pbuf.flush();
                pbuf.смещение += objmod.reftoident(pbuf.seg,pbuf.смещение,s,0,flags) - 2;
            }
            else if (s.Sseg == pbuf.seg &&
                     (s.Sclass == SCstatic || s.Sclass == SCglobal) &&
                     s.Sxtrnnum == 0 && flags & CFselfrel)
            ...
            {
                pbuf.flush();
                objmod.reftoident(pbuf.seg,pbuf.смещение,s,0,flags);
            }
            break;
...
private проц do8bit(MiniCodeBuf *pbuf, FL fl, evc *uev)
{
    сим c;
    ...
    switch (fl)
    {
        ...
        case FLblock:
            ...
            if (cast(byte)delta != delta)
            {
                ...
                printf("block displacement of %lld exceeds the maximum смещение of -128 to 127.\n", cast(long)delta);
                err_exit();
            }
            c = cast(сим)delta;
...
проц code_hydrate(code **pc)
{
    ...
    while (*pc)
    {
        c = cast(code *) ph_hydrate(cast(ук*)pc);
        if (c.Iflags & CFvex && c.Ivex.pfx == 0xC4)
            ins = vex_inssize(c);
        else if ((c.Iop & 0xFFFD00) == 0x0F3800)
            ins = inssize2[(c.Iop >> 8) & 0xFF];
        ...
        switch (c.Iop)
        {
            ...
            case ASM:
                ph_hydrate(cast(ук*)&c.IEV1.bytes);
                goto done;
        }
        if (!(ins & M) ||
            ((rm = c.Irm) & 0xC0) == 0xC0)
        ...
        switch (fl)
        {
            ...
            case FLcode:
                ph_hydrate(cast(ук*)&c.IEV1.Vcode);
                break;

            case FLblock:
            case FLblockoff:
                ph_hydrate(cast(ук*)&c.IEV1.Vblock);
                break;
version (SCPP)
{
            case FLctor:
    ...
    do2:
        ...
        switch (fl)
        {
            ...
            case FLcode:
                ph_hydrate(cast(ук*)&c.IEV2.Vcode);
                break;

            case FLblock:
            case FLblockoff:
                ph_hydrate(cast(ук*)&c.IEV2.Vblock);
                break;
...
extern (C) проц code_print(code* c)
{
    ббайт ins;
    ббайт rexb;
    ...
    if (ins & M)
    {
        ...
        if ((rm & 0xC7) == BPRM || (rm & 0xC0) == 0x80 || (rm & 0xC0) == 0x40)
        {
            switch (c.IFL1)
            {
                ...
                case FLdatseg:
                    printf(" %d.%llx",c.IEV1.Vseg,cast(бдол)c.IEV1.Vpointer);
                    break;
                ...
                case FLextern:
                    printf(" FLextern смещение = %4d",cast(цел)c.IEV1.Voffset);
                    break;
    ...
    if (ins & T)
    {
        ...
        switch (c.IFL2)
        {
            ...
            case FLdatseg:
                printf(" %d.%llx",c.IEV2.Vseg,cast(бдол)c.IEV2.Vpointer);
                break;
