...
version (SCPP)
    const SCPP_OR_NTEXCEPTIONS = да;
else static if (NTEXCEPTIONS)
    const SCPP_OR_NTEXCEPTIONS = да;
else
    const SCPP_OR_NTEXCEPTIONS = нет;
...
extern (C) проц *mem_fcalloc(т_мера numbytes); // tk/mem.c
extern (C) проц mem_free(ук); // tk/mem.c

alias mem_free MEM_PH_FREE;

version (HTOD)
    проц *util_realloc(ук p, бцел n, бцел size);
else
    import drc.backend.gflow : util_realloc;

//__gshared
//{
    бцел numblks;           // number of basic blocks in current function
    ...
    block *block_last;      // last block читай in
...
pragma(inline, да) block *block_calloc_i()
{
    block *b;
...
/**************************
 * Завершить этот блок и начать следующий.
 */
...
{
    bctx.curblock.BC = cast(ббайт) bc;
    block_last = bctx.curblock;
    if (!bn)
        bn = block_calloc_i();
    bctx.curblock.Bnext = bn;                // следщ block
...
{
проц block_next(цел bc,block *bn)
{
    curblock.BC = cast(ббайт) bc;
    curblock.Bsymend = globsym.top;
    block_last = curblock;
    if (!bn)
        bn = block_calloc_i();
    curblock.Bnext = bn;                     // следщ block
...
version (Dinrus)
{
block *block_goto(Blockx *bx,цел bc,block *bn)
{
    block *b;
...
/****************************
 * Перейти к блоку с названием gotolbl.
...
проц block_goto(block *bgoto,block *bnew)
{
    ...
    if (curblock.Bcode)         // If there is already code in the block
        ...
        bc = BCgoto;            // fall thru to следщ block
    block_next(bc,bnew);
}
...
{
    block_clearvisit();
    block_visit(startblock);                    // посети all reachable blocks
...
/********************************
 * Free optimizer gathered данные.
 */

проц block_optimizer_free(block *b)
{
    static проц vfree(ref vec_t v) { vec_free(v); v = null; }
    vfree(b.Bdom);
    vfree(b.Binrd);
    vfree(b.Boutrd);
    vfree(b.Binlv);
...
проц blocklist_hydrate(block **pb)
{
    while (isdehydrated(*pb))
    {
        ...
        block *b = cast(block *)ph_hydrate(cast(ук*)pb);
        /*printf(" %p\n",b);*/
        el_hydrate(&b.Belem);
        list_hydrate(&b.Bsucc,FPNULL);
        list_hydrate(&b.Bpred,FPNULL);
        cast(проц) ph_hydrate(cast(ук*)&b.Btry);
        cast(проц) ph_hydrate(cast(ук*)&b.Bendscope);
        ...
        switch (b.BC)
        {
            ...
            case BCswitch:
                ph_hydrate(cast(ук*)&b.Bswitch);
                break;

            case BC_finally:
                //(проц) ph_hydrate(&b.B_ret);
                break;

            case BC_lpad:
                symbol_hydrate(&b.флаг);
                break;
 ...
 * Read in an Выражение and put it in curblock.Belem.
 * If there is one already there, создай a tree like:
 *              ,
 *             / \
 *           old  e
 */
...
проц blockopt(цел iter)
{
    if (OPTIMIZER)
    {
        blassertsplit();                // only need this once

        цел iterationLimit = 200;
        ...
        do
        {
            ...
            brcombine();                // convert graph to Выражения
            if (iter >= 2)
                brmin();                // minimize branching
            do
            {
                compdfo();              /* compute depth first order (DFO) */
                elimblks();             /* удали blocks not in DFO      */
    ...
    else
    {
        ...
        bropt();                        /* branch optimization           */
        brrear();                       /* branch rearrangement          */
        comsubs();                      /* eliminate common subВыражения */
...
/***********************************
 * Try to удали control structure.
        ...
        for (block *b = startblock; b; b = b.Bnext)   // for each block
        {
            ...
            if (bc == BCiftrue)
            {
                        ...
                        )
                {
                    if (PARSER)
                    {
                        тип *t = (bc2 == BCretexp) ? b2.Belem.ET : tstypes[TYvoid];
            ...
            {
                block *b2 = b.nthSucc(0);
                if (!list_next(b2.Bpred) && b2.BC != BCasm    // if b is only родитель
                    && b2 != startblock
                    && b2.BC != BCtry
                    && b2.BC != BC_try
                    && b.Btry == b2.Btry
...
private проц bropt()
{
    ...
    for (block *b = startblock; b; b = b.Bnext)   // for each block
    {
        ...
        {   /* see we can evaluate this switch now  */
            ...
            targ_llong значение = el_tolong(n);
            targ_llong* pv = b.Bswitch;      // ptr to switch данные
            assert(pv);
            бцел ncases = cast(бцел) *pv++;  // # of cases
            бцел i = 1;                      // first case
            while (1)
            {
                if (i > ncases)
            ...
            foreach (bl; ListRange(b.Bsucc))
                ...
                {
                    проц *p = list_subtract(&(list_block(bl).Bpred),b);
                    assert(p == b);
                }

            /* dump old successor list and создай a new one */
            list_free(&b.Bsucc,FPNULL);
            b.appendSucc(db);
            b.BC = BCgoto;
            b.Belem = doptelem(b.Belem,GOALnone | GOALagain);
        ...
        {   /* For each transfer of control block pointer   */
            ...
            version (SCPP)
            {
                static if (NTEXCEPTIONS)
                    const additionalAnd = "b.Btry == bt.Btry &&
...
проц compdfo(ref Barray!(block*) dfo, block* startblock)
{
...
private проц elimblks()
{
    ...
    for (block **pb = &startblock; (b = *pb) != null;)
    {
        if (((b.Bflags & BFLvisited) == 0)   // if block is not visited
            && ((b.Bflags & BFLlabel) == 0)  // need label смещение
            )
        {
            /* for each marked successor S to b                     */
            /*      удали b from S.Bpred.                          */
            ...
            *pb = b.Bnext;         // удали from linked list
...
/**********************************
 * Merge together blocks where the first block is a goto to the следщ
 * block and the следщ block has only the first block as a predecessor.
 * Example:
 *      e1; GOTO L2;
 *      L2: return e2;
 * becomes:
    ...
    for (block *bn = startblock; bn; bn = bnext)
    {
        ...
        for (block *b = bnext; b; b = b.Bnext)
        {
            ...
            static if (SCPP_OR_NTEXCEPTIONS)
                бул additionalAnd = b.Btry == bn.Btry;
            else
                const additionalAnd = да;
            if (b.BC == bn.BC &&
            ...
            {   /* Eliminate block bn           */
                ...
                static if(0) // && SCPP
                {
                    // Predecessors must all be at the same btry уровень.
                    if (bn.Bpred)
                    {
                ...
                /* Entirely удали predecessor list from bn.            */
...
private проц blreturn()
{
    if (!(go.mfoptim & MFtime))            /* if optimized for space       */
    {
        цел retcount = 0;               // number of return counts
...
/*****************************************
 * Convert Выражение into a list.
 * Construct the list in reverse, that is, so that the right-most
 * Выражение occurs first in the list.
 */
...
/*****************************************
 * Take a list of Выражения and convert it back into an Выражение tree.
 */
...
private проц bltailmerge()
{
    ...
    if (!(go.mfoptim & MFtime))            /* if optimized for space       */
    {
        ...
        /* Search for two blocks that have the same successor list.
           If the first Выражения both lists are the same, split
           off a new block with that Выражение in it.
         */
        static if (SCPP_OR_NTEXCEPTIONS)
            const additionalAnd = "b.Btry == bn.Btry";
        else
            const additionalAnd = "да";
        for (block *b = startblock; b; b = b.Bnext)
        {
            ...
            for (block *bn = b.Bnext; bn; bn = bn.Bnext)
            {
                   ...
                   )
                {
                    ...
                    /* Find all the Выражения we can merge    */
                    do
                    {
...
nextb:
        /* Recombine elem lists into Выражение trees   */
        for (block *b = startblock; b; b = b.Bnext)
...
private проц block_check()
{
    for (block *b = startblock; b; b = b.Bnext)
    {
        цел nsucc = list_nitems(b.Bsucc);
        цел npred = list_nitems(b.Bpred);
        switch (b.BC)
        {
            case BCgoto:
                assert(nsucc == 1);
...
private проц brtailrecursion()
{
    version (SCPP)
    {
    //    if (tyvariadic(funcsym_p.Stype))
            return;
        return;             // haven't dealt with struct парамы, and ctors/dtors
    }
    if (funcsym_p.Sfunc.Fflags3 & Fnotailrecursion)
        return;
    if (localgot)
    {   /* On OSX, tail recursion will результат in two OPgot's:
            цел status5;
            struct MyStruct5 { }
            проц rec5(цел i, MyStruct5 s)
            {
                if( i > 0 )
                {   status5++;
                    rec5(i-1, s);
...
/*****************************************
 * Convert параметр Выражение to assignment statements.
 */

private elem * assignparams(elem **pe,цел *psi,elem **pe2)
{
    elem *e = *pe;
 ...
 * This means, determine if all the function does is return a значение;
...
private цел funcsideeffect_walk(elem *e)
{
    assert(e);
    elem_debug(e);
...
private цел el_anyframeptr(elem *e)
{
    while (1)
    {
        if (OTunary(e.Eoper))
...
private проц blassertsplit()
{
    ...
    L1:
        ...
        foreach (el; ListRange(bel))
        {
            ...
            b = b2;
            bel = bex;  // remainder of Выражение list goes into b2
            go.changes++;
            goto L1;
        }
        b.Belem = bl_delist(list_reverse(bel));
