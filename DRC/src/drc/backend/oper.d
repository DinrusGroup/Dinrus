...
enum
{
        ...
        OPneg,                  // unary -
        OPuadd,                 // unary +
        OPvoid,                 // where casting to проц is not a no-op
        OPabs,                  // absolute значение
...
/*      8,16,32,64      integral тип of unspecified sign
        s,u             signed/unsigned
...
*/
        ...
        OPu8_16,        // unsigned сим to short
        OPs8_16,        // signed сим to short
        ...
        OPoffset,       // get смещение of far pointer
...
//**************** End of conversion operators *****************
        ...
        OPparam,                // function параметр separator
        ...
        OPconst,                // constant
        OPrelconst,             // constant that содержит an address
        OPvar,                  // variable
        OPreg,                  // register (используется in inline asm operand Выражения)
        OPcolon,                // : as in ?:
        OPcolon2,               // alternate version with different EH semantics
        OPstring,               // address of ткст
        OPnullptr,              // null pointer
        OPasm,                  // in-line assembly code
        OPinfo,                 // attach info (используется to attach ctor/dtor
                                // info for exception handling)
        OPhalt,                 // вставь HLT instruction
        OPctor,
        OPdtor,
        OPmark,
        ...
        OPgot,                  // load pointer to глоб2 смещение table
 ...
 * parallel массив invconvtab[] in cgelem.c
 * Параметры:
 *   op = conversion operator
 */
цел convidx(OPER op) { return op - CNVOPMIN; }
 ...
 *                      post ?: && ||)
 *      OTconv          тип conversion operator that could appear on lhs of
 *                      assignment operator
 *      OTlogical       logical operator (результат is 0 or 1)
 ...
 *      OTae            potential common subВыражение operator
 *      OTбулnop       operation is a nop if булean результат is desired
 */
...
ббайт OTbinary(OPER op)    { return optab1[op] & _OTbinary; }
ббайт OTunary(OPER op)     { return optab1[op] & _OTunary; }
бул  OTleaf(OPER op)      { return !(optab1[op] & (_OTunary|_OTbinary)); }
ббайт OTcommut(OPER op)    { return optab1[op] & _OTcommut; }
ббайт OTassoc(OPER op)     { return optab1[op] & _OTassoc; }
ббайт OTassign(OPER op)    { return optab2[op]&_OTassign; }
бул  OTpost(OPER op)      { return op == OPpostinc || op == OPpostdec; }
ббайт OTeop0e(OPER op)     { return optab1[op] & _OTeop0e; }
ббайт OTeop00(OPER op)     { return optab1[op] & _OTeop00; }
ббайт OTeop1e(OPER op)     { return optab1[op] & _OTeop1e; }
ббайт OTsideff(OPER op)    { return optab1[op] & _OTsideff; }
бул  OTconv(OPER op)      { return op >= CNVOPMIN && op <= CNVOPMAX; }
ббайт OTlogical(OPER op)   { return optab2[op] & _OTlogical; }
ббайт OTwid(OPER op)       { return optab2[op] & _OTwid; }
бул  OTopeq(OPER op)      { return op >= OPaddass && op <= OPashrass; }
бул  OTop(OPER op)        { return op >= OPadd && op <= OPor; }
ббайт OTcall(OPER op)      { return optab2[op] & _OTcall; }
ббайт OTrtol(OPER op)      { return optab2[op] & _OTrtol; }
бул  OTrel(OPER op)       { return op >= OPle && op <= OPnue; }
бул  OTrel2(OPER op)      { return op >= OPle && op <= OPge; }
ббайт OTdef(OPER op)       { return optab2[op] & _OTdef; }
ббайт OTae(OPER op)        { return optab2[op] & _OTae; }
ббайт OTбулnop(OPER op)   { return optab3[op] & _OTбулnop; }
бул  OTcalldef(OPER op)   { return OTcall(op) || op == OPstrcpy || op == OPstrcat || op == OPmemcpy; }
 ...
 * EBIN         binary узел?
 * EUNA         unary узел?
 * EOP          operator узел (unary or binary)?
...
extern (C) const ббайт[OPMAX] optab1 =
() {
    ббайт[OPMAX] tab;
...
const ббайт[OPMAX] optab3 =
() {
    ббайт[OPMAX] tab;
    foreach (i; Eбулnop) { tab[i] |= _OTбулnop; }
    return tab;
} ();

private const RELMAX = RELOPMAX - RELOPMIN + 1;

const ббайт[RELMAX] _rel_exception =
() {
    ббайт[RELMAX] tab;
    foreach (i; Eexception) { tab[cast(цел)i - RELOPMIN] = 1; }
    return tab;
} ();

const ббайт[RELMAX] _rel_unord =
() {
    ббайт[RELMAX] tab;
    foreach (i; Eunord) { tab[cast(цел)i - RELOPMIN] = 1; }
    return tab;
} ();

/// Logical negation
const ббайт[RELMAX] _rel_not =
() {
    ббайт[RELMAX] tab;
    foreach (op; new бцел[RELOPMIN .. RELOPMAX + 1])
    {
        OPER opnot;
        switch (op)
        {
        ...
        tab[cast(цел)op - RELOPMIN] = cast(ббайт)opnot;
    }

    foreach (op; new бцел[RELOPMIN .. RELOPMAX + 1])
    {
        OPER opnot = tab[cast(цел)op - RELOPMIN];
        assert(op == tab[cast(цел)opnot - RELOPMIN]);  // symmetry check
    }
    return tab;
} ();
...
() {
    ...
    foreach (op; new бцел[RELOPMIN .. RELOPMAX + 1])
    {
        ...
        tab[cast(цел)op - RELOPMIN] = cast(ббайт)opswap;
    }

    foreach (op; new бцел[RELOPMIN .. RELOPMAX + 1])
    {
        OPER opswap = tab[cast(цел)op - RELOPMIN];
        assert(op == tab[cast(цел)opswap - RELOPMIN]);  // symmetry check
    }
    return tab;
} ();

/// If operands are integral types
const ббайт[RELMAX] _rel_integral =
() {
    ббайт[RELMAX] tab;
    foreach (op; new бцел[RELOPMIN .. RELOPMAX + 1])
    {
        ...
        tab[cast(цел)op - RELOPMIN] = cast(ббайт)opintegral;
    }
    return tab;
} ();
 ...
 *
 * Used for reordering elem trees to minimize register использование.
 */

const ббайт[OPMAX] opcost =
() {
    ббайт[OPMAX] tab;
    foreach (op; new бцел[0 .. OPMAX])
    {
        ббайт c = 0;        // default cost
        foreach (o; Eunary)
        {
            if (o == op)
            {
...
const Eassoc = [ OPadd,OPand,OPor,OPxor,OPmul ];
...
const Eeop00 = [ OPmul,OPand,OPmulass,OPandass ];

const Eeop1e = [ OPmul,OPdiv,OPmulass,OPdivass ];

const Elogical =
    [
        OPeqeq,OPne,OPle,OPlt,OPgt,OPge,OPandand,OPoror,OPnot,OPбул,
        OPunord,OPlg,OPleg,OPule,OPul,OPuge,OPug,OPue,OPngt,OPnge,
        OPnlt,OPnle,OPord,OPnlg,OPnleg,OPnule,OPnul,OPnuge,OPnug,OPnue,
        OPbt,OPbtst,
    ];
...
const Ecall = [ OPcall,OPucall,OPcallns,OPucallns ];
...
const Eae =
    [
        ...
        OPnot,OPбул,OPcom,OPadd,OPmin,OPmul,OPand,OPor,OPmemcmp,
...
const Eбулnop =
    [
        OPuadd,OPбул,OPs16_32,OPu16_32,
        OPs16_d,
...
/** результат of unordered operands */
const Eunord =
    [
        OPne,
