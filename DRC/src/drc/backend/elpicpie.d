 ...
 * Параметры: s = symbol
 * Возвращает: `elem` with address of `s`
 */
...
elem * el_ptr(Symbol *s)
{
    ...
    static if (TARGET_OSX)
    {
        if (config.flags3 & CFG3pic && tyfunc(s.ty()) && I32)
        {
             ...
             * Instead, создай a данные variable, put the address of the
             * code in that данные variable, and return the elem for
             * that данные variable.
             */
...
static if (TARGET_LINUX || TARGET_OSX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
{
    ...
    if (I32 && !localgot)
    {
        ...
        сим[15] имя = проц;
         цел tmpnum;
        sprintf(имя.ptr, "_LOCALGOT%d".ptr, tmpnum++);
        тип *t = type_fake(TYnptr);
        /* Make it volatile because we need it for calling functions, but that isn't
         * noticed by the данные flow analysis. Hence, it may get deleted if we don't
         * make it volatile.
         */
        type_setcv(&t, mTYvolatile);
        localgot = symbol_name(имя.ptr, SCauto, t);
        symbol_add(localgot);
        localgot.Sfl = FLauto;
...
static if (1)
{
            if (I32 && s.Stype.Tty & mTYthread)
            {
                if (!tls_get_addr_sym)
                {
                    /* проц *___tls_get_addr(проц *ptr);
                     * Параметр2 ptr is passed in RDI, matching TYnfunc calling convention.
                     */
...
static if (1)
{
             ...
             * }
             *
             * To access the значение of the variable, the variable is accessed
             * like a plain глоб2 () variable of the тип
             * TLVDescriptor. The thunk is called and a pointer to the variable
             * itself is passed as the argument. The return значение of the thunk
             * is a pointer to the значение of the thread local variable.
             *
             * module foo;
             *
             * цел bar;
             * pragma(mangle, "_D3foo3bari") extern  TLVDescriptor!(цел) barTLV;
             *
             * цел a = *barTLV.thunk(&barTLV);
             */
            if (I64 && s.Stype.Tty & mTYthread)
            {
...
{
    assert(0);  // опция not needed on TARGET_OSX
}
...
private elem *el_picvar(Symbol *s)
{
     ...
     *      ADD  EBX,смещение _GLOBAL_OFFSET_TABLE_@GOTPC[2]
     * Generate for var locals:
    ...
    if (I64)
    {
        switch (s.Sclass)
        {
            ...
            case_got64:
            {
                ...
                if (s.Stype.Tty & mTYthread)
                {
                    /* Add "volatile" to prevent e from being common subВыражениеed.
    ...
    else
    {
        switch (s.Sclass)
        {
            /* local (and thread) symbols get only one уровень of indirection;
            ...
            case_got:
            {
                ...
                if (s.Stype.Tty & mTYthread)
                {
                    ...
                    {
                        /* проц *___tls_get_addr(проц *ptr);
                         * Параметр2 ptr is passed in EAX, matching TYjfunc calling convention.
                         */
 ...
 * Use PIE protocol.
 * Параметры: s = variable's symbol
 * Возвращает: elem created
 */
private elem *el_pievar(Symbol *s)
{
    цел x;
...
private elem *el_pieptr(Symbol *s)
{
    ...
    if (I64)
    {
        Obj.refGOTsym();    // even though not используется, generate reference to _GLOBAL_OFFSET_TABLE_
        switch (s.Sclass)
        {
            case SCstatic:
            case SClocstat:
            case SCglobal:
            {
                /* Generate:
                 *   mov RAX,FS:[0000]
                 *   add EAX,смещение FLAG:global_tls@TPOFF32
                 */
