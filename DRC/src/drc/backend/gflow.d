...
проц vec_setclear(т_мера b, vec_t vs, vec_t vc) { vec_setbit(b, vs); vec_clearbit(b, vc); }

бул Eunambig(elem* e) { return OTassign(e.Eoper) && e.EV.E1.Eoper == OPvar; }

сим symbol_isintab(Symbol *s) { return sytab[s.Sclass] & SCSS; }

проц util_free(ук p) { if (p) free(p); }
проц *util_calloc(бцел n, бцел size) { ук p = calloc(n, size); assert(!(n * size) || p); return p; }
проц *util_realloc(ук p, бцел n, бцел size) { ук q = realloc(p, n * size); assert(!(n * size) || q); return q; }
...
private 
{
    цел flowxx;              // one of the above values
 ...
 * найди all elems that could be the last elem that defines
...
проц flowrd()
{
    ...
    static if (0)
    {
        ...
        foreach (i, b; dfo[])    // for each block
        {
            ...
            dbg_printf("B%d Bin ", cast(цел)i); vec_println(b.Binrd);
...
private проц rdgenkill()
{
    /* Compute number of definition elems. */
    бцел num_unambig_def = 0;
    бцел deftop = 0;
    foreach (b; dfo[])    // for each block
        if (b.Belem)
        {
    ...
    /* Allocate массив of pointers to all definition elems   */
    ...
    /* Allocate буфер for the DNunambig vectors
     */
    const т_мера dim = (deftop + (VECBITS - 1)) >> VECSHIFT;
    ...
    foreach (b; dfo[])    // for each block
    {
        ...
        /* calculate and создай new vectors */
...
private бцел numdefelems(elem *e, бцел *pnum_unambig_def)
{
    бцел n = 0;
    while (1)
    {
...
private проц asgdefelems(block *b,elem *n)
{
    ...
    if (OTdef(op))
    {
        n.Edef = cast(бцел)go.defnod.length;
        go.defnod.сунь(DefNode(n, b, null));
    }
    else
        n.Edef = ~0;       // just to ensure it is not in the массив
}
...
private проц initDNunambigVectors()
{
    //printf("initDNunambigVectors()\n");
    const т_мера numbits = go.defnod.length;
    const т_мера dim = (numbits + (VECBITS - 1)) >> VECSHIFT;
...
private проц rdelem(vec_t *pgen,vec_t *pkill,   /* where to put результат          */
...
private проц accumrd(vec_t GEN,vec_t KILL,elem *n)
{
    assert(GEN && KILL && n);
    const op = n.Eoper;
    if (OTunary(op))
    ...
    else if (OTbinary(op))
    {
        if (op == OPcolon || op == OPcolon2)
        {
            ...
            auto rrr = el_returns(n.EV.E1) * 2 | el_returns(n.EV.E2);
            switch (rrr)
            {
...
/******************** AVAILABLE ВыражениеS ***********************/

/************************************
 * Compute доступно Выражения (AEs).
 * That is, Выражения whose результат is still current.
...
/***************************************
 * Compute копируй propagation info (CPs).
 * Very similar to AEs (the same code is используется).
 * Using RDs for копируй propagation is WRONG!
 * That is, set of копируй statements still valid.
...
/*****************************************
 * Common flow analysis routines for Available Выражения and
 * Copy Propagation.
 * Input:
 *      flowxx
 */

private проц flowaecp()
{
    aecpgenkill();          /* Compute Bgen and Bkill for AEs or CPs */
    if (go.exptop <= 1)        /* if no Выражения                    */
        return;
    ...
    foreach (b; dfo[1 .. $])
    {
        vec_set(b.Bin);        /* Bin = all Выражения        */
...
/******************************
 * A variable to avoid параметр overhead to asgexpelems().
 */

private  block *this_block;

/***********************************
 * Compute Bgen and Bkill for AEs, CPs, and VBEs.
 */

private проц aecpgenkill()
{
    go.expnod.setLength(0);             // dump any existing one

    /* Compute number of Выражения */
    ...
    /* Allocate массив of pointers to all Выражение elems.          */
    /* (The elems are in order. Also, these Выражения must not    */
    ...
    const бцел exptopsave = go.exptop;
    go.exptop = 1;
    foreach (b; dfo[])
    {
        ...
        switch (b.BC)
        {
            case BCiftrue:
                ...
                if (e.Eoper == OPandand || e.Eoper == OPoror)
                {
                    ...
                    const бцел dim = cast(бцел)vec_dim(Kr);
                    vec_t KILL = b.Bkill;
                    vec_t GEN = b.Bgen;

                    foreach (j; new бцел[0 .. dim])
                    {
...
/*****************************
 * Accumulate number of Выражения in go.exptop.
 * Set NFLaecp as a флаг indicating an AE elem.
 * Возвращает:
 *      да if this elem is a possible AE elem.
 */

private цел numaeelems(elem *n)
{
    бцел ae;
...
/****************************
 * Compute number of cp (копируй propagation) elems.
 * Mark cp elems by setting NFLaecp флаг.
 * Возвращает:
 *      number of cp elems
 */

private цел numcpelems(elem *n)
{
    while (1)
    {
        const op = n.Eoper;
...
private проц asgexpelems(elem *n)
{
    assert(n);
    if (OTunary(n.Eoper))
        asgexpelems(n.EV.E1);
 ...
 *      starkill:       set of Выражения killed when a variable is
 *                      changed that somebody could be pointing to.
 *                      (not needed for cp)
 *                      starkill is a subset of defkill.
 *      defkill:        set of Выражения killed by an ambiguous
 *                      definition.
 *      vptrkill:       set of Выражения killed by an access to a vptr.
 */

private проц defstarkill()
{
    ...
    if (flowxx != CP)
    {
        go.starkill = vec_calloc(go.exptop);      /* and создай new ones  */
        go.vptrkill = vec_calloc(go.exptop);      /* and создай new ones  */
    }
    else /* CP */
    {
        go.starkill = null;
    ...
    if (flowxx == CP)
    {
        foreach (бцел i; new бцел[1 .. go.exptop])
        {
            elem *n = go.expnod[i];
            const op = n.Eoper;
...
private проц aecpelem(vec_t *pgen,vec_t *pkill, elem *n)
{
...
private 
{
    vec_t GEN;       // use static copies to save on параметр passing
    vec_t KILL;
}

private проц accumaecp(vec_t g,vec_t k,elem *n)
...
private проц accumaecpx(elem *n)
{
    elem *t;
    ...
    switch (op)
    {
        ...
        case OPddtor:
        case OPasm:
            assert(!n.Eexp);                   // no ASM доступно Выражения
    ...
    /* Do копируй propagation stuff first  */
    ...
    if (flowxx == CP)
    {
        ...
        else                                    /* unambiguous def elem */
        {
            ...
            foreach (бцел i; new бцел[1 .. go.exptop])        // for each ae elem
            {
                elem *e = go.expnod[i];
    ...
    /* Else Available Выражение stuff  */

    if (n.Eexp)
    {
        const бцел b = n.Eexp;             // add elem to GEN
 ...
 * Bgen = set of variables используется before any definition in B.
...
проц flowlv()
{
    ...
    foreach (бцел i; new бцел[0 .. globsym.top])
    {
    ...
    static if (0)
    {
        ...
        foreach (i, b; dfo[])
        {
            printf("B%d IN\t", cast(цел)i);
            vec_println(b.Binlv);
            printf("B%d GEN\t", cast(цел)i);
            vec_println(b.Bgen);
            printf("  KILL\t");
            vec_println(b.Bkill);
            printf("  OUT\t");
...
private проц lvelem(vec_t *pgen,vec_t *pkill,elem *n)
{
...
private проц accumlv(vec_t GEN,vec_t KILL,elem *n)
{
    assert(GEN && KILL && n);
        ...
        switch (op)
        {
            case OPvar:
                if (symbol_isintab(n.EV.Vsym))
                {
                    ...
                    assert(cast(бцел)si < globsym.top);
            ...
            case OPstreq:
            {
                ...
                else /* unambiguous assignment */
                {
                    ...
                    {
                        // printf("%s\n", s.Sident);
                        assert(cast(бцел)s.Ssymnum < globsym.top);
...
/********************* VERY BUSY ВыражениеS ********************/

/**********************************************
 * Compute very busy Выражения(VBEs).
 * That is,Выражения that are evaluated along
 * separate paths.
 * Bin = the set of VBEs at the beginning of B.
 * Bout = the set of VBEs at the end of B.
 * Bgen = set of Выражения X+Y such that X+Y is
 *      evaluated before any def of X or Y.
 * Bkill = set of Выражения X+Y such that X or Y could
...
private проц accumvbe(vec_t GEN,vec_t KILL,elem *n)
{
    elem *t;
    ...
    if (n.Eexp)                    /* if a vbe elem                */
    {
        ...
        {
            /* GEN this Выражение only if it hasn't        */
            /* already been GENed in this block.            */
            /* (Don't GEN common subВыражения.)           */
            if (vec_testbit(ne,GEN))
                vec_clearbit(ne,GEN);
            else
            {
                vec_setbit(ne,GEN); /* GEN this Выражение  */
                /* GEN all identical Выражения            */
    ...
    else if (OTdef(op))             /* if definition elem           */
    {
        ...
        else                    /* unambiguous definition       */
        {
            ...
            if (!(s.Sflags & SFLunambig))
                vec_orass(KILL,go.starkill);/* kill all 'starred' refs */
            foreach (бцел i; new бцел[1 .. go.exptop])        // for each vbe elem
            {
                elem *e = go.expnod[i];
                бцел eop = e.Eoper;
