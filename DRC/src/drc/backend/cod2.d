...
extern (C)
{
    бул choose_multiplier(цел N, бдол d, цел prec, бдол *pm, цел *pshpost);
    бул udiv_coefficients(цел N, бдол d, цел *pshpre, бдол *pm, цел *pshpost);
}
...
private проц swap(reg_t *a,reg_t *b)
{
    const tmp = *a;
    *a = *b;
    *b = tmp;
...
/*******************************************
 * Возвращает: да if cannot use this EA in anything other than a MOV instruction.
 */
...
/********************************
 * Determine index registers используется by addressing mode.
...
regm_t idxregm(code* c)
{
    ...
    if ((rm & 0xC0) != 0xC0)            /* if register is not the destination */
    {
        if (I16)
        {
            static const ббайт[8] idxrm  = [mBX|mSI,mBX|mDI,mSI,mDI,mSI,mDI,0,mBX];
            idxm = idxrm[rm & 7];
        }
        else
        {
...
проц opdouble(ref CodeBuilder cdb, elem *e,regm_t *pretregs,бцел clib)
{
    ...
    codelem(cdb,e.EV.E1, &retregs1,нет);
    if (retregs1 & mSTACK)
        cgstate.stackclean++;
    scodelem(cdb,e.EV.E2, &retregs2, retregs1 & ~mSTACK, нет);
    if (retregs1 & mSTACK)
        cgstate.stackclean--;
...
проц cdorth(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    elem *e1 = e.EV.E1;
    elem *e2 = e.EV.E2;
    if (*pretregs == 0)                   // if don't want результат
    {
        codelem(cdb,e1,pretregs,нет); // eval left leaf
        *pretregs = 0;                          // in case they got set
        codelem(cdb,e2,pretregs,нет);
        return;
    }
    ...
    бул test = нет;                // assume we destroyed lvalue
       ...
       )
    {
        ...
        if (e2.Eoper == OPconst && el_signx32(e2))
        {
            ...
            targ_т_мера значение = e2.EV.Vpointer;
            if (sz == 2)
                значение &= 0xFFFF;
            else if (sz == 4)
                значение &= 0xFFFFFFFF;
            reg_t reg;
            if (reghasvalue(isbyte ? BYTEREGS : ALLREGS,значение,&reg))
            {
                code_newreg(&cs, reg);
                if (I64 && isbyte && reg >= 4)
                    cs.Irex |= REX;
            ...
            else
            {
                if (sz == 8 && !I64)
                {
                    assert(значение == cast(цел)значение);    // sign extend imm32
                }
                op1 = 0xF7;
                cs.IEV2.Vint = cast(targ_int)значение;
                cs.IFL2 = FLconst;
            }
            cs.Iop = op1 ^ isbyte;
        ...
        (sz == REGSIZE || (I64 && sz == 4)))  // far pointers aren't handled
    {
           ...
           )
        {
            ...
            {
                const co1 = cast(targ_т_мера)el_tolong(e11.EV.E2);
                if (e11.Eoper == OPshl)
                {
                    if (co1 > 3)
                        goto L13;
            ...
            if (e11.Eoper == OPvar && isregvar(e11,&regm,&reg11))
            {
                ...
                codelem(cdb,e11,&retregs,нет);

            regm_t rretregs = ALLREGS & ~retregs & ~mBP;
            scodelem(cdb,ebase,&rretregs,retregs,да);
            reg_t reg;
            {
    ...
    {     /* Generate code for add/subtract of huge pointers.
        ...
        {   // hptr +- long
            ...
            scodelem(cdb,e2,&retregs,rretregs,да);
        }
        else
        {   // long + hptr
            ...
            if (!(rretregs & mLSW))
                rretregs |= mLSW;
            scodelem(cdb,e2,&rretregs,retregs,да);
        }
        ...
        genshift(cdb);                             // MOV CX,смещение __AHSHIFT
    ...
    {     /* The only possibilities are (TYfptr + tyword) or (TYfptr - tyword) */
        ...
        reg = findreglsw(retregs);      /* reg is the register with the смещение*/
    }
    else
    {
        regm_t regm;
    ...
    switch (e2oper)
    {
        ...
        L2:
            ...
            scodelem(cdb,e2,&rretregs,retregs,да);       // get rvalue
            ...
            {
                /* reverse operands to avoid moving around the segment значение */
                if (_tysize[ty2] > REGSIZE)
                {
        ...
        case OPrelconst:
            ...
            if (sz != REGSIZE)
                goto L2;
            if (segfl[el_fl(e2)] != 3)              /* if not in данные segment */
                goto L2;
        ...
        case OPconst:
            ...
            L3:
                ...
                if (I64 && isbyte && (reg >= 4 || (rval && rreg >= 4)))
                    cs.Irex |= REX;
                cs.IFL2 = cast(ббайт)((e2.Eoper == OPconst) ? FLconst : el_fl(e2));
                ...
                switch (e.Eoper)
                {
                    case OPadd:
                    {
                        ...
                        {
                            cs.Irm = cast(ббайт)(modregrm(3,0,reg & 7) | iop);
                            cs.Iop = 0xFF;
                        }
                        break;
                    }
            ...
            else if (numwords == 2)
            {
                ...
                cs.Irm = cast(ббайт)((cs.Irm & modregrm(3,7,0)) | reg);
                cs.IEV2.Vint = msw;
                if (e.Eoper == OPadd)
    ...
    {
        /* If the Выражение is (_tls_array + ...), then the flags are not set
...
проц cdmul(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    regm_t resreg;
    ббайт op;
    const uns = tyuns(tyml) || tyuns(e2.Ety);  // 1 if бцел operation, 0 if not
    ...
    const ббайт rex = (I64 && sz == 8) ? REX_W : 0;
    const бцел grex = rex << 16;
    ...
    switch (e2.Eoper)
    {
        ...
        case OPs32_64:
        {
            if (sz != 2 * REGSIZE || oper != OPmul || e1.Eoper != e2.Eoper ||
                e1.Ecount || e2.Ecount)
                goto L2;
            const ббайт opx = (e2.Eoper == opunslng) ? 4 : 5;
            regm_t retregsx = mAX;
            codelem(cdb,e1.EV.E1,&retregsx,нет);    // eval left leaf
            ...
            else
            {
                ...
                scodelem(cdb,e2.EV.E1,&rretregsx,retregs,да); // get rvalue
                getregs(cdb,mAX | mDX);
            ...
            freenode(e.EV.E1);
            freenode(e2);
            fixрезультат(cdb,e,mAX | mDX,pretregs);
            return;
        }

        case OPconst:
            auto d = cast(targ_т_мера)el_tolong(e2);
            бул neg = нет;
            const e2factor = d;
            ...
            if (oper == OPmul && I32 && sz == REGSIZE * 2)
            {
                /*  IMUL    EDX,EDX,lsw
                    ...
                    ADD     EDX,reg
                 */
                codelem(cdb,e1,&retregs,нет);    // eval left leaf
                ...
                freenode(e2);
                fixрезультат(cdb,e,resregx,pretregs);
                return;
            }
                ...
                config.flags4 & CFG4speed && !uns)
            {
                ...
                цел shpost;
                const цел N = sz * 8;
                const бул mhighbit = choose_multiplier(N, d, N - 1, &m, &shpost);

                regm_t regm = allregs & ~(mAX | mDX);
                codelem(cdb,e1,&regm,нет);       // eval left leaf
                 ...
                 *    q = -q
                 */
                const бул mgt = mhighbit || m >= (1UL << (N - 1));
                movregconst(cdb, AX, cast(targ_т_мера)m, (sz == 8) ? 0x40 : 0);  // MOV EAX,m
                cdb.gen2(0xF7,grex | modregrmx(3,5,reg));               // IMUL R1
                if (mgt)
                    cdb.gen2(0x03,grex | modregrmx(3,DX,reg));          // ADD EDX,R1
                getregsNoSave(mAX);                                     // EAX no longer содержит 'm'
                ...
                {   case OPdiv:
                    ...
                    case OPmod:
                        ...
                        else
                        {
                            ...
                            getregsNoSave(mAX);                             // EAX no longer содержит 'd'
                        }
                ...
                config.flags4 & CFG4speed && uns)
            {
                ...
                if (udiv_coefficients(sz * 8, e2factor, &shpre, &m, &shpost))
                {
                    ...
                    movregconst(cdb, DX, cast(targ_т_мера)m, (sz == 8) ? 0x40 : 0);  // MOV EDX,m
                    getregs(cdb,regm | mDX | mAX);
                ...
                freenode(e2);
                fixрезультат(cdb,e,resreg,pretregs);
                return;
            }
            ...
            {   reg_t reg;
                ...
                {   // See if we can use an LEA instruction
                    ...
                    switch (e2factor)
                    {
                        ...
                        else
                        {
                            ...
                            codelem(cdb,e.EV.E1,&resreg,нет);
                            reg = findreg(resreg);
                            getregs(cdb,resreg);
                        ...
                        L5:
                        {
                            ...
                            codelem(cdb,e.EV.E1,&sregm,нет);
                            бцел sreg = findreg(sregm);
                ...
                scodelem(cdb,e.EV.E1,&retregs,0,да);     // eval left leaf
               ...
               )
            {
                if (pow2 == 1 && oper == OPdiv && config.target_cpu > TARGET_80386)
                {
                    ...
                    retregs = allregs;
                    codelem(cdb,e.EV.E1,&retregs,нет);  // eval left leaf
            ...
            else if (sz == 2 * REGSIZE)
            {
                if (config.target_cpu >= TARGET_PentiumPro && oper == OPmul)
                {
                     ...
                     */
                     ...
                     fixрезультат(cdb,e,mDX|mAX,pretregs);
                }
                else
        ...
        L1:
            ...
            else
            {
                if (sz == 2 * REGSIZE)
                {
                    ...
                    codelem(cdb,e1.EV.E1,&retregs,нет);    // eval left leaf
                    const reg = findreg(retregs);
                    getregs(cdb,mAX);
...
проц cdnot(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    if (*pretregs == 0)
        goto L1;
    if (*pretregs == mPSW)
    {   //assert(e.Eoper != OPnot && e.Eoper != OPбул);*/ /* should've been optimized
    L1:
        codelem(cdb,e1,pretregs,нет);      // evaluate e1 for cc
        return;
    }

    OPER op = e.Eoper;
    бцел sz = tysize(e1.Ety);
    бцел rex = (I64 && sz == 8) ? REX_W : 0;
    бцел grex = rex << 16;
    ...
    {   code cs;
        ...
        allocreg(cdb,&retregs,&reg,TYint);
        op ^= (OPбул ^ OPnot);                 // switch operators
        goto L2;
    }
    else if (config.target_cpu >= TARGET_80486 &&
        tysize(e.Ety) == 1)
    {
        цел jop = jmpopcode(e.EV.E1);
        retregs = mPSW;
        ...
        цел iop = 0x0F90 | (jop & 0x0F);        // SETcc rm8
        if (op == OPnot)
            iop ^= 1;
    ...
    logexp(cdb,e.EV.E1,(op == OPnot) ? нет : да,FLcode,ctrue);
    forflags = *pretregs & mPSW;
    if (I64 && sz == 8)
        forflags |= 64;
    assert(tysize(e.Ety) <= REGSIZE);              // результат better be цел
    CodeBuilder cdbfalse;
    cdbfalse.ctor();
    allocreg(cdbfalse,pretregs,&reg,e.Ety);        // размести reg for результат
    code *cfalse = cdbfalse.finish();
    CodeBuilder cdbtrue;
    cdbtrue.ctor();
    cdbtrue.приставь(ctrue);
    ...
    cdb.приставь(cfalse);
    cdb.приставь(cdbfalse2);
    cdb.приставь(cdbtrue);
    cdb.приставь(cnop);
}
...
проц cdcond(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    бцел stackpushold,stackpushsave;
    цел ehindexold,ehindexsave;
    бцел sz2;

    /* vars to save state of 8087 */
    цел stackusedold,stackusedsave;
    ...
    const op1 = e1.Eoper;
    бцел sz1 = tysize(e1.Ety);
    бцел rex = (I64 && sz1 == 8) ? REX_W : 0;
    бцел grex = rex << 16;
    бцел jop = jmpopcode(e1);

    бцел jop1 = jmpopcode(e21);
    бцел jop2 = jmpopcode(e22);
       ...
       )
    {
        ...
        cdcmp_flag = 1;
        v1 = cast(targ_т_мера)e21.EV.Vllong;
        v2 = cast(targ_т_мера)e22.EV.Vllong;
        if (jop == JNC)
        {   v1 = v2;
            v2 = cast(targ_т_мера)e21.EV.Vllong;
        }
        ...
        {
            // only нуль-extension from 32-bits is доступно for 'or'
        }
        else if (I64 && cast(targ_llong)v2 != cast(targ_llong)cast(targ_long)v2)
        {
            // only sign-extension from 32-bits is доступно for 'and'
        }
        else
        {
            codelem(cdb,e1,&retregs,нет);
    ...
    {   // Recognize (e ? c : f)
        ...
        code *cnop1 = gennop(null);
        regm_t retregs = mPSW;
        jop = jmpopcode(e1);            // get jmp условие
        codelem(cdb,e1,&retregs,нет);
        ...
        reg_t reg;
        regwithvalue(cdb,retregs,cast(targ_т_мера)e21.EV.Vllong,&reg,tysize(e21.Ety) == 8 ? 64|8 : 8);
        retregs = mask(reg);
    ...
    logexp(cdb,e1,нет,FLcode,cnop1);  // evaluate условие
    regconold = regcon;
    ...
    if (psw && jop1 != JNE)
    {
        ...
        if (!retregs)
            retregs = ALLREGS;
        codelem(cdb1,e21,&retregs,нет);
        fixрезультат(cdb1,e21,retregs,pretregs);
    }
    else
        codelem(cdb1,e21,&retregs,нет);
    ...
    if (psw && jop2 != JNE)
    {
        ...
        if (!retregs)
            retregs = ALLREGS;
        codelem(cdb2,e22,&retregs,нет);
        fixрезультат(cdb2,e22,retregs,pretregs);
    }
    else
        codelem(cdb2,e22,&retregs,нет);   // use same regs as E1
...
{
    regm_t retregs = 0;
    codelem(cdb,e.EV.E1,&retregs,нет);   // ignore значение from left leaf
    codelem(cdb,e.EV.E2,pretregs,нет);   // do right leaf
}
 ...
 * Do && and || operators.
 * Generate:
 *              (evaluate e1 and e2, if да goto cnop1)
 *      cnop3:  NOP
 *      cg:     [save reg code]         ;if we must preserve reg
 *              CLR     reg             ;нет результат (set Z also)
 *              JMP     cnop2
 *
 *      cnop1:  NOP                     ;if e1 evaluates to да
 *              [save reg code]         ;preserve reg
 *
 *              MOV     reg,1           ;да результат
 *                  or
 *              CLR     reg             ;if return результат in flags
 *              INC     reg
 *
...
проц cdloglog(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    if (*pretregs == 0)                 // if don't want результат
    {
        цел noreturn = !el_returns(e2);
        codelem(cdb,e2,pretregs,нет);
        if (noreturn)
        {
            regconsave.используется |= regcon.используется;
            regcon = regconsave;
        }
        else
            andregcon(&regconsave);
        assert(stackpush == stackpushsave);
        cdb.приставь(cnop3);
        cdb.приставь(cdb1);        // eval code, throw away результат
        cgstate.stackclean--;
        return;
    }
    code *cnop2 = gennop(null);
    бцел sz = tysize(e.Ety);
    if (tybasic(e2.Ety) == TYбул &&
      sz == tysize(e2.Ety) &&
      !(*pretregs & mPSW) &&
      e2.Eoper == OPcall)
    {
        ...
        assert(sz <= 4);                                        // результат better be цел
        regm_t retregs = *pretregs & allregs;
        reg_t reg;
        allocreg(cdb1,&retregs,&reg,TYint);                     // размести reg for результат
    ...
    cdbcg.ctor();
    reg_t reg;
    allocreg(cdbcg,&retregs,&reg,TYint);                     // размести reg for результат
...
проц cdshift(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    if (*pretregs == 0)                   // if don't want результат
    {
        codelem(cdb,e1,pretregs,нет); // eval left leaf
        *pretregs = 0;                  // in case they got set
        codelem(cdb,e.EV.E2,pretregs,нет);
        return;
    }
...
version (SCPP)
{
    ...
    reg_t sreg = NOREG;                   // guard against using значение without assigning to sreg
    elem *e2 = e.EV.E2;
    regm_t forccs = *pretregs & mPSW;            // if return результат in CCs
    regm_t forregs = *pretregs & (ALLREGS | mBP); // mask of possible return regs
    бул e2isconst = нет;                    // assume for the moment
    бцел isbyte = (sz == 1);
    switch (e2.Eoper)
    {
        case OPconst:
            e2isconst = да;               // e2 is a constant
               ...
               )
            {
                ...
                {   // Handle (shtlng)s << 16
                    ...
                    codelem(cdb,e1.EV.E1,&r,нет);      // eval left leaf
...

                {
                    ...
                    {   resreg = findreg(retregs);
                        sreg = NOREG;              // an invalid значение
                    }
                        ...
                        sz <= REGSIZE)
                    {
                        ...
                        if (I64 && sz == 1 && resreg >= 4)
                            cdb.last().Irex |= REX;
                        // See if we need operand size префикс
                        if (!I16 && oper != OPshl && sz == 2)
                            cdb.last().Iflags |= CFopsize;
                        if (forccs)
                            cdb.last().Iflags |= CFpsw;         // need flags результат
                    }
                    else if (shiftcnt == 8)
        ...
        L1:
            ...
            if (sz <= REGSIZE)
            {
                scodelem(cdb,e2,&rretregs,retregs,нет); // get rvalue
                ...
                if (e2isconst)
                    *pretregs &= mBP | ALLREGS; // flags already set with результат
            }
            else if (sz == 2 * REGSIZE &&
                     config.target_cpu >= TARGET_80386)
            {
                ...
                {
                    scodelem(cdb,e2,&rretregs,retregs,нет); // get rvalue in CX
                    getregs(cdb,retregs);          // modify these regs
                    if (oper == OPshl)
                    {
                        /*
            ...
            {
                scodelem(cdb,e2,&rretregs,retregs,нет);
...
проц cdind(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    regm_t idxregs = idxregm(&cs);               // mask of index regs используется
    ...
    else                                // else return результат in reg
    {
    L1:
        retregs = *pretregs;
        if (sz == 8 &&
        ...
        {   цел i;
            ...
            // Optimizer should not CSE these, as the результат is worse code!
            assert(!e.Ecount);
        ...
        // the смещение loaded into a different register.
        ...
        {   бцел lsreg;
            ...
            if (!I16)
            {
                // Can't handle if both результат registers are используется in
                // the addressing mode.
...
{
/********************************
 * Generate code to load ES with the right segment значение,
...
проц cdstrlen(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    /* Generate strlen in CX:
        LES     DI,e1
        CLR     AX                      ;scan for 0
        MOV     CX,-1                   ;largest possible ткст
        REPNE   SCASB
        NOT     CX
        DEC     CX
     */
    ...
    // Make sure ES содержит proper segment значение
    cdb.приставь(cod2_setES(ty1));

    ббайт rex = I64 ? REX_W : 0;

    getregs_imm(cdb,mAX | mCX);
    movregconst(cdb,AX,0,1);               // MOV AL,0
    movregconst(cdb,CX,-cast(targ_т_мера)1,I64 ? 64 : 0);  // MOV CX,-1
    getregs(cdb,mDI|mCX);
    cdb.gen1(0xF2);                                     // REPNE
    cdb.gen1(0xAE);                                     // SCASB
    ...
    fixрезультат(cdb,e,mCX,pretregs);
}
...
проц cdstrcmp(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    ...
    /*
        ...
        MOV     CX,-1                   ;largest possible ткст
        REPNE   SCASB
        NOT     CX                      ;CX = ткст length of s2
        SUB     DI,CX                   ;point DI back to beginning
        REPE    CMPSB                   ;compare ткст
        POP     DS
    ...
    */
    ...
    if (!tyreg(ty2))
        retregs |= mES;
    scodelem(cdb,e.EV.E2,&retregs,retregs1,нет);

    // Make sure ES содержит proper segment значение
    cdb.приставь(cod2_setES(ty2));
    getregs_imm(cdb,mAX | mCX);

    ббайт rex = I64 ? REX_W : 0;

    // Load DS with right значение
    switch (tybasic(ty1))
    {
        case TYnptr:
        case TYimmutPtr:
    ...
    *pretregs &= ~mPSW;
    cdb.приставь(c4);
    fixрезультат(cdb,e,mAX,pretregs);
}
...
проц cdmemcmp(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    */
    ...
    if (!tyreg(ty1))
        retregs1 |= mDX;
    codelem(cdb,e1.EV.E1,&retregs1,нет);
    ...
    if (!tyreg(ty2))
        retregs |= mES;
    scodelem(cdb,e1.EV.E2,&retregs,retregs1,нет);
    freenode(e1);

    // Get члобайт into CX
    regm_t retregs3 = mCX;
    scodelem(cdb,e.EV.E2,&retregs3,retregs | retregs1,нет);
...
проц cdstrcpy(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    /*
        ...
        LES     DI,s1
        POP     DS
        MOV     AX,DI                   ;return значение is s1
        REP     MOVSB
        POP     DS
    */
    ...
    if (!tyreg(ty1))
        retregs |= mES;
    scodelem(cdb,e.EV.E1,&retregs,mCX|mSI,нет);
    getregs(cdb,mAX|mCX|mSI|mDI);
    ...
    fixрезультат(cdb,e,mAX | mES,pretregs);
}
...
проц cdmemcpy(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    */
    ...
    if (!tyreg(ty2))
        retregs2 |= mDX;
    codelem(cdb,e2.EV.E1,&retregs2,нет);

    // Get члобайт into CX
    regm_t retregs3 = mCX;
    scodelem(cdb,e2.EV.E2,&retregs3,retregs2,нет);
    freenode(e2);
    ...
    if (!tyreg(ty1))
        retregs1 |= mES;
    scodelem(cdb,e.EV.E1,&retregs1,retregs2 | retregs3,нет);
    ...
    if (*pretregs)                              // if need return значение
    {   getregs(cdb,mAX);
        genmovreg(cdb,AX,DI);
    }
    ...
    fixрезультат(cdb,e,mES|mAX,pretregs);
}
...
проц cdmemset(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    {
        значение = cast(targ_т_мера)el_tolong(e2.EV.E2);
        значение &= 0xFF;
        значение |= значение << 8;
        значение |= значение << 16;
        static if (значение.sizeof > 4)
            значение |= значение << 32;
        e2E2isConst = да;
    }
    else if (e2.EV.E2.Eoper == OPstrpar)  // happens if e2.EV.E2 is a struct of 0 size
    {
        ...
        значение = 0xDEADBEEF;     // stop annoying нет positives that значение is not inited

    if (e2.EV.E1.Eoper == OPconst)
    {
        static бцел REP_THRESHOLD() { return REGSIZE * (6 + (REGSIZE == 4)); }
        numbytes = cast(бцел)cast(targ_т_мера)el_tolong(e2.EV.E1);
            ...
            e2E2isConst)
        {
            ...
            if (e2.EV.E2.Eoper == OPconst)
            {
                const бцел mrm = buildModregrm(0,0,reg);
                switch (numbytes)
                {
                    case 4:                     // MOV [reg],imm32
                        cdb.genc2(0xC7,mrm,значение);
                        goto fixres;
            ...
            regwithvalue(cdb, BYTEREGS & ~retregs1, значение, &vreg, I64 ? 64 : 0);
            freenode(e2.EV.E2);
            freenode(e2);

            m = (rex << 16) | buildModregrm(2,vreg,reg);
            while (numbytes >= REGSIZE)
            {                           // MOV dword ptr смещение[reg],vreg
                cdb.gen2(0x89,m);
                cdb.last().IEV1.Voffset = смещение;
                cdb.last().IFL1 = FLconst;
                numbytes -= REGSIZE;
                смещение += REGSIZE;
            }
            ...
            if (numbytes & 2)
            {                           // MOV word ptr смещение[reg],vreg
                cdb.gen2(0x89,m);
                cdb.last().IEV1.Voffset = смещение;
                cdb.last().IFL1 = FLconst;
                cdb.last().Iflags = CFopsize;
            ...
            if (numbytes & 1)
            {                           // MOV byte ptr смещение[reg],vreg
                cdb.gen2(0x88,m);
                cdb.last().IEV1.Voffset = смещение;
                cdb.last().IFL1 = FLconst;
                if (I64 && vreg >= 4)
...
fixres:
            fixрезультат(cdb,e,retregs1,pretregs);
            return;
        }
    }
    ...
    if (!I16 && e2E2isConst)
    {
        regwithvalue(cdb, mAX, значение, null, I64?64:0);
        freenode(e2.EV.E2);
    }
    else
    {
    ...
    regimmed_set(CX,0);
    fixрезультат(cdb,e,mES|mBX,pretregs);
}
...
проц cdstreq(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    elem *e2 = e.EV.E2;
    цел segreg;
    бцел numbytes = cast(бцел)type_size(e.ET);          // # of bytes in structure/union
    ббайт rex = I64 ? REX_W : 0;
    ...
    else if (e2.Eoper == OPvar)
    {
        ...
        else
        {
            ...
            {
                need_DS = да;         // we need to reload DS
    ...
    if (e1.Eoper == OPind)               // if (*p = ..)
    {
        if (tyreg(e1.EV.E1.Ety))
            dstregs = mDI;
        cdb.приставь(cod2_setES(e1.EV.E1.Ety));
        scodelem(cdb,e1.EV.E1,&dstregs,srcregs,нет);
    }
    else
...
static if (1)
{
        бцел remainder = numbytes & (REGSIZE - 1);
...
проц cdrelconst(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
     ...
     * Assertion can be tripped by func("ткст" == 0); and similar
    ...
    //assert(tym & typtr);              // don't fail on (цел)&a

    SC sclass;
    reg_t mreg,            // segment of the address (TYfptrs only)
          lreg;            // смещение of the address
    ...
    if (_tysize[tym] > REGSIZE)            // fptr could've been cast to long
    {
        if (*pretregs & mES)
        {
            /* Do not размести CX or SI here, as cdstreq() needs
         ...
         * current segment (i.e. CS doesn't have the right значение in it)
         */
        Symbol *s = e.EV.Vsym;
        if (s.Sfl == FLdatseg)
        {   assert(0);
...
/*********************************
 * Load the смещение portion of the address represented by e into
 * reg.
 */
...
проц getoffset(ref CodeBuilder cdb,elem *e,reg_t reg)
{
    ...
    else static if (TARGET_WINDOS)
    {
            ...
            L5:
                ...
                cs.IEV2.Vsym = e.EV.Vsym;
                cs.IEV2.Voffset = e.EV.Voffset;
                cs.Iop = 0xB8 + (reg & 7);      // MOV Ereg,смещение s
                if (reg & 8)
                    cs.Irex |= REX_B;
                cs.Iflags = CFoff;              // want смещение only
                cs.IFL2 = cast(ббайт)fl;
                cdb.gen(&cs);
                break;
            }
...
проц cdneg(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    else if (sz == 2 * REGSIZE)
    {
        ...
        code_orflag(cdb.last(), CFpsw);           // need флаг результат of previous NEG
...
static if (TARGET_WINDOS)
{
        ...
        cgstate.stackclean++;
        scodelem(cdb,e2,&retregs,idxregs,нет);
        cgstate.stackclean--;
        ...
        if (tyml == TYdouble || tyml == TYdouble_alias)
        {   if (*pretregs == mSTACK)
                retregs = mSTACK;       /* leave результат on stack        */
            else
            {
                if (I32)
                {
        ...
        (!I16 || (idxregs & (mBX | mSI | mDI | mBP))))
    {
        ...
        if (I16)
        {
            static const byte[8] regtorm = [ -1,-1,-1, 7,-1, 6, 4, 5 ]; // copied from cod1.c
            rm = regtorm[reg];
        }
    ...
    else if (sz <= REGSIZE || tyfv(tyml))
    {
        ...
        cs.Iop = 0x81 ^ isbyte;
        cs.Irm &= ~cast(цел)modregrm(0,7,0);             // reg field = 0
        cs.Irex &= ~REX_R;
        if (op == OPpostdec)
            ...
            config.flags4 & CFG4speed)
        {
            ...
            cs.Irm = (cs.Irm & ~cast(цел)modregrm(3,0,7)) | modregrm(3,0,reg & 7);
            if (reg & 8)
            {   cs.Irex &= ~REX_R;
                cs.Irex |= REX_B;
            }
    ...
    else if (tyml == TYhptr)
    {
        ...
        {   cs.Iop = 0xC4;
            retregs |= mES;
            getregs(cdb,mES|mCX);       // размести ES
    ...
    else if (sz == 2 * REGSIZE)
    {
        ...
        cs.Iop = 0x81;
        cs.Irm &= ~cast(цел)modregrm(0,7,0);     /* reg field = 0 for ADD        */
        if (op == OPpostdec)
            cs.Irm |= modregrm(0,5,0);  /* SUB                          */
        getlvalue_lsw(&cs);
        cs.IFL2 = FLconst;
...
проц cderr(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    //printf("ткст = '%.*s'\n", cast(цел)e.EV.ss.Vstrlen, e.EV.ss.Vstring);
    assert(0);
}
...
проц cdddtor(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    else
    {
        ...
        */
        ...
        CodeBuilder cdbx;
        cdbx.ctor();
        codelem(cdbx,e.EV.E1,pretregs,нет);
        cdbx.gen1(0xC3);                      // RET
        code *c = cdbx.finish();
