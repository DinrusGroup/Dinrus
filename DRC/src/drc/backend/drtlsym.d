...
private  Symbol*[RTLSYM_MAX] rtlsym;

version (Dinrus)
    // This varies depending on C ABI
    alias fregsaved FREGSAVED;
else
    const FREGSAVED = (mBP | mBX | mSI | mDI);
...
Symbol *getRtlsym(цел i)
{
    ...
    // Only используется by dmd1 for RTLSYM_THROW
    тип *tw = null;
 ...
 *    имя = имя of function
 *    flags = значение for Sflags
 *    t = тип of function
 */
private проц symbolz(Symbol** ps, цел fl, regm_t regsaved, ткст0 имя, SYMFLGS flags, тип *t)
{
...
проц rtlsym_reset()
{
    clib_inited = 0;            // сбрось CLIB symbols, too
    for (т_мера i = 0; i < RTLSYM_MAX; i++)
    {
        if (rtlsym[i])
        {
            rtlsym[i].Sxtrnnum = 0;
