...
цел REGSIZE();
code *gen1(code *c, бцел op);
code *gen2(code *c, бцел op, бцел rm);
...
// assembler.
private бул is32bitaddr(бул x, бцел Iflags) { return I64 || (x ^ ((Iflags & CFaddrsize) != 0)); }

// If we use Pentium Pro scheduler
private бул PRO() { return config.target_cpu >= TARGET_PentiumPro; }

private enum FP : ббайт
{
    fstp = 1,       /// FSTP mem
    fld  = 2,       /// FLD mem
...
private enum CIFL : ббайт
{
    arraybounds = 1,     /// this instruction is a jmp to массив bounds
    ea          = 2,     /// this instruction has a memory-referencing
                             /// modregrm EA byte
    nostage     = 4,     /// don't stage these instructions
    сунь        = 8,     /// it's a сунь we can swap around
}
...
struct Cinfo
{
    ...
    ббайт pair;         // pairing information
    ббайт sz;           // operand size
    ббайт isz;          // instruction size
    ...
    бцел r;             // читай mask
    бцел w;             // пиши mask
    бцел a;             // registers используется in addressing mode
    ббайт reg;          // reg field of modregrm byte
    ббайт uops;         // Pentium Pro micro-ops
    бцел sibmodrm;      // (sib << 8) + mod__rm byte
    бцел spadjust;      // if !=0, then amount ESP changes as a результат of this
                        // instruction being executed
    цел fpuadjust;      // if !=0, then amount FPU stack changes as a результат
     ...
     проц print()        // pretty-printer
    {
        ...
        if (ci.flags & CIFL.nostage)
            printf("nostage,");
        if (ci.flags & CIFL.сунь)
            printf("сунь,");
        if (ci.flags & ~(CIFL.arraybounds|CIFL.nostage|CIFL.сунь|CIFL.ea))
            printf("bad флаг,");
        printf("\n\tr %x w %x a %x reg %x uops %x sibmodrm %x spadjust %d\n",
                cast(цел)r,cast(цел)w,cast(цел)a,reg,uops,sibmodrm,cast(цел)spadjust);
        if (ci.fp_op)
        {
             сим*[3] fpops = ["fstp","fld","fop"];
...
private проц cgsched_pentium(code **pc,regm_t scratch)
{
...
проц cgsched_block(block* b)
{
        ...
        b.BC != BCasm)
    {
        ...
        scratch &= ~(b.Bregcon.используется | b.Bregcon.парамы | mfuncreg);
...
extern (D) private const ббайт[256] pentcycl =
[
...
/********************************************
 * For each opcode, determine читай [0] and written [1] masks.
 */
...
extern (D) private const бцел[2][256] oprw =
[
      // 00
      [ EA|R|B, F|EA|B ],       // ADD
      [ EA|R,   F|EA   ],
      ...
      [ N,      N ],            // SEG FS
      [ N,      N ],            // SEG GS
      [ N,      N ],            // operand size префикс
      [ N,      N ],            // address size префикс
...
extern (D) private const бцел[2][8][8] grprw =
[
    [
        // Grp 1
      [ EA,     F|EA ],           // ADD
...
extern (D) private const бцел[2][8][8] grpf1 =
[
    [
        // 0xD8
      [ EA|S,   S|C ],    // FADD  float
...
extern (D) private const ббайт[8][8] uopsgrpf1 =
[
    [
        // 0xD8
        2,              // FADD  float
...
extern (D) private const ббайт[256] insuops =
[       0,0,0,0,        1,1,4,5,                /* 00 */
        0,0,0,0,        1,1,4,0,                /* 08 */
        0,0,0,0,        2,2,4,5,                /* 10 */
        0,0,0,0,        2,2,4,5,                /* 18 */
...
extern (D) private const ббайт[8] uopsx = [ 1,1,2,5,1,1,1,5 ];
...
{   ббайт op;
    ...
    switch (op)
    {
        ...
        L1:
                pc = UV;
        L2:
                // if EA содержит a displacement then
    ...
    if (c.Iflags & CFPREFIX && pc == UV)       // if префикс byte
        pc = PU;
    return pc;
}

/******************************************
 * For an instruction, determine what is читай
 * and what is written, and what is используется for addressing.
 * Determine operand size if EA (larger is ok).
 */

private проц getinfo(Cinfo *ci,code *c)
{
    ...
    if (PRO)
    {
        ci.uops = cast(ббайт)uops(c);
        ci.isz = cast(ббайт)calccodsize(c);
    }
    else
        ci.pair = cast(ббайт)pair_class(c);
    ...
    switch (op)
    {
        ...
        case 0x83:
            ...
            {
                assert(c.IFL2 == FLconst);
                ci.spadjust = (op == 0x81) ? -c.IEV2.Vint : -cast(цел)cast(byte)c.IEV2.Vint;
            }
...
version (CLASSINIT2)
        ...
        case 0xDF:
            ...
            {   ci.fpuadjust = -1;             // вынь versions of Fop's
                if (irm == 0xD9)
    ...
    {   ббайт sib;
        ...
        switch (mod)
        {
            case 0:
                ...
                {
                    const ббайт[8] ea16 = [mBX|mSI,mBX|mDI,mBP|mSI,mBP|mDI,mSI,mDI,0,mBX];
                    ci.a |= ea16[rm];
                }
                goto Lmem;
            ...
            case 2:
                ...
                {
                    const ббайт[8] ea16 = [mBX|mSI,mBX|mDI,mBP|mSI,mBP|mDI,mSI,mDI,mBP,mBX];
                    ci.a |= ea16[rm];
                }
...
Lret:
    ...
    if (op == LEA)                     // if LEA
        ci.r &= ~mMEM;                 // memory is not actually читай
    ci.sz = cast(ббайт)sz;
...
private цел pair_test(Cinfo *cu,Cinfo *cv)
{
    бцел pcu;
    бцел pcv;
    бцел r1,w1;
...
private цел pair_agi(Cinfo *c1, Cinfo *c2)
{
    бцел x = c1.w & c2.a;
...
private цел triple_test(Cinfo *c0, Cinfo *c1, Cinfo *c2)
{
    assert(c0);
    if (!c1)
        return 0;
    цел c2isz = c2 ? c2.isz : 0;
...
/********************************************
 * Get следщ instruction worth looking at for scheduling.
 * Возвращает:
...
private цел conflict(Cinfo *ci1,Cinfo *ci2,цел fpsched)
{
    code *c1;
    code *c2;
    бцел r1,w1,a1;
...
if (c2.IEV1.Vpointer + sz2 <= c1.IEV1.Vpointer) printf("t5\n");
        ...
        if (w1 & w2 & F &&              // if both instructions пиши to flags
            w1 != F &&
            w2 != F &&
        ...
        if (((c1.Iop & ~7) == 0x50 ||          // PUSH reg
            ...
            !(a2 & mBP && cast(цел)c2.IEV1.Vpointer < 0)
           )
        {
            if (c1.Iop == 0xFF)
            {
...
Lswap:
    ...
    {
        //printf("\tfpsched %d,%d:\n",ci1.fp_op,ci2.fp_op);
        ббайт x1 = ci1.fxch_pre;
        ббайт y1 = ci1.fxch_post;
        ббайт x2 = ci2.fxch_pre;
        ббайт y2 = ci2.fxch_post;

        static бцел X(бцел a, бцел b) { return (a << 8) | b; }
...
Lconflict:
    ...
    {
        // Look for partial register пиши stalls
...
const TBLMAX = 2*3*20;        // must be divisible by both 2 and 3
...
struct Schedule
{
    ...
    цел tblmax;                 // max number of slots используется
    ...
    цел fpustackused;           // number of slots in FPU stack that are используется

    проц initialize(цел fpustackinit)          // initialize scheduler
    {
...
code **assemble(code **pc)  // reassemble scheduled instructions
{
    ...
    // Try to вставь the rest of the staged instructions
    т_мера sli;
    ...
    assert(cast(бцел)tblmax <= TBLMAX);
    for (цел i = 0; i < tblmax; i++)
    {
        Cinfo* ci = tbl[i];
...
L1:
    // Just приставь any instructions left in the staging area
...
{   code *c;
    ...
    цел movesp = 0;
    цел reg2 = -1;              // avoid "may be uninitialized" warning

    //printf("вставь "); ci.c.print();
    //printf("вставь() %d\n", fpustackused);
    c = ci.c;
    //printf("\tc.Iop %x\n",c.Iop);
    ...
    {
        movesp = 1;                     // this is a MOV reg2,смещение[ESP]
        смещение = cast(byte)c.IEV1.Vpointer;
        reg2 = (c.Irm >> 3) & 7;
    }
    ...
    for (i = tblmax; i >= 0; i--)
    {
        ...
        if (movesp &&
            cit.c.Iop == 0x83 &&
            cit.c.Irm == modregrm(3,5,SP) &&          // if SUB ESP,смещение
            cit.c.IFL2 == FLconst &&
            (cast(byte)c.IEV1.Vpointer) >= -cit.spadjust
           )
        {
    ...
    for (i = imin; i < TBLMAX; i++)
    {
        ...
        {
            if (PRO)
            {   цел i0 = (i / 3) * 3;           // index of decode unit 0
                Cinfo *ci0;
...
Lnoinsert:
    //printf("\tnoinsert\n");
    c.IEV1.Vpointer = vpointer;  // сбрось to original значение
    return 0;
...
Linsert:
    ...
    if (ci.fp_op)
    {
        ...
        цел fpu = fpustackused;
        for (цел j = 0; j < tblmax; j++)
        {
            if (tbl[j])
            {
                fpu += tbl[j].fpuadjust;
    ...
    // and see if we can swap with a сунь.
    if (0 && movesp)
    {
        while (1)
        {
                   ...
                   cast(byte)c.IEV1.Vpointer >= REGSIZE)
            {
                ...
                assert(cast(бцел)i < TBLMAX);
                assert(cast(бцел)(i - j) < TBLMAX);
                tbl[i] = tbl[i - j];
                tbl[i - j] = ci;
                i -= j;
 ...
 * Возвращает:
 *      нет if could not be scheduled; have to start a new one
 */
...
бул stage(code *c)
{
    ...
    // Look through stagelist, and вставь any AGI conflicting instructions
    бул agi = нет;
    foreach (ref cs; stagelist[])
    {
        if (cs)
        {
            if (pair_agi(cs,ci))
            {
                if (!вставь(cs))
                    goto Lnostage;
                cs = null;
                agi = да;                    // we put out an AGI
            }
        }
    }

    // Look through stagelist, and вставь any other conflicting instructions
    foreach (i, ref cs; stagelist[])
    {
        if (!cs)
            continue;
        if (conflict(cs,ci,0) &&                // if conflict
            !(cs.flags & ci.flags & CIFL.сунь))
        {
            if (cs.spadjust)
            {
                // We need to вставь all previous adjustments to ESP
    ...
    stagelist.сунь(ci);         // приставь to staging list
    return да;
...
private code * csnip(code *c)
{
    if (c)
    {
        бцел iflags = c.Iflags & CFclassinit;
        code **pc;
        while (1)
        {
            pc = &c.следщ;
...
{
    code *cрезультат = null;
    code **pctail = &cрезультат;
    Schedule sch = проц;
    ...
    while (c)
    {
        ...
        {   code *cn;
            ...
            // Just приставь this instruction to pctail and go to the следщ one
            *pctail = c;
            cn = code_next(c);
            c.следщ = null;
            pctail = &c.следщ;
...
private проц repEA(code *c,бцел r1,бцел r2)
{
    бцел mod,reg,rm;
    бцел rmn;
        ...
        (rmn & modregrm(3,0,7)) != modregrm(0,0,5))
    {
        ...
        {   // SIB byte addressing
            ...
            if (index == r1)
                index = r2;
            c.Isib = cast(ббайт)((sib & 0xC0) | (index << 3) | base);
        }
        else if (rm == r1)
        {
            if (r1 == BP && r2 == SP)
            {   // Replace [EBP] with [ESP]
                c.Irm = cast(ббайт)(mod | reg | 4);
                c.Isib = modregrm(0,4,SP);
            }
            else if (r2 == BP && mod == 0)
            {
                c.Irm = cast(ббайт)(modregrm(1,0,0) | reg | r2);
                c.IFL1 = FLconst;
                c.IEV1.Vint = 0;
            }
            else
                c.Irm = cast(ббайт)(mod | reg | r2);
        }
    }
}
...
private проц code_swap(code *c1,code *c2)
{   code cs;
...
code *peephole(code *cstart,regm_t scratch)
{
    ...
    Ln:
        ...
        {   цел i = cast(byte)c.IEV2.Vint;
            цел i1 = cast(byte)c1.IEV2.Vint;
        ...
        switch (c1.Iop)
        {
            ...
            case 0x84:                  // TEST reg,byte ptr EA
                ...
                {
                    if ((rmn & 3) == r1)
                    {   c1.Irm = rmn = cast(ббайт)((rmn & modregrm(3,7,4)) | r2);
                        //printf("schedule 1\n");
                    }
                }
            ...
            case 0x85:                  // TEST reg,word ptr EA
                ...
                {
                    if ((rmn & 7) == r1)
                    {   c1.Irm = rmn = cast(ббайт)((rmn & modregrm(3,7,0)) | r2);
                        //printf("schedule 3\n");
                    }
                }
            ...
            case 0x8B:                  // MOV reg,EA
                if ((rmn & 0xC0) == 0xC0 &&
                    (rmn & 7) == r1)            // if EA == r1
                {   c1.Irm = cast(ббайт)((rmn & modregrm(3,7,0)) | r2);
                    //printf("schedule 6\n");
                    if (c1.Irm == modregrm(3,r2,r2))
                        goto Lnop;
                }
...
{   code *cstart;
    ...
    cstart = c;
    for (code** pc = &cstart; *pc; pc = &(*pc).следщ)
    {
        c = *pc;
