...
extern  бул[FLMAX] datafl;

private extern (D) бцел mask(бцел m) { return 1 << m; }

                        /*   AX,CX,DX,BX                */
 const reg_t[4] dblreg = [ BX,DX,NOREG,CX ];
...
private цел intree(Symbol *s,elem *e)
{
    if (!OTleaf(e.Eoper))
...
/***********************************
 * Determine if Выражение e can be evaluated directly into register
 * variable s.
...
/****************************
 * Return code for saving common subВыражения if EA
...
private проц opassdbl(ref CodeBuilder cdb,elem *e,regm_t *pretregs,OPER op)
{
    static const бцел[OPdivass - OPpostinc + 1] clibtab =
    /* OPpostinc,OPpostdec,OPeq,OPaddass,OPminass,OPmulass,OPdivass       */
    [  CLIB.dadd, CLIB.dsub, cast(бцел)-1,  CLIB.dadd,CLIB.dsub,CLIB.dmul,CLIB.ddiv ];
    ...
    бцел clib = clibtab[op - OPpostinc];
    elem *e1 = e.EV.E1;
    ...
    if ((cs.Iflags & CFSEG) == CFes)
        idxregs |= mES;
    cgstate.stackclean++;
    scodelem(cdb,e.EV.E2,&retregs2,idxregs,нет);
    cgstate.stackclean--;
...
private проц opnegassdbl(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    if (config.inline8087)
    {
    ...
    if (*pretregs || e1.Ecount)
    {
        ...
        else
        {
            if (I32)
            {
                ...
                getregs(cdb,DOUBLEREGS_32);
                cs.Iop = 0x8B;
                cs.Irm &= ~cast(бцел)modregrm(0,7,0);
                cs.Irm |= modregrm(0,AX,0);
                cdb.gen(&cs);
                cs.Irm |= modregrm(0,DX,0);
                getlvalue_msw(&cs);
...
проц cdeq(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    code cs;
    elem *e11;
    бул regvar;                  // да means evaluate into register variable
    regm_t varregm;
    reg_t varreg;
    targ_int postinc;
    ...
    бцел sz = _tysize[tyml];           // # of bytes to transfer
    assert(cast(цел)sz > 0);

    if (retregs == 0)                     // if no return значение
    {
        цел fl;
            ...
            !e1.Ecount)        // and no CSE headaches
        {
            ...
            // If loading результат into a register
            if ((cs.Irm & 0xC0) == 0xC0)
            {
                modEA(cdb,&cs);
            ...
            else
            {
                ...
                cs.IFL2 = FLconst;
                targ_т_мера *p = cast(targ_т_мера *) &(e2.EV);
                cs.IEV2.Vт_мера = *p;
                ...
                if ((cs.Irm & 0xC0) == 0xC0)
                {
                    ...
                    {   getlvalue_msw(&cs);
                        if (REGSIZE == 2)
                            ...
                            movregconst(cdb,cs.Irm & 7,(cast(бцел *)p)[1],0);
                        else if (REGSIZE == 8)
        ...
        if (sz == 2 * REGSIZE)
            retregs &= ~mBP;      // BP cannot be используется for register pair
    }
    if (retregs == mPSW)
    {
        retregs = allregs;
    ...
    {   // Be careful that this строки up with jmpopcode()
        retregs |= mPSW;
        *pretregs &= ~mPSW;
    }
    scodelem(cdb,e2,&retregs,0,да);    // get rvalue
        ...
        if (!I16)
        {
            ...
            for (; да; sz -= REGSIZE)
            {
                ...
                if (sz == 2)            // if 16 bit operand
                    ...
                    cs.Irex |= REX;
                cdb.gen(&cs);           // MOV EA+смещение,reg
                if (sz <= REGSIZE)
                    break;
                getlvalue_msw(&cs);
...
проц cdaddass(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    tym_t tyml = tybasic(e1.Ety);            // тип of lvalue
    цел sz = _tysize[tyml];
    цел isbyte = (sz == 1);                     // 1 for byte operation, else 0
    ...
    бцел opsize = (I16 && tylong(tyml) && config.target_cpu >= TARGET_80386)
        ? CFopsize : 0;
    бцел cflags = 0;
    regm_t forccs = *pretregs & mPSW;            // return результат in flags
    regm_t forregs = *pretregs & ~mPSW;          // return результат in regs
    // да if we want the результат in a register
    бцел wantres = forregs || (e1.Ecount && !OTleaf(e1.Eoper));
            ...
            )
    {
        ...
        modEA(cdb,&cs);
        cs.IFL2 = FLconst;
        cs.IEV2.Vт_мера = e2.EV.Vint;
        if (sz <= REGSIZE || tyfv(tyml) || opsize)
        {
            targ_int i = cs.IEV2.Vint;

            // Handle shortcuts. Watch out for if результат has
            // to be in flags.
            ...
            else if (!I16 && cs.Iflags & CFopsize)
            {
                ...
                {   case OPorass:
                    case OPxorass:
                        cs.IEV2.Vт_мера &= 0xFFFF;
                        cs.Iflags &= ~CFopsize; // don't worry about MSW
                        break;

                    case OPandass:
                        cs.IEV2.Vт_мера |= ~0xFFFFL;
                        cs.Iflags &= ~CFopsize; // don't worry about MSW
                        break;
                ...
                config.flags4 & CFG4speed)
            {
                ...
                allocreg(cdb,&sregm,&reg,tyml);      // размести register
        ...
        else if (sz == 2 * REGSIZE)
        {
            ...
            msw = cast(бцел)MSREG(e.EV.E2.EV.Vllong);
            ...
            )
    {
        ...
        if (OTconv(e2.Eoper))
        {
            scodelem(cdb,e2.EV.E1,&retregs,keepmsk,да);
            freenode(e2);
        }
        else
            scodelem(cdb,e2,&retregs,keepmsk,да);
        ...
        if (forccs)
            cs.Iflags |= CFpsw;
        cs.IFL2 = FLconst;
        cs.IEV2.Vт_мера = (jop == JC) ? 0 : ~cast(targ_т_мера)0;
        cdb.gen(&cs);
        retregs = 0;            // to trigger a bug if we attempt to use it
    }
    else // evaluate e2 into register
    {
        ...
        scodelem(cdb,e.EV.E2,&retregs,0,да);   // get rvalue
        ...
        {
            бцел mreg = findregmsw(retregs);
            бцел lreg = findreglsw(retregs);
    ...
    // See if we need to reload результат into a register.
    // Need результат in registers in case we have a 32 bit
    // результат and we want the flags as a результат.
...
проц cdmulass(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    tym_t tyml = tybasic(e1.Ety);              // тип of lvalue
    сим uns = tyuns(tyml) || tyuns(e2.Ety);
    бцел sz = _tysize[tyml];

    бцел rex = (I64 && sz == 8) ? REX_W : 0;
    бцел grex = rex << 16;          // 64 bit operands
    ...
    if (sz <= REGSIZE)                  // if word or byte
    {
        ...
        resreg = AX;                    // результат register for * or /
        if (uns)                        // if бцел operation
            opr = 4;                    // MUL
        else                            // else signed
            opr = 5;                    // IMUL
        ...
        if (op == OPmulass)             // if multiply
        {
            ...
            {
                targ_т_мера e2factor = cast(targ_т_мера)el_tolong(e2);
                if (I64 && sz == 8 && e2factor != cast(цел)e2factor)
                    goto L1;
                freenode(e2);
                ...
                cs.IFL2 = FLconst;
                cs.IEV2.Vint = cast(цел)e2factor;
                opr = resreg;
            }
            else if (!I16 && !isbyte)
            {
             L1:
                ...
                if (!retregs)
                    retregs = ALLREGS;
                codelem(cdb,e2,&retregs,нет); // load rvalue in reg
                getlvalue(cdb,&cs,e1,retregs);  // get EA
                getregs(cdb,retregs);           // разрушь these regs
            ...
            {
                retregs = mAX;
                codelem(cdb,e2,&retregs,нет);      // load rvalue in AX
                getlvalue(cdb,&cs,e1,mAX);           // get EA
                getregs(cdb,isbyte ? mAX : mAX | mDX); // разрушь these regs
        ...
        else // /= or %=
        {
            ...
            if (config.flags4 & CFG4speed &&
                ...
                (sz == REGSIZE || (I64 && sz == 4)) &&
                (pow2 = ispow2(e2factor = cast(targ_т_мера)el_tolong(e2))) != -1 &&
                e2factor == cast(цел)e2factor &&
            ...
            else
            {
                ...
                codelem(cdb,e2,&retregs,нет); // load rvalue in retregs
                reg = findreg(retregs);
                getlvalue(cdb,&cs,e1,mAX | mDX | retregs);     // get EA
                getregs(cdb,mAX | mDX);         // разрушь these regs
                cs.Irm |= modregrm(0,AX,0);
                cs.Iop = 0x8B;
                cdb.gen(&cs);                   // MOV AX,EA
                if (uns)                        // if бцел
        ...
        freenode(e1);
        fixрезультат(cdb,e,mask(resreg),pretregs);
        return;
    }
    else if (sz == 2 * REGSIZE)
    {
...
проц cdshass(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    tym_t tyml = tybasic(e1.Ety);              // тип of lvalue
    бцел sz = _tysize[tyml];
    бцел isbyte = tybyte(e.Ety) != 0;        // 1 for byte operations
    tym_t tym = tybasic(e.Ety);                // тип of результат
    OPER oper = e.Eoper;
    assert(tysize(e2.Ety) <= REGSIZE);

    бцел rex = (I64 && sz == 8) ? REX_W : 0;

    // if our lvalue is a cse, make sure we evaluate for результат in register
    ...
    // Select opcodes. op2 is используется for msw for long shifts.
    ...
    бцел v = 0xD3;                  // for SHIFT xx,CL cases
    бцел loopcnt = 1;
    бцел conste2 = нет;
    бцел shiftcnt = 0;              // avoid "use before initialized" warnings
    if (e2.Eoper == OPconst)
    {
        conste2 = да;                 // e2 is a constant
    ...
    if (*pretregs == 0 ||               // if don't return результат
        (*pretregs == mPSW && conste2 && _tysize[tym] <= REGSIZE) ||
        sz > REGSIZE
       )
    {
        retregs = 0;            // значение not returned in a register
        cs.Iop = v ^ isbyte;
        while (loopcnt--)
        {
        ...
        // If we want the результат, we must load it from the EA
        // into a register.
        ...
        if (sz == 2 * REGSIZE && *pretregs)
        {
            ...
            if (retregs)
            {
                ...
                cdb.gen(&cs);
                if (*pretregs & mPSW)
                    tstрезультат(cdb,retregs,tyml,да);
            }
            else        // flags only
            {
    ...
    else                                // else must evaluate in register
    {
        if (sz <= REGSIZE)
        {
            ...
            {
                assert(shiftcnt);
                *pretregs &= ~mPSW;     // результат is already in flags
                code_orflag(cdb.last(),CFpsw);
            }
            ...
            // If результат is not in correct register
            fixрезультат(cdb,e,retregs,pretregs);
            retregs = *pretregs;
        }
        else
 ...
 * Handles lt,gt,le,ge,eqeq,ne for all данные types.
 */
...
проц cdcmp(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    // Collect extra параметр. This is pretty ugly...
    цел флаг = cdcmp_flag;
    cdcmp_flag = 0;
    ...
    {
        codelem(cdb,e1,pretregs,нет);
        *pretregs = 0;                  // in case e1 changed it
        codelem(cdb,e2,pretregs,нет);
        return;
    }

    бцел jop = jmpopcode(e);        // must be computed before
                                        // leaves are free'd
    бцел reverse = 0;

    OPER op = e.Eoper;
    assert(OTrel(op));
    бул eqorne = (op == OPeqeq) || (op == OPne);
    ...
    if (tyfloating(tym))                  // if floating operation
    {
        ...
        else
        {
            static if (TARGET_WINDOS)
            {
                ...
                retregs = 0;                /* skip результат for now          */
                if (iffalse(e2))            /* second operand is constant 0 */
                {
                    assert(!eqorne);        /* should be OPбул or OPnot    */
                    if (tym == TYfloat)
                    {
                        retregs = FLOATREGS;
                        clib = CLIB.ftst0;
       ...
       )
    {
        ...
        codelem(cdb,e1,&retregs,нет);
        retregs = mCX | mBX;
        scodelem(cdb,e2,&retregs,mDX | mAX,нет);
    ...
    switch (e2.Eoper)
    {
        default:
        L2:
            scodelem(cdb,e1,&retregs,0,да);      // compute left leaf
            rretregs = allregs & ~retregs;
            if (isbyte)
                rretregs &= BYTEREGS;
            scodelem(cdb,e2,&rretregs,retregs,да);     // get right leaf
        ...
        case OPconst:
                ...
                (!I16 || op == OPlt || op == OPge))
            {
                ...
                {
                    case OPle:
                        cdb.genc2(0x81,grex | modregrmx(3,0,reg),cast(бцел)-1);   // ADD reg,-1
                        code_orflag(cdb.last(), CFpsw);
                    ...
                    case OPgt:
                        cdb.gen2(0xF7,grex | modregrmx(3,3,reg));         // NEG reg
                            /* Flips the sign bit unless the значение is 0 or цел.min.
                            Also sets the carry bit when the значение is not 0. */
                        code_orflag(cdb.last(), CFpsw);
                        cdb.genc2(0x81,grex | modregrmx(3,3,reg),0);  // SBB reg,0
                            /* Subtracts the carry bit. This turns цел.min into
                            цел.max, flipping the sign bit.
                    ...
                    case OPlt:
                    oplt:
                        // Get the sign bit, i.e. 1 if the значение is negative.
                        if (!I16)
            ...
            cs.IFL2 = FLconst;
            if (sz == 16)
                cs.IEV2.Vт_мера = cast(targ_т_мера)e2.EV.Vcent.msw;
            else if (sz > REGSIZE)
                cs.IEV2.Vint = cast(цел)MSREG(e2.EV.Vllong);
            else
                cs.IEV2.Vт_мера = cast(targ_т_мера)e2.EV.Vllong;

            // The cmp immediate relies on sign extension of the 32 bit immediate значение
            if (I64 && sz >= REGSIZE && cs.IEV2.Vт_мера != cast(цел)cs.IEV2.Vint)
                goto L2;
          L4:
            cs.Iop = 0x81 ^ isbyte;

            /* if ((e1 is данные or a '*' reference) and it's not a
             * common subВыражение
             */
                ...
                !evalinregister(e1))
            {
                ...
                else
                {
                    ...
                    if (sz > REGSIZE)
                    {
                        ...
                        genjmp(cdb,JNE,FLcode, cast(block *) ce); // JNE nop
                        if (e2.Eoper == OPconst)
                            cs.IEV2.Vint = cast(цел)e2.EV.Vllong;
        ...
        case OPvar:
          ...
          L5:
            ...
            {
                reg = findreg(retregs & allregs);   // get reg that e1 is in
                бцел opsize = cs.Iflags & CFopsize;
...
L3:
    if ((retregs = (*pretregs & (ALLREGS | mBP))) != 0) // if return результат in register
    {
        if (config.target_cpu >= TARGET_80386 && !флаг && !(jop & 0xFF00))
        {
            ...
            *pretregs &= ~mPSW;
            fixрезультат(cdb,e,resregs,pretregs);
        }
        else
        {
            code *nop = null;
...
проц longcmp(ref CodeBuilder cdb,elem *e,бул jcond,бцел fltarg,code *targ)
{
                                         // <=  >   <   >=
    static const ббайт[4] jopmsw = [JL, JG, JL, JG ];
    static const ббайт[4] joplsw = [JBE, JA, JB, JAE ];
    ...
    бцел jop = jopmsw[op - OPle];
    if (!(jcond & 1)) jop ^= (JL ^ JG);                   // toggle jump условие
    CodeBuilder cdbjmp;
    cdbjmp.ctor();
    ...
    switch (e2.Eoper)
    {
        ...
        case OPconst:
            cs.IEV2.Vint = cast(цел)MSREG(e2.EV.Vllong);            // MSW first
            cs.IFL2 = FLconst;
            cs.Iop = 0x81;
        ...
        case OPvar:
            if (!e1.Ecount && e1.Eoper == OPs32_64)
            {
                ...
                retregs = allregs;
                scodelem(cdb,e1.EV.E1,&retregs,0,да);
                freenode(e1);
                reg = findreg(retregs);
    ...
    jop = joplsw[op - OPle];
    if (!(jcond & 1)) jop ^= 1;                           // toggle jump условие
    ...
    if (config.inline8087)
    {
        switch (e.Eoper)
        {
            ...
            Lcomplex:
                    ...
                    codelem(cdb,e.EV.E1, &retregsx, нет);
                    fixрезультат_complex87(cdb, e, retregsx, pretregs);
                    return;
                }
                regm_t retregsx = mST0 | (*pretregs & mPSW);
                codelem(cdb,e.EV.E1, &retregsx, нет);
                fixрезультат87(cdb, e, retregsx, pretregs);
                return;
            }
            ...
            case OPu32_d:
                ...
                else if (!I16)
                {
                    ...
                    fixрезультат87(cdb, e, retregsy, pretregs);
                    return;
                }
                break;
...
проц cdshtlng(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    цел e1comsub = e.EV.E1.Ecount;
    ббайт op = e.Eoper;
    if ((*pretregs & (ALLREGS | mBP)) == 0)    // if don't need результат in regs
    {
        codelem(cdb,e.EV.E1,pretregs,нет);     // then conversion isn't necessary
        return;
    }
    else if (
             op == OPnp_fp ||
    ...
    {
        /* результат goes into a register pair.
         * Zero extend by putting a нуль into most significant reg.
         */
        ...
        fixрезультат(cdb,e,retregsx | regm,pretregs);
        return;
    }
        ...
        L2:
            ...
            *pretregs &= ~mPSW;             // flags are already set
            ...
            else
            {
                ...
                {
                    бцел iop = (op == OPu16_32) ? 0x0FB7 : 0x0FBF; // MOVZX/MOVSX reg,reg
    ...
    else if (*pretregs & mPSW || config.target_cpu < TARGET_80286)
    {
        ...
        retregs = mAX | mPSW;               // want integer результат in AX
        *pretregs &= ~mPSW;                 // flags are already set
        codelem(cdb,e.EV.E1,&retregs,нет);
        ...
        if (e1comsub)
            getregs(cdb,retregs);
        fixрезультат(cdb,e,mDX | retregs,pretregs);
        return;
    }
    else
    {
...
проц cdbyteint(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    if (!I16)
    {
        if (e1.Eoper == OPvar || (e1.Eoper == OPind && !e1.Ecount))
        {
            ...
            freenode(e1);
            fixрезультат(cdb,e,retregsx,pretregs);
            return;
        }
        size = tysize(e.Ety);
        retregs = *pretregs & BYTEREGS;
    ...
    {
        if (op == OPu8_16)              // if бцел conversion
        {
            retregs = *pretregs & BYTEREGS;
            if (retregs == 0)
                retregs = BYTEREGS;
        ...
        else
        {
            ...
            // math to provide the flags.
            retregs = mAX | (*pretregs & mPSW); // want integer результат in AX
        }
    }

    CodeBuilder cdb1;
    cdb1.ctor();
    codelem(cdb1,e1,&retregs,нет);
    code *c1 = cdb1.finish();
    cdb.приставь(cdb1);
    ...
    // If previous instruction is an AND bytereg,значение
    c = cdb.last();
     ...
     L1:
        if (!I16)
        {
            ...
            else
            {
                ...
                {
                    бцел iop = (op == OPu8_16) ? 0x0FB6 : 0x0FBE; // MOVZX/MOVSX reg,reg
                    genregs(cdb,iop,reg,reg);
                    if (I64 && reg >= 4)
                        code_orrex(cdb.last(), REX);
                }
 ...
 * Get смещение of far pointer (OPoffset).
 * Convert цел to byte (OP16_8).
...
проц cdlngsht(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    /* We "разрушь" a reg by assigning it the результат of a new e, even
    ...
    fixрезультат(cdb,e,retregs,pretregs);  // lsw only
}

/**********************************************
 * Get top 32 bits of 64 bit значение (I32)
 * or top 16 bits of 32 bit значение (I16)
 * or top 64 bits of 128 bit значение (I64).
 * OPmsw
 */
...
проц cdmsw(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    assert(!*pretregs || retregs);
    fixрезультат(cdb,e,retregs,pretregs);  // msw only
}
...
{
    //printf("cdport\n");
    ббайт op = 0xE4;            // root of all IN/OUT opcodes
    elem *e1 = e.EV.E1;
    ...
    if (e.Eoper == OPoutp)
    {
        ...
        scodelem(cdb,e.EV.E2,&retregs,((op & 0x08) ? mDX : 0),да);
        op |= 0x02;                     // OUT opcode
    }
    else // OPinp
    {
...
проц cdasm(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    cdb.genasm(cast(сим *)e.EV.Vstring, cast(бцел)e.EV.Vstrlen);
    fixрезультат(cdb,e,(I16 ? mDX | mAX : mAX),pretregs);
}
...
проц cdfar16(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    ...
    getregs(cdb,*pretregs);      // we will разрушь the regs
    ...
    if (e.Eoper == OPnp_f16p)
    {
        ...
        цел isbyte = (mask(reg) & BYTEREGS) == 0;
...
проц cdbtst(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    ...
    opcode_t op = 0xA3;                        // BT EA,значение
    цел mode = 4;
    ...
    if ((e1.Eoper == OPind && !e1.Ecount) || e1.Eoper == OPvar)
    {
        ...
        idxregs = idxregm(&cs);             // mask if index regs используется
    }
    else
    {
    ...
    ббайт word = (!I16 && _tysize[ty1] == SHORTSIZE) ? CFopsize : 0;
...
//    if (e2.Eoper == OPconst && e2.EV.Vuns < 0x100)  // should do this instead?
    ...
    {
        retregs = ALLREGS & ~idxregs;
        scodelem(cdb,e2,&retregs,idxregs,да);
        reg = findreg(retregs);
...
проц cdbt(ref CodeBuilder cdb,elem *e, regm_t *pretregs)
{
    ...
    ббайт word = (!I16 && _tysize[ty1] == SHORTSIZE) ? CFopsize : 0;
    regm_t idxregs = idxregm(&cs);         // mask if index regs используется
...
проц cdpopcnt(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    else
    {
        regm_t retregs = allregs;
        codelem(cdb,e.EV.E1, &retregs, нет);
        reg_t reg = cast(ббайт)findreg(retregs);
        cs.Irm = modregrm(3,0,reg & 7);
        cs.Iflags = 0;
        cs.Irex = 0;
        if (reg & 8)
...
проц cdpair(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    if (*pretregs == 0)                         // if don't want результат
    {
        codelem(cdb,e.EV.E1,pretregs,нет);     // eval left leaf
        *pretregs = 0;                          // in case they got set
    ...
    codelem(cdb,e.EV.E1, &regs1, нет);
    scodelem(cdb,e.EV.E2, &regs2, regs1, нет);
    ...
    fixрезультат(cdb,e,regs1 | regs2,pretregs);
}
...
проц cdcmpxchg(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    ...
    if (I32 && sz == 8)
    {
        ...
        codelem(cdb,e2.EV.E1,&retregs,нет);           // [DX,AX] = e2.EV.E1

        retregs = mCX|mBX;
        scodelem(cdb,e2.EV.E2,&retregs,mDX|mAX,нет);  // [CX,BX] = e2.EV.E2
    ...
    {
        бцел isbyte = (sz == 1);                  // 1 for byte operation
        ббайт word = (!I16 && sz == SHORTSIZE) ? CFopsize : 0;
        бцел rex = (I64 && sz == 8) ? REX_W : 0;

        regm_t retregs = mAX;
        codelem(cdb,e2.EV.E1,&retregs,нет);        // AX = e2.EV.E1

        retregs = (ALLREGS | mBP) & ~mAX;
        scodelem(cdb,e2.EV.E2,&retregs,mAX,нет);   // load rvalue in reg
