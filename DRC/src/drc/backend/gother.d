...
struct Elemdata
{
     ...
     *      null if not
     */
    Elemdata* найди(elem *e)
    {
        Elemdata* edl = &this;
        for (; edl; edl = edl.следщ)
        {
            if (edl.pelem == e)
                break;
        }
...
private проц elemdatafree(Elemdata **plist)
{
    Elemdata *eln;
    ...
    foreach (i, b; dfo[])    // for each block
    {
        ...
        if (b.Belem)
        {
            ...
            if (!(vec_equal(b.Binrd,b.Boutrd)))
            {
                ...
                j = cast(цел)vec_index(0,b.Binrd);
...
private проц conpropwalk(elem *n,vec_t IN)
{
    Elemdata *pdata;
    vec_t L,R;
    elem *t;
    ...
    // Collect данные for subsequent optimizations
...
private проц chkrd(elem *n,list_t rdlist)
{
    Symbol *sv;
    цел unambig;
    ...
    // If variable содержит bit fields, don't print message (because if
    // bit field is the first set, then we get a spurious warning).
    // STL uses 0 sized structs to transmit тип information, so
    // don't complain about them being используется before set.
    ...
    static if (0)
    {
        // If variable is нуль length static массив, don't print message.
        // BUG: Suppress error even if variable is initialized with проц.
    ...
    version (SCPP)
    {
        {   Outbuffer буф;
            ...
            type_tostring(&буф, sv.Stype);
            буф.пишиБайт(' ');
            буф.пиши(sv.Sident.ptr);
            p2 = буф.вТкст();
            warerr(WM.WM_used_b4_set, p2);     // variable используется before set
        }
    }
    ...
    version (Dinrus)
    {
        ...
        {
            выведиОшибку(n.Esrcpos.Sfilename, n.Esrcpos.Slinnum, n.Esrcpos.Scharnum,
                "variable %s используется before set", sv.Sident.ptr);
        }
    }
...
extern (C) list_t listrds(vec_t IN,elem *e,vec_t f)
{
    ...
    ty = e.Ety;
    if (tyscalar(ty))
        nsize = cast(бцел)size(ty);
    noff = e.EV.Voffset;
    unambig = s.Sflags & SFLunambig;
    if (f)
        vec_clear(f);
    for (i = 0; (i = cast(бцел) vec_index(i, IN)) < go.defnod.length; ++i)
    {
    ...
    listit:
        ...
        if (f)
            ...
            list_prepend(&rdlist,d);     // add the definition узел
    }
    return rdlist;
}

/********************************************
 * Look at reaching defs for Выражения of the form (v == c) and (v != c).
 * If all definitions of v are c or are not c, then we can replace the
 * Выражение with 1 or 0.
 */

private проц eqeqranges()
{
    Symbol *v;
    цел sz;
    elem *e;
    ...
    for (Elemdata *rel = eqeqlist; rel; rel = rel.следщ)
    {
        e = rel.pelem;
        v = e.EV.E1.EV.Vsym;
        ...
        результат = -1;                    // результат not known yet
        ...
        if (результат >= 0)
        {
            ...
            el_free(e.EV.E1);
            el_free(e.EV.E2);
            e.EV.Vint = (e.Eoper == OPeqeq) ? результат : результат ^ 1;
            e.Eoper = OPconst;
        }
    //L1:
    }
...
private проц intranges()
{
    ...
    for (Elemdata *rel = rellist; rel; rel = rel.следщ)
    {
        rb = rel.pblock;
        ...
        /* Гарант that the only defs reaching the increment elem (rdinc) */
        /* are rdeq and rdinc.                                          */
        for (Elemdata *iel = inclist; да; iel = iel.следщ)
        {
            elem *rd1;
            elem *rd2;
...
бул findloopparameters(elem* erel, ref elem* rdeq, ref elem* rdinc)
{
     ...
     */
    Elemdata* rel = rellist.найди(erel);
    if (!rel)
    {
        if (log) printf("\trel not found\n");
        return returnрезультат(нет);
    }
    ...
    Elemdata* iel = inclist.найди(rdinc);
    if (!iel)
    {
     ...
     */
    цел i = loopcheck(iel.pblock,iel.pblock,rel.pblock);
    block_clearvisit();
    if (i)
    {
    ...
    return returnрезультат(да);
}

/***********************
 * Return да if there is a path from start to inc without
 * passing through rel.
 */

private цел loopcheck(block *start,block *inc,block *rel)
{
...
Louter:
    while (1)
    {
        flowcp();               /* compute доступно копируй statements    */
        if (go.exptop <= 1)
            return;             // none доступно
        static if (0)
        {
            foreach (i; new бцел[1 .. go.exptop])
            {
        ...
        foreach (i, b; dfo[])    // for each block
        {
            if (b.Belem)
            {
                ...
                /*      a=b; d=b; a=b;                          */
                /* and the d=b копируй elem now reaches the end    */
...
/*****************************
 * Walk tree n, doing копируй propagation as we go.
 * Keep IN up to date.
 * Параметры:
 *      n = tree to walk & do копируй propagation in
 *      IN = vector of live копируй Выражения, updated as progress is made
 * Возвращает:
 *      да if need to recalculate данные flow equations and try again
 */

private бул copyPropWalk(elem *n,vec_t IN)
{
    бул recalc = нет;
    цел nocp = 0;

    проц cpwalk(elem* n, vec_t IN)
    {
        assert(n && IN);
        ...
        else if (OTunary(op))
        {
            ...
            else if (op == OPctor || op == OPdtor)
            {
                 ...
                 * an el_match is done on the lvalue. If копируй propagation
        ...
        if (OTdef(op))                  // if definition elem
        {
            цел ambig;              /* да if ambiguous def        */

            ambig = !OTassign(op) || t.Eoper == OPind;
            бцел i;
            for (i = 0; (i = cast(бцел) vec_index(i, IN)) < go.exptop; ++i) // for each active копируй elem
            {
                Symbol *v;
            ...
            clr:                        /* this копируй elem is not доступно */
                vec_clearbit(i,IN);     /* so удали it from the vector */
            } /* foreach */

            /* If this is a копируй elem in go.expnod[]   */
            /*      Set bit in IN.                     */
        ...
        else if (op == OPvar && !nocp)  // if reference to variable v
        {
            ...
            symbol_debug(v);
            const ty = n.Ety;
            бцел sz = tysize(n.Ety);
            if (sz == -1 && !tyfunc(n.Ety))
                sz = cast(бцел)type_size(v.Stype);

            elem *foundelem = null;
            Symbol *f;
            for (бцел i = 0; (i = cast(бцел) vec_index(i, IN)) < go.exptop; ++i) // for all active копируй elems
            {
                elem* c = go.expnod[i];
                assert(c);

                бцел csz = tysize(c.EV.E1.Ety);
                if (c.Eoper == OPstreq)
                    csz = cast(бцел)type_size(c.ET);
                assert(cast(цел)csz >= 0);
            ...
            if (foundelem)          /* if we can do the копируй prop   */
            {
                debug if (debugc)
                {
                    printf("Copyprop, from '%s'(%d) to '%s'(%d)\n",
                        (v.Sident[0]) ? cast(сим *)v.Sident.ptr : "temp".ptr, v.Ssymnum,
                        (f.Sident[0]) ? cast(сим *)f.Sident.ptr : "temp".ptr, f.Ssymnum);
                }

                тип *nt = n.ET;
                targ_т_мера noffset = n.EV.Voffset;
                el_copy(n,foundelem.EV.E2);
                n.Ety = ty;    // retain original тип
                n.ET = nt;
...
private 
{
    Barray!(elem*) assnod;      /* массив of pointers to asg elems       */
...
проц rmdeadass()
{
    ...
    foreach (b; dfo[])         // for each block b
    {
        ...
        assnod.setLength(assnum);         // pre-размести sufficient room
        ...
        for (бцел j = 0; (j = cast(бцел) vec_index(j, POSS)) < assnum; ++j) // for each possible dead asg.
        {
...
{   elem *e1;
    ...
    switch (n.Eoper)
    {
        ...
        case OPshrass:
        case OPashrass:
            n.Eoper = cast(ббайт)opeqtoop(n.Eoper);
            break;
...
private проц accumda(elem *n,vec_t DEAD, vec_t POSS)
{
    assert(n && DEAD && POSS);
    const op = n.Eoper;
    switch (op)
    {
        ...
        case OPcolon2:
        {
            ...
            /* P = P & (Pl & Pr) | ~P & (Pl | Pr)   */
            /*   = Pl & Pr | ~P & (Pl | Pr)         */
            const vecdim = cast(бцел)vec_dim(DEAD);
            for (бцел i = 0; i < vecdim; i++)
            {
        ...
        {
            Symbol *v = n.EV.Vsym;
            targ_т_мера voff = n.EV.Voffset;
            бцел vsize = tysize(n.Ety);

            // We have a reference. Clear all bits in POSS that
            // could be referenced.

            foreach ( i; new бцел[0 .. cast(бцел)assnod.length])
            {
        ...
        case OPbt:
            ...
            vec_subass(POSS,ambigref);      // удали possibly refed
            break;
        ...
        default:
            if (OTassign(op))
            {
                ...
                if (ERTOL(n))
                    accumda(n.EV.E2,DEAD,POSS);
                t = n.EV.E1;
                // if not (v = Выражение) then gen refs of left tree
                if (op != OPeq && op != OPstreq)
                    accumda(n.EV.E1,DEAD,POSS);
                else if (OTunary(t.Eoper))         // if (*e = Выражение)
                ...
                {
                    бцел tsz = tysize(t.Ety);
                    if (n.Eoper == OPstreq)
                        tsz = cast(бцел)type_size(n.ET);
                    foreach ( i; new бцел[0 .. cast(бцел)assnod.length])
                    {
                        elem *ti = assnod[i].EV.E1;

                        бцел tisz = tysize(ti.Ety);
                        if (assnod[i].Eoper == OPstreq)
                            tisz = cast(бцел)type_size(assnod[i].ET);

                        // There may be some problem with this следщ
                        // инструкция with unions.
                ...
                {
                    const i = cast(бцел)assnod.length;
                    vec_setbit(i,POSS);
 ...
 * Be careful not to compute live ranges for члены of structures (CLMOS).
 */
проц deadvar()
{
        assert(dfo);
            ...
            if (globsym.tab[i].Srange /*&& globsym.tab[i].Sclass != CLMOS*/)
                foreach (j, b; dfo[])
                    if (vec_testbit(i,b.Binlv))
                        vec_setbit(cast(бцел)j,globsym.tab[i].Srange);
        }
 ...
 *      i = block index
 */
private проц dvwalk(elem *n,бцел i)
{
    for (; да; n = n.EV.E1)
    {
        assert(n);
...
/*********************************
 * Optimize very busy Выражения (VBEs).
 */

private  vec_t blockseen; /* which blocks we have visited         */
...
проц verybusyexp()
{
    ...
    /* close as possible to where they are используется.                    */
    foreach_reverse (i, b; dfo[])     // for each block
    {
        ...
        /* Find pointer to last инструкция in current elem */
        pn = &(b.Belem);
        if (*pn)
        {
            while ((*pn).Eoper == OPcomma)
                pn = &((*pn).EV.E2);
            /* If last инструкция has side effects,  */
            /* don't do these VBEs. Potentially we  */
            /* could by assigning the результат to     */
        ...
        for (j = 0; (j = cast(бцел) vec_index(j, b.Bout)) < go.exptop; ++j)
        {
            if (go.expnod[j] == null ||
      ...
      //  L2:
        ...
        foundvbe:                   /* we got one           */
            ...
            *pn = el_bin(OPcomma,(*pn).Ety,
            ...
            /* go.expnod[j] one) so that the Выражение will   */
            /* only be hoisted again if other occurrences   */
            /* of the Выражение are found later. This      */
            /* will substitute for the fact that the        */
            /* el_copytree() Выражение does not appear in go.expnod[]. */
            l = k;
            do
            {
...
/****************************
 * Return да if elem j is killed somewhere
 * between b and bp.
 */

private цел killed(бцел j,block *bp,block *b)
{
...
/***************************
 * Return да if there is a path from b to bp along which
 * elem j is not используется.
 * Input:
 *      b .    block where we want to put the VBE
 *      bp .   block somewhere between b and block containing j
 *      j =     VBE Выражение elem candidate (index into go.expnod[])
 */

private цел ispath(бцел j,block *bp,block *b)
{
    /*chkvecdim(go.exptop,0);*/
    if (bp == b) return да;              /* the trivial case             */
    if (vec_testbit(bp.Bdfoidx,blockseen))
        return нет;                      /* already seen this block      */
    vec_setbit(bp.Bdfoidx,blockseen);      /* we've visited this block     */

    /* нет if elem j is используется in block bp (and reaches the end     */
    /* of bp, indicated by it being an AE in Bgen)                  */
    бцел i;
    for (i = 0; (i = cast(бцел) vec_index(i, bp.Bgen)) < go.exptop; ++i) // look thru используется Выражения
    {
        if (i != j && go.expnod[i] && el_match(go.expnod[i],go.expnod[j]))
            return нет;
    }

    /* Not используется in bp, see if there is a path through a predecessor */
    /* of bp                                                        */
    foreach (bl; ListRange(bp.Bpred))
        if (ispath(j,list_block(bl),b))
            return да;

    return нет;           /* j is используется along all paths            */
}
