...
module drc.backend.iasm;

// Online documentation: https://dlang.org/phobos/dmd_backend_iasm.html

import drc.backend.cc : block;
import drc.backend.code_x86 : opcode_t;
...
бцел VEX_128_W0(opcode_t op)            { return _VEX(op)|_VEX_NOO; }
бцел VEX_128_W1(opcode_t op)            { return _VEX(op)|_VEX_NOO|_VEX_W; }
бцел VEX_128_WIG(opcode_t op)           { return  VEX_128_W0(op); }
бцел VEX_256_W0(opcode_t op)            { return _VEX(op)|_VEX_NOO|_VEX_L; }
бцел VEX_256_W1(opcode_t op)            { return _VEX(op)|_VEX_NOO|_VEX_W|_VEX_L; }
бцел VEX_256_WIG(opcode_t op)           { return  VEX_256_W0(op); }
бцел VEX_NDS_128_W0(opcode_t op)        { return _VEX(op)|_VEX_NDS; }
бцел VEX_NDS_128_W1(opcode_t op)        { return _VEX(op)|_VEX_NDS|_VEX_W; }
бцел VEX_NDS_128_WIG(opcode_t op)       { return  VEX_NDS_128_W0(op); }
бцел VEX_NDS_256_W0(opcode_t op)        { return _VEX(op)|_VEX_NDS|_VEX_L; }
бцел VEX_NDS_256_W1(opcode_t op)        { return _VEX(op)|_VEX_NDS|_VEX_W|_VEX_L; }
бцел VEX_NDS_256_WIG(opcode_t op)       { return  VEX_NDS_256_W0(op); }
бцел VEX_NDD_128_W0(opcode_t op)        { return _VEX(op)|_VEX_NDD; }
бцел VEX_NDD_128_W1(opcode_t op)        { return _VEX(op)|_VEX_NDD|_VEX_W; }
бцел VEX_NDD_128_WIG(opcode_t op)       { return  VEX_NDD_128_W0(op); }
бцел VEX_NDD_256_W0(opcode_t op)        { return _VEX(op)|_VEX_NDD|_VEX_L; }
бцел VEX_NDD_256_W1(opcode_t op)        { return _VEX(op)|_VEX_NDD|_VEX_W|_VEX_L; }
бцел VEX_NDD_256_WIG(opcode_t op)       { return  VEX_NDD_256_W0(op); }
бцел VEX_DDS_128_W0(opcode_t op)        { return _VEX(op)|_VEX_DDS; }
бцел VEX_DDS_128_W1(opcode_t op)        { return _VEX(op)|_VEX_DDS|_VEX_W; }
бцел VEX_DDS_128_WIG(opcode_t op)       { return  VEX_DDS_128_W0(op); }
бцел VEX_DDS_256_W0(opcode_t op)        { return _VEX(op)|_VEX_DDS|_VEX_L; }
бцел VEX_DDS_256_W1(opcode_t op)        { return _VEX(op)|_VEX_DDS|_VEX_W|_VEX_L; }
бцел VEX_DDS_256_WIG(opcode_t op)       { return  VEX_DDS_256_W0(op); }
...
бцел _VEX(opcode_t op) { return (0xC4 << 24) | _VEX_MM(op >> 8) | (op & 0xFF); }

бцел _VEX_MM(opcode_t op)
{
    return
...
debug
{
     бцел _VEX_ASSERT0() { assert(0); }
}
else
{
     бцел _VEX_ASSERT0() { return 0; }
}
...
////////////////////////////////////////////////////////////////////
// Operand flags for floating point opcodes are all just ники for
...
// bit size                      5            3          3              7
бцел CONSTRUCT_FLAGS(бцел uSizemask, бцел aopty, бцел amod, бцел uRegmask)
{
    return uSizemask | (aopty << 5) | (amod << 8) | (uRegmask << 11);
}

бцел ASM_GET_uSizemask(бцел us) { return us & 0x1F; }
бцел ASM_GET_aopty(бцел us)     { return cast(ASM_OPERAND_TYPE)((us >> 5) & 7); }
бцел ASM_GET_amod(бцел us)      { return cast(ASM_MODIFIERS)((us >> 8) & 7); }
бцел ASM_GET_uRegmask(бцел us)  { return (us >> 11) & 0x7F; }
...
// For amod (3 bits)
alias бцел ASM_MODIFIERS;
enum
{
    _normal,        // Normal register значение
...
const _rplus_r        = 0x20;
const _plus_r = CONSTRUCT_FLAGS( 0, 0, 0, _rplus_r );
...
/* from iasm.c */
цел asm_state(цел iFlags);

проц asm_process_fixup( block **ppblockLabels );
...
const ASM_END = 0xffff;      // special opcode meaning end of PTRNTABx table
