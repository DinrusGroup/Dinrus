...
бул isXMMstore(opcode_t op)
{
    ...
    case STOHPD: case STOHPS: case STOLPD: case STOLPS: return да;
    default: return нет;
    }
}

/*******************************************
 * Move constant значение into xmm register xreg.
 */

private проц movxmmconst(ref CodeBuilder cdb, reg_t xreg, бцел sz, targ_т_мера значение, regm_t flags)
{
    /* Generate:
     *    MOV reg,значение
     *    MOV xreg,reg
    ...
    if (I32 && sz == 8)
    {
        ...
        union U { targ_т_мера s; targ_long[2] l; }
        U u = проц;
        u.l[1] = 0;
        u.s = значение;
        targ_long *p = &u.l[0];
        ...
        const op = xmmload(TYdouble, да);
        cdb.genxmmreg(op,xreg,0,TYdouble);          // MOVSD XMMreg,floatreg
    }
    else
    {
        reg_t reg;
        regwithvalue(cdb,ALLREGS,значение,&reg,(sz == 8) ? 64 : 0);
...
проц orthxmm(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
         ...
         (tyreal(e2.Ety) && tyimaginary(e1.Ety))))
    {
        ...
        scodelem(cdb, e2, &rretregs, retregs, да);  // eval right leaf
        ...
        if (e.Eoper == OPmin)
        {
            ...
            const бцел sz = tysize(e1.Ety);
            allocreg(cdb,&nretregs,&sreg,e2.Ety);
            targ_т_мера signbit = 0x80000000;
            if (sz == 8)
                signbit = cast(targ_т_мера)0x8000000000000000L;
...
проц xmmcnvt(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    case OPu32_d: rex = REX_W; zx = да; goto Litod;
    Litod:
        regs = ALLREGS;
        op = CVTSI2SD;
        ty = TYdouble;
    ...
    {   assert(I64);
        getregs(cdb,regs);
        genregs(cdb,0x8B,reg,reg); // MOV reg,reg to нуль upper 32-bit
...
{   elem *e1 = e.EV.E1;
    ...
    codelem(cdb,e2,&rretregs,нет); // eval right leaf
    ...
    if (!regvar)
    {
        ...
        cs.Iop = xmmload(ty1, да);            // MOVSD xmm,xmm_m64
    ...
    if (!regvar)
    {
        cs.Iop = xmmstore(ty1,да);      // reverse operand order of MOVS[SD]
        cdb.gen(&cs);
        checkSetVex(cdb.last(), ty1);
    }
...
проц xmmpost(ref CodeBuilder cdb,elem *e,regm_t *pretregs)
{
    ...
    // результат register
    regm_t результатregs = XMMREGS & *pretregs & ~retregs;
    if (!результатregs)
        результатregs = XMMREGS & ~retregs;
    reg_t результатreg;
    allocreg(cdb,&результатregs, &результатreg, ty1);

    cdb.gen2(xmmload(ty1,да),modregxrmx(3,результатreg-XMM0,reg-XMM0));   // MOVSS/D результатreg,reg
    checkSetVex(cdb.last(), ty1);

    regm_t rretregs = XMMREGS & ~(*pretregs | retregs | результатregs);
    if (!rretregs)
        rretregs = XMMREGS & ~(retregs | результатregs);
    codelem(cdb,e2,&rretregs,нет); // eval right leaf
    const rreg = findreg(rretregs);
    ...
    fixрезультат(cdb,e,результатregs,pretregs);
    freenode(e1);
}
...
/*****************************
 * Get correct load operator based on тип.
 ...
 * Параметры:
 *      tym = тип of данные to load
 *      aligned = for vectors, да if aligned to 16 bytes
 */

opcode_t xmmload(tym_t tym, бул aligned)
{
    opcode_t op;
    if (tysize(tym) == 32)
        aligned = нет;
...
/*****************************
 * Get correct store operator based on тип.
 */

opcode_t xmmstore(tym_t tym, бул aligned)
{
    opcode_t op;
    switch (tybasic(tym))
    {
...
/************************************
 * Get correct XMM operator based on тип and operator.
 */
...
проц cdvector(ref CodeBuilder cdb, elem *e, regm_t *pretregs)
{
    ...
    elem **парамы = cast(elem **)malloc(n * (elem *).sizeof);
    assert(парамы);
    elem **tmp = парамы;
...
static if (0)
{
    ...
    {   /* Evaluate for side effects only
        ...
        {
            codelem(cdb,парамы[i], pretregs, нет);
    ...
    {   // Handle: op xmm,imm8
        ...
        if (!retregs)
            retregs = XMMREGS;
        codelem(cdb,op1,&retregs,нет); // eval left leaf
        const reg = findreg(retregs);
        цел r;
        switch (op)
        {
        ...
        getregs(cdb,retregs);
        cdb.genc2(op,modregrmx(3,r,reg-XMM0), cast(бцел)el_tolong(op2));
    }
    else if (n == 2)
    {   /* Handle: op xmm,mem
         * where xmm is written only, not читай
         */
        code cs;
    ...
    {   /* Handle:
        ...
        {
            regm_t rretregs = XMMREGS & ~retregs;
            scodelem(cdb, op2, &rretregs, retregs, да);
            const rreg = findreg(rretregs) - XMM0;
            cs.Irm = modregrm(3,0,rreg & 7);
            cs.Iflags = 0;
            cs.Irex = 0;
...
version (Dinrus)
{
            ...
            {
                выведиОшибку(imm8.Esrcpos.Sfilename, imm8.Esrcpos.Slinnum, imm8.Esrcpos.Scharnum, "last параметр to `__simd()` must be a constant");
                cs.IEV2.Vт_мера = 0;
            }
            else
                cs.IEV2.Vт_мера = cast(targ_т_мера)el_tolong(imm8);
}
else
{
            cs.IEV2.Vт_мера = cast(targ_т_мера)el_tolong(imm8);
 ...
 * OPvecfill takes the single значение in e1 and
 * fills the vector тип with it.
 */
...
static if (0)
{
    ...
    {
        regm_t rretregs = XMMREGS & ~retregs;
        cr = scodelem(op2, &rretregs, retregs, да);
        const rreg = findreg(rretregs) - XMM0;
        cs.Irm = modregrm(3,0,rreg & 7);
        cs.Iflags = 0;
        cs.Irex = 0;
    ...
    switch (ty)
    {
        ...
        case TYuchar32:
            ...
            {
                regm_t regm = ALLREGS;
                codelem(cdb,e1,&regm,да); // eval left leaf
        ...
        case TYulong4:
            ...
            {
                codelem(cdb,e1,&retregs,да); // eval left leaf
                const reg = cast(reg_t)(findreg(retregs) - XMM0);
                getregs(cdb,retregs);
                if (config.avx >= 2)
                {
...
бул xmmIsAligned(elem *e)
{
    if (tyvector(e.Ety) && e.Eoper == OPvar)
    {
        ...
        if (Symbol_Salignsize(s) < alignsz ||
            ...
            return нет;       // definitely not aligned
    }
    return да;        // assume aligned
}

/**************************************
 * VEX prefixes can be 2 or 3 bytes.
 * If it must be 3 bytes, set the CFvex3 флаг.
 */
...
проц checkSetVex(code *c, tym_t ty)
{
    if (config.avx || tysize(ty) == 32)
    {
        ...
        {
            static бцел MM_PP(бцел mm, бцел pp) { return (mm << 16) | (pp << 8); }
