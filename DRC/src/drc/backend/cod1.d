...
extern  бул[FLMAX] stackfl;

private extern (D) бцел mask(бцел m) { return 1 << m; }

private проц genorreg(ref CodeBuilder c, бцел t, бцел f) { genregs(c, 0x09, f, t); }

/* массив to convert from index register to r/m field    */
                                       /* AX CX DX BX SP BP SI DI       */
private  const byte[8] regtorm32 =   [  0, 1, 2, 3,-1, 5, 6, 7 ];
 const   byte[8] regtorm   =   [ -1,-1,-1, 7,-1, 6, 4, 5 ];

targ_т_мера paramsize(elem *e, tym_t tyf);
//проц funccall(ref CodeBuilder cdb,elem *e,бцел numpara,бцел numalign,
//        regm_t *pretregs,regm_t keepmsk, бул usefuncarg);

/*********************************
 * Determine if we should leave параметр `s` in the register it
 * came in, or размести a register it using the register
 * allocator.
 * Параметры:
 *      s = параметр Symbol
 * Возвращает:
 *      `да` if `s` is a register параметр and leave it in the register it came in
 */
 ...
 *      !=0     the значение for ss in the SIB byte
 */

цел isscaledindex(elem *e)
{
    targ_uns ss;
...
/*********************************************
 * Generate code for which isscaledindex(e) returned a non-нуль результат.
 */

/*private*/ проц cdisscaledindex(ref CodeBuilder cdb,elem *e,regm_t *pidxregs,regm_t keepmsk)
{
    // Load index register with результат of e.EV.E1
    while (e.Eoper == OPcomma)
    {
        regm_t r = 0;
        scodelem(cdb, e.EV.E1, &r, keepmsk, да);
        freenode(e);
        e = e.EV.E2;
    }
    assert(e.Eoper == OPshl);
    scodelem(cdb, e.EV.E1, pidxregs, keepmsk, да);
    freenode(e.EV.E2);
    freenode(e);
}
...
struct Ssindex
{
    ...
    ббайт ss1;
    ббайт ss2;
    ббайт ssflags;       /// SSFLxxxx
}

private  const Ssindex[21] ssindex_array =
[
...
цел ssindex(OPER op,targ_uns product)
{
    if (op == OPshl)
        product = 1 << product;
    for (т_мера i = 1; i < ssindex_array.length; i++)
    {
...
проц genEEcode()
{
    ...
    assert(EEStack.смещение >= REGSIZE);
    cod3_stackadj(cdb, cast(цел)(EEStack.смещение - REGSIZE));
    cdb.gen1(0x50 + SI);                      // PUSH ESI
    cdb.genadjesp(cast(цел)EEStack.смещение);
    gencodelem(cdb, eecontext.EEelem, &retregs, нет);
    code *c = cdb.finish();
    assignaddrc(c);
    pinholeopt(c,null);
    jmpaddr(c);
...
бцел gensaverestore(regm_t regm,ref CodeBuilder cdbsave,ref CodeBuilder cdbrestore)
{
 ...
 *      keepmsk         mask of registers to not разрушь
 */
...
+/
        {
            ...
            {
                scratchm = ALLREGS & ~keepmsk & regcon.используется & ~regcon.mvar;
            }
...
/*********************************
 * Generate code for a logical Выражение.
 * Input:
 *      e       elem
 *      jcond
 *         bit 1 if да then goto jump address if e
 *               if нет then goto jump address if !e
 *         2    don't call save87()
...
проц logexp(ref CodeBuilder cdb, elem *e, цел jcond, бцел fltarg, code *targ)
{
    ...
    if (!OTleaf(e.Eoper) && !e.Ecount)     // if operator and not common sub
    {
        switch (e.Eoper)
        {
            ...
            case OPcond:
            {
                ...
                code *cnop = gennop(null);
                logexp(cdb, e.EV.E1, нет, FLcode, cnop2);   // eval условие
    ...
    regm_t retregs = mPSW;                // return результат in flags
    opcode_t op = jmpopcode(e);           // get jump opcode
    if (!(jcond & 1))
        op ^= 0x101;                      // toggle jump условие(s)
    codelem(cdb, e, &retregs, да);         // evaluate elem
    if (no87)
 ...
 * Look for common subВыражение.
 * Can handle indirection operators, but not if they're common subs.
 * Input:
 *      e ->    elem where we get some of the данные from
 *      cs ->   partially filled code to add
 *      op =    opcode
 *      reg =   reg field of (mod reg r/m)
 *      смещение = данные to be added to Voffset field
 *      keepmsk = mask of registers we must not разрушь
 *      desmsk  = mask of registers destroyed by executing the instruction
 * Возвращает:
 *      pointer to code generated
 */

проц loadea(ref CodeBuilder cdb,elem *e,code *cs,бцел op,бцел reg,targ_т_мера смещение,
            regm_t keepmsk,regm_t desmsk)
{
    ...
    debug
    if (debugw)
        printf("loadea: e=%p cs=%p op=x%x reg=%s смещение=%lld keepmsk=%s desmsk=%s\n",
               e, cs, op, regstring[reg], cast(бдол)смещение, regm_str(keepmsk), regm_str(desmsk));
    assert(e);
    cs.Iflags = 0;
    cs.Irex = 0;
    cs.Iop = op;
        ...
        (op & 0xFFF8) != 0xD8)            // and not 8087 opcode
    {
        ...
        if (sz > REGSIZE)               // значение is in 2 or 4 registers
        {
            if (I16 && sz == 8)     // значение is in 4 registers
            {
                static const regm_t[4] rmask = [ mDX,mCX,mBX,mAX ];
                rm &= rmask[cast(т_мера)(смещение >> 1)];
            }
            else if (смещение)
        ...
        for (бцел i = 0; rm; i++)
        {
            if (mask(i) & rm)
            {
                if (regcon.cse.значение[i] == e && // if register has elem
    ...
    if (!I16)
    {
        ...
        cs.Iflags |= CFwait;
L2:
    getregs(cdb, desmsk);                  // save any regs we разрушь
...
бцел getaddrmode(regm_t idxregs)
{
    ...
    if (I16)
    {
        static ббайт выведиОшибку() { assert(0); }
...
проц getlvalue(ref CodeBuilder cdb,code *pcs,elem *e,regm_t keepmsk)
{
    ...
    switch (fl)
    {
        case FLoper:
             ...
             */
            static бул relativeToRIP(Symbol* s)
            {
                if (!I64)
                    return нет;
               ...
               )
            {
                ...
                {   /* Any register can be an index register        */
                    ...
                    if (ss)
                    {
                        /* Load index register with результат of e11.EV.E1       */
                            ...
                            )
                    {
                        ...
                        сим ssflags = ssindex_array[ssi].ssflags;
                        if (ssflags & SSFLnobp && stackfl[f])
                            goto L6;

                        // Load index register with результат of e11.EV.E1
                        scodelem(cdb, e11.EV.E1, &idxregs, keepmsk, да);
                        reg = findreg(idxregs);

                        цел ss1 = ssindex_array[ssi].ss1;
                        if (ssflags & SSFLlea)
                        {
                            assert(!stackfl[f]);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    static const бцел[4] imm32 = [1+1,2+1,4+1,8+1];
                    ...
                    {
                     L6:
                        /* Load index register with результат of e11   */
                        scodelem(cdb, e11, &idxregs, keepmsk, да);
                ...
                else
                {
                    ...
                    assert(idxregs);
                    if (stackfl[f])                 /* if stack данные тип   */
                    {
                        ...
                        t = 0;                      /* [SI + disp]          */
                    scodelem(cdb, e11, &idxregs, keepmsk, да); // load idx reg
                    pcs.Irm = cast(ббайт)(getaddrmode(idxregs) ^ t);
                }
                if (f == FLpara)
                    refparam = да;
                ...
                if (e1.Ecount)
                {
                    ...
                    /* If desired результат is a far pointer, we'll have       */
               ...
               )
            {
                ...
                {
                    scodelem(cdb, e11, &idxregs, keepmsk, да); // load index reg
                    setaddrmode(pcs, idxregs);
                }
                goto Lptr;
            }
            ...
            L4:
                ...
                else if ((ss = isscaledindex(e11)) != 0)
                {
                    ...
                    scodelem(cdb, e12, &idxregs, keepmsk | idxregs2, да);
                }
                ...
                else
                {
                    scodelem(cdb, e11, &idxregs, keepmsk, да);
                    idxregs2 = allregs & ~(idxregs | keepmsk);
                    scodelem(cdb, e12, &idxregs2, keepmsk | idxregs, да);
                }
             ...
             * we have a pointer to a long and need an смещение to the second
             * word.
             */

            assert(e1free);
            scodelem(cdb, e1, &idxregs, keepmsk, да);  // load index register
            setaddrmode(pcs, idxregs);
        Lptr:
        ...
        case FLfast:
            if (regParamInPreg(s))
            {
                ...
                /* See if the параметр is still hanging about in a register,
                 * and so can we load from that register instead.
                 */
                if (regcon.парамы & pregm /*&& s.Spreg2 == NOREG && !(pregm & XMMREGS)*/)
                {
                    if (keepmsk & RMload && !anyiasm)
                    {
                        ...
                        if (sz <= REGSIZE)
                        {
                             ...
                             */
                            if (preg != NOREG && regcon.парамы & mask(preg))
                            {
                                //printf("sz %d, preg %s, Voffset %d\n", cast(цел)sz, regm_str(mask(preg)), cast(цел)voffset);
        ...
        case FLextern:
            ...
            if (s.ty() & mTYcs && cast(бул) LARGECODE)
                goto Lfardata;
            goto L3;
    ...
    static if (TARGET_LINUX || TARGET_OSX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS)
    {
        ...
        L2:
            if (fl == FLreg)
            {
                ...
                // s.Sident.ptr, cast(цел)e.EV.Voffset, regm_str(regcon.mvar));
                ...
                {   refparam = да;
                    reflocal = да;        // kludge to set up prolog
                }
            ...
            {   /* Don't use SI or DI for this variable     */
                ...
                {
                    debug if (debugr) printf("'%s' not reg cand due to byte смещение\n", s.Sident.ptr);
                    s.Sflags &= ~GTregcand;
                }
            }
            else if (e.EV.Voffset || sz > tysize(s.Stype.Tty))
            {
                debug if (debugr) printf("'%s' not reg cand due to смещение or size\n", s.Sident.ptr);
                s.Sflags &= ~GTregcand;
            }

            if (config.fpxmmregs && tyfloating(s.ty()) && !tyfloating(ty))
            {
                debug if (debugr) printf("'%s' not reg cand due to mix float and цел\n", s.Sident.ptr);
                // Can't successfully mix XMM register variables accessed as integers
                s.Sflags &= ~GTregcand;
            }

            if (!(keepmsk & RMstore))               // if not store only
                s.Sflags |= SFLread;               // assume we are doing a читай
            break;
...
/*****************************
 * Given a результат in registers, test it for да or нет.
 * Will fail if TYfptr and the reg is ES!
 * If saveflag is да, preserve the contents of the
 * registers.
 */
...
проц tstрезультат(ref CodeBuilder cdb, regm_t regm, tym_t tym, бцел saveflag)
{
    ...
    //if (!(regm & (mBP | ALLREGS)))
        //printf("tstрезультат(regm = %s, tym = x%x, saveflag = %d)\n",
        ...
        if (!I16)
        {
            if (tym == TYfloat)
            {
                if (saveflag)
                {
                    ...
                    allocreg(cdb, &scrregm, &scrreg, TYoffset); // размести scratch reg
...
{
    //printf("fixрезультат(e = %p, retregs = %s, *pretregs = %s)\n",e,regm_str(retregs),regm_str(*pretregs));
    if (*pretregs == 0) return;           // if don't want результат
    ...
    if ((retregs & forregs) == retregs)   // if already in right registers
        *pretregs = retregs;
    else if (forregs)             // if return the результат in registers
    {
        ...
        if (I16 && sz == 8)
        {
            ...
            else if (retregs & mSTACK)
            {
                ...
                cdb.gen1(0x58 + AX);
                stackpush -= DOUBLESIZE;
                retregs = DOUBLEREGS_16; // for tstрезультат() below
            }
            else
            {
                debug
        ...
        {
            allocreg(cdb, pretregs, &rreg, tym);  // размести return regs
            if (retregs & XMMREGS)
            {
            ...
            {
                бцел msreg = findregmsw(retregs);
                бцел lsreg = findreglsw(retregs);
                бцел msrreg = findregmsw(*pretregs);
                бцел lsrreg = findreglsw(*pretregs);
            ...
            else
            {
                ...
                reg = findreg(retregs & (mBP | ALLREGS));
                if (I64 && sz <= 4)
                    genregs(cdb, 0x89, reg, rreg);  // only move 32 bits, and нуль the top 32 bits
                else
        ...
        //forregs = 0;    // don't care about результат in reg cuz real результат is in rreg
        retregs = *pretregs & ~mPSW;
    }
    if (forccs)                           // if return результат in flags
    {
        if (retregs & (mST01 | mST0))
            fixрезультат87(cdb, e, retregs, pretregs);
        else
            tstрезультат(cdb, retregs, tym, forregs);
    }
}
...
enum
{
    ...
    INFpushebx    = 0x10,   /// сунь EBX before load_localgot()
    INFpusheabcdx = 0x20,   /// pass EAX/EBX/ECX/EDX on stack, callee does ret 16
}

struct ClibInfo
{
    regm_t retregs16;   /* registers that 16 bit результат is returned in  */
    regm_t retregs32;   /* registers that 32 bit результат is returned in  */
    ббайт вынь;          // # of bytes popped off of stack upon return
    ббайт flags;        /// INFxxx
    byte push87;                        // # of pushes onto the 8087 stack
    byte pop87;                         // # of pops off of the 8087 stack
}

 цел clib_inited = нет;          // да if initialized

Symbol* symboly(ткст0 имя, regm_t desregs)
{
...
проц getClibInfo(бцел clib, Symbol** ps, ClibInfo** pinfo)
{
     Symbol*[CLIB.MAX] clibsyms;
     ClibInfo[CLIB.MAX] clibinfo;

    if (!clib_inited)
    {
        for (т_мера i = 0; i < CLIB.MAX; ++i)
        {
            Symbol* s = clibsyms[i];
            if (s)
            {
    ...
    const бцел ex_unix = (EX_LINUX   | EX_LINUX64   |
                          EX_OSX     | EX_OSX64     |
    ...
    if (!s)
    {
        ...
        switch (clib)
        {
            case CLIB.lcmp:
                {
                    ткст0 имя = (config.exe & ex_unix) ? "__LCMP__" : "_LCMP@";
                    s = symboly(имя, 0);
                }
                break;
            ...
            case CLIB.lmul:
                {
                    ткст0 имя = (config.exe & ex_unix) ? "__LMUL__" : "_LMUL@";
                    s = symboly(имя, mAX|mCX|mDX);
            ...
            case CLIB.ldiv:
                ...
                {
                    ткст0 имя = (config.exe & ex_unix) ? "__LDIV__" : "_LDIV@";
                    s = symboly(имя, (config.exe & ex_unix) ? mAX|mBX|mCX|mDX : ALLREGS);
                    cinfo.retregs32 = mDX|mAX;
                }
                break;
            ...
            case CLIB.uldiv:
                ...
                {
                    ткст0 имя = (config.exe & ex_unix) ? "__ULDIV__" : "_ULDIV@";
                    s = symboly(имя, (config.exe & ex_unix) ? mAX|mBX|mCX|mDX : ALLREGS);
                    cinfo.retregs32 = mDX|mAX;
                }
                break;
            ...
            case CLIB.dbllng:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLLNG" : "_DBLLNG@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = mDX | mAX;
                cinfo.retregs32 = mAX;
                cinfo.flags = INFfloat;
                cinfo.push87 = 1;
            ...
            case CLIB.lngdbl:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__LNGDBL" : "_LNGDBL@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = DOUBLEREGS_16;
                cinfo.retregs32 = DOUBLEREGS_32;
                cinfo.flags = INFfloat;
            ...
            case CLIB.dblint:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLINT" : "_DBLINT@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = mAX;
                cinfo.retregs32 = mAX;
                cinfo.flags = INFfloat;
            ...
            case CLIB.intdbl:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__INTDBL" : "_INTDBL@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = DOUBLEREGS_16;
                cinfo.retregs32 = DOUBLEREGS_32;
                cinfo.flags = INFfloat;
            ...
            case CLIB.dbluns:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLUNS" : "_DBLUNS@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = mAX;
                cinfo.retregs32 = mAX;
                cinfo.flags = INFfloat;
            ...
            {
                ткст0 имя = (config.exe & ex_unix) ? "__UNSDBL" : "_UNSDBL@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = DOUBLEREGS_16;
                cinfo.retregs32 = DOUBLEREGS_32;
                cinfo.flags = INFfloat;
            ...
            case CLIB.dblulng:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLULNG" : "_DBLULNG@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = mDX|mAX;
                cinfo.retregs32 = mAX;
            ...
            case CLIB.ulngdbl:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__ULNGDBL@" : "_ULNGDBL@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = DOUBLEREGS_16;
                cinfo.retregs32 = DOUBLEREGS_32;
                cinfo.flags = INFfloat;
            ...
            case CLIB.dblflt:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLFLT" : "_DBLFLT@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = FLOATREGS_16;
                cinfo.retregs32 = FLOATREGS_32;
                cinfo.flags = INFfloat;
            ...
            case CLIB.fltdbl:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__FLTDBL" : "_FLTDBL@";
                s = symboly(имя, I16 ? ALLREGS : DOUBLEREGS_32);
                cinfo.retregs16 = DOUBLEREGS_16;
                cinfo.retregs32 = DOUBLEREGS_32;
                cinfo.flags = INFfloat;
                cinfo.push87 = 1;
            ...
            case CLIB.dblllng:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLLLNG" : "_DBLLLNG@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = DOUBLEREGS_16;
                cinfo.retregs32 = mDX|mAX;
                cinfo.flags = INFfloat;
            ...
            case CLIB.llngdbl:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__LLNGDBL" : "_LLNGDBL@";
                s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                cinfo.retregs16 = DOUBLEREGS_16;
                cinfo.retregs32 = DOUBLEREGS_32;
                cinfo.flags = INFfloat;
            ...
            case CLIB.dblullng:
            {
                ...
                {
                    ткст0 имя = (config.exe & ex_unix) ? "__DBLULLNG" : "_DBLULLNG@";
                    s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                    cinfo.retregs16 = DOUBLEREGS_16;
            ...
            case CLIB.ullngdbl:
            {
                ...
                {
                    ткст0 имя = (config.exe & ex_unix) ? "__ULLNGDBL" : "_ULLNGDBL@";
                    s = symboly(имя, I16 ? DOUBLEREGS_16 : DOUBLEREGS_32);
                    cinfo.retregs16 = DOUBLEREGS_16;
            ...
            case CLIB.dtst:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DTST" : "_DTST@";
                s = symboly(имя, 0);
                cinfo.flags = INFfloat;
                break;
            }
            ...
            case CLIB.vptrfptr:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__HTOFPTR" : "_HTOFPTR@";
                s = symboly(имя, mES|mBX);
            ...
            case CLIB.cvptrfptr:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__HCTOFPTR" : "_HCTOFPTR@";
                s = symboly(имя, mES|mBX);
                cinfo.retregs16 = mES|mBX;
                cinfo.retregs32 = mES|mBX;
                break;
            ...
            case CLIB._87topsw:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__87TOPSW" : "_87TOPSW@";
                s = symboly(имя, 0);
                cinfo.flags = INFfloat;
                break;
            }
            ...
            case CLIB.fltto87:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__FLTTO87" : "_FLTTO87@";
                s = symboly(имя, mST0);
            ...
            case CLIB.dblto87:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLTO87" : "_DBLTO87@";
                s = symboly(имя, mST0);
                cinfo.retregs16 = mST0;
                cinfo.retregs32 = mST0;
                cinfo.flags = INFfloat;
            ...
            case CLIB.dblint87:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLINT87" : "_DBLINT87@";
                s = symboly(имя, mST0|mAX);
                cinfo.retregs16 = mAX;
                cinfo.retregs32 = mAX;
                cinfo.flags = INFfloat;
                break;
            ...
            case CLIB.dbllng87:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__DBLLNG87" : "_DBLLNG87@";
                s = symboly(имя, mST0|mAX|mDX);
                cinfo.retregs16 = mDX|mAX;
                cinfo.retregs32 = mAX;
                cinfo.flags = INFfloat;
                break;
            ...
            case CLIB.ftst:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__FTST" : "_FTST@";
                s = symboly(имя, 0);
                cinfo.flags = INFfloat;
                break;
            }
            ...
            case CLIB.fcompp:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__FCOMPP" : "_FCOMPP@";
                s = symboly(имя, 0);
            ...
            case CLIB.ftest:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__FTEST" : "_FTEST@";
                s = symboly(имя, 0);
                cinfo.retregs16 = mPSW;
                cinfo.retregs32 = mPSW;
                cinfo.flags = INFfloat;
            ...
            case CLIB.ftest0:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__FTEST0" : "_FTEST0@";
                s = symboly(имя, 0);
                cinfo.retregs16 = mPSW;
                cinfo.retregs32 = mPSW;
                cinfo.flags = INFfloat;
            ...
            case CLIB.fdiv87:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__FDIVP" : "_FDIVP";
                s = symboly(имя, mST0|mAX|mBX|mCX|mDX);
                cinfo.retregs16 = mST0;
                cinfo.retregs32 = mST0;
                cinfo.flags = INFfloat;
                cinfo.push87 = 1;
            ...
            case CLIB.u64_ldbl:
            {
                ткст0 имя = (config.exe & ex_unix) ? "__U64_LDBL" : "_U64_LDBL";
                s = symboly(имя, mST0);
                cinfo.retregs16 = mST0;
                cinfo.retregs32 = mST0;
            ...
            case CLIB.ld_u64:
            {
                ткст0 имя = (config.exe & ex_unix) ? (config.objfmt == OBJ_ELF ||
 ...
 *      clib = CLIB.xxxx
 *      keepmask = mask of registers not to разрушь. Currently can
...
проц callclib(ref CodeBuilder cdb, elem* e, бцел clib, regm_t* pretregs, regm_t keepmask)
{
    ...
    if (config.target_cpu >= TARGET_80386 && clib == CLIB.lmul && !I32)
    {
        ...
        cdb.genasm(cast(сим*)lmul.ptr, lmul.sizeof);
    }
    else
    {
        makeitextern(s);
        цел nalign = 0;
        цел pushebx = (cinfo.flags & INFpushebx) != 0;
        цел pushall = (cinfo.flags & INFpusheabcdx) != 0;
        if (STACKALIGN >= 16)
        {   // Align the stack (assume no args on stack)
            цел npush = (npushed + pushebx + 4 * pushall) * REGSIZE + stackpush;
        ...
        {
            // Note: not for OSX
            /* Pass EBX on the stack instead, this is because EBX is используется
             * for shared library function calls
             */
            if (config.flags3 & CFG3pic)
            {
                load_localgot(cdb);     // EBX gets set to this значение
            }
        }
...
/*************************************************
 * Helper function for converting OPparam's into массив of Параметры.
 */
struct Параметр2 { elem* e; reg_t reg; reg_t reg2; бцел numalign; }

//проц fillParameters(elem* e, Параметр2* parameters, цел* pi);
...
/***********************************
 * tyf: тип of the function
 */
FuncParamRegs FuncParamRegs_create(tym_t tyf)
{
    FuncParamRegs результат;

    результат.tyf = tyf;

    if (I16)
    {
        результат.numintegerregs = 0;
        результат.numfloatregs = 0;
    }
    else if (I32)
    {
        if (tyf == TYjfunc)
        {
            static const ббайт[1] reglist1 = [ AX ];
            результат.argregs = &reglist1[0];
            результат.numintegerregs = reglist1.length;
        }
        else if (tyf == TYmfunc)
        {
            static const ббайт[1] reglist2 = [ CX ];
            результат.argregs = &reglist2[0];
            результат.numintegerregs = reglist2.length;
        }
        else
            результат.numintegerregs = 0;
        результат.numfloatregs = 0;
    ...
    {
        static const ббайт[4] reglist3 = [ CX,DX,R8,R9 ];
        результат.argregs = &reglist3[0];
        результат.numintegerregs = reglist3.length;

        static const ббайт[4] freglist3 = [ XMM0, XMM1, XMM2, XMM3 ];
        результат.floatregs = &freglist3[0];
        результат.numfloatregs = freglist3.length;
    }
    else if (I64)
    {
        static const ббайт[6] reglist4 = [ DI,SI,DX,CX,R8,R9 ];
        результат.argregs = &reglist4[0];
        результат.numintegerregs = reglist4.length;

        static const ббайт[8] freglist4 = [ XMM0, XMM1, XMM2, XMM3, XMM4, XMM5, XMM6, XMM7 ];
        результат.floatregs = &freglist4[0];
        результат.numfloatregs = freglist4.length;
    }
    else
        assert(0);
    return результат;
...
/*****************************************
 * Allocate параметр of тип t and ty to registers *preg1 and *preg2.
 * Параметры:
 *      t = тип, valid only if ty is TYstruct or TYarray
 * Возвращает:
 *      нет       not allocated to any register
 *      да        *preg1, *preg2 set to allocated register pair
 */

//бул type_jparam2(тип* t, tym_t ty);

private бул type_jparam2(тип* t, tym_t ty)
{
    ty = tybasic(ty);
...
цел FuncParamRegs_alloc(ref FuncParamRegs fpr, тип* t, tym_t ty, reg_t* preg1, reg_t* preg2)
{
    ...
    *preg2 = NOREG;
    ...
    if (tyb == TYstruct && type_zeroSize(t, fpr.tyf))
        return 0;               // don't размести into registers
    ...
    if (tyb == TYstruct && tybasic(t.Tty) == TYstruct)
    {
        ...
        else
        {
            ...
            тип* targ2 = t.Ttag.Sstruct.Sarg2type;
            if (targ1)
            {
                t = targ1;
                ty = t.Tty;
    ...
    reg_t* preg = preg1;
    цел regcntsave = fpr.regcnt;
    цел xmmcntsave = fpr.xmmcnt;
    ...
    for (цел j = 0; j < 2; j++)
    {
        ...
        // Failed to размести to a register
        if (j == 1)
...
проц cdfunc(ref CodeBuilder cdb, elem* e, regm_t* pretregs)
{
    //printf("cdfunc()\n"); elem_print(e);
    assert(e);
    бцел numpara = 0;               // bytes of parameters
    бцел numalign = 0;              // bytes to align stack before pushing parameters
    бцел stackpushsave = stackpush;            // so we can compute # of parameters
    cgstate.stackclean++;
    regm_t keepmsk = 0;
    цел xmmcnt = 0;
    tym_t tyf = tybasic(e.EV.E1.Ety);        // the function тип

    // Easier to deal with parameters as an массив: parameters[0..np]
    цел np = OTbinary(e.Eoper) ? el_nparams(e.EV.E2) : 0;
    Параметр2 *parameters = cast(Параметр2 *)alloca(np * Параметр2.sizeof);
    ...
    if (config.exe & (EX_LINUX | EX_LINUX64 | EX_OSX | EX_FREEBSD | EX_FREEBSD64) &&
        ...
        cgstate.accessedTLS = да;

    /* Special handling for call to __tls_get_addr, we must save registers
     * before evaluating the параметр, so that the параметр load and call
     * are adjacent.
     */
    if (np == 1 && sf)
    {
    ...
    for (цел i = np; --i >= 0;)
    {
        ...
        бцел psize = cast(бцел)_align(stackalign, paramsize(ep, tyf));     // align on stack boundary
        if (config.exe == EX_WIN64)
        {
        ...
        {
            if (config.exe == EX_WIN64)
                numpara += REGSIZE;             // размести stack space for it anyway
            continue;   // goes in register, not stack
        }

        // Параметр2 i goes on the stack
        parameters[i].reg = NOREG;
        бцел alignsize = el_alignsize(ep);
                ...
                (config.exe & (EX_OSX | EX_LINUX) && (tyaggregate(ep.Ety) || tyvector(ep.Ety))))))
        {
            ...
            бцел newnumpara = (numpara + (alignsize - 1)) & ~(alignsize - 1);
    ...
    /* Determine if we should use cgstate.funcarg for the parameters or сунь them
     */
    бул usefuncarg = нет;
    static if (0)
    {
       ...
       )
    {
        ...
        {
            elem* ep = parameters[i].e;
            цел preg = parameters[i].reg;
            //printf("параметр[%d] = %d, np = %d\n", i, preg, np);
            if (preg == NOREG)
            {
                switch (ep.Eoper)
                {
        ...
        {   // New high water mark
            //printf("increasing size from %d to %d\n", (цел)cgstate.funcarg.size, (цел)numpara);
  ...
  Lno:
    ...
    цел[XMM7 + 1] regsaved = проц;
    memset(regsaved.ptr, -1, regsaved.sizeof);
    CodeBuilder cdbrestore;
    cdbrestore.ctor();
    regm_t saved = 0;
    targ_т_мера funcargtossave = cgstate.funcargtos;
    targ_т_мера funcargtos = numpara;
    //printf("funcargtos1 = %d\n", cast(цел)funcargtos);

    /* Параметры go into the registers RDI,RSI,RDX,RCX,R8,R9
     * float and double parameters go into XMM0..XMM7
     * For variadic functions, count of XMM registers используется goes in AL
     */
    for (цел i = 0; i < np; i++)
    {
        elem* ep = parameters[i].e;
        ...
        if (preg == NOREG)
        {
            /* Push параметр on stack, but keep track of registers используется
            ...
            cdbparams.ctor();
            if (usefuncarg)
                movParams(cdbparams, ep, stackalign, cast(бцел)funcargtos, tyf);
            else
            ...
            for (reg_t j = 0; tosave; j++)
            {
                ...
                if (mi & tosave)
                {
                    ...
                    saved |= mi;
                    keepmsk &= ~mi;             // don't need to keep these for rest of парамы
                    tosave &= ~mi;
                }
            }

            cdb.приставь(cdbsave);
            cdb.приставь(cdbparams);

            // Alignment for параметр comes after it got pushed
            const бцел numalignx = parameters[i].numalign;
            if (usefuncarg)
            {
        ...
        else
        {
            ...
            if (preg2 != NOREG)
            {
                ...
                scodelem(cdb, ep, &retregs, keepmsk, нет);

                // Move результат [mreg,lreg] into параметр registers from [preg2,preg]
                retregs = 0;
                if (preg != lreg)
                    retregs |= mask(preg);
                if (preg2 != mreg)
                ...
                if (ty1 == TYstruct)
                {
                    тип* targ1 = ep.ET.Ttag.Sstruct.Sarg1type;
                    тип* targ2 = ep.ET.Ttag.Sstruct.Sarg2type;
                    if (targ1)
                        ty1 = targ1.Tty;
                    if (targ2)
                        ty2 = targ2.Tty;
            ...
            else if (ep.Eoper == OPstrthis)
            {
                getregs(cdb,retregs);
                // LEA preg,np[RSP]
                бцел delta = stackpush - ep.EV.Vuns;   // stack delta to параметр
                cdb.genc1(LEA,
    ...
    getregs(cdb,saved);
    cdb.приставь(cdbrestore);
    keepmsk |= saved;

    // Variadic functions store the number of XMM registers используется in AL
...
проц cdstrthis(ref CodeBuilder cdb, elem* e, regm_t* pretregs)
{
    ...
    getregs(cdb,mask(reg));
    // LEA reg,np[ESP]
    бцел np = stackpush - e.EV.Vuns;        // stack delta to параметр
    cdb.genc1(LEA,(modregrm(0,4,SP) << 8) | modregxrm(2,reg,4),FLconst,np);
    if (I64)
        code_orrex(cdb.last(), REX_W);
    fixрезультат(cdb, e, mask(reg), pretregs);
}
 ...
 *      pretregs   = where return значение goes
...
private проц funccall(ref CodeBuilder cdb, elem* e, бцел numpara, бцел numalign,
                      regm_t* pretregs,regm_t keepmsk, бул usefuncarg)
{
    ...
    сим farfunc = tyfarfunc(tym1) || tym1 == TYifunc;
    ...
    {   // Call function directly
        ...
        if (sytab[s.Sclass] & SCSS)    // if function is on stack (!)
        {
            ...
            if (farfunc)
            {
                ...
                floatreg = да;                // use float register
                reflocal = да;
    ...
    {   // Call function via pointer
            ...
            (e11.Eoper != OPind || e11.Ecount))
        {
            retregs = allregs & ~keepmsk;
            cgstate.stackclean++;
            scodelem(cdbe,e11,&retregs,keepmsk,да);
            cgstate.stackclean--;
...
static if (0)
{
    if (!needframe)
    {
        // If there is a register доступно for this basic block
        if (config.flags4 & CFG4optimized && (ALLREGS & ~regcon.используется))
        { }
        else
        {
    ...
    if (!usefuncarg)
    {
        ...
        {
            if (tym1 == TYhfunc)
            {   // Hidden параметр is popped off by the callee
    ...
    /* Special handling for functions which return a floating point
       значение in the top of the 8087 stack.
     */

    if (retregs & mST0)
    {
        cdb.genadjfpu(1);
        if (*pretregs)                  // if we want the результат
        {
            ...
            // Pop unused результат off 8087 stack
...
private проц movParams(ref CodeBuilder cdb, elem* e, бцел stackalign, бцел funcargtos, tym_t tyf)
{
    ...
    цел grex = I64 ? REX_W << 16 : 0;

    targ_т_мера szb = paramsize(e, tyf);          // size before alignment
    targ_т_мера sz = _align(stackalign, szb);       // size after alignment
    ...
    switch (e.Eoper)
    {
        ...
        case OPconst:
            if (!evalinregister(e))
            {
                ...
                cs.IFL2 = FLconst;
                targ_т_мера *p = cast(targ_т_мера *) &(e.EV);
                cs.IEV2.Vт_мера = *p;
                ...
                if (I64 && sz >= 8)
                {
                    ...
                    do
                    {
                        ...
                        p = cast(targ_т_мера *)(cast(сим *) p + REGSIZE);
                        i -= REGSIZE;
                    } while (i > 0);
                    p = cast(targ_т_мера *) &(e.EV);
                }
                ...
                {   цел regsize = REGSIZE;
                    ...
                    p = cast(targ_т_мера *)(cast(сим *) p + regsize);
                    cs.Iop = 0xC7;
                    cs.Irm &= cast(ббайт)~cast(цел)modregrm(0, 7, 0);
                    cs.Irex &= ~REX_R;
...
/***************************
 * Generate code to сунь argument e on the stack.
 * stackpush is incremented by stackalign for each PUSH.
 */

проц pushParams(ref CodeBuilder cdb, elem* e, бцел stackalign, tym_t tyf)
{
    //printf("парамы(e = %p, stackalign = %d)\n", e, stackalign);
    //printf("парамы()\n"); elem_print(e);
    stackchanged = 1;
    assert(e && e.Eoper != OPparam);
    ...
    version (SCPP)
    {
        ...
        {
            elem* e1 = e.EV.E1;
            docommas(cdb,&e1);              // skip over any comma Выражения
            ...
            regm_t retregs = 0;
            codelem(cdb, e1, &retregs, да);
            freenode(e);
            return;
        }
        case OPstrthis:
            // This is the параметр for the 'this' pointer corresponding to
            // OPstrctor. We сунь a pointer to an объект that was already
        ...
        case OPstrpar:
        {
            ...
            бул doneoff = нет;
            бцел pushsize = REGSIZE;
            бцел op16 = 0;
            if (!I16 && sz & 2)     // if odd number of words to сунь
            {
                pushsize = 2;
                op16 = 1;
            }
            ...
            {
                pushsize = 4;       // сунь DWORDs at a time
                op16 = 1;
            }
            бцел npushes = cast(бцел)(sz / pushsize);
            switch (e1.Eoper)
            {
                case OPind:
                    if (sz)
                    ...
                    codelem(cdb, e1.EV.E1, &retregs, нет);
                    freenode(e1);
                    break;
                ...
                case OPvar:
                    ...
                    {
                        static const бцел[4] segtocf = [ CFes,CFcs,CFss,0 ];
        ...
        case OPnp_fp:
            if (!e.Ecount)                         /* if (far *)e1 */
            {
                ...
                if (I32 && stackalign == 2)
                    cdb.gen1(0x66);                 // сунь a word
                cdb.gen1(0x06 + segreg);            // PUSH SEGREG
                if (I32 && stackalign == 2)
                    code_orflag(cdb.last(), CFopsize);        // сунь a word
        ...
        case OPrelconst:
            ...
            {
                /* Determine if we can just сунь the segment register           */
                /* Test size of тип rather than TYfptr because of (long)(&v)   */
                Symbol* s = e.EV.Vsym;
                //if (sytab[s.Sclass] & SCSS && !I32)  // if variable is on stack
                //    needframe = да;                 // then we need stack frame
                цел fl;
            ...
            break;                          /* else must evaluate Выражение */
        ...
        case OPconst:
        {
            ...
            if (I64 && tyfloating(tym) && sz > 4 && boolres(e))
                // Can't сунь 64 bit non-нуль args directly
                break;

            if (I32 && szb == 10)           // special case for long double constants
            {
                assert(sz == 12);
                targ_int значение = e.EV.Vushort8[4]; // pick upper 2 bytes of Vldouble
                stackpush += sz;
                cdb.genadjesp(cast(цел)sz);
                for (цел i = 0; i < 3; ++i)
                {
                    reg_t reg;
                    if (reghasvalue(allregs, значение, &reg))
                        cdb.gen1(0x50 + reg);           // PUSH reg
                    else
                        cdb.genc2(0x68,0,значение);        // PUSH значение
                    значение = e.EV.Vulong4[i ^ 1];       // treat Vldouble as 2 element массив of 32 bit бцел
                }
                freenode(e);
                return;
            }
    ...
    //       && (e.Ecount == 0 || e.Ecount != e.Ecomsub))
            ...
            do
            {
                ...
                switch (regsize)
                {
                    ...
                    case 4:
                        if (tym == TYldouble || tym == TYildouble)
                             ...
                             * just читай those 2 bytes, not 4.
                             * Otherwise we're reading uninitialized данные.
                             * I.e. читай 4 bytes, 4 bytes, then 2 bytes
                             */
                            значение = i == 2 ? ps[4] : pi[i]; // 80 bits
                        else
                            значение = pi[i];
                        break;

                    case 8:
                        значение = cast(targ_т_мера)pl[i];
                        break;
                ...
                if (pushi)
                {
                    if (I64 && regsize == 8 && значение != cast(цел)значение)
                    {
                        regwithvalue(cdb,allregs,значение,&reg,64);
                        goto Preg;          // cannot сунь imm64 unless it is sign extended 32 bit значение
                    }
                    if (regsize == REGSIZE && reghasvalue(allregs,значение,&reg))
                        goto Preg;
                    cdb.genc2((szb == 1) ? 0x6A : 0x68, 0, значение); // PUSH значение
                }
                else
                {
                    regwithvalue(cdb, allregs, значение, &reg, 0);
                Preg:
                    genpush(cdb,reg);         // PUSH reg
                }
                code_orflag(cdb.last(), флаг);              // operand size
            } while (i);
            freenode(e);
            return;
        }
    ...
    if (tyvector(tym) || (tyxmmreg(tym) && config.fpxmmregs))
    {
        ...
        codelem(cdb, e, &retxmm, нет);
        stackpush += sz;
        cdb.genadjesp(cast(цел)sz);
        cod3_stackadj(cdb, cast(цел)sz);
        const op = xmmstore(tym);
    ...
    if (sz <= REGSIZE)
    {
        ...
        cdb.genadjesp(cast(цел)REGSIZE);
    }
    else if (sz == REGSIZE * 2)
    {
...
/*******************************
 * Get смещение portion of e, and store it in an index
...
/******************************
 * Generate code to load данные into registers.
 */
...
проц loaddata(ref CodeBuilder cdb, elem* e, regm_t* pretregs)
{
    ...
    if (*pretregs == mPSW)
    {
        ...
        regm = allregs;
        if (e.Eoper == OPconst)
        {       /* да:        OR SP,SP        (SP is never 0)         */
                /* нет:       CMP SP,SP       (always equal)          */
        ...
        {
            tstрезультат(cdb,s.Sregm,e.Ety,да);
        }
        else if (sz <= REGSIZE)
        {
            if (!I16 && (tym == TYfloat || tym == TYifloat))
            {
                ...
                loadea(cdb, e, &cs, 0x8B, reg, 0, 0, 0);    // MOV reg,данные
                cdb.gen2(0xD1,modregrmx(3,4,reg));           // SHL reg,1
            }
            else if (I64 && (tym == TYdouble || tym ==TYidouble))
            {
                allocreg(cdb, &regm, &reg, TYoffset);   // get a register
                loadea(cdb, e,&cs, 0x8B, reg, 0, 0, 0);    // MOV reg,данные
                // удали sign bit, so that -0.0 == 0.0
                cdb.gen2(0xD1, modregrmx(3, 4, reg));           // SHL reg,1
                code_orrex(cdb.last(), REX_W);
            }
        ...
        else if (sz < 8)
        {
            ...
            if (I32)                                    // it's a 48 bit pointer
                loadea(cdb, e, &cs, 0x0FB7, reg, REGSIZE, 0, 0); // MOVZX reg,данные+4
            else
            {
                loadea(cdb, e, &cs, 0x8B, reg, REGSIZE, 0, 0); // MOV reg,данные+2
                if (tym == TYfloat || tym == TYifloat)       // dump sign bit
                    cdb.gen2(0xD1, modregrm(3, 4, reg));        // SHL reg,1
            }
            loadea(cdb,e,&cs,0x0B,reg,0,regm,0);     // OR reg,данные
        }
        else if (sz == 8 || (I64 && sz == 2 * REGSIZE && !tyfloating(tym)))
        {
            allocreg(cdb, &regm, &reg, TYoffset);       // get a register
            цел i = sz - REGSIZE;
            loadea(cdb, e, &cs, 0x8B, reg, i, 0, 0);        // MOV reg,данные+6
            if (tyfloating(tym))                             // TYdouble or TYdouble_alias
                cdb.gen2(0xD1, modregrm(3, 4, reg));            // SHL reg,1

            while ((i -= REGSIZE) >= 0)
            {
                loadea(cdb, e, &cs, 0x0B, reg, i, regm, 0); // OR reg,данные+i
                code *c = cdb.last();
                if (i == 0)
    ...
    if (e.Eoper == OPconst)
    {
        targ_т_мера значение = e.EV.Vint;
        if (sz == 8)
            значение = cast(targ_т_мера)e.EV.Vullong;

        if (sz == REGSIZE && reghasvalue(forregs, значение, &reg))
            forregs = mask(reg);

        regm_t save = regcon.immed.mval;
        allocreg(cdb, &forregs, &reg, tym);        // размести registers
        ...
        if (sz <= REGSIZE)
        {
            ...
            {   /* This comes about because 0, 1, pi, etc., constants don't get stored
                 * in the данные segment, because they are x87 opcodes.
                 * Not so efficient. We should at least do a PXOR for 0.
                 */
                reg_t r;
                targ_т_мера unsvalue = e.EV.Vuns;
                if (sz == 8)
                    unsvalue = cast(targ_т_мера)e.EV.Vullong;
            ...
            {
                movregconst(cdb, reg, значение, flags);
                flags = 0;                          // flags are already set
            }
        }
        ...
        else if (sz == 8)
        {
            if (I32)
            {
                targ_long *p = cast(targ_long *)cast(ук)&e.EV.Vdouble;
                if (isXMMreg(reg))
                {   /* This comes about because 0, 1, pi, etc., constants don't get stored
                     * in the данные segment, because they are x87 opcodes.
                     * Not so efficient. We should at least do a PXOR for 0.
                    ...
                    reg_t r;
                    regm_t rm = ALLREGS;
                    allocreg(cdb, &rm, &r, TYint);    // размести scratch register
        ...
        {
            movregconst(cdb, findreglsw(forregs), cast(targ_т_мера)e.EV.Vcent.lsw, 64);
            movregconst(cdb, findregmsw(forregs), cast(targ_т_мера)e.EV.Vcent.msw, 64);
        }
        else
            assert(0);
        // Flags may already be set
        *pretregs &= flags | ~mPSW;
        fixрезультат(cdb, e, forregs, pretregs);
        return;
    }
    else
    {
        // See if we can use register that параметр was passed in
        if (regcon.парамы &&
            regParamInPreg(e.EV.Vsym) &&
            !anyiasm &&   // may have written to the memory for the параметр
            (regcon.парамы & mask(e.EV.Vsym.Spreg) && e.EV.Voffset == 0 ||
             regcon.парамы & mask(e.EV.Vsym.Spreg2) && e.EV.Voffset == REGSIZE) &&
            sz <= REGSIZE)                  // make sure no 'paint' to a larger size happened
        {
            ...
            if (debugr)
                printf("%s.%d is fastpar and using register %s\n",
                       ...
                       cast(цел)e.EV.Voffset,
                       regm_str(forregs));

            mfuncreg &= ~forregs;
            regcon.используется |= forregs;
            fixрезультат(cdb,e,forregs,pretregs);
            return;
        }
...
//                    opmv = tyuns(tym) ? 0x0FB6 : 0x0FBE;      // MOVZX/MOVSX
                ...
                loadea(cdb, e, &cs, opmv, reg, 0, 0, 0);     // MOV regL,данные
            }
            else
            {
                ...
                loadea(cdb, e, &cs, opmv, nreg, 0, 0, 0);    // MOV nregL,данные
                if (reg != nreg)
                {
                    genmovreg(cdb, reg, nreg);   // MOV reg,nreg
                    cssave(e, mask(nreg), нет);
                }
            }
        }
        else if (forregs & XMMREGS)
        {
            ...
            loadea(cdb, e, &cs, opmv, reg, 0, RMload, 0); // MOVSS/MOVSD reg,данные
            checkSetVex(cdb.last(),tym);
        }
        else if (sz <= REGSIZE)
        {
            opcode_t opmv = 0x8B;                     // MOV reg,данные
...
//                opmv = tyuns(tym) ? 0x0FB7 : 0x0FBF;  // MOVZX/MOVSX
        ...
        {
            loadea(cdb, e, &cs, 0xC4, reg, 0, 0, mES);    // LES данные
        }
    ...
    +/
            ...
            reg = findregmsw(forregs);
            loadea(cdb, e, &cs, 0x8B, reg, REGSIZE, forregs, 0); // MOV reg,данные+2
            if (I32 && sz == REGSIZE + 2)
                cdb.last().Iflags |= CFopsize;                   // seg is 16 bits
            reg = findreglsw(forregs);
            loadea(cdb, e, &cs, 0x8B, reg, 0, forregs, 0);       // MOV reg,данные
        }
        else if (sz >= 8)
        {
            assert(!I32);
            ...
            {
                assert(reg == AX);
                loadea(cdb, e, &cs, 0x8B, AX, 6, 0,           0); // MOV AX,данные+6
                loadea(cdb, e, &cs, 0x8B, BX, 4, mAX,         0); // MOV BX,данные+4
                loadea(cdb, e, &cs, 0x8B, CX, 2, mAX|mBX,     0); // MOV CX,данные+2
                loadea(cdb, e, &cs, 0x8B, DX, 0, mAX|mCX|mCX, 0); // MOV DX,данные
            }
        }
        else
            assert(0);
