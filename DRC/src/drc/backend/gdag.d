 ...
 * Возвращает:
 *      да if should be cse'd
 */

private бул cse_float(elem *e)
{
...
/************************************
 * Build DAGs (basically найди all the common subВыражения).
 ...
 *      For each block
 *              unstick unprofitable common subВыражения
...
проц builddags()
{
    ...
    else
    {
        ...
        foreach (i, b; dfo[])
        {
            ...
            /* previous block), then нуль out the доступно         */
            /* Выражения.                                         */
            if ((i != 0 &&
                ...
                vec_clear(aevec);
            if (b.Belem)           /* if there is an Выражение    */
 ...
 * Параметры:
 *      pn = pointer to Выражение tree to convert to DAG
 *      ae = vector of доступно Выражения
 */

private проц aewalk(elem **pn,vec_t ae)
{
    elem* n = *pn;
    assert(n && ae);
    ...
    const op = n.Eoper;
    if (n.Eexp)                            // if an AE
    {   // Try to найди an equivalent AE, and point to it instead
        assert(go.expnod[n.Eexp] == n);
        if (aetype == Aetype.cse)
        {
            for (бцел i = 0; (i = cast(бцел) vec_index(i, ae)) < go.exptop; ++i)
            {   elem *e = go.expnod[i];

                // Attempt to replace n with e
                if (e == null)              // if elem no longer exists
                    vec_clearbit(i,ae);     // it's not доступно
                else if (n != e &&
                    el_match(n,e) &&
                    e.Ecount < 0xFF-1 &&   // must fit in unsigned сим
                    cse_float(n)
                    )
                {
    ...
    if (OTdef(op))
    {
        ...
        /* удали all AEs that could be affected by this def    */
        if (Eunambig(n))        // if unambiguous definition
        {
            ...
            for (бцел i = 0; (i = cast(бцел) vec_index(i, ae)) < go.exptop; ++i) // for each ae elem
            {
                elem *e = go.expnod[i];
    ...
    if (n.Eexp)            // if an AE
    {
        ...
        /*printf("доступно: ("); WReqn(n); printf(")\n");
        elem_print(n);*/
        vec_setbit(n.Eexp,ae);     /* mark this elem as доступно  */
    }
}
...
private elem * delcse(elem **pe)
{
    ...
    (*pe).Nflags |= NFLdelcse;     // not generating узел
    e.Ecount = 0;
    *pe = e;
    return *pe;
}
...
private проц removecses(elem **pe)
{
L1:
    elem* e = *pe;
    ...
    if (OTunary(op))
    {
        if (op == OPind)
        {
            бул scaledIndex = I32 || I64;      // if scaled index addressing mode support
            elem *e1 = e.EV.E1;
            if (e1.Eoper == OPadd &&
                e1.Ecount
               )
        ...
        // OPremquo is only worthwhile if its результат is используется more than once
...
/*****************************************
 * Do optimizations based on if we know an Выражение is
...
проц булopt()
{
    ...
    // Mark each Выражение that we know starts off with a non-нуль значение
    for (бцел i = 0; i < go.exptop; i++)
    {
        elem *e = go.expnod[i];
        if (e)
...
/****************************
 * Walk tree, replacing бул Выражения that we know
 *      ae = vector of доступно булean Выражения
 *      aeval = parallel vector of values corresponding to whether бул
 *               значение is 1 or 0
 *      n = elem tree to look at
 */

private проц abewalk(elem *n,vec_t ae,vec_t aeval)
{
    elem *t;
...
/************************************
 * Elem e is to be evaluated for a булean результат.
 * See if we already know its значение.
 */

private проц abeboolres(elem *n,vec_t ae,vec_t aeval)
{
    ...
    {   /* Try to найди an equivalent AE, and point to it instead */
        assert(go.expnod[n.Eexp] == n);
        бцел i;
        for (i = 0; (i = cast(бцел) vec_index(i, ae)) < go.exptop; ++i) // for each ae elem
        {   elem *e = go.expnod[i];

            // Attempt to replace n with the булean результат of e
...
/****************************
 * Remove e from доступно Выражения, and its children.
 */

private проц abefree(elem *e,vec_t ae)
{
...
/************************************
 * Elem e is to be evaluated for a булean результат.
 * Set its результат according to флаг.
 */

private проц abeset(elem *e,vec_t ae,vec_t aeval,цел флаг)
{
    while (1)
    {
        бцел i = e.Eexp;
