...
проц clearFE();
цел statusFE();
бул have_float_except();


/**********************
 * Return булean результат of constant elem.
 */
        ...
        case OPconst:
            ...
            {   case TYchar:
                ...
                case TYfgPtr:
                    b = el_tolong(e) != 0;
                    break;
                case TYnref: // reference can't be converted to бул
                    assert(0);
                ...
                default:
                    break;  // can be the результат of other errors
            }
            break;
        default:
            assert(0);
...
/***************************
 * Return да if Выражение will always evaluate to да.
 */
...
/***************************
 * Return да if Выражение will always evaluate to нет.
 */
...
/******************************
 * Evaluate a узел with only constants as leaves.
 * Return with the результат.
 */
...
elem * evalu8(elem *e, goal_t goal)
{
    ...
    static бул unordered(T)(T d1, T d2) { return isnan(d1) || isnan(d2); }
    ...
    if (e1.Eoper == OPconst && !tyvector(e1.Ety))
    {
        ...
        else
        {
            tym2 = 0;
            e2 = null;
            i2 = 0;             // not используется, but static analyzer complains
            l2 = 0;             // "
            d2 = 0;             // "
        }
        i1 = cast(targ_int)(l1 = el_tolong(e1));
        d1 = el_toldoubled(e1);
        tym = tybasic(typemask(e1));    /* тип of op is тип of left child */
...
version (DigitalMars)
    ...
    case OPle:
    {
        ...
        if (uns)
        {
            b = cast(цел)((cast(targ_ullong) l1) <= (cast(targ_ullong) l2));
        }
        else
        {
            if (tyfloating(tym))
                b = cast(цел)(!unordered(d1, d2) && d1 <= d2);
            else
                b = cast(цел)(l1 <= l2);
        }
        e.EV.Vint = (op != OPle) ^ b;
            ...
            switch (tybasic(tym))
            {
                case TYcfloat:
                    if (isnan(e1.EV.Vcfloat.re) || isnan(e1.EV.Vcfloat.im) ||
                        ...
                        b = cast(цел)((e1.EV.Vcfloat.re == e2.EV.Vcfloat.re) &&
                ...
                case TYcdouble:
                    if (isnan(e1.EV.Vcdouble.re) || isnan(e1.EV.Vcdouble.im) ||
                        ...
                        b = cast(цел)((e1.EV.Vcdouble.re == e2.EV.Vcdouble.re) &&
                ...
                case TYcldouble:
                    if (isnan(e1.EV.Vcldouble.re) || isnan(e1.EV.Vcldouble.im) ||
                        ...
                        b = cast(цел)((e1.EV.Vcldouble.re == e2.EV.Vcldouble.re) &&
    ...
    case OPd_s32:
        ...
        break;
    case OPu32_d:
        e.EV.Vdouble = cast(бцел) l1;
        break;
    case OPs32_d:
        e.EV.Vdouble = cast(цел) l1;
        break;
    case OPd_s16:
    ...
    case OPb_8:
        ...
        break;
    case OPbswap:
        e.EV.Vint = core.bitop.bswap(cast(бцел) i1);
        break;
    ...
    case OPpopcnt:
    {
        ...
        e.EV.Vllong = core.bitop.popcnt(cast(бдол) l1);
        break;
    }
    ...
    {   бцел n = i2;
        ...
        switch (tysize(tym))
        {
            case 1:
                n &= 7;
                e.EV.Vuchar = cast(ббайт)((i1 << n) | ((i1 & 0xFF) >> (8 - n)));
                break;
            case 2:
                n &= 0xF;
...
static if (0) // && Dinrus
{
        ...
        if (l1 >= 0 && l1 < 4096)
        {
            выведиОшибку(e.Esrcpos.Sfilename, e.Esrcpos.Slinnum, e.Esrcpos.Scharnum,
...
version (CRuntime_Microsoft)
{
    ...
    static if (!is(targ_ldouble == real))
        extern (D) private цел isnan(targ_ldouble x)
        {
            return isnan(cast(real)x);
        }
    import core.stdc.math : fabsl;
    import util.longdouble : fabsl; // needed if longdouble is longdouble_soft
}
else
{
