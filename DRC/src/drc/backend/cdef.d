...
module drc.backend.cdef;

// Online documentation: https://dlang.org/phobos/dmd_backend_cdef.html

import drc.backend.cc: Classsym, Symbol, param_t, config;
import drc.backend.el;
import drc.backend.ty : I32;
...
const VERSION = "9.00.0";        // for banner and imbedding in .OBJ файл
const VERSIONHEX = "0x900";      // for __DMC__ macro
const VERSIONINT = 0x900;        // for precompiled headers and DLL version
...
version (XVERSION)
{
    extern (D) template xversion(ткст s)
    {
        const xversion = mixin(`{ version (` ~ s ~ `) return да; else return нет; }`)();
    }

    const TARGET_LINUX   = xversion!(`linux`);
    const TARGET_OSX     = xversion!(`OSX`);
    const TARGET_FREEBSD = xversion!(`FreeBSD`);
    const TARGET_OPENBSD = xversion!(`OpenBSD`);
    const TARGET_SOLARIS = xversion!(`Solaris`);
    const TARGET_WINDOS  = xversion!(`Windows`);
    const TARGET_DRAGONFLYBSD  = xversion!(`DragonFlyBSD`);
}
...
enum
{
    ...
    ATTR_TYPEMOD    = 2,     // basic тип modifier
    ATTR_FUNCINFO   = 4,     // function information
    ATTR_DATAINFO   = 8,     // данные information
...
enum
{
    ...
    ALOC_PARAM      = 4,   // follows function параметр
...
// C++ Language Features
const ANGLE_BRACKET_HACK = 0;       // >> means two template arglist closes

// C/C++ Language Features
const IMPLIED_PRAGMA_ONCE = 1;       // include guards count as #pragma once
const бул HEADER_LIST = да;

// Support generating code for 16 bit memory models
version (SCPP)
    const SIXTEENBIT = TARGET_WINDOS != 0;
else
    const SIXTEENBIT = нет;
 ...
 */
version (Dinrus)
    const TARGET_SEGMENTED = нет;
else
    const TARGET_SEGMENTED = TARGET_WINDOS;


бул LDOUBLE() { return config.exe == EX_WIN32; }   // support да long doubles
...
const EXIT_BREAK = 255;     // aborted compile with ^C
...
//#else
//#define TOWORD(ptr,val) (((ptr)[0] = (unsigned сим)(val)),\
//                         ((ptr)[1] = (unsigned сим)((val) >> 8)))
//#define TOLONG(ptr,val) (((ptr)[0] = (unsigned сим)(val)),\
//                         ((ptr)[1] = (unsigned сим)((val) >> 8)),\
//                         ((ptr)[2] = (unsigned сим)((val) >> 16)),\
//                         ((ptr)[3] = (unsigned сим)((val) >> 24)))
//#endif
//
//#define TOOFFSET(a,b)   (I32 ? TOLONG(a,b) : TOWORD(a,b))

/***************************
 * Target machine данные types as they appear on the host.
 */

//import core.stdc.stdint : int64_t, uint64_t;

alias  byte targ_char;
alias  ббайт targ_uchar;
alias  byte targ_schar;
alias  short targ_short;
alias ushort targ_ushort;
alias  цел targ_long;
alias  бцел targ_ulong;
alias  int64_t targ_llong;
alias  uint64_t targ_ullong;
alias  float targ_float;
alias  double targ_double;
public import util.longdouble : targ_ldouble = longdouble;

// Extract most significant register from constant
цел REGSIZE();
бдол MSREG(бдол p) { return (REGSIZE == 2) ? p >> 16 : ((targ_llong.sizeof == 8) ? p >> 32 : 0); }

alias  цел targ_int;
alias  бцел targ_uns;

/* Sizes of base данные types in bytes */
enum
{
    CHARSIZE       = 1,
    SHORTSIZE      = 2,
...
//   auto NPTRSIZE() { return _tysize[TYnptr]; }
//#define FPTRSIZE        _tysize[TYfptr]
const REGMASK = 0xFFFF;

// targ_llong is also используется to store host pointers, so it should have at least their size
version (SCPP)
{
    // No 64 bit support yet
    alias targ_int targ_ptrdiff_t;  // ptrdiff_t for target machine
    alias targ_uns targ_т_мера;     // т_мера for the target machine
}
else version (SPP)
{
    alias targ_int targ_ptrdiff_t;  // ptrdiff_t for target machine
...
else
{
    ...
    alias int64_t targ_ptrdiff_t;  // ptrdiff_t for target machine
    alias uint64_t targ_т_мера;    // т_мера for the target machine
}
...
бул MFUNC() { return I32 != 0; } // && config.exe == EX_WIN32)       // member functions are TYmfunc
const CV3 = 0;          // 1 means support CV3 debug format
...
//#endif
const ELFOBJ = TARGET_LINUX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS;
const MACHOBJ = TARGET_OSX;

version (XVERSION)
{
    const SYMDEB_CODEVIEW = TARGET_WINDOS;
    const SYMDEB_DWARF = TARGET_LINUX || TARGET_FREEBSD || TARGET_OPENBSD || TARGET_DRAGONFLYBSD || TARGET_SOLARIS || TARGET_OSX;
}
...
enum
{
    Smodel = 0,        // 64k code, 64k данные, or flat model
    Mmodel = 1,        // large code, 64k данные
    Cmodel = 2,        // 64k code, large данные
    Lmodel = 3,        // large code, large данные
    Vmodel = 4,        // large code, large данные, vcm
}

version (Dinrus)
    const MEMMODELS = 1; // number of memory models
else
    const MEMMODELS = 5;
...
{
    CODE      = 1,     // code segment
    DATA      = 2,     // initialized данные
    CDATA     = 3,     // constant данные
    UDATA     = 4,     // uninitialized данные
    CDATAREL  = 5,     // constant данные with relocs
    UNKNOWN   = -1,    // unknown segment
    DGROUPIDX = 1,     // group index of DGROUP
}

const REGMAX = 29;      // registers are numbered 0..10

alias бцел tym_t;    // данные тип big enough for тип masks
alias цел SYMIDX;    // symbol table index
 ...
 */
    const COPYRIGHT_SYMBOL = "\xA9";
    const COPYRIGHT = "Copyright " ~ COPYRIGHT_SYMBOL ~ " 2001 Digital Mars";
}
else
{
    debug
    {
        const COPYRIGHT = "Copyright (C) Digital Mars 2000-2019.  All Rights Reserved.
...
enum
{
    ...
    CVTDB     = 4,           // Symantec format written to файл
...
enum
{
    ...
    WFsaveds     = 0x2000, // use сунь/вынь DS for far functions
...
alias бцел config_flags2_t;
enum
{
    CFG2comdat      = 1,       // use initialized common blocks
    CFG2nodeflib    = 2,       // no default library imbedded in OBJ файл
...
enum
{
    ...
    CFG3relax       = 0x100,   // relaxed тип checking (C only)
...
enum
{
    ...
    CFG4pascal           = 0x80,       // default to pascal компонаж
    ...
    CFG4implicitfromvoid = 0x800000,   // allow implicit cast from ук to T*
...
struct Config
{
    сим language;              // 'C' = C, 'D' = C++
    ткст _version;            /// Compiler version
    сим[3] exetype;            // distinguish exe types so PH
    ...
    short versionint;           // intermediate файл version (= VERSIONINT)
    цел defstructalign;         // struct alignment specified by command line
    ...
    ббайт ansi_c;               // strict ANSI C
                                // 89 for ANSI C89, 99 for ANSI C99
    ббайт asian_char;           // 0: normal, 1: Japanese, 2: Chinese
                                // and Taiwanese, 3: Korean
    бцел threshold;             // данные larger than threshold is assumed to
                                // be far (16 bit models only)
                                // if threshold == THRESHMAX, all данные defaults
                                // to near
    linkage_t компонаж;          // default function call компонаж
...
struct Configv
{
    ббайт addlinenumbers;       // put line number info in .OBJ файл
    ббайт verbose;              // 0: compile quietly (no messages)
                                // 1: show progress to DLL (default)
                                // 2: full verbosity
    ткст0 csegname;             // code segment имя
    ткст0 deflibname;           // default library имя
    LANG language;              // message language
    цел errmax;                 // max error count
}

alias  ббайт reg_t;            // register number
alias  бцел regm_t;            // Register mask тип
struct immed_t
{
    targ_т_мера[REGMAX] значение;  // immediate values in registers
    regm_t mval;                // Mask of which values in regimmed.значение[] are valid
}


struct cse_t
{
    elem*[REGMAX] значение;        // Выражение values in registers
    regm_t mval;                // mask of which values in значение[] are valid
...
struct con_t
{
    ...
    regm_t используется;                // mask of registers используется
    regm_t парамы;              // mask of registers which still contain register
                                // function parameters
}
...
import drc.backend.bcomplex;

/*********************************
 * Union of all данные types. Storage allocated must be the right
 * size of the данные on the TARGET, not the host.
 */
...
union eve
{
        ...
        struct
        {
            ...
            union
            {
                param_t* Vtal;  // template-argument-list for SCfunctempl,
                                // используется only to transmit it to cpp_overload()
                LIST* Erd;      // OPvar: reaching definitions
            }
        }
        struct
        {
            targ_т_мера Voffset2;// member pointer смещение
            Classsym* Vsym2;    // struct tag
            elem* ethis;        // OPrelconst: 'this' for member pointer
        }
        struct
        {
            targ_т_мера Voffset3;// смещение from ткст
            ткст0 Vstring;      // pointer to ткст (OPstring or OPasm)
            targ_т_мера Vstrlen;// length of ткст
        }
        struct
        {
        ...
        struct
        {
            ...
            ук Edecl;        // VarDeclaration being constructed
        }                       // OPdctor,OPddtor
}                               // variants for each тип of elem
...
enum
{
    ...
    SCglobal,         // top уровень глоб2 definition
    SCcomdat,         // initialized common block
    SCparameter,      // function параметр
    SCregpar,         // function register параметр
    SCfastpar,        // function параметр passed in register
    SCshadowreg,      // function параметр passed in register, shadowed on stack
    SCtypedef,        // тип definition
    SCexplicit,       // explicit
    SCmutable,        // mutable
    SClabel,          // goto label
    SCstruct,         // struct/class/union tag имя
    SCenum,           // enum tag имя
    SCfield,          // bit field of struct or union
    SCconst,          // constant integer
    SCmember,         // member of struct or union
    SCanon,           // member of анонимный union
    SCinline,         // for inline functions
    SCsinline,        // for static inline functions
    SCeinline,        // for extern inline functions
    SCoverload,       // for overloaded function имена
    ...
    SClinkage,        // function компонаж symbol
    SCpublic,         // generate a pubdef for this
    SCcomdef,         // uninitialized common block
    SCbprel,          // variable at fixed смещение from frame pointer
    ...
    SCstack,          // смещение from stack pointer (not frame pointer)
    SCadl,            // list of ADL symbols for overloading
    SCMAX
}

цел ClassInline(цел c) { return c == SCinline || c == SCsinline || c == SCeinline; }
цел SymInline(Symbol* s) { return ClassInline(s.Sclass); }
