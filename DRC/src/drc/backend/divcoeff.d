...
проц SHL128(out ullong dh, out ullong dl, ullong xh,ullong xl)
{
    dh = (xh << 1) | SIGN64(xl);
    dl = xl << 1;
}

проц SHR128(out ullong dh, out ullong dl, ullong xh,ullong xl)
{
    dl = (xl >> 1) | ((xh & 1) << 63);
    dh = xh >> 1;
}

бул XltY128(ullong xh, ullong xl, ullong yh, ullong yl)
{
    return xh < yh || (xh == yh && xl < yl);
}

проц u128Div(ullong xh, ullong xl, ullong yh, ullong yl, ullong *pqh, ullong *pql)
{
...
extern (C) бул choose_multiplier(цел N, ullong d, цел prec, ullong *pm, цел *pshpost)
{
    assert(N == 32 || N == 64);
    assert(prec <= N);
    assert(d > 1 && (d & (d - 1)));
...
extern (C) бул udiv_coefficients(цел N, ullong d, цел *pshpre, ullong *pm, цел *pshpost)
{
    бул mhighbit = choose_multiplier(N, d, N, pm, pshpost);
    if (mhighbit && (d & 1) == 0)
    {
        цел e = 0;
        while ((d & 1) == 0)
...
unittest
{
    ...
    {   const ps = &table[i];
        ...
        цел shpre;
        цел shpost;
        бул mhighbit = udiv_coefficients(ps.N, ps.d, &shpre, &m, &shpost);
...
version (none)
{
    ...
    extern (D) цел main(ткст[] args)
    {
        if (args.length == 2)
        {
            ...
            цел shpre;
            цел shpost;
            бул mhighbit = udiv_coefficients(64, d, &shpre, &m, &shpost);
