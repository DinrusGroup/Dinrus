...
бул findloopparameters(elem* erel, ref elem* rdeq, ref elem* rdinc);

/*********************************
 * Loop данные structure.
 */
...
struct famlist
{
    ...
    elem **FLpelem;         /* родитель of elem in the family         */
    elem *c1;
    elem *c2;               // c1*(basic IV) + c2
    Symbol *FLtemp;         // symbol index of temporary (FLELIM if */
                            /* this entry has no temporary)         */
    tym_t FLty;             /* тип of this induction variable      */
    tym_t FLivty;           /* тип of the basic IV elem (which is  */
                            /* not necessarilly the тип of the IV  */
                            /* elem!)                               */
    famlist *FLnext;        // следщ in list
...
const FLELIM = cast(Symbol *)-1;

struct Iv
{

    Symbol *IVbasic;        // symbol of basic IV
    elem **IVincr;          // pointer to родитель of IV increment elem
    famlist *IVfamily;      // variables in this family
    Iv *IVnext;             // следщ iv in list
...
private  бул addblk;                    /* if TRUE, then we added a block */

/* is elem loop invariant?      */
цел isLI(elem *n) { return n.Nflags & NFLli; }

/* make elem loop invariant     */
проц makeLI(elem *n) { n.Nflags |= NFLli; }

/******************************
 *      Only variables that could only be unambiguously defined
 *      are candidates for loop invariant removal and induction
 *      variables.
 *      This means only variables that have the SFLunambig флаг
 *      set for them.
...
private extern (D) проц compdom(block*[] dfo)
{
    assert(dfo.length);
    ...
    vec_t t1 = vec_calloc(vec_numbits(sb.Bdom));       // размести a temporary
    бцел cntr = 0;                       // # of times thru loop
    бул chgs;
    do
    {
        chgs = нет;
...
private extern (D) проц findloops(block*[] dfo, loop **ploops)
{
    freeloop(ploops);

    //printf("findloops()\n");
    foreach (b; dfo)
        b.Bweight = 1;             // сбрось Bweights
...
private бцел loop_weight(бцел weight, цел factor) 
{
...
private проц buildloop(loop **ploops,block *head,block *tail)
{
    loop *l;
    ...
    *ploops = l;                         // put l at beginning of list
    ...
    l.Lloop = vec_calloc(maxblks);       /* размести loop bit vector     */
    l.Lexit = vec_calloc(maxblks);       /* bit vector for exit blocks   */
    l.Lhead = head;
    l.Ltail = tail;

    vec_setbit(head.Bdfoidx,l.Lloop);    /* add head to the loop         */
    head.Bweight = loop_weight(head.Bweight, 2);  // *20 использование for loop header

    вставь(tail,l.Lloop);                /* вставь tail in loop          */
...
L1:
    ...
    бцел i;
    for (i = 0; (i = cast(бцел) vec_index(i, l.Lloop)) < dfo.length; ++i)  // for each block in this loop
    {
...
private проц вставь(block *b, vec_t lv)
{
    assert(b && lv);
    if (!vec_testbit(b.Bdfoidx,lv))     /* if block is not in loop      */
    {
        vec_setbit(b.Bdfoidx,lv);       /* add block to loop            */
        b.Bweight = loop_weight(b.Bweight,1);   // *10 использование count
        foreach (bl; ListRange(b.Bpred))
            вставь(list_block(bl),lv);  /* вставь all its predecessors  */
    }
}
 ...
 * Возвращает:
 *      да need to recompute loop данные
 */

private цел looprotate(loop *l)
{
    block *tail = l.Ltail;
    block *head = l.Lhead;
    ...
    {   // Duplicate the header past the tail (but doing
        ...
        head2 = block_calloc(); // создай new head block
        ...
        if (head.Belem)                // копируй Выражение tree
        ...
        for (list_t *pbl = &(head.Bpred); *pbl; pbl = pbln)
        {
            ...
            {   // if this predecessor is inside the loop
                ...
                *pbl2 = *pbl;
                *pbl = list_next(*pbl);
                pbln = pbl;                     // don't skip this следщ one
                (*pbl2).следщ = null;
       ...
       // L2:
                ...
                pbln = &((*pbl).следщ);      // следщ predecessor in list
        } // for each pred(head)
...
private 
{
    цел gref;                // параметр for markinvar()
    block *gblock;           // параметр for markinvar()
    vec_t lv;                // параметр for markinvar()
    vec_t gin;               // параметр for markinvar()
    бул doflow;             // да if flow analysis has to be redone
}
...
restart:
    file_progress();
    if (blockinit())                    // init block данные
    {
    ...
    findloops(dfo[], &startloop);              // найди the loops
    ...
    addblk = нет;                     /* assume no blocks added        */
    ...
    for (loop *l = startloop; l; l = l.Lnext)
    {
        ...
        if (doflow)
        {
            ...
            flowae();               // compute доступно Выражения
            doflow = нет;         /* no need to redo it           */
        ...
        for (i = 0; (i = cast(бцел) vec_index(i, lv)) < dfo.length; ++i)
            if (dfo[i].Belem)
                unmarkall(dfo[i].Belem);       /* unmark all elems     */

        /* Find & mark all LIs   */
        gin = vec_clone(l.Lpreheader.Bout);
        rd = vec_calloc(go.defnod.length);        /* размести our running RD vector */
        for (i = 0; (i = cast(бцел) vec_index(i, lv)) < dfo.length; ++i) // for each block in loop
        {
            block *b = dfo[i];
            ...
            if (b.Belem)
            {
                ...
                static if (0)
                {
                    printf("i = %d\n",i);
                    {
                        for (цел j = 0; j < go.defnod.length; j++)
                ...
                static if (0)
                {
                    printf("B%d\n", i);
                    {
                        foreach (j; new бцел[0 .. go.defnod.length])
                        {
                            printf("  [%2d] ", j);
        ...
        for (i = 0; (i = cast(бцел) vec_index(i, lv)) < dfo.length; ++i)
        {
            цел domexit;                // да if this block dominates all
                                        // exit blocks of the loop

            for (j = 0; (j = cast(бцел) vec_index(j, l.Lexit)) < dfo.length; ++j) // for each exit block
            {
...
private проц markinvar(elem *n,vec_t rd)
{
    vec_t tmp;
    бцел i;
    Symbol *v;
    ...
    switch (n.Eoper)
    {
        ...
        case OPmsw:
        case OPneg:     case OPбул:    case OPnot:     case OPcom:
        ...
        case OPstreq:
            ...
            if (isLI(n.EV.E2) && n1.Eoper == OPvar)          /* 1 & 2 */
            {
                ...
                if (v.Sflags & SFLunambig)
                {
                    ...
                    //filterrd(tmp,rd,v);
                    listrds(rd,n1,tmp);
                    for (i = 0; (i = cast(бцел) vec_index(i, tmp)) < go.defnod.length; ++i)
        ...
        case OPind:                     /* must assume this is not LI   */
            ...
            if (isLI(n.EV.E1))
            {
                static if (0)
                {
                    ...
                    // the loop dimension for the [i] массив if
                    // a[j][i] was in a loop. This meant the a[j] массив bounds
                    ...
                    else if (n.Ejty)
                    {
                        ...
                        for (i = 0; (i = cast(бцел) vec_index(i, tmp)) < go.defnod.length; ++i)  // for each RD
                        ...
                        if (gref)
                        {
                            ...
                            for (j = 0; (j = cast(бцел) vec_index(j, gin)) < go.exptop; ++j)
                            {
                                elem *e = go.expnod[j];
...
проц fillInDNunambig(vec_t v, elem *e)
{
    ...
    targ_т_мера toff = t.EV.Voffset;
    targ_т_мера tsize = (e.Eoper == OPstreq) ? type_size(e.ET) : tysize(t.Ety);
    targ_т_мера ttop = toff + tsize;

    //printf("updaterd: "); WReqn(n); printf(" toff=%d, tsize=%d\n", toff, tsize);


    // for all unambig defs in go.defnod[]
    foreach ( i; new бцел[0 .. go.defnod.length])
    {
...
проц updaterd(elem *n,vec_t GEN,vec_t KILL)
{
    ...
    else
    {
        static if (0)
        {
            ...
            {
                // for all unambig defs in go.defnod[]
                foreach (бцел i; new бцел[0 .. go.defnod.length])
                {
                    elem *tn = go.defnod[i].DNelem;
                    elem *tn1;
 ...
 * Возвращает:
 *    да if there are any refs of v in n before nstop is encountered
 */

private бул refs(Symbol *v,elem *n,elem *nstop)
{
    symbol_debug(v);
 ...
 *      1) All exit blocks are dominated (да before this is called).
 *                      -- OR --
 *      2) Variable assigned by a инструкция is not live on entering
 *         any successor outside the loop of any exit block of the
 *         loop.
 *
 *      3) Cannot move assignment to variable if there are any other
 *         assignments to that variable within the loop (да or
 ...
 *         def of it.
 *      5) Cannot move Выражения that have side effects.
 *      6) Do not move assignments to variables that could be affected
 *         by ambiguous defs.
 *      7) It is not worth it to move Выражения of the form:
 *              (var == const)
 * Input:
...
private проц movelis(elem *n,block *b,loop *l,цел *pdomexit)
{
    vec_t tmp;
    elem *ne;
    elem *t;
    ...
    switch (op)
    {
        ...
        case OPeq:
            ...
            if (isLI(n) && n.EV.E1.Eoper == OPvar)
            {
                ...
                {
                    бцел i;
                    for (i = 0; (i = cast(бцел) vec_index(i, l.Lexit)) < dfo.length; ++i)  // for each exit block
                    {
                        foreach (bl; ListRange(dfo[i].Bsucc))
                        {
                ...
                tmp = vec_calloc(go.defnod.length);
                бцел i;
                for (i = 0; (i = cast(бцел) vec_index(i, l.Lloop)) < dfo.length; ++i)  // for each block in loop
                {
                    ...
                    бцел j;
                    for (j = 0; (j = cast(бцел) vec_index(j, tmp)) < go.defnod.length; ++j)  // for each RD of v in Binrd
                    {
                ...
                go.changes++;
                doflow = да;                  // redo flow analysis
                ne = el_calloc();
                el_copy(ne,n);                  // создай assignment elem
                assert(l.Lpreheader);          // make sure there is one
                appendelem(ne,&(l.Lpreheader.Belem)); // приставь ne to preheader
...
L3:
    // Do leaves of non-LI Выражения, leaves of = elems that didn't
     ...
     )
    {
        if (OTassign(op))
        {
            ...
            // Do lvalue only if it is an Выражение
            if (n1.Eoper == OPvar)
                goto Lret;
            n11 = n1.EV.E1;
            ...
            {
                //printf("assign to глоб2 => domexit |= 2\n");
                *pdomexit |= 2;
            }
        }
        else if (OTunary(op))
...
Lpass:
    // Move the LI Выражение to the preheader
    if (debugc) printf("Moved LI Выражение ");
    ...
    foreach (nl; ListRange(l.Llis))
    {
        ...
        if (tysize(ty) == tysize(ty2))
        {
            ...
            if (el_match(n,el.EV.E2))
            {
                ...
                el_copy(n,el.EV.E1);      // make копируй of temp
                n.Ety = ty;

                debug
                    if (debugc)
                    {   printf("Already moved: LI Выражение replaced with ");
                        WReqn(n);
                        printf("\nPreheader %d Выражение %p ",
    ...
    t = el_alloctmp(n.Ety);                     /* размести temporary t */

    debug
    {
        if (debugc) printf("movelis() introduced new variable '%s' of тип ",t.EV.Vsym.Sident.ptr);
        if (debugc) WRTYxx(t.Ety);
        if (debugc) printf("\n");
    }

    n2 = el_calloc();
    el_copy(n2,n);                              /* создай копируй n2 of n  */
    ne = el_bin(OPeq,t.Ety,t,n2);               /* создай elem t=n2     */
    assert(l.Lpreheader);                       /* make sure there is one */
    appendelem(ne,&(l.Lpreheader.Belem));       /* приставь ne to preheader */
    ...
    debug
        if (debugc)
        {
            printf("Preheader %d Выражение %p\n\t",
            l.Lpreheader.Bdfoidx,l.Lpreheader.Belem);
            WReqn(l.Lpreheader.Belem);
            printf("\nLI Выражение replaced with "); WReqn(t);
            printf("\n");
        }

    el_copy(n,t);                                 /* replace this elem with t */

    // Remember LI Выражение in elem list
    list_prepend(&l.Llis,ne);
 ...
 * Input:
 *      n       elem to приставь
 *      *pn     elem to приставь to
 */

private проц appendelem(elem *n,elem **pn)
{
    assert(n && pn);
...
private проц freeivlist(Iv *biv)
{
    Iv *bivnext;
...
private проц loopiv(loop *l)
{
    ...
    findbasivs(l);              /* найди basic induction variables       */
    findopeqs(l);               // найди op= variables
    findivfams(l);              /* найди IV families                     */
    ...
    /* Do копируй propagation and dead assignment elimination        */
...
private проц findbasivs(loop *l)
{
    vec_t poss,notposs;
    elem *n;
    бул ambdone;
    ...
    foreach ( i; new бцел[0 .. go.defnod.length])
    {
        ...
        else                            /* else ambiguous def           */
        {
            ...
            {
                foreach (бцел j; new бцел[0 .. globsym.top])
                {
    ...
    /* создай list of IVs */
    бцел i;
    for (i = 0; (i = cast(бцел) vec_index(i, poss)) < globsym.top; ++i)  // for each basic IV
    {
        Iv *biv;
        Symbol *s;
        ...
        // is at смещение 0). We could perhaps amend this by allowing basic IVs
        // if the struct consists of only one данные member.
        if (tyaggregate(s.ty()))
            continue;
        ...
        /* We have the sym idx of the basic IV. We need to найди         */
        /* the родитель of the increment elem for it.                     */

        /* First найди the go.defnod[]      */
        foreach (j; new бцел[0 .. go.defnod.length])
        {
...
private проц findopeqs(loop *l)
{
    ...
    foreach (i; new бцел[0 .. go.defnod.length])
    {
        ...
        else                            // else ambiguous def
        {
            ...
            if (!ambdone)           // avoid redundant loops
            {
                foreach (j; new бцел[0 .. globsym.top])
                {
                    if (!(globsym.tab[j].Sflags & SFLunambig))
                        vec_setbit(j,notposs);
                }
    ...
    // создай list of IVs
    бцел i;
    for (i = 0; (i = cast(бцел) vec_index(i, poss)) < globsym.top; ++i)  // for each opeq IV
    {
        Iv *biv;
        Symbol *s;
...
private проц findivfams(loop *l)
{
    if (debugc) printf("findivfams(%p)\n",l);
    for (Iv *biv = l.Livlist; biv; biv = biv.IVnext)
    {
        for (бцел i = 0; (i = cast(бцел) vec_index(i, l.Lloop)) < dfo.length; ++i)  // for each block in loop
            if (dfo[i].Belem)
                ivfamelems(biv,&(dfo[i].Belem));
        /* Fold all the constant Выражения in c1 and c2.      */
...
private проц ivfamelems(Iv *biv,elem **pn)
{
    tym_t ty,c2ty;
    elem *n;
    elem *n1;
...
private проц elimfrivivs(loop *l)
{
    for (Iv *biv = l.Livlist; biv; biv = biv.IVnext)
    {
        цел nfams;
...
private проц intronvars(loop *l)
{
    elem *T;
    elem *ne;
    elem *t2;
    ...
    for (Iv *biv = l.Livlist; biv; biv = biv.IVnext)      // for each basic IV
    {
        ...
        {                               /* for each IV in family of biv  */
            ...
            ty = fl.FLty;
            T = el_alloctmp(ty);        /* размести temporary T          */
            fl.FLtemp = T.EV.Vsym;

            debug
            {
                if (debugc) printf("intronvars() introduced new variable '%s' of тип ",T.EV.Vsym.Sident.ptr);
                if (debugc) WRTYxx(ty);
                if (debugc) printf("\n");
            }

            /* приставь elem T=biv*C1+C2 to preheader */
            /* ne = biv*C1      */
            tyr = fl.FLivty;                   /* тип of biv              */
            ...
            /* префикс T+=C1*C to elem biv+=C                            */
            /* Must префикс in case the значение of the Выражение (biv+=C) */
            /* is используется by somebody up the tree.                         */
 ...
 * Возвращает:
 *      нет           Caller should создай a new induction variable.
 *      да            *FLpelem is replaced with function of a previous
 *                      induction variable. FLtemp is set to FLELIM to
 *                      indicate this.
 */

private бул funcprev(Iv *biv,famlist *fl)
{
    tym_t tymin;
    цел sz;
    elem *e1;
    ...
    for (famlist *fls = biv.IVfamily; fls != fl; fls = fls.FLnext)
    {
        ...
        if (typtr(fls.c2.Ety))
        {
            ...
            if (tybasic(fls.c2.Ety) == TYhptr)
                ...
                tymin = I64 ? TYllong : TYint;         /* тип of (ptr - ptr) */
        }
        ...
        if (tyfv(e1.Ety) && tyfv(fls.c2.Ety))
        {
            if (e1.Eoper != OPrelconst || fls.c2.Eoper != OPrelconst)
                goto L1;                /* assume Выражения have diff segs */
        ...
        /* Some more тип checking...   */
        sz = tysize(fl.FLty);
        if (sz != tysize(e1.Ety) &&
            sz != tysize(tymin))
            goto L1;

        /* Do some тип checking (can't add pointers and get a pointer!) */
    ...
    L2:
        ...
        *fl.FLpelem = flse1;
        ...
        /* Fix the iv so when we do loops again, we won't создай        */
        ...
        go.changes++;
        doflow = да;
        return да;                    /* it was replaced              */
    }
    return нет;                       /* need to создай a new variable */
}
...
private проц elimbasivs(loop *l)
{
    ...
    for (Iv *biv = l.Livlist; biv; biv = biv.IVnext)        // for each basic IV
    {
        ...
        if (pref != null && refcount <= 1)
        {
            ...
            fl = simfl(fl,ty);      /* найди simplest elem in family */
            if (!fl)
                continue;
            ...
            if (ref_.EV.E2.Eoper == OPvar && ref_.EV.E2.EV.Vsym == X)
            {
                ...
                ref_.EV.E1 = tmp;
                ref_.Eoper = cast(ббайт)swaprel(ref_.Eoper);
            }

            // If e*c1+c2 would результат in a sign change or an overflow
            // then we can't do it
            if (fl.c1.Eoper == OPconst)
            {
                targ_llong c1 = el_tolong(fl.c1);
                const цел sz = tysize(ty);
                if (sz == SHORTSIZE &&
             ...
             * https://issues.dlang.org/show_bug.cgi?ид=16189
             */
            ...
            {
                /* Don't do it if replacement will результат in
            ...
            el_free(ref_.EV.E2);
            ref_.EV.E2 = refE2;
            ref_.Eoper = cast(ббайт)refEoper;
            ...
            /* If sizes of Выражение worked out wrong...
               Which can happen if we have (цел)ptr==e
             */
            if (OTbinary(fofe.Eoper))         // if didn't optimize it away
            {
                const tym_t fofety = fofe.Ety;
                const цел sz = tysize(fofety);
                tym_t ty1 = fofe.EV.E1.Ety;
                const tym_t ty2 = fofe.EV.E2.Ety;
                /* Sizes of + Выражение must all be the same       */
                if (sz != tysize(ty1) &&
                    sz != tysize(ty2)
                   )
                {
                    if (tyuns(fofety))      // if unsigned comparison
                        ty1 = touns(ty1);   /* to unsigned тип     */
                    fofe.Ety = ty1;
                    ref_.EV.E1.Ety = ty1;
                }
            }
            ...
            for (бцел i = 0; (i = cast(бцел) vec_index(i, l.Lexit)) < dfo.length; ++i)  // for each exit block
            {
                elem *ne;
                block *b;
...
/***********************
 * Eliminate opeq IVs that are not используется outside the loop.
 */

private проц elimopeqs(loop *l)
{
    elem **pref;
    Symbol *X;
    цел refcount;
...
private elem ** onlyref(Symbol *x,loop *l,elem *incn,цел *prefcount)
{
    бцел i;

    //printf("onlyref('%s')\n", x.Sident.ptr);
    X = x;                                /* save some параметр passing  */
    assert(symbol_isintab(x));
    sincn = incn;
 ...
 * Do not count the узел if it is the increment узел (sincn).
 * Input:
 *      флаг:   да if block wants to test the elem
 */

private проц countrefs(elem **pn,бул флаг)
{
    elem *n = *pn;
    ...
    {
        if (OTbinary(n.Eoper))
            countrefs(&n.EV.E2, нет);
        return;                         // don't count lvalue
    }
    if (OTunary(n.Eoper))
        countrefs(&n.EV.E1,нет);
    else if (OTbinary(n.Eoper))
    {
        if (OTrel(n.Eoper))
        {
            ...
            /* Check both subtrees to see if n is the comparison узел,
            ...
            if (e1.Eoper == OPvar && e1.EV.Vsym == X && !countrefs2(n.EV.E2) ||
                ...
                nd = pn;                /* found the relop узел */
        }
    L1:
        countrefs(&n.EV.E1,нет);
        countrefs(&n.EV.E2,(флаг && n.Eoper == OPcomma));
    }
...
private проц elimspecwalk(elem **pn)
{
    elem *n;
    ...
    else if (OTbinary(n.Eoper))
    {
        ...
        if (OTrel(n.Eoper))
        {
                ...
                !el_sideeffect(e1.EV.E1))
            {
                ...
                /* increment узел is now guaranteed to have no goal */
                e1.Nflags |= NFLnogoal;
                n.Eoper = OPcomma;
                //go.changes++;
                doflow = да;
...
struct UnrollWalker
{
     ...
     * state == 0: when rdinc is found, удали it, advance to state 1
...
//    extern цел el_length(elem *e);
    цел cost = el_length(ehead);
    //printf("test4 cost: %d\n", cost);
    ...
    /* Find the initial, increment elem, and final значение of s
     */
    elem *einitial;
    elem *eincrement;
    ...
    /* If loop would only execute once anyway, just удали the test at the end
     */
     ...
     * When eincrement is found, удали it.
    ...
    /* If unrolled loop would only execute once anyway, just удали the test at the end
     */
