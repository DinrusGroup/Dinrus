...
{ /// Флаги, устанавливаемые Лексером.
  ...
  ткст0 пп;
  ткст0 старт; /// Указывает на первый символ семы.
  ткст0 конец;   /// На +1 символ после окончания семы.
  ...
  union
  {
    ...
    дол   дол_;    /// Целочисленное значение дол (дол).
  ...
  /// Возвращает: 'эту' сему, если предыдущая является ТОК.ГОЛОВА или пусто (пусто).
  Сема* следщНепроб()
  out(сема)
  {
  ...
  body
  {
    ...
    while (сема !is пусто && сема.пробел)
      сема = сема.следщ;
    if (сема is пусто || сема.вид == ТОК.КФ)
      return this;
    return сема;
  }

  /// Находит предшествующую непробельную сему.
  /// Возвращает: 'эту' сему, если предыдущая является ТОК.ГОЛОВА или пусто.
  Сема* предшНепроб()
  out(сема)
  {
    assert(сема !is пусто);
  ...
  body
  {
    ...
    while (сема !is пусто && сема.пробел)
      сема = сема.предш;
    if (сема is пусто || сема.вид == ТОК.ГОЛОВА)
      return this;
    return сема;
  }
...
version(D2)
{
  ...
  {
    return вид == ТОК.Ткст && ткт_семы !is пусто;
  }
}
...
/// Возвращает "да", если эта сема начинает ДефиницияДекларации.
бул семаНачалаДеклДеф(ТОК лекс)
{
  switch (лекс)
  {
  alias ТОК T;
...
бул семаНачалаАсмИнстр(ТОК лекс)
{
  switch(лекс)
  {
  alias ТОК T;
