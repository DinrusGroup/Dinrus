...
/// Возвращает: да, еслиf d является строкой Юникод или разделителем параграфа.
бул симНовСтрЮ_ли(дим d)
{
  return d == РСд || d == РАд;
}

/// Возвращает: да, если p указывает на строку или разделитель параграфа.
...
/// Возвращает: да, если c является символом Новстр.
бул новСтр_ли(дим c)
{
  return c == '\n' || c == '\r' || симНовСтрЮ_ли(c);
}

/// Возвращает: да, если p указыват на символ КФ.
/// $(PRE
/// КФ := "\0" | _Z_
/// _Z_ := "\x1A"
/// )
...
/// Возвращает: идентификатор, если он действителен (устанавливает ref_p на 1 после ид,) или
