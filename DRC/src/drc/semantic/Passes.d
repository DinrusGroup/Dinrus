...
class ПерваяСемантическаяПроходка : СемантическаяПроходка
{
  ...
  // Атрибуты:
  ТипКомпоновки типКомпоновки; /// Current компонаж тип.
...
override
{
  ...
  Д посети(ДекларацияПеречня d)
  {
    ...
    foreach (член; d.члены)
    {
      ...
      if (анонимен) // Also вставь в родительский Масштаб if enum is анонимный.
  ...
  /// The current surrounding, breakable инструкция.
...
//     выйдиИзМасштаба();
  ...
  И посети(ИнструкцияСКаждым s)
  {
    auto сохранённое = setBS(s);
    // TODO:
    // найди overload opApply или opApplyReverse.
    restилиeBS(сохранённое);
    return s;
  }
  ...
  /// Возвращает a call выражение if 'e' overrides
...
//     устМасштабИда(т.пв.символ);
  ...
  Т посети(ТУказательНаФункцСи т)
  {
    return т;
  }
