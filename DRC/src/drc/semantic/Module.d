...
class Модуль : СимволМасштаба
{
  ...
  ткст пкиМодуля; /// Fully qualified имя of the module. Напр.: drc.ast.Node
  ...
  проц  разбор()
  {
    ...
    { // деклМодуля will be null if first узел isn't a ДекларацияМодуля.
      ...
      if (this.деклМодуля)
        this.установиПКН(деклМодуля.дайПКН()); // Напр.: drc.ast.Node
    }
  ...
  /// Напр.: ["dil/ast/Узел", "dil/semantic/Модуль"]
  ткст[] дайПутиИмпорта()
  {
    ткст[] результат;
