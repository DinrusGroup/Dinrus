...
class ЗагрузчикКонфиг : ЗагрузчикНастроек
{
  ...
  {
    ткст сделайТекст(ткст0 начало, ткст0 конец)
    {
    ...
    ткст результат;
    ткст0 у = знач.ptr, конец = у + знач.length;
    ткст0 началоКуска = у; // Указывает на участок ткст после переменной.
...
{
private extern(Windows) бцел GetModuleFileNameA(ук, ткст0, бцел);
...
{
private extern(C) т_мера readlink(ткст0 путь, ткст0 буф, т_мера буфразм);
