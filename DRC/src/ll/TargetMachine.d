...
|* classes, which can be используется to generate assembly or объект files.           *|
\*===----------------------------------------------------------------------===*/
...
/** Возвращает the first llvm::Target in the registered targets list. */
ЛЛЦель ЛЛДайПервЦель();
/** Возвращает the следщ llvm::Target given a previous one (or null if there's none) */
ЛЛЦель ЛЛДайСледщЦель(ЛЛЦель T);

/*===-- Target ------------------------------------------------------------===*/
/** Finds the target corresponding to the given имя and stores it in \p T.
  Возвращает 0 on успех. */
ЛЛЦель ЛЛДайЦельИзИмени(ткст0 Name);

/** Finds the target corresponding to the given triple and stores it in \p T.
  Возвращает 0 on успех. Optionally returns any error in ошСооб.
  Use LLVMDisposeMessage to dispose the message. */
ЛЛБул ЛЛДайЦельИзТриады(ткст0 Triple, ЛЛЦель *T,
                                 ткст0 *ошСооб);

/** Возвращает the имя of a target. See llvm::Target::getName */
ткст0 ЛЛДайИмяЦели(ЛЛЦель T);

/** Возвращает the description  of a target. See llvm::Target::getDescription */
ткст0 ЛЛДайОписаниеЦели(ЛЛЦель T);

/** Возвращает if the target has a JIT */
ЛЛБул ЛЛЦель_ЕстьДжИТ_ли(ЛЛЦель T);

/** Возвращает if the target has a TargetMachine associated */
ЛЛБул ЛЛЦель_ЕстьЦелМаш_ли(ЛЛЦель T);

/** Возвращает if the target as an ASM backend (required for emitting output) */
...
/** Emits an asm or объект файл for the given module to the имяФайла. This
  wraps several конст++ only classes (among them a файл stream). Возвращает any
...
/** Compile the LLVM IR stored in \p M and store the результат in \p выхБуфПам. */
...
/** Get the host CPU as a ткст. The результат needs to be disposed with
  LLVMDisposeMessage. */
ткст0 ЛЛДайИмяЦПБХоста();

/** Get the host CPU's features as a ткст. The результат needs to be disposed
