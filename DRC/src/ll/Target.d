...
/* Declare all of the target-initialization functions that are доступно. */
...
/* Declare all of the доступно assembly printer initialization functions. */
#define LLVM_ASM_PRINTER(TargetName) \
  проц LLVMInitialize##TargetName##AsmPrinter();
#include "llvm/Config/AsmPrinters.def"
#undef LLVM_ASM_PRINTER  /* Explicit undef to make SWIG happier */

/* Declare all of the доступно assembly parser initialization functions. */
#define LLVM_ASM_PARSER(TargetName) \
  проц LLVMInitialize##TargetName##AsmParser();
#include "llvm/Config/AsmParsers.def"
#undef LLVM_ASM_PARSER  /* Explicit undef to make SWIG happier */

/* Declare all of the доступно disassembler initialization functions. */
...
/** LLVMInitializeAllTargetInfos - The main program should call this function if
    it wants access to all доступно targets that LLVM is configured to
    support. */
проц ЛЛНициализуйВсеИнфОЦели();

/** LLVMInitializeAllTargets - The main program should call this function if it
    wants to link in all доступно targets that LLVM is configured to
    support. */
проц ЛЛНициализуйВсеЦели();

/** LLVMInitializeAllTargetMCs - The main program should call this function if
    it wants access to all доступно target MC that LLVM is configured to
    support. */
...
/** Computes the call frame alignment of a тип in bytes for a target.
    Смотри метод llvm::DataLayout::getTypeABISize. */
бцел ЛЛРаскладкаФреймаВызДляТипа(ЛЛДанныеОЦели доц, ЛЛТип тип);

/** Computes the preferred alignment of a тип in bytes for a target.
    Смотри метод llvm::DataLayout::getTypeABISize. */
бцел ЛЛПредпочтРаскладкаТипа(ЛЛДанныеОЦели доц, ЛЛТип тип);

/** Computes the preferred alignment of a глоб2 variable in bytes for a target.
    Смотри метод llvm::DataLayout::getPreferredAlignment. */
бцел ЛЛПредпочтРаскладкаГлоба(ЛЛДанныеОЦели доц,
                                        ЛЛЗначение глобПеремн);

/** Computes the structure element that содержит the byte смещение for a target.
    Смотри метод llvm::StructLayout::getElementContainingOffset. */
бцел ЛЛЭлтПоСмещ(ЛЛДанныеОЦели доц, ЛЛТип типСтрукт,
                             бдол смещ);

/** Computes the byte смещение of the indexed struct element for a target.
    Смотри метод llvm::StructLayout::getElementContainingOffset. */
