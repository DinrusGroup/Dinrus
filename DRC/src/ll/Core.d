...
enum ЛЛКомпоновка {
  ...
  СлабаяЛюбая,     /**< Keep one копируй of function when linking (weak) */
...
enum ЛЛПредикатРеала {
  False, /**< Always нет (always folded) */
...
enum ЛЛАтомныйПорядок {
  НеАтомный = 0, /**< атр load or store which is not atomic */
  Неупорядоченный = 1, /**< Нижайший уровень атомарности, гарантирует
                                     некоторые здравые результаты, lock free. */
  ...
  AcquireRelease = 6, /**< provides both an Acquire and a
                                          ...
                                          operations which both читай and пиши
                                           memory). */
...
enum LLVMAtomicRMWBinOp {
    ...
    And, /**< Добавить какое-то значение и вернуть старое */
    Nand, /**< Not-And значение и вернуть старое */
    Or, /**< OR значение и вернуть старое */
    Xor, /**< Xor значение и вернуть старое */
 ...
 *
 * Смотрите http://llvm.org/docs/LangRef.html#параметр-attributes
 ...
 *
 * LLVMGetDataLayout is Депрекировано, as the имя is not only incorrect,
 ...
 *
 * Возвращает NULL if the iterator was already at the beginning and there are
 ...
 * не все API доступны.
 *
 * Callers can determine the тип of an ЛЛЗначение by calling the
 * LLVMIsA* family of functions (e.g. LLVMIsAArgument()). These
 * functions are defined by a macro, so it isn't obvious which are
 * доступно by looking at the Doxygen source code. Instead, look at the
 * source definition of LLVM_FOR_EACH_VALUE_SUBCLASS and note the list
 * of знач имена given. These знач имена also correspond to classes in
 * the llvm::Значение hierarchy.
 *
 * @{
 */
/+
 ...
 * regardless of their sub-тип. They correspond to functions доступно
 * on llvm::Значение.
 *
 * @{
 */
...
/**
 * Return a ткст representation of the знач. Use
 * LLVMDisposeMessage to free the ткст.
 *
 * @see llvm::Значение::print()
 */
 ...
 *
 * Internally, an ЛЛЗначение is "pinned" to a specific тип. This
 * series of functions allows you to cast an instance to a specific
 * тип.
 *
 * If the cast is not valid for the specified тип, NULL is returned.
 *
 ...
 * the final use and no more are доступно.
 */
...
/**
 * Obtain a constant знач referring to an undefined знач of a тип.
 *
 ...
 * ткст's length is доступно, it is preferred to call that function
 * instead.
 *
 * @see llvm::ConstantInt::get()
 */
...
/**
 * Create a ConstantDataSequential and initialize it with a ткст.
 *
 ...
 * глоб2 variables, load and store instructions.
 */
 ...
 * it already exists for the given вид.
 *
...
/**
 * Erases a metadata attachment of the given вид if it exists.
 *
...
/**
 * Возвращает the вид of a знач metadata entry at a specific инд.
 */
бцел ЛЛЗначение_ЗаписиМетаданных_ДайРод(ЛЛЗаписьМетаданныхЗначения *записи,
                                         бцел индекс);

/**
 * Возвращает the underlying metadata узел of a знач metadata entry at a
 * specific инд.
 */
ЛЛМетаданные
 ...
 *
 * This group содержит functions that operate on глоб2 variable values.
 *
...
/**
 * Obtain a GlobalAlias знач from a Module by its имя.
 *
 ...
 *
 * Возвращает NULL if the iterator was already at the end and there are no more
 * глоб2 ники.
 */
...
/**
 * Create or вставь the declaration of an intrinsic.  For overloaded intrinsics,
...
/**
 * Copies the имя of an overloaded intrinsic identified by a given list of
 * параметр types.
 *
...
/**
 * Obtain the имя of the garbage collector to use during code
 * generation.
 *
 ...
 * at least LLVMCountParams() дол. This массив will be filled with
 ...
 * actually a wrapped iterator) and obtains the следщ параметр from the
 * underlying iterator.
 */
...
/**
 * Add a глоб2 indirect function to a module under a specified имя.
 *
...
/**
 * Obtain a GlobalIFunc знач from a Module by its имя.
 *
...
/**
 * Remove a глоб2 indirect function from its родитель module and delete it.
 *
 ...
 *
 * The MDString знач does not take ownership of the given ткст, it remains
 ...
 * @param длина Memory address which will hold length of returned ткст.
 ...
 * pointer to a pre-allocated массив of ЛЛБазовыйБлок of at least
 ...
 * llvm::Значение and in the к API, instructions are modeled by
 * ЛЛЗначение.
 *
 ...
 *   * The instruction has no родитель
 *   * The instruction has no имя
 *
...
/**
 * Set the given укНаЭлт instruction to be inbounds or not.
 */
 ...
 * @param обры Memory address of an массив to be filled with basic blocks.
 */
...
/**
 * Changes the тип of м so it can be passed to FunctionPassManagers and the
...
/** Constructs a new function-by-function pass pipeline over the module
    provider. It does not take ownership of the module provider. This тип of
