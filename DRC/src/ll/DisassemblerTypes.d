...
/**
 * The тип for the operand information call back function.  This is called to
 * get the symbolic information for an operand of an instruction.  Typically
 * this is from the relocation information, symbol table, etc.  That блок of
 * information is saved when the disassembler context is created and passed to
 * the call back in the DisInfo параметр.  The instruction containing operand
 * is at the PC параметр.  For some instruction sets, there can be more than
 * one operand with symbolic information.  To determine the symbolic operand
 * information for each operand, the bytes for the specific operand in the
 * instruction are specified by the Offset параметр and its byte widith is the
 * size параметр.  For instructions sets with fixed widths and one symbolic
 * operand per instruction, the Offset параметр will be нуль and разм параметр
 ...
 * the fields of the structure with the Offset of the relocatable Выражение
 ...
 * тип is one of the Target specific #defines below and is используется to принт
 * operands like "_foo@GOT", ":lower16:_foo", etc.
 */
struct LLVMOpInfoSymbol1
{
  бдол Present;  /* 1 if this symbol is present */
  ткст0 Name;  /* symbol имя if not NULL */
...
/**
 * The тип for the symbol lookup function.  This may be called by the
 * disassembler for things like adding a коммент for a PC plus a constant
 * смещение load instruction to use a symbol имя instead of a load address знач.
 ...
 */
/* No input reference тип or no output reference тип. */
