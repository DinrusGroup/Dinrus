 ...
 * параметр and specifying the типТэга and callback functions as described
 ...
 * in the инфоДиз параметр and specifying the типТэга and callback functions as
...
/* The опция use the other assembler printer variant */
 ...
 * the параметр DC.  The bytes of the instruction are specified in the
 * параметр байты, and содержит at least BytesSize number of bytes.  The
 * instruction is at the address specified by the PC параметр.  If a valid
 * instruction can be disassembled, its ткст is returned indirectly in
 * OutString whose size is specified in the параметр OutStringSize.  This
 * function returns the number of bytes in the instruction or нуль if there was
