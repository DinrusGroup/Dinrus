...
abstract class ЭмиттерДДок : ДефолтныйВизитёр
{
  ...
  static бул файлДДок(Модуль мод)
  {
    ...
    if (данные.length >= 5 + 1 && // Проверим на минимальную длину.
        сравнилюб(данные[0..4], "ddoc") == 0 && // Проверим первых четыре символа.
  ...
  {
    auto данные = мод.исходныйТекст.данные;
    ткст0 у = данные.ptr + "ddoc".length;
 ...
 scope class МасштабДДок
  ...
  КомментарийДДок ддок(Узел узел)
  {
    ...
    if (this.коммент)
    {
      if (this.коммент.дитто) // атр определено коммент.
        (this.коммент = this.предшКоммент), (this.cmntIsDitto = да);
      else // атр nилиmal коммент.
  ...
  /// Эскапирует '<', '>' и '&' именованными сущностями ГЯР (HTML).
  ткст эскапируй(ткст текст)
  {
    ...
    if (результат.length != текст.length)
      return результат;
    // Nothing escaped. Итог - оригинальный текст.
    delete результат;
    return текст;
  }

  /// Записывает массив тксты в текстовый буфер.
  проц  пиши(сим[][] тксты...)
  {
    foreach (s; тксты)
      текст ~= s;
  ...
  /// Записывает парамы в текстовый буфер.
  проц  пишиПарамы(Параметры парамы)
  {
    ...
    foreach (парам; парамы.элементы)
    {
      ...
      else
      {
        ...
        пиши(эскапируй(участокТекста(typeBegin, парам.тип.конец))); // Write тип.
        if (парам.имя)
          пиши(" $(DDOC_PARAM ", парам.имя.ткт, ")");
        if (парам.ДиВариадический)
          пиши("...");
        if (парам.дефЗначение)
          пиши(" = ", эскапируй(участокТекста(парам.дефЗначение.начало, парам.дефЗначение.конец)));
      }
      пиши(", ");
    }
    if (парамы.элементы)
  ...
  /// Writes the current template параметры в текстовый буфер.
  проц  пишиПарамыШаблона()
  {
    if (!шпарамы)
      return;
    auto текст = участокТекста(шпарамы.начало, шпарамы.конец);
    текст = эскапируй(текст)[1..$-1]; // Escape and удали '(', ')'.
    пиши("$(DRC_TEMPLATE_PARAMS ", текст, ")");
    шпарамы = пусто;
  }

  /// Writes основы в текстовый буфер.
  проц  пишиСписокНаследования(ТипКлассОснова[] основы)
  {
    if (основы.length == 0)
      return;
    auto basesBegin = основы[0].начало.предшНепроб;
    if (basesBegin.вид == ТОК2.Двоеточие)
      basesBegin = основы[0].начало;
    auto текст = эскапируй(участокТекста(basesBegin, основы[$-1].конец));
    пиши(" $(DRC_BASE_CLASSES ", текст, ")");
  }

  /// Offset at which в вставь a declaration with a "определено" коммент.
  бцел предшСмещДекл;

  /// Writes a declaration в текстовый буфер.
  проц  ДЕКЛ(проц  delegate() dg, Декларация d, бул writeSemicolon = да)
  {
    ...
    { // The declaration has a определено коммент.
      alias предшСмещДекл offs;
      assert(offs != 0);
      auto savedText = текст;
      текст = "";
      пишиДЕКЛ();
      // Вставить текст at смещение.
      auto len = текст.length;
      текст = savedText[0..offs] ~ текст ~ savedText[offs..$];
      offs += len; // Add length of the inserted текст в the смещение.
    }
    else
    {
      пишиДЕКЛ();
      // Установить the смещение. At this смещение другой declarations with a определено
      // коммент will be inserted, if present.
      предшСмещДекл = текст.length;
    }
  }

  /// Wraps the DDOC_DECL_DD macro around the текст Автор dg().
  /// Writes the коммент befилиe dg() is called.
  ...
  /// Записывает символ в  текстовый буфер.
  /// Напр.: &#36;(DRC_SYMBOL сканируй, Лексер.сканируй, функц, 229, 646);
  ...
  проц  пишиАлиасИлиТипдеф(T)(T d)
  {
    ...
    if (auto vd = d.декл.Является!(ДекларацияПеременных))
    {
      ...
      foreach (имя; vd.имена)
        ДЕКЛ({ пиши(вид, " "); пиши(эскапируй(тип), " ");
...
override:
  ...
  D посети(ДекларацияПеременных d)
  {
    ...
    foreach (имя; d.имена)
      ДЕКЛ({ пиши(эскапируй(тип), " "); СИМВОЛ(имя.ткт, "переменная", d); }, d);
    ДЕСК();
    return d;
  }
