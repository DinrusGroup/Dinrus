...
/// Эскапирует символы '<', '>' и '&' именованными символьными сущностями.
ткст xml_escape(ткст текст)
{
  ...
  if (результат.length != текст.length)
    return результат;
  //Нечто не эскапируется. Итог - оригинальный текст.
  delete результат;
  return текст;
}

/// Помещает семы в (форматируй) тксты.
class КартаТегов
{
  ткст[ткст] таблица;
  ткст[ТОК2.МАКС] таблицаСем;
  ...
  /// Возвращает значение для ткт, или 'fallback', если ткт не в таблице.
  ...
  /// Возвращает значение для лекс за O(1) время.
  ткст opIndex(ТОК2 лекс)
  {
...
проц  печатайСему(Сема* сема, КартаТегов тэги, Принт принт)
{
  ...
  case ТОК2.ХэшСтрочка:
    ...
    проц  выводитьШС(ткст0 старт, ткст0 конец)
    {
