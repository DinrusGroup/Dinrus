...
version (СОКЕТЫБСД)
 {
        ...
        extern  (C)
                ...
                цел прими_от(т_сокет s, ук буф, цел длин, цел флаги, адрессок* является, цел* fromlen);
...
class Сокет
{
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Depending on the аргумент значение, it will:
                    ...
                    -   stop trying куда transmit данные является this сокет. Also
        ...
        ***********************************************************************/
         ...
         */
        цел принять_от(проц[] буф, ПФлагиСокета флаги, Адрес является)
        {
                if (!буф.length)
                     плохойАрг ("Сокет.принять_от :: у целевого буфера нулевая длина");

                assert(является.семействоАдресов() == семейство);
                цел длинаИмени = является.длинаИмени();
                return .прими_от(cast(СОКЕТ) сок, буф.ptr, буф.length, cast(ПФлагиСокета)флаги, cast(адрессок*)является.имя(), &длинаИмени);
        }


        /// ditto
        цел принять_от(проц[] буф, Адрес является)
        {
                return принять_от(буф, ПФлагиСокета.Неук, является);
        }
