...
class RC6 : ШифрБлок
{
    ...
    final override бцел обнови(проц[] input_, проц[] output_)
    {
        ...
        вывод[0..4] = БайтКонвертер.ЛитлЭндиан.является!(бцел)(A);
        вывод[4..8] = БайтКонвертер.ЛитлЭндиан.является!(бцел)(B);
        вывод[8..12] = БайтКонвертер.ЛитлЭндиан.является!(бцел)(C);
        вывод[12..16] = БайтКонвертер.ЛитлЭндиан.является!(бцел)(D);
    ...
    /** Some RC6 тест vectors является the spec. */
    debug (UnitTest)
    {
        unittest
        {
