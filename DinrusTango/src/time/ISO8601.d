/*******************************************************************************
        ...
        Если указан часовой пояс, то время просто преобразуется в UTC: является может
...
/** An extended дата тип, wrapping a Время together with some добавьitional
 * information. */
public struct РасширеннаяДата
{
    ...
    private проц год(цел y)
    {
        ...
        {
            year_ = 0;
            // дайГод returns бцел: be careful with promotion в_ unsigned
            цел toдобавь = y - Грегориан.генерный.дайГод(знач);
            знач = Грегориан.генерный.добавьГоды(знач, toдобавь);
        }
        else
            year_ = y < 0 ? y-1 : y;
    }

    private байт маска; // leap секунда and конецДня

    /** Returns the сек часть of the дата: may be 60 if a leap секунда
    ...
    debug (Tango_ISO8601) private ткст toStr()
    {
        return Стдвыв.выкладка.преобразуй(
                   ...
                   year_, годы(*this), месяцы(*this), дни(*this),
                   часы(*this), минуты(*this), .секунды(*this), ms(*this),
 ...
 *
 * A third parameter is available for the РасширеннаяДата version: this allows for
 * parsing expanded год representations. The parameter is the число of extra
 * год цифры beyond four, and дефолты в_ zero. It must be внутри the range
 ...
 * РасширеннаяДата.год instead of the Время's год значение.
 *
 * Examples:
 * ---
 * Время t;
 * РасширеннаяДата ed;
 *
 * разборДаты("19",             t);    // January 1st, 1900
 * разборДаты("1970",           t);    // January 1st, 1970
 * разборДаты("1970-02",        t);    // February 1st, 1970
 * разборДаты("19700203",       t);    // February 3rd, 1970
 * разборДаты("+19700203",     ed, 2); // March 1st, 197002
 * разборДаты("-197002-04-01", ed, 2); // April 1st, -197003 (197003 BCE)
 * разборДаты("00000101",       t);    // January 1st, -1 (1 BCE)
 * разборДаты("1700-W14-2",     t);    // April 6th, 1700
 * разборДаты("2008W01",        t);    // December 31st, 2007
 * разборДаты("1987-221",       t);    // August 9th, 1987
 * разборДаты("1234abcd",       t);    // January 1st, 1234; return значение is 4
 * разборДаты("12abcdef",       t);    // January 1st, 1200; return значение is 2
 * разборДаты("abcdefgh",       t);    // January 1st, 0001; return значение is 0
 * ---
 */
public т_мера разборДаты(T)(T[] ист, ref ДВ dt)
{
    auto fd = ПолнаяДата(dt);

    auto возвр = разборДаты(ист, fd);
    dt = fd.знач;
    return возвр;
}
/** ditto */
public т_мера разборДаты(T)(T[] ист, ref ПолнаяДата fd, ббайт expanded = 0)
{
    ббайт dummy =void;
    T* p = ист.ptr;
...
)
{
    ...
    т_мера remaining()
    {
        return ист.length - eaten();
    }
    бул готово(T[] s)
    {
        return .готово(eaten(), ист.length, p, s);
    }
    ...
    if (прими(p, 'W'))
    {
        ...
        цел i = парсируйЦел(p, мин(cast(т_мера)3, remaining()));
    ...
    case 2:
        ...
        цел день = парсируйЦел(p, мин(cast(т_мера)2, remaining()));
 ...
 * разборВремени("24:00:01",       t); // 00:00:00; return значение is 5
 * разборВремени("24:00:01",      ed); // 00:00:00; return значение is 5; конецДня
 * разборВремени("30",             t); // 00:00:00; return значение is 0
 * разборВремени("21:32:43-12:34", t); // 10:06:43; день increased by one
 * ---
 */
public т_мера разборВремени(T)(T[] ист, ref ДВ dt)
{
    auto fd = ПолнаяДата(dt);

    auto возвр = разборВремени(ист, fd);
    dt = fd.знач;
    return возвр;
}
/** ditto */
public т_мера разборВремени(T)(T[] ист, ref ПолнаяДата fd)
{
    бул dummy =void;
    T* p = ист.ptr;
...
// ВсёValопр is used only в_ получи разборДатыИВремени() в_ catch ошибки correctly
...
)
{
    ...
    {
        return .дайВременнуюЗону(p, remaining(), fd, разделители, &готово);
    }
    ...
    цел час =void;
    if (парсируйЦел(p, мин(cast(т_мера)2, remaining()), час) != 2 || час > 24)
        return 0;
    ...
    цел минуты =void;
    if (
        парсируйЦел(p, мин(cast(т_мера)2, remaining()), минуты) != 2 ||
        минуты > 59 ||
    ...
    цел sec =void;
    if (
        парсируйЦел(p, мин(cast(т_мера)2, remaining()), sec) != 2 ||
        sec > 60 ||
        (fd.конецДня && sec != 0)
    )
        return onlyMinute;
 ...
 * case, the значение of the resulting Время or РасширеннаяДата is undefined.
 *
 * This function is stricter than just calling разборДаты followed by
 * разборВремени: there are no allowances for expanded годы or reduced dates
 ...
 * ---
 */
public т_мера разборДатыИВремени(T)(T[] ист, ref ДВ dt)
{
    ПолнаяДата fd;
    auto возвр = разборДатыИВремени(ист, fd);
    dt = fd.знач;
    return возвр;
}
/** ditto */
public т_мера разборДатыИВремени(T)(T[] ист, ref ПолнаяДата fd)
{
    T* p = ист.ptr;
    ббайт sep;
    бул ВсёValопр = нет;
...
бул парсируйГод(T)(ref T* p, т_мера длин, ббайт expanded, ref ПолнаяДата fd)
{
    ...
    бул doParse()
    {
        ...
        if (!парсируйЦел(p, мин(cast(т_мера)(expanded + 4), длин), год))
            return нет;
...
байт getDecimal(T)(ref T* p, т_мера длин, ref ПолнаяДата fd, ббайт which)
{
    ...
    {
    case час:
        добавьМинуты(fd, 6 * i / степень);
        добавьСекунды(fd, 6 * i % степень);
        break;
    case минута:
        добавьСекунды(fd, 6    * i / степень);
        добавьМс  (fd, 6000 * i / степень % 1000);
        break;
    case сукунда:
        добавьМс(fd, 100 * i / степень);
        break;
...
байт дайВременнуюЗону(T)(ref T* p, т_мера длин, ref ПолнаяДата fd, ббайт разделители, бул delegate(T[]) готово)
{
    ...
    // if we go вперёд in время в_ mопрnight, it's 24:00
    if (
        factor > 0 &&
...
Extract an целое является the ввод, прими no ещё than max цифры
...
debug (Tango_ISO8601_Valgrind) import cidrus :
    ...
    unittest
    {
        ...
        {
            fd = fd.init;
            return разборДатыИВремени(s, fd);
        }
