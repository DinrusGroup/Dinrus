...
//export:
                ...
                /**************************************************************
                        ...
                        Attach console ввод в_ the предоставленный устройство
...
//export:
                ...
                /**************************************************************
                        ...
                        Attach console вывод в_ the предоставленный устройство
                        ...
                        *******************************************************/
						...
						//}          
							...
							if (вв.указатель is пусто || вв.указатель is sys.WinConsts.НЕВЕРНХЭНДЛ)
                                    вв.указатель = sys.WinFuncs.СоздайФайлА ( cast(ткст) f[ук_], 
                                                sys.WinConsts.ППраваДоступа.ГенерноеЧтение| sys.WinConsts.ППраваДоступа.ГенернаяЗапись, sys.WinConsts.ПСовмИспФайла.Чтение|sys.WinConsts.ПСовмИспФайла.Запись, 
                                                пусто,cast(sys.WinConsts.ПРежСоздФайла) 3, cast(sys.WinConsts.ПФайл) 0, cast(ук) 0);
                        ...
                        *******************************************************/
                        ...
                        version (Win32SansUnicode) 
								...
								эхо("redirected!");
                                   ...
                                   i = MultiByteToWideChar (cast(UINT)sys.WinConsts.ПКодСтр.УТФ8, 0, cast(PCHAR) ист.ptr, i, 
                                   ...
                                   for (шим* p=вывод.ptr, конец=вывод.ptr+i; p < конец; p+=i)
                                       ...
                                       if (! WriteConsoleW (cast(HANDLE) вв.указатель,cast(POINTER) p, длин, cast(LPDWORD) &i, пусто))
                                             ошибка();
                                       }
                                   return ист.length;
                                   }
                        ...
                        *******************************************************/
                        ...
                        version (Win32SansUnicode) 
                                ...
                                if (перенапр)
                                   ...
                                   i = WideCharToMultiByte (cast(UINT)sys.WinConsts.ПКодСтр.УТФ8, cast(DWORD) 0,cast(LPCWSTR) ввод.ptr, i, 
                                                            cast(LPSTR) приёмн.ptr, приёмн.length, пусто, пусто);
                                   if (i is 0)
                                       ошибка ();
