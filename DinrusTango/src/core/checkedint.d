...
{
    const long r = cast(long)x - cast(long)y;
    if (r < int.min || r > int.max)
        overflow = true;
    return cast(int)r;
}
...
{
    const long r = cast(ulong)x - cast(ulong)y;
    if (x <  0 && y >= 0 && r >= 0 ||
...
{
    const cent r = cast(ucent)x - cast(ucent)y;
    if (x <  0 && y >= 0 && r >= 0 ||
        x >= 0 && y <  0 && (r <  0 || y == long.min))
        overflow = true;
    return r;
...
unittest
{
    void test(ucent x, ucent y, ucent r, bool overflow) 
    {
        bool o;
        assert(mulu(x, y, o) == r);
        assert(o == overflow);
