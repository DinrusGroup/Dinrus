...
alias MAKELONG MAKEWPARAM,MAKELPARAM,MAKELRESULT;
...
const LPTSTR WC_DIALOG = MAKEINTATOM_T(0x8002);
...
const WS_EX_NOREDIRECTIONBITMAP = 0x00200000; // w8, s2012
...
static if (_WIN32_WINNT >= 0x600) {
    const WM_MOUSELAST = WM_MOUSEHWHEEL;
} else static if (_WIN32_WINNT >= 0x501) {
    const WM_MOUSELAST = WM_XBUTTONDBLCLK;
} else {
    const WM_MOUSELAST = WM_MOUSEWHEEL;
}
...
alias DLGTEMPLATE* LPCDLGTEMPLATE, LPCDLGTEMPLATEA, LPCDLGTEMPLATEW;
...
enum {
    ...
    MIM_MENUDATA        =  8,
    MIM_STYLE           = 16,
    MIM_APPLYTOSUBMENUS = cast(uint) 0x80000000L
}
...
static if (_WIN32_WINNT >= 0x501) {
    ...
    const LONG CONSOLE_APPLICATION_16BIT = 1;
}
