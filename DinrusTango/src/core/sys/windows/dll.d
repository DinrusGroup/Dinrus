...
version (Win32)
{
    ...
    void* function(void* HeapHandle, uint Flags, size_t Size) fnRtlAllocateHeap;
    ...
    static  int* pNtdllBaseTag; // remembered for reusage in addTlsData

    static  ubyte[] jmp_LdrpInitialize = [ 0x33, 0xED, 0xE9 ]; // xor ebp,ebp; jmp _LdrpInitialize
    static  ubyte[] jmp__LdrpInitialize = [ 0x5D, 0xE9 ]; // pop ebp; jmp __LdrpInitialize
    static  ubyte[] jmp__LdrpInitialize_xp64 = [ 0x5D, 0x90, 0x90, 0x90, 0x90, 0x90 ]; // pop ebp; nop; nop; nop; nop; nop;
    static  ubyte[] call_LdrpInitializeThread = [ 0xFF, 0x75, 0x08, 0xE8 ]; // push [ebp+8]; call _LdrpInitializeThread
    static  ubyte[] call_LdrpAllocateTls = [ 0x00, 0x00, 0xE8 ]; // jne 0xc3; call _LdrpAllocateTls
    static  ubyte[] call_LdrpAllocateTls_svr03 = [ 0x65, 0xfc, 0x00, 0xE8 ]; // and [ebp+fc], 0; call _LdrpAllocateTls
    static  ubyte[] jne_LdrpAllocateTls = [ 0x0f, 0x85 ]; // jne body_LdrpAllocateTls
    static  ubyte[] mov_LdrpNumberOfTlsEntries = [ 0x8B, 0x0D ]; // mov ecx, _LdrpNumberOfTlsEntries
    static  ubyte[] mov_NtdllBaseTag = [ 0x51, 0x8B, 0x0D ]; // push ecx; mov ecx, _NtdllBaseTag
    static  ubyte[] mov_NtdllBaseTag_srv03 = [ 0x50, 0xA1 ]; // push eax; mov eax, _NtdllBaseTag
    static  ubyte[] mov_LdrpTlsList = [ 0x8B, 0x3D ]; // mov edi, _LdrpTlsList
    ...
    static bool addTlsData( void** teb, void* tlsstart, void* tlsend, int tlsindex ) 
    {
        ...
        cidrus.memcpy( tlsdata, tlsstart, sz );
        ...
        if ( tlsindex > 0 && teb[11] )
            cidrus.memcpy( array, teb[11], tlsindex * (void*).sizeof);
...
bool dll_fixTLS( HINSTANCE hInstance, void* tlsstart, void* tlsend, void* tls_callbacks_a, int* tlsindex ) 
{
    ...
    scope (failure) assert(0); // enforce , Bugzilla 13561
...
private extern (Windows) ULONGLONG VerSetConditionMask(ULONGLONG, DWORD, BYTE)  ;

private bool isWindows8OrLater()  
{
