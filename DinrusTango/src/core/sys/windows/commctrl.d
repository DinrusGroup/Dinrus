...
template HANDLE_WM_NOTIFY(R) {
    private alias  extern (Windows)
        R function(HWND, int, NMHDR*) _prm_HANDLE_WM_NOTIFY; // to inject linkage type
    R HANDLE_WM_NOTIFY(HWND hwnd, WPARAM wParam, LPARAM lParam, _prm_HANDLE_WM_NOTIFY fn) {
        return fn(hwnd, wParam, cast(NMHDR*) lParam);
    }
}
private alias extern (Windows)
    LRESULT function(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam) _prm_FORWARD_WM_NOTIFY; // to inject linkage type
