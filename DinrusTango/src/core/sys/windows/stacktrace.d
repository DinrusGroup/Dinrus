...
import cidrus, stdrus: разманглируй;
...
private  RtlCaptureStackBackTraceFunc RtlCaptureStackBackTrace;
...
public:
    ...
    this(size_t skip, CONTEXT* context)
    {
        ...
        {
            version (Win64)
                static const INTERNALFRAMES = 3;
            else version (Win32)
                static const INTERNALFRAMES = 2;
     ...
     */
     static char[][] resolve(ulong[] addresses)
    {
        synchronized( typeid(StackTrace) )
        {
...
private:
    ...
    static ulong[] traceNoSync(size_t skip, CONTEXT* context)
    {
        ...
        with (stackframe)
        {
            version (X86)
            {
                const Flat = ADDRESS_MODE.AddrModeFlat;
        ...
        version (X86)         const imageType = IMAGE_FILE_MACHINE_I386;
        else version (X86_64) const imageType = IMAGE_FILE_MACHINE_AMD64;
    ...
    static char[][] resolveNoSync(ulong[] addresses)
    {
        auto dbghelp  = DbgHelp.get();
        if (dbghelp is null)
            return []; // dbghelp.dll not available
    ...
    static char[] formatStackFrame(void* pc)
    {
        ...
        auto len = snprintf(buf.ptr, buf.length, "0x%p", pc);
    ...
    static char[] formatStackFrame(void* pc, char* symName)
    {
        ...
        res ~= " in ";
        char[] tempSymName = symName[0 .. strlen(symName)];
       ...
       // res ~= demangle(tempSymName, demangleBuf);
        res ~= разманглируй(tempSymName);
        return res;
    }

    static char[] formatStackFrame(void* pc, char* symName,
                                     char* fileName, uint lineNum)
    {
          char[11] buf=void;
        ...
        res ~= "(";
        auto len = snprintf(buf.ptr, buf.length, "%u", lineNum);
        cast(uint)len < buf.length || assert(0);
        res ~= buf[0 .. len];
        res ~= ")";
        return res;
                                      ...
                                      ulong CallbackContext, ulong UserContext)
{
    if (ActionCode == CBA_READ_MEMORY)
    {
        ...
        {
            auto base = DbgHelp.get().SymGetModuleBase64(hProcess, p.addr);
...
{
     string[3] defaultPathList = ["_NT_SYMBOL_PATH",
