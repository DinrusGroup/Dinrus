...
struct thread_aux
{
    ...
    static void impersonate_thread( uint id, externCVoidFunc dg)
    {
        ...
        version (Win64)
        {
            const TEB_offset_TlsSlots = 0x1480;
            const TEB_offset_TlsExpansionSlots = 0x1780;
        }
        else
        {
            const TEB_offset_TlsSlots = 0xE10;
            const TEB_offset_TlsExpansionSlots = 0xF94;
        }
        void* tlsSlotsAdr(void** teb) { return cast(void*) teb + TEB_offset_TlsSlots; }
        void* tlsExpansionSlots(void** teb) { return *cast(void**)(cast(void*) teb + TEB_offset_TlsExpansionSlots); }
        ...
        auto rez1 = tlsExpansionSlots(curteb);
		auto rez2 = tlsExpansionSlots(teb);
        rez1 = rez2;
        ...
        auto re1 = tlsExpansionSlots(teb);
		auto re2 = tlsExpansionSlots(curteb);
        re1 = re2;

        memcpy(tlsSlotsAdr(curteb), slots.ptr, slots.sizeof);
        auto r1 = tlsExpansionSlots(curteb);
		r1 = extraSlots;
    }
}
