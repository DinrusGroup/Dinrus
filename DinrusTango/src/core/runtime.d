...
struct Runtime
{
     ...
     * ---------
     */
    static void extendedModuleUnitTester( ExtendedModuleUnitTester h )
    {
     ...
     *  set.
     */
    static ExtendedModuleUnitTester extendedModuleUnitTester()
    {
...
private:
    ...
    //       make it __gshared.
     ExtendedModuleUnitTester sm_extModuleUnitTester = null;
...
static if (hasExecinfo) private class DefaultTraceInfo : Throwable.TraceInfo
{
    ...
    override int opApply( int delegate(ref size_t, ref cchar[]) dg ) 
    {
        ...
        static if (enableDwarf)
        {
            ...
            alias traceHandlerOpApplyImpl = externDFunc!(
                ...
                int function(const void*[], int delegate(ref size_t, ref char[]))
                );
...
private:
    char[] fixline( char[] buf, ref char[4096] fixbuf )
    {
