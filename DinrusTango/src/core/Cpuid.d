...
public:
    ...
    CacheInfo[] dataCaches() {
		CacheInfo[] rez = _dataCaches.dup;
		return rez; }
...
public:
    ...
    uint cacheLevels() { return numCacheLevels; }
private:
...
//    pragma(inline, false);
   ...
   bool hyperThreadingBit() { return (cpuFeatures.features&HTT_BIT)!=0;}
...
version (GNU) {
    version (X86)
        const supportedX86 = true;
    else version (X86_64)
        const supportedX86 = true;
    else
        const supportedX86 = false;
...
void getcacheinfoCPUID2()
{
    ...
    // CPUBUG: Pentium M reports 0x2C but tests show it is only 4-way associative
        ...
        for (size_t i=0; i< ids.length; ++i) {
            if (x==ids[i]) {
                ...
                _dataCaches[level].size=sizes[i];
                _dataCaches[level].associativity=ways[i];
                if (level == 3 || x==0x2C || x==0x0D || (x>=0x48 && x<=0x80)
                                   || x==0x86 || x==0x87
                                   || (x>=0x66 && x<=0x68) || (x>=0x39 && x<=0x3E)){
                    _dataCaches[level].lineSize = 64;
                } else _dataCaches[level].lineSize = 32;
            }
        }
    }
    ...
    do {
        ...
        // (L1 cache = 16K, 4way, linesize16)
                _dataCaches[0].size=8;
                _dataCaches[0].associativity=4;
                _dataCaches[0].lineSize=16;
                return;
            }
...
void getcacheinfoCPUID4()
{
    ...
    for (;;) {
        ...
        if (level > _dataCaches.length) continue; // ignore deep caches
        _dataCaches[level].associativity = a & 0x200 ? ubyte.max :cast(ubyte)((b>>22)+1);
        _dataCaches[level].lineSize = (b & 0xFFF)+ 1; // system coherency line size
        ...
        // and must convert to Kb, also dividing by the number of hyperthreads using this cache.
         ulong sz = (_dataCaches[level].associativity< ubyte.max)? number_of_sets *
            _dataCaches[level].associativity : number_of_sets;
        _dataCaches[level].size = cast(size_t)(
                (sz * _dataCaches[level].lineSize * line_partitions ) / (numthreads *1024));
        if (level == 0 && (a&0xF)==3) {
            // Halve the size for unified L1 caches
            _dataCaches[level].size/=2;
        }
    }
}
...
void getAMDcacheinfo()
{
    ...
    _dataCaches[0].size = ( (c5>>24) & 0xFF);
    _dataCaches[0].associativity = cast(ubyte)( (c5 >> 16) & 0xFF);
    _dataCaches[0].lineSize = c5 & 0xFF;
    ...
    if (max_extended_cpuid >= 0x8000_0006) {
        ...
        static const ubyte [] assocmap = [ 0, 1, 2, 0, 4, 0, 8, 0, 16, 0, 32, 48, 64, 96, 128, 0xFF ];
        _dataCaches[1].size = (c6>>16) & 0xFFFF;
        _dataCaches[1].associativity = assocmap[(c6>>12)&0xF];
        _dataCaches[1].lineSize = c6 & 0xFF;

        // The L3 cache value is TOTAL, not per core.
        _dataCaches[2].size = ((d6>>18)*512)/numcores; // could be up to 2 * this, -1.
        _dataCaches[2].associativity = assocmap[(d6>>12)&0xF];
        _dataCaches[2].lineSize = d6 & 0xFF;
    }
}
...
extern(C) uint _xgetbv(uint xcr);
//Этого опкода нет у процессора. Он появился недавно.!!!
void cpuidX86()
{
    ...
    version (GNU)
    {
        asm  { "cpuid" : "=a" max_cpuid, "=b" venptr[0], "=d" venptr[1], "=c" venptr[2] : "a" 0; }
        asm  { "cpuid" : "=a" max_extended_cpuid : "a" 0x8000_0000 : "ebx", "ecx", "edx"; }
    }
    else
    {
        uint a2;
    ...
    if (cf.miscfeatures & OSXSAVE_BIT)
    {
        version (GNU) asm {
            ...
            mov ECX, 0;
            xgetbv;//extern (C) uint _xgetbv(uint xcr);
			//Этого опкода нет у процессора. Он появился недавно.!!!
            mov d, EDX;
            mov a, EAX;
        }*/
    ...
    if (max_extended_cpuid >= 0x8000_0004) {
        ...
        cf.processorName = "Неизвестный ЦПБ";
    }
    ...
    if (max_cpuid<2 || !cf.probablyIntel) {
        if (max_extended_cpuid >= 0x8000_0005) {
            ...
            _dataCaches[0].size = 8;
            _dataCaches[0].associativity = 4;
            _dataCaches[0].lineSize = 32;
        } else {
            // Some obscure CPU.
            // Values for Cyrix 6x86MX (family 6, model 0)
            _dataCaches[0].size = 64;
            _dataCaches[0].associativity = 4;
            _dataCaches[0].lineSize = 32;
        }
    }
    if ((_dataCaches[0].size == 0) && max_cpuid>=4) {
        getcacheinfoCPUID4();
    }
    if ((_dataCaches[0].size == 0) && max_cpuid>=2) {
        getcacheinfoCPUID2();
    }
    if (_dataCaches[0].size == 0) {
        // Pentium, PMMX, late model 486, or an obscure CPU
        if (mmx) { // Pentium MMX. Also has 8kB code cache.
            _dataCaches[0].size = 16;
            _dataCaches[0].associativity = 4;
            _dataCaches[0].lineSize = 32;
...
void cpuidPPC()
{
    ...
    _dataCaches[0].size = sizes[cputype];
    _dataCaches[0].associativity = ways[cputype];
    _dataCaches[0].lineSize = (cputype==PPCG5)? 128 :
        (cputype == PPC620 || cputype == PPCG3)? 64 : 32;
    _dataCaches[1].size = L2size[cputype];
    _dataCaches[2].size = L3size[cputype];
    _dataCaches[1].lineSize = _dataCaches[0].lineSize;
    _dataCaches[2].lineSize = _dataCaches[0].lineSize;
}
...
static this()
{
    ...
    for (size_t i=1; i< _dataCaches.length; ++i) {
        if (_dataCaches[i].size==0) {
            // Set all remaining levels of cache equal to full address space.
            _dataCaches[i].size = size_t.max/1024;
            _dataCaches[i].associativity = 1;
            _dataCaches[i].lineSize = _dataCaches[i-1].lineSize;
        }
        else
            ++numCacheLevels;
    }
    ...
    const avx_mask = XF_SSE_BIT|XF_YMM_BIT;
