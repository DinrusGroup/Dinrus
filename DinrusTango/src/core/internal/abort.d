 ...
 */
void abort(string msg, string filename = __FILE__, size_t line = __LINE__)
{    
    // use available OS system calls to print the message to stderr
    version (Posix)
    {        
        static void writeStr(scope const(char)[][] m...)
        {
            foreach (s; m)
