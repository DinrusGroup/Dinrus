...
alias char[20] UnsignedStringBuf;
...
Returns:
    The unsigned integer value as a string of characters
*/
auto unsignedToTempString()(ulong value, uint radix = 10)
{
...
alias char[20] SignedStringBuf;

string signedToTempString(long value,  char[] buf, uint radix = 10) 
{
    bool neg = value < 0;
    if (neg)
    ...
    if (neg)
    {
        ...
        void[] trustedSlice( char[] r)  { assert(r.ptr > buf.ptr); return (r.ptr-1)[0..r.length+1]; }
        r = trustedSlice(r);
        r[0] = '-';
    }
    return r;
...
string  signedToTempString(long value, uint radix = 10)
{
    bool neg = value < 0;
    if (neg)
        value = cast(ulong)-value;
...
int dstrcmp()( char[] s1, char[] s2 )
{
    ...
    if (__ctfe)
    {
        foreach (const u; new uint[0 .. len])
        {
            if (s1[u] != s2[u])
