...
private union Split64
{
   ...
   // pragma(inline, true)
    static Split64 opCall(ulong u64)
        if (__ctfe)
        {
            lo = cast(uint) u64;
            hi = cast(uint) (u64 >>> 32);
...
unittest
{
    ...
    auto test_ctfe = bsr(ulong.max);
    assert(test_ctfe == 63);
}

private alias softScan!(N, true) softBsf(N);
private alias softScan!(N, false) softBsr(N);
...
*/
private int softScan(N, bool forward)(N v)
...
struct BitRange
{
    ...
    const bitsPerWord = size_t.sizeof * 8;

    private
    {
        size_t* bits; // points at next word of bits to check
     ...
     */
    this(size_t* bitarr, size_t numBits) 
    {
        bits = bitarr;
        len = numBits;
        if (len)
...
version (DigitalMars) version (AnyX86)  // not pure
{
    /**
...
unittest
{
    ...
    const test_ctfe = popcnt(uint.max);
    assert(test_ctfe == 32);
}
...
unittest
{
    ...
    // Make sure popcnt() is available at CTFE
    const test_ctfe = popcnt(ulong.max);
    assert(test_ctfe == 64);
}

private int softPopcnt(N)(N x)
    if (is(N == uint) || is(N == ulong))
{
...
version (AsmX86)
{
    private uint asmBitswap32(uint x) 
    {
        asm  { naked; }
...
version (D_InlineAsm_X86_64)
{
    private ulong asmBitswap64(ulong x)
    {
        asm { naked; }
