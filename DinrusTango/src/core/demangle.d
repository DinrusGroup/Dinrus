...
private struct Demangle(Hooks = NoHooks)
{
    ...
    this( char[] buf_,  char[] dst_ = null )
    {
        this( buf_, AddType.yes, dst_ );
    }


    this( char[] buf_, AddType addType_,  char[] dst_ = null )
    {
        buf     = buf_;
        addType = addType_;
        dst     = dst_;
    ...
    static void error( string msg = "Неверный символ" ) /* exception only used in module */
    {
    ...
    static void overflow( string msg = "Переполнение буфера" )  /* exception only used in module */
    {
        pragma(inline, false); // tame dmd inliner

        //throw new OverflowException( msg );
        debug(info) printf( "переполнение: %.*s\n", cast(int) msg.length, msg.ptr );
    ...
    static bool contains( char[] a, char[] b )
    {
    ...
    */
    ...
    {
        static const string[23] primitives = [
            "char", // a
            "bool", // b
            "creal", // c
            "double", // d
