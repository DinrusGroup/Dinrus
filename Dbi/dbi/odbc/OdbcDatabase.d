...
class OdbcDatabase : БазаДанных {
	...
	override проц выполни (ткст эскюэл) {
		...
		scope (failure)
			if (!SQL_SUCCEEDED(SQLEndTran(SQL_HANDLE_DBC, подключение, SQL_ROLLBACK))) {
				throw new ИсклДБИ("Не удался откат после неудачного запроса.  ODBC вернуло " ~ дайСообПоследнОш, эскюэл, дайКодПоследнОш);
			}
		...
		if (!SQL_SUCCEEDED(SQLExecDirect(инстр, cast(SQLCHAR*)эскюэл.ptr, эскюэл.length))) {
			throw new ИсклДБИ("Не удалось выполнить код SQL.  ODBC вернуло " ~ дайСообПоследнОш, эскюэл, дайКодПоследнОш);
		}
	...
	override РезультатОДБЦ запрос (ткст эскюэл) {
		...
		if (SQL_SUCCEEDED(SQLExecDirect(инстр, cast(SQLCHAR*)эскюэл.ptr, эскюэл.length))) {
			...
			throw new ИсклДБИ("Не удаётся запрос к базе данных.  ODBC вернуло " ~ дайСообПоследнОш, эскюэл, дайКодПоследнОш);
		}
	}
