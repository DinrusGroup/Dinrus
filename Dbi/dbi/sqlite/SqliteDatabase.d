...
class БДЭскюлайт : БазаДанных {
	...
	override проц подключись (ткст парамы, ткст имя_пользователя = пусто, ткст пароль = пусто) {
		if ((кодОш = sqlite3_open(toCString(парамы), &бд)) != SQLITE_OK) {
			throw new ИсклДБИ("Не удалось открыть или создать " ~ парамы, кодОш, спецВОбщ(кодОш));
		}
	}
	...
	override проц закрой () {
		if (бд !is пусто) {
			if ((кодОш = sqlite3_close(бд)) != SQLITE_OK) {
				throw new ИсклДБИ(asString(sqlite3_errmsg(бд)), кодОш, спецВОбщ(кодОш));
			}
			бд = пусто;
		}
	}
	...
	override проц выполни (ткст эскюэл) {
		char** errorMessage;
		if ((кодОш = sqlite3_exec(бд, эскюэл.dup.ptr, пусто, пусто, errorMessage)) != SQLITE_OK) {
			throw new ИсклДБИ(toDString(sqlite3_errmsg(бд)), эскюэл, кодОш, спецВОбщ(кодОш));
		}
	}
	...
	override РезультатЭскюлайт запрос (ткст эскюэл) {
		char** errorMessage;
		sqlite3_stmt* инстр;
		if ((кодОш = sqlite3_prepare(бд, toCString(эскюэл), эскюэл.length, &инстр, errorMessage)) != SQLITE_OK) {
			throw new ИсклДБИ(toDString(sqlite3_errmsg(бд)), эскюэл, кодОш, спецВОбщ(кодОш));
		}
		return new РезультатЭскюлайт(инстр);
	}
	...
	deprecated override ткст дайСообОшибки () {
		return toDString(sqlite3_errmsg(бд));
	}
	...
	дол дайИдСтрокиПоследнейВставки () {
		return sqlite3_last_insert_rowid(бд);
	}
	 ...
	 */
	ткст[] дайИменаТаблиц () {
		return дайИменаЭлементов("table");
	}
	...
	ткст[] дайИменаИндексов () {
		return дайИменаЭлементов("index");
	}
	...
	ткст[] дайИменаЭлементов(ткст тип) {
		ткст[] items;
		Ряд[] ряды = запросПолучиВсе("SELECT имя FROM sqlite_master WHERE тип='" ~ тип ~ "'");
		for (т_мера i = 0; i < ряды.length; i++) {
			items ~= ряды[i].дай(0);
		}
		return items;
	}
	...
	бул естьЭлемент(ткст тип, ткст имя) {
		Ряд[] ряды = запросПолучиВсе("SELECT имя FROM sqlite_master WHERE тип='" ~ тип ~ "' AND имя='" ~ имя ~ "'");
		if (ряды !is пусто && ряды.length > 0) {
			return да;
		}
		return false;
	}
