...
/*******************************************************************************
        ...
        on a scheme being present in the Уир, and it doesn't implement the
        ...
        путь, запрос, and fragment are UTF-8 кодирован 
...
class Уир : ОбзорУИР
{
        ...
        /***********************************************************************
                ...
                Initialize the Уир character maps and so on

        ***********************************************************************/

        static this ()
        {
                // Map known generic schemes в_ their default порт. Specify
                ...
                // загрузи the character карта with valid symbols
                for (цел i='a'; i <= 'z'; ++i)  
                     карта[i] = IncGeneric;
                ...
                // exclude these из_ parsing elements
                карта[':'] |= ExcScheme;
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Clone другой Уир. This can be used в_ сделай a mutable Уир
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Return a valid порт число by performing a отыщи on the 
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Return whether or not the Уир scheme is consопрered generic.
        ...
        ***********************************************************************/
        ...
        final т_мера произведи (Consumer используй)
        {
                ...
                if (scheme_.length)
                    ret += используй (scheme_), ret += используй (":");
                ...
                if (userinfo_.length || host_.length || port_ != InvalКСЕРort)
                   ...
                   if (userinfo_.length)
                       ret += кодируй (используй, userinfo_, IncUser), ret +=используй ("@");
                   ...
                   if (port_ != InvalКСЕРort && port_ != getDefaultPort(scheme_))
                      {
                      сим[8] врем;
                      ret += используй (":"), ret += используй (Целое.itoa (врем, cast(бцел) port_));
                      }
                   }

                if (path_.length)
                    ret += кодируй (используй, path_, IncPath);

                if (query_.length)
                   {
                   ret += используй ("?");
                   ret += кодируй (используй, query_, IncQueryAll);
                   }

                if (fragment_.length)
                   {
                   ret += используй ("#");
                   ret += кодируй (используй, fragment_, IncQuery);
                   }
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Decode a character ткст with potential %hex values in it.
                The decoded strings are placed преобр_в a нить-safe expanding
                буфер, and a срез of it is returned в_ the caller.
        ...
        ***********************************************************************/
        ...
        private ткст decoder (ткст s, сим ignore=0)
        {
                ...
                // take a Просмотр first, в_ see if there's work в_ do
        ...
        /***********************************************************************
                ...
                Decode a duplicated ткст with potential %hex values in it
        ...
        ***********************************************************************/
        ...
        final Уир разбор (ткст уир, бул relative = нет)
        {
                ...
                // isolate authority
                if (mark < длин-1 && уир[mark] is '/' && уир[mark+1] is '/')
                   {
                   for (mark+=2, i=mark; i < длин && !(карта[уир[i]] & ExcAuthority); ++i) {}
                   parseAuthority (уир[mark .. i]); 
                   mark = i;
                   }
                else
                   if (relative)
                ...
                // isolate путь
                for (i=mark; i < длин && !(карта[уир[i]] & ExcPath); ++i) {}
                path_ = decoder (уир[mark .. i]);
                mark = i;

                // isolate запрос
                if (mark < длин && уир[mark] is '?')
                   {
                   for (++mark, i=mark; i < длин && уир[i] != '#'; ++i) {}
                   query_ = decoder (уир[mark .. i], '&');
                   mark = i;
                   }

                // isolate fragment
                if (mark < длин && уир[mark] is '#')
                    fragment_ = decoder (уир[mark+1 .. длин]);
        ...
        ***********************************************************************/
        ...
        final проц сбрось()
        {
                decoded.сбрось;
                port_ = InvalКСЕРort;
                host_ = path_ = query_ = scheme_ = userinfo_ = fragment_ = пусто;
        }
        ...
        **********************************************************************/
        ...
        private final static ткст toLower (ref ткст ист)
        {
...
private struct СрезКучи
{
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Potentially расширь the контент пространство, and return a pointer
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Return a срез of the контент из_ the current позиция 
                with the specified размер. Adjusts the current позиция в_ 
...
{
    auto уир = new Уир;
    auto uristring = "http://www.example.com/click.html/c=37571:RoS_Intern_search-link3_LB_Sky_Rec/b=98983:news-время-search-link_leader_neu/l=68%7C%7C%7C%7Cde/url=http://ads.ad4max.com/adclick.aspx?опр=cf722624-efd5-4b10-ad53-88a5872a8873&pubad=b9c8acc4-e396-4b0b-b665-8bb3078128e6&avопр=963171985&adcpc=xrH%2f%2bxVeFaPVkbVCMufB5A%3d%3d&a1v=6972657882&a1lang=de&a1ou=http%3a%2f%2fad.search.ch%2fiframe_ad.html%3fcampaignname%3dRoS_Intern_search-link3_LB_Sky_Rec%26bannername%3dnews-время-search-link_leader_neu%26iframeопр%3dsl_if1%26content%3dvZLLbsIwEEX3%2bQo3aqUW1XEgkAckSJRuKqEuoDuELD%2bmiSEJyDEE%2fr7h0cKm6q6SF9bVjH3uzI3vMOaVYdpgPIwrodXGIHPYQGIb2BuyZDt2Vu2hRUh8Nx%2b%2fjj5Gc4u0UNAJ95H7jCbAJGi%2bZlqix3eoqyfUIhaT3YLtabpVEiXI5pEImRBdDF7k4y53Oea%2b38Mh554bhO1OCL49%2bO6qlTTZsa3546pmoNLMHOXIvaoapNIgTnpmzKZPCJNOBUyLzBEZEbkSKyczRU5E4gW9oN2frmf0rTSgS3quw7kqVx6dvNDZ6kCnIAhPojAKvX7Z%2bMFGFYBvKml%2bskxL2JI88cOHYPxzJJCtzpP79pXQaCZWqkxppcVvlDsF9b9CqiJNLiB1Xd%2bQqIKlUBHXSdWnjQbN1heLoRWTcwz%2bCAlqLCZXg5VzHoEj1gW5XJeVffOcFR8TCKVs8vcF%26crc%3dac8cc2fa9ec2e2de9d242345c2d40c25";
