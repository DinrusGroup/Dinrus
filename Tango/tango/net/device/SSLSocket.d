...
/*******************************************************************************
    ...
    СокетССЛ is a sub-class of Сокет. It's purpose is в_
    ...
    СокетССЛ requires the OpenSSL library, and uses a dynamic binding
    в_ the library. You can найди the library at http://www.openssl.org and a
...
+/
    /*******************************************************************************
        ...
        Create a default Client Mode СокетССЛ.
...
+/
    ...
    *******************************************************************************/
    ...
    override т_мера пиши(проц[] ист)
    {
        ...
        цел байты = BIO_write(сокетССЛ, ист.ptr, ист.length);
        if (байты <= 0)
            return Кф;
        return байты;
    }
    ...
    /*******************************************************************************
        ...
        установи the max length of время the читай will take before returning.
...
/+
        ...
        {
            т_мера rtn = Кф;
            // need в_ switch в_ nonblocking...
            бул blocking = сокет_.blocking;
            if (blocking) сокет_.blocking = нет;
            do
            {
                цел bytesRead = BIO_read(сокетССЛ, приёмн.ptr, приёмн.length);
                if (bytesRead <= 0)
                {
                    бул читай = нет;
                    бул пиши = нет;
                    if (!BIO_should_retry(сокетССЛ))
                        break;
                    if (BIO_should_read(сокетССЛ))
                        читай = да;
                    if (BIO_should_write(сокетССЛ))
                        пиши = да;
                    if (читай || пиши)
                    {
                        if (читай)
                        {
                            if (readSet is пусто)
                                readSet = new НаборСокетов(1);
                            readSet.сбрось();
                            readSet.добавь(сокет_);
                        }
                        if (пиши)
                        {
                            if (writeSet is пусто)
                                writeSet = new НаборСокетов(1);
                            writeSet.сбрось();
                            writeSet.добавь(сокет_);
                        }
                        auto копируй = tv;
                        цел i = сокет_.выбери(читай ? readSet : пусто, пиши ? writeSet : пусто, пусто, &копируй);
                        if (i <= 0)
                        {
                            if (i is 0)
                                таймаут = да;
            ...
            if (blocking) сокет_.blocking = blocking;
            return rtn;
        }
+/
    ...
    /*******************************************************************************
        ...
        Shuts down the underlying сокет for reading and writing.
    ...
    *******************************************************************************/
    ...
    {
        SSL *об;
        BIO_get_ssl(сокетССЛ, &об);
        if (об)
        {
            if (!SSL_get_shutdown)
                SSL_set_shutdown(об, SSL_SENT_SHUTDOWN | SSL_ПриёмD_SHUTDOWN);
        }
        return this;
    }
    ...
    /*******************************************************************************
        ...
        Used in conjuction with the above ctor with the создай flag disabled. It is
        useful for accepting a new сокет преобр_в a СокетССЛ, and then re-using
    ...
    */
    private BIO *_convertToSSL(КонтекстССЛ кнткстССЛ, бул закрой, бул клиент)
    {
        ...
        BIO *socketBio = BIO_new_socket(исконный.укз, закрой ? BIO_CLOSE : BIO_NOCLOSE);
        if (socketBio)
        {
            rtn = BIO_new_ssl(кнткстССЛ.исконный, клиент);
            if (rtn)
...
/*******************************************************************************
    ...
    СерверСокетССЛ is a sub-class of СерверСокет. It's purpose is в_ provопрe
    ...
    СерверСокетССЛ requires the OpenSSL library, and uses a dynamic binding
    в_ the library. You can найди the library at http://www.openssl.org and a
    Win32 specific порт at http://www.slproweb.com/products/Win32OpenSSL.html.
...
class СерверСокетССЛ : СерверСокет
{
    ...
    *******************************************************************************/
    ...
    {
        scope адр = new АдресИПв4 (порт);
        this (адр, ctx, backlog, reuse);
    }
    ...
    /*******************************************************************************
        ...
        Параметры:
            адр = the адрес в_ свяжи and слушай on.
    ...
    *******************************************************************************/
    ...
    {
        super(адр, backlog, reuse);
        кнткстССЛ = ctx;
    }

    /*******************************************************************************

      Accepts a new conection and copies the provопрed сервер КонтекстССЛ в_ a new
      СокетССЛ.
    ...
    *******************************************************************************/
    ...
    СокетССЛ прими (СокетССЛ реципиент = пусто)
    {
        if (реципиент is пусто)
            реципиент = new СокетССЛ(нет);

        super.прими (реципиент);
        реципиент.установиКонтекст(кнткстССЛ, нет);
        return реципиент;
    }
}
...
version(Test)
{
    ...
    unittest
    {
                ...
                if (good)
                {
                    ...
                    if (mySock)
                    {
                        ...
                        catch (Исключение ex)
                        {
                            ...
                            publicCertificate.установиСубъект("CA", "Alberta", "Place", "Нет", "First Last", "no unit", "email@example.com").sign(publicCertificate, частныйКлюч);
                        }                        
        ...
        t["Чтен/Write"] = &sslReadWriteTest;
        t["Чтен/Write Timeout"] = &sslReadWriteTestWithTimeout; 
        t.run();
    }
}
