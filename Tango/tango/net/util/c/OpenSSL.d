...
private import sys.Environment;
...
private проц bindFunc(T)(ref T func, ткст funcName, Длл lib)
in
{
    assert(funcName);
    assert(lib);
    ...
    if (ERR_Просмотр_error())
    {
        ...
        while (код != 0)
        {
            if (данные && (флаги & ERR_TXT_STRING))
                exceptionString ~= Формат.преобразуй("ssl ошибка код: {} {}:{} - {}\r\n", код, изТкст0(файл), строка, изТкст0(данные));
            else
...
Длл loadLib(ткст[] loadPath)
{
    ...
    foreach(путь; loadPath)
    {
        ...
        catch (ИсклДлл ex)
        {
            ...
            try
                rtn = Длл.загрузи(fp.абсолютный(Среда.текрабпап()).вТкст);
            catch (ИсклДлл ex)
            {}
        }
    }
