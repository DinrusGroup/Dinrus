...
/*******************************************************************************
  ...
  PKI provопрes Public Key Infrastructure. 

  Specifically, it provопрes the ability в_:

  - Make a X509 Сертификат (SSL Сертификат)

  - Make a Public and Private ключ pair

  - Valопрate a X509 Сертификат against a Сертификат Authority

  - Generate a КонтекстССЛ for СокетССЛ and СерверСокетССЛ

  - Wrap a SSLVerifyCallback so that retrieving the peer серт is easier

  PKI requires the OpenSSL library, and uses a dynamic binding в_ the library.
  You can найди the library at http://www.openssl.org and a Win32 specific порт 
...
/*******************************************************************************
    ...
    КонтекстССЛ is provопрed в_ СокетССЛ and СерверСокетССЛ.
...
class КонтекстССЛ
{
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
        ...
        Return the исконный контекст из_ OpenSSL
...
class ХранилищеСертификатов
{
    ...
    *******************************************************************************/
    ...
    {
        if (X509_STORE_добавь_cert(_store, серт._cert))
...
class ПубличныйКлюч
{
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
        ...
        Return a ПубличныйКлюч in the PEM форматируй.
    ...
    *******************************************************************************/
    ...
    ббайт[] зашифруй(ббайт[] данные)
    {
        ...
        if (данные.length > maxSize)
            throw new Исключение("The specified данные is larger than the размер that can be encrypted by this public ключ.");
...
class ЧастныйКлюч
{
    ...
    *******************************************************************************/
    ...
    ббайт[] sign(ббайт[] данные, ббайт[] sigbuf)
    {
        бцел maxSize = RSA_size(cast(RSA *)_evpKey.pkey);
        if (sigbuf.length < maxSize)
            throw new Исключение("The сигнатура буфер is too small в_ fit the сигнатура for this ключ.");
        ббайт[MD5_DIGEST_LENGTH] дайджест;
...
/*******************************************************************************
    ...
    серт.sign(серт, newPkey); // сам signed серт
...
class Сертификат
{
    ...
    *******************************************************************************/
    ...
    ткст датаПосле()
    {
        ...
        if (genTime)
        {
            rtn = genTime.данные[0..genTime.length].dup;
    ...
    body
    {
        ...
        if (имя)
        {
            ...
            if (email) // this might have в_ go after the CN
                добавьЗаписьИмени(имя, "emailадрес", email);
    ...
    *******************************************************************************/
    ...
    Сертификат sign(Сертификат caCert, ЧастныйКлюч caKey)
    in
    {
        assert(caCert);
        assert(caKey);
    ...
    *******************************************************************************/
    ...
    {
        if (!X509_NAME_добавь_entry_by_txt(имя, тип, MBSTRING_ASC, вТкст0(значение), значение.length, -1, 0))
            выдайОшибкуОпенССЛ();
    }
    ...
    {
        if (readOnly)
            throw new Исключение("The серт is signed already, and cannot be изменён.");
    }
}
...
version (Test)
{
    ...
    unittest
    {
        Test.Status _pkeyGenTest(ref ткст[] messages)
        {
        ...
        Test.Status _certGenTest(ref ткст[] messages)
        {
            auto серт = new Сертификат();
            auto pkey = new ЧастныйКлюч(2048);
            серт.частныйКлюч(pkey).серийныйНомер(123).смещениеКДатеДо(t1).смещениеКДатеПосле(t2);
            серт.установиСубъект("CA", "Alberta", "Place", "Нет", "First Last", "no unit", "email@example.com").sign(серт, pkey);
...
//            Стдвыв.форматнс("{}\n{}\n{}\n{}", cert2.серийныйНомер, cert2.субъект, cert2.датаДо, cert2.датаПосле);
        ...
        Test.Status _chainValопрation(ref ткст[] messages)
        {
        ...
        Test.Status _rsaCrypto(ref ткст[] messages)
        {
        ...
        Test.Status _rsaSignVerify(ref ткст[] messages)
        {
            ...
            ббайт[512] sigBuf;
            ббайт[512] sigBuf2;
            auto sig1 = ключ.sign(данные, sigBuf);
            auto sig2 = key2.sign(данные, sigBuf2);
        ...
        t["RSA sign/проверь"] = &_rsaSignVerify;
        t.run();
    }
}
