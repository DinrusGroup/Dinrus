...
class КэшПам : private Нить
{
        ...
        **********************************************************************/
        ...
        /**********************************************************************
                ...
                Удали the specified ключ and сделай ключ "не_годится" for the 
                duration of таймаут, causing добавь(), получи() and удали() on
                the same ключ в_ краш within that период
...
private class Подключение
{
        ...
        private Line            строка;           // reading lines из_ сервер
                    ...
                    try {
                        ...
                        if (строка.следщ)
                           ...
                           if (контент.length > 4 && контент[0..5] == "VALUE")
                              ...
                              // ensure вывод буфер есть enough пространство
                              буфер.расширь (i);
                              ...
                              // съешь the CR and тест terminator
                              строка.следщ;
                              строка.следщ;
...
проц main()
{
        ...
        auto cache = new КэшПам (хосты);

        cache.установи ("foo", "bar");
        cache.установи ("foo", "wumpus");

        auto буфер = cache.буфер (1024);
        if (cache.получи ("foo", буфер))
