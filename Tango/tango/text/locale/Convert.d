...
/******************************************************************************
   ...
   * Converts the значение of this экземпляр в_ its equivalent ткст representation using the specified _format and культура-specific formatting information.
   * Параметры: 
   *   форматируй = A _format ткст.
   *   службаФормата = An ИСлужбаФормата that provопрes культура-specific formatting information.
   * Возвращает: A ткст representation of the значение of this экземпляр as specified by форматируй and службаФормата.
   * Remarks: See $(LINK2 datetimeformat.html, Время Formatting) for ещё information about дата and время formatting.
   * Examples:
   * ---
   * import io.stream.Format, text.locale.Core, time.WallClock;
   *
   * проц main() {
   *   Культура культура = Культура.current;
   *   Время сейчас = Куранты.сейчас;
   *
   *   Println("Текущий дата and время: %s", сейчас.вТкст());
   *   Println();
   *
   *   // Формат the current дата and время in a число of ways.
   *   Println("Культура: %s", культура.englishName);
   *   Println();
   *
   ...
   *   Println("Sortable:                %s", сейчас.вТкст("s"));
   *   Println("Год:                    %s", сейчас.вТкст("Y"));
   *   Println();
   *
   *   // Display the same values using a different культура.
   ...
   *   Println("Sortable:                %s", сейчас.вТкст("s", культура));
   *   Println("Год:                    %s", сейчас.вТкст("Y", культура));
   *   Println();
   * }
   *
   * // Produces the following вывод:
   * // Текущий дата and время: 26/05/2006 10:04:57 AM
   * //
...
public ткст форматируйДатуВремя (ткст вывод, Время dateTime, ткст форматируй, ИСлужбаФормата службаФормата = пусто) 
{
    return форматируйДатуВремя (вывод, dateTime, форматируй, ФорматДатыВремени.дайЭкземпляр(службаФормата));
}

ткст форматируйДатуВремя (ткст вывод, Время dateTime, ткст форматируй, ФорматДатыВремени dtf)
{
        /**********************************************************************

        **********************************************************************/

        ткст разверниИзвестныйФормат(ткст форматируй, ref Время dateTime)
        {
                ткст f;
...
version (Full)
{
                       case 'u':
                            ...
                            break;
                       case 'U':
                            dtf = cast(ФорматДатыВремени) dtf.clone();
                            dateTime = dateTime.toUniversalTime();
                       ...
                       case 'Y':
                            ...
                            break;
                       default:
                           throw new ИсклНелегальногоАргумента("Invalid дата форматируй.");
                       }
        ...
        **********************************************************************/
        ...
        ткст форматируйОсобо (ref Результат результат, Время dateTime, ткст форматируй)
        {

                цел повториРазбор(ткст форматируй, цел поз, сим c)
                {
                        цел n = поз + 1;
                ...
                ткст форматируйДеньНедели(Calendar.ДеньНедели ДеньНедели, цел rpt)
                {
                        if (rpt is 3)
                                return dtf.getAbbreviatedDayName(ДеньНедели);
                        return dtf.getDayName(ДеньНедели);
                }

                ткст форматируйМесяц(цел месяц, цел rpt)
                {
                        if (rpt is 3)
                                return dtf.getAbbreviatedMonthName(месяц);
                        return dtf.getMonthName(месяц);
                }
                ...
                цел разборКавычек(ткст форматируй, цел поз, out ткст результат)
                {
                        цел старт = поз;
                ...
                if (форматируй[0] is '%')
                    {
                    // specifiers for Всё стандарт форматируй strings and custom ones
                    const ткст commonSpecs = "dmMsty";
                    foreach (c; commonSpecs)
                        if (форматируй[1] is c)
                            {
                            индекс += 1;
                            break;
                            }
                ...
                while (индекс < форматируй.length)
                      ...
                      switch (c)
                             ...
                             case 'd':  // день
                                  ...
                                  if (длин <= 2)
                                     {
                                     цел день = calendar.дайДеньМесяца(dateTime);
                                     результат ~= форматируйЦел (врем, день, длин);
                                     }
                                  else
                                     результат ~= форматируйДеньНедели(calendar.дайДеньНедели(dateTime), длин);
                                  justTime = нет;
                                  break;

                             case 'M':  // месяц
                                  длин = повториРазбор(форматируй, индекс, c);
                                  цел месяц = calendar.дайМесяц(dateTime);
                                  if (длин <= 2)
                             ...
                             case 'y':  // год
                                  длин = повториРазбор(форматируй, индекс, c);
                                  цел год = calendar.дайГод(dateTime);
                                  ...
                                  if (длин is 1)
                                     ...
                                     if (время.часы < 12)
                                        ...
                                        if (dtf.amDesignator.length != 0)
                                            результат ~= dtf.amDesignator[0];
                                        }
                                     else
                                        {
                                        if (dtf.pmDesignator.length != 0)
                                            результат ~= dtf.pmDesignator[0];
                                        }
                                     }
                                  else
                                     результат ~= (время.часы < 12) ? dtf.amDesignator : dtf.pmDesignator;
                                  break;
                             case 'z':  // timezone смещение
                                  длин = повториРазбор(форматируй, индекс, c);
version (Full)
{
                                  ИнтервалВремени смещение = (justTime && dateTime.тики < TICKS_PER_DAY)
                                                     ? TimeZone.current.getUtcOffset(Куранты.сейчас)
...
else
{
                             ...
                             case '\'':  // сим literal
                                  ткст quote;
                                  длин = разборКавычек(форматируй, индекс, quote);
                                  результат ~= quote;
                                  break;
                             default:
                                 длин = 1;
        ...
        if (форматируй is пусто)
            форматируй = "G"; // Default в_ general форматируй.

        if (форматируй.length is 1) // It might be one of our shortcuts.
            форматируй = разверниИзвестныйФормат (форматируй, dateTime);

        return форматируйОсобо (результат, dateTime, форматируй);
}
...
private extern (C) сим* ecvt(дво d, цел digits, out цел decpt, out бул sign);
...
// Must match ФорматЧисла.валютнПоложитОбразец
package const   ткст[] positiveCurrencyFormats =
                [
                "$#", "#$", "$ #", "# $"
                ];

// Must match ФорматЧисла.валютнОтрицатОбразец
...
ткст долВТкст (ткст буфер, дол значение, цел digits, ткст negativeSign)
{
        if (digits < 1)
            digits = 1;
        ...
        if (значение < 0)
           {
           for (цел i = negativeSign.length - 1; i >= 0; i--)
                буфер[--n] = negativeSign[i];
           }
...
ткст долВГексТкст (ткст буфер, бдол значение, цел digits, сим форматируй)
{
        if (digits < 1)
            digits = 1;

        цел n = буфер.length;
        while (--digits >= 0 || значение != 0)
              {
              auto v = cast(бцел) значение & 0xF;
              буфер[--n] = cast(сим)((v < 10) ? v + '0' : v + форматируй - ('X' - 'A' + 10));
              значение >>= 4;
              }
...
ткст долВБинТкст (ткст буфер, бдол значение, цел digits)
{
        if (digits < 1)
            digits = 1;
...
сим разборОпределенияФормата (ткст форматируй, out цел length)
{
        цел     i = -1;
        сим    specifier;

        if (форматируй.length)
           {
           auto s = форматируй[0];
...
ткст форматируйЦелое (ткст вывод, дол значение, ткст форматируй, ФорматЧисла nf)
{
        цел     length;
        auto    specifier = разборОпределенияФормата (форматируй, length);
        ...
        switch (specifier)
               ...
               case 'd':
               case 'D':
                    return долВТкст (вывод, значение, length, nf.negativeSign);
        ...
        return число.вТкстФормат (результат, форматируй, nf);
}
...
ткст форматируйДво (ткст вывод, дво значение, ткст форматируй, ФорматЧисла nf)
{
        ...
        цел точность = 6;
        Результат результат = Результат (вывод);
        сим specifier = разборОпределенияФормата (форматируй, length);
        ...
        switch (specifier)
               ...
               case 'R':
                    ...
                    if (число.шкала == ФЛАГ_БЕСК)
                        return число.sign ? nf.negativeInfinitySymbol
...
проц форматируйВОбщем (ref Число число, ref Результат мишень, цел length, сим форматируй, ФорматЧисла nf)
{
        цел поз = число.шкала;

        auto p = число.digits.ptr;
        if (поз > 0)
           {
           while (поз > 0)
                 {
...
{
        ткст форматируй = число.sign ? negativeNumberFormats[nf.члоОтрицатОбразцов]
                                    : positiveNumberFormat;

        // Parse the форматируй.
        foreach (c; форматируй)
                {
                switch (c)
                       {
                       case '#':
                            форматируйФиксированно (число, мишень, length, nf.numberGroupSizes,
                                         nf.разделительЧисловыхДесятков, nf.разделительЧисловыхГрупп);
                            break;

                       case '-':
                            мишень ~= nf.negativeSign;
                            break;
...
{
        ткст форматируй = число.sign ? negativeCurrencyFormats[nf.валютнОтрицатОбразец]
        ...
        foreach (c; форматируй)
                ...
                switch (c)
                       {
                       case '#':
                            форматируйФиксированно (число, мишень, length, nf.currencyGroupSizes,
...
проц форматируйФиксированно (ref Число число, ref Результат мишень, цел length,
                  цел[] groupSizes, ткст decimalSeparator, ткст groupSeparator)
{
        цел поз = число.шкала;
        auto p = число.digits.ptr;
        ...
        if (поз > 0)
           ...
           if (groupSizes.length != 0)
              {
              // Calculate whether we have enough digits в_ форматируй.
              ...
              // Insert the разделитель according в_ groupSizes.
              ...
              // questionable: use the back конец of the вывод буфер в_
              // форматируй the разделители, and then копируй back в_ старт
              ткст temp = мишень.черновик;
              бцел ii = temp.length;

              for (цел c, i = поз - 1; i >= 0; i--)
                  {
                  temp[--ii] = (i < старт) ? число.digits[i] : '0';
                  if (размер > 0)
                     {
                     c++;
                     if (c == размер && i != 0)
...
ткст вТкст (ref Число число, ref Результат результат, сим форматируй, цел length, ФорматЧисла nf)
{
        switch (форматируй)
               {
               case 'c':
               ...
               case 'G':
                     ...
                     форматируйВОбщем (число, результат, length, (форматируй == 'g') ? 'e' : 'E', nf);
                     break;

               default:
                     return "{не_годится FP форматируй specifier '" ~ форматируй ~ "'}";
               }
        return результат.получи;
}
...
private struct Число
{
        ...
        **********************************************************************/
        ...
        private static Число opCall (дол значение)
        {
                ...
                число.digits[0 .. конец] = буфер[n .. n + конец];
                число.digits[конец] = '\0';
        ...
        **********************************************************************/
        ...
        private static Число opCall (дво значение, цел точность)
        {
                ...
                if (эксп == ЭКСП)
                   ...
                   сим* chars = ecvt(значение, число.точность, число.шкала, число.sign);
                   if (*chars != '\0')
                      {
                      while (*chars != '\0')
                             *p++ = *chars++;
        ...
        **********************************************************************/
        ...
        private бул вДво(out дво значение)
        {
                ...
                цел шкала = this.шкала - (счёт - left);
        ...
        **********************************************************************/
        ...
        private ткст вТкстФормат (ref Результат результат, ткст форматируй, ФорматЧисла nf)
        {
                бул hasGroups;
                цел groupCount;
                цел groupPos = -1, pointPos = -1;
                цел first = цел.max, последний, счёт;
                бул scientific;
                цел n;
                сим c;

                while (n < форматируй.length)
                      {
                      c = форматируй[n++];
                      switch (c)
                             {
                             ...
                             case '\'':
                             case '\"':
                                   while (n < форматируй.length && форматируй[n++] != c)
                                         {}
                                   break;

                             case '\\':
                                  if (n < форматируй.length)
                                      n++;
                                  break;
                ...
                if (hasGroups)
                   ...
                   if (nf.numberGroupSizes.length == 0)
                      ...
                      {
                      цел groupSizesTotal = nf.numberGroupSizes[0];
        ...
        **********************************************************************/
        ...
        {
                цел индекс;
                while (индекс < поз && digits[индекс] != '\0')
                       индекс++;

                if (индекс == поз && digits[индекс] >= '5')
                   {
                   while (индекс > 0 && digits[индекс - 1] == '9')
                          индекс--;
                   ...
                   while (индекс > 0 && digits[индекс - 1] == '0')
                          индекс--;
