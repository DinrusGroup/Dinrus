...
package Время парсируйВремя(ткст s, ФорматДатыВремени dtf) {
  РезультатРазбораДатыВремени результат;
  if (!пробуйРазборТочноНесколько(s, dtf.дайВсеОбразцыДатыВремени(), dtf, результат))
    throw new ИсклНелегальногоАргумента("Строка was not a valid Время.");
...
private бул пробуйРазборТочноНесколько(ткст s, ткст[] formats, ФорматДатыВремени dtf, ref РезультатРазбораДатыВремени результат) {
  foreach (ткст форматируй; formats) {
    if (пробуйРазборТочно(s, форматируй, dtf, результат))
      return да;
  }
  return нет;
...
private бул пробуйРазборТочно(ткст s, ткст образец, ФорматДатыВремени dtf, ref РезультатРазбораДатыВремени результат) {
  ...
  бул doParse() {
    ...
    цел разборЦифр(ткст s, ref цел поз, цел max) {
    ...
    цел разбериНесколько(ткст s, ref цел поз, ткст[] values ...) {
      цел результат = -1, max;
      foreach (цел i, ткст значение; values) {
        if (значение.length == 0 || s.length - поз < значение.length)
          continue;

        if (s[поз .. поз + значение.length] == значение) {
          if (результат == 0 || значение.length > max) {
            результат = i + 1;
            max = значение.length;
          }
        }
    ...
    ИнтервалВремени parseTimeZoneOffset(ткст s, ref цел поз) {
      бул sign;
      if (поз < s.length) {
        if (s[поз] == '-') {
          sign = да;
      ...
      //Due в_ dmd bug, this doesn't компилируй
      //ИнтервалВремени результат = ИнтервалВремени.часы(час) +  ИнтервалВремени.минуты(минута);
      ИнтервалВремени результат = ИнтервалВремени(ИнтервалВремени.ТиковВЧас * час +  ИнтервалВремени.ТиковВМин * минута);
      if (sign)
        результат = -результат;
      return результат;
    }
    ...
    результат.calendar = dtf.calendar;
    результат.год = результат.месяц = результат.день = -1;
    результат.час = результат.минута = результат.сукунда = 0;
    результат.дробь = 0.0;
      ...
      switch (c) {
        case 'd': // день
          if (счёт == 1 || счёт == 2)
            ...
            результат.день = разбериНесколько(s, i, dtf.abbreviatedDayNames);
          else
            результат.день = разбериНесколько(s, i, dtf.dayNames);
          if (результат.день == -1)
            return нет;
          break;
        case 'M': // месяц
          if (счёт == 1 || счёт == 2)
            ...
            результат.месяц = разбериНесколько(s, i, dtf.abbreviatedMonthNames);
          else
            результат.месяц = разбериНесколько(s, i, dtf.monthNames);
        ...
        case 'm': // минута
          ...
          break;
        case 's': // сукунда
          результат.сукунда = разборЦифр(s, i, 2);
          break;
        case 't': // время mark
          if (счёт == 1)
            результат.timeMark = разбериНесколько(s, i, stringOf(dtf.amDesignator[0]), stringOf(dtf.pmDesignator[0]));
          else
            результат.timeMark = разбериНесколько(s, i, dtf.amDesignator, dtf.pmDesignator);
          break;
        case 'z':
          результат.timeZoneOffset = parseTimeZoneOffset(s, i);
          break;
        default:
          break;
      }
    ...
    if (результат.timeMark == 1) { // am
      if (результат.час == 12)
        результат.час = 0;
    }
    else if (результат.timeMark == 2) { // pm
      if (результат.час < 12)
        результат.час += 12;
    }
    ...
    if (результат.год == -1 || результат.месяц == -1 || результат.день == -1) {
      ...
      if (результат.месяц == -1 && результат.день == -1) {
        if (результат.год == -1) {
          результат.год = результат.calendar.дайГод(сейчас);
          результат.месяц = результат.calendar.дайМесяц(сейчас);
          результат.день = результат.calendar.дайДеньМесяца(сейчас);
        }
        else
  ...
  if (doParse()) {
    результат.разобраннаяДата = результат.calendar.воВремя(результат.год, результат.месяц, результат.день, результат.час, результат.минута, результат.сукунда, 0);
    return да;
  }
  return нет;
}
