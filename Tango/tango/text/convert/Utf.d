/*******************************************************************************
        ...
        conversion, but есть little effect on the другие.
        ...
        registers. On a 3GHz P4, the дим/сим conversions take around
        2500ns в_ process an Массив of 1000 ASCII elements. Invoking the
        память manager doubles that период, and quadruples the время for
        массивы of 100 elements. Memory allocation can slow down notably
        ...
        transcoding between utf16 and utf8. Such cases are consопрered
        ...
        целое is returned indicating как many elements of the ввод
...
public extern (C) проц onUnicodeError (ткст сооб, т_мера инд = 0);
...
ткст  вТкст (ткст ист, ткст приёмн, бцел* ate=пусто) {return ист;}
шим[] вТкст (шим[] ист, шим[] приёмн, бцел* ate=пусто) {return ист;}
дим[] вТкст (дим[] ист, дим[] приёмн, бцел* ate=пусто) {return ист;}
...
/*******************************************************************************
        ...
        If the вывод is provопрed off the stack, it should be large
        ...
        elements consumed из_ the ввод, and the вывод буфер 
...
ткст вТкст (шим[] ввод, ткст вывод=пусто, бцел* ate=пусто)
{
        ...
        foreach (цел eaten, шим b; ввод)
                ...
                if (pOut > pMax)
                   {
                   // if Потокing, just return the unused ввод
                   if (ate)
                      {
                      *ate = eaten;
                      break;
                ...
                if (b < 0x80)
                   ...
                   if (b < 0x0800)
                      ...
                      if (b < 0xd800 || b > 0xdfff)
                         ...
                         return вТкст (toString32(ввод, пусто, ate), вывод);
                }
        ...
        foreach (ref шим d; вывод)
                ...
                if (b & 0x80)
                    if (b < 0xe0)
                       ...
                       if (b < 0xf0)
                          ...
                          // deal with surrogate-pairs
                          return вТкст16 (toString32(ввод, пусто, ate), вывод);
                ...
                if (++pIn >= pMax)
                    if (pIn > pMax)
                       ...
                       onUnicodeError ("Unicode.вТкст16 : incomplete utf8 ввод", pIn - ввод.ptr);
                       }
                    else
                       break;
                }
                   ...
                   if (b < 0x0800)
                      ...
                      if (b < 0x10000)
                         ...
                         if (b < 0x110000)
                            ...
                            onUnicodeError ("Unicode.вТкст : непригодный символ дим", eaten);
                }
...
дим[] toString32 (ткст ввод, дим[] вывод=пусто, бцел* ate=пусто)
{
        цел     produced;
        сим*   pIn = ввод.ptr;
        сим*   pMax = pIn + ввод.length;
        ...
        foreach (ref дим d; вывод)
                ...
                if (b & 0x80)
                    if (b < 0xe0)
                       ...
                       if (b < 0xf0)
                          ...
                          if (b >= 0x110000)
                              onUnicodeError ("Unicode.toString32 : введённый utf8 непригоден", pIn - ввод.ptr);
                          pIn += 3;
                          }
                ...
                if (++pIn >= pMax)
                    if (pIn > pMax)
                       ...
                       onUnicodeError ("Unicode.toString32 : незавершённый ввод utf8", pIn - ввод.ptr);
                       }
                    else
                       break;
                }
        ...
        if (ate)
           ...
           if (pIn < pMax)
               // this should never happen!
               onUnicodeError ("Unicode.toString32 : utf8 перебор", pIn - ввод.ptr);
        ...
        foreach (цел eaten, дим b; ввод)
                ...
                if (b < 0x10000)
                   ...
                   if (b < 0x110000)
                      ...
                      onUnicodeError ("Unicode.вТкст16 : не_годится дим", eaten);
                }
...
дим[] toString32 (шим[] ввод, дим[] вывод=пусто, бцел* ate=пусто)
{
        цел     produced;
        шим*  pIn = ввод.ptr;
        ...
        foreach (ref дим d; вывод)
                ...
                if (b >= 0x110000)
                    onUnicodeError ("Unicode.toString32 :  введённый текст utf16 имеет ошибки", pIn - ввод.ptr);
                ...
                if (++pIn >= pMax)
                    if (pIn > pMax)
                       ...
                       onUnicodeError ("Unicode.toString32 : utf16 - ввод незавершённый", pIn - ввод.ptr);
                       }
                    else
                       break;
                }
        ...
        if (ate)
           ...
           if (pIn < pMax)
               // this should never happen!
               onUnicodeError ("Unicode.toString32 : utf16 перебор", pIn - ввод.ptr);
...
/*******************************************************************************
        ...
        Decodes a single дим из_ the given ист текст, and indicates как
...
{
        дим[1] ret;
        return toString32 (ист, ret, &ate)[0];
}
...
/*******************************************************************************
        ...
        Encode a дим преобр_в the provопрed приёмн Массив, and return a срез of 
...
/*******************************************************************************
        ...
        the original ввод where no conversion is required.

*******************************************************************************/

T[] fromString8(T) (ткст s, T[] приёмн)
{
        static if (is (T == сим))
                   return s;
...
T[] fromString16(T) (шим[] s, T[] приёмн)
{
        static if (is (T == шим))
                   return s;
...
T[] fromString32(T) (дим[] s, T[] приёмн)
{
        static if (is (T == дим))
                   return s;
...
T[] cropLeft(T) (T[] s)
{
        ...
        static if (is (T == шим))
                   // пропусти if first сим is a trailing surrogate
        ...
        if (s.length)
           ...
           static if (is (T == сим))
                      while (i && (s[i] & 0x80))
                             if ((s[i] & 0xc0) is 0xc0)
                                {
                                // located the first байт of a sequence
                                ббайт b = s[i];
                                цел d = s.length - i;
...
debug (Utf)
{
        ...
        проц main()
        {
                ...
                Квывод (cropLeft(s[0..$])).нс;
                Квывод (cropLeft(s[1..$])).нс;
                Квывод (cropLeft(s[2..$])).нс;
                Квывод (cropLeft(s[3..$])).нс;
                Квывод (cropLeft(s[4..$])).нс;
                Квывод (cropLeft(s[5..$])).нс;

                Квывод (cropRight(s[0..$])).нс;
                Квывод (cropRight(s[0..$-1])).нс;
                Квывод (cropRight(s[0..$-2])).нс;
                Квывод (cropRight(s[0..$-3])).нс;
                Квывод (cropRight(s[0..$-4])).нс;
                Квывод (cropRight(s[0..$-5])).нс;
        }
}
