/*******************************************************************************
        ...
        This module provопрes a general-purpose formatting system в_
        преобразуй values в_ текст suitable for display. There is support
        for alignment, justification, and common форматируй specifiers for
        numbers.

        Выкладка can be customized via configuring various handlers and
        associated meta-данные. This is utilized в_ plug in текст.locale
        for handling custom formats, дата/время and культура-specific
        conversions.

        The форматируй notation is influenced by that used by the .NET
        and ICU frameworks, rather than C-стиль printf or D-стиль
        writef notation.
...
class Выкладка(T)
{
        ...
        public alias бцел delegate (T[]) Sink;
       
        static if (is (typeof(МестнДатаВремя)))
                   private МестнДатаВремя* dateTime = &ДефДатаВремя;
        ...
        **********************************************************************/
        ...
        /**********************************************************************
                ...
                items of the form: $(BR)$(BR)
                  ...
                  $(UL $(LI индекс $(BR)
                    An целое indicating the element in a список в_ _format.)
                  $(LI alignment $(BR)
                ...
                characters.$(BR)$(BR)
                If строкаФмт is "{0} bottles of beer on the wall" and the
        ...
        **********************************************************************/
        ...
        public final бцел преобразуй (Sink сток, T[] строкаФмт, ...)
        {
...
version (old)
{
        ...
        **********************************************************************/
        ...
        public final T[] convertOne (T[] результат, ИнфОТипе ti, Арг арг)
        {
                return депешируй (результат, пусто, ti, арг);
        }
}
        /**********************************************************************
        ...
        **********************************************************************/
        ...
        public final бцел преобразуй (Sink сток, ИнфОТипе[] аргументы, АргСписок арги, T[] строкаФмт)
        {
                assert (строкаФмт, "пустой определитель формата"w);
                assert (аргументы.length < 64, "слишком много аргов в Выкладка.преобразуй"w);
                ...
                version (GNU)
                        ...
                        foreach (i, арг; аргументы)
                                 ...
                                 * the stack, they must be использовался by the
                                 * correct тип. */
        ...
        /**********************************************************************
                ...
                Parse the форматируй-ткст, излейting formatted арги and текст
                fragments as we go

        **********************************************************************/

        private бцел разбор (T[] выкладка, ИнфОТипе[] ti, Арг[] арги, Sink сток)
        {
                ...
                while (да)
                      ...
                      if (*s != '}')
                         {
                         length += сток ("{ошибочный формат}"w);
                         continue;
                         }

                      // check for default индекс & установи следщ default counter
                      if (! indexed)
                            индекс = nextIndex;
                      nextIndex = индекс + 1;
                                ...
                                if (crop)
                                   ...
                                   if (паддинг < 0)
                                      ...
                                      if (left)
                                         {
                                         length += сток ("...");
                                         length += сток (Utf.cropLeft (ткт[-паддинг..$]));
                                         }
                                      else
                                         {
                                         length += сток (Utf.cropRight (ткт[0..width]));
                                         length += сток ("...");
                                         }
                                      }
                                   else
                      ...
                      {
                                // Because Вариантs can contain AAs (and maybe
...
version (WithВариант)
{
                                ...
                                if (_ti.classinfo.имя.length is 25 && _ti.classinfo.имя[9..$] == "AssociativeArray")
                                   ...
                                   alias ббайт AV; // any тип for ключ, значение might be ok, the sizes are corrected later
                                   alias ббайт AK;
                                   ...
                                   цел roundUp (цел sz)
                                   {
                                        return (sz + (проц*).sizeof -1) & ~((проц*).sizeof - 1);
                                   }
                                ...
                                if (_ti.classinfo.имя[9] is КодТипа.ARRAY)
                                   ...
                                   if (_ti is typeid(ткст))
                                       излей (Utf.fromString8 (*cast(ткст*) _arg, результат));
                                   else
                                   if (_ti is typeid(шим[]))        
                                       излей (Utf.fromString16 (*cast(шим[]*) _arg, результат));
                                   else
                                   if (_ti is typeid(дим[]))
                                       излей (Utf.fromString32 (*cast(дим[]*) _arg, результат));
                                   else
                                      {
                      ...
                      if (индекс >= ti.length)
                          излей ("{неверный индекс}"w);
                      else
                         process (ti[индекс], арги[индекс]);
                      }
                return length;
        }
        ...
        ***********************************************************************/
        ...
        private T[] депешируй (T[] результат, T[] формат, ИнфОТипе тип, Арг p)
        {
                switch (тип.classinfo.имя[9])
                       ...
                       case КодТипа.CHAR:
                            return Utf.fromString8 ((cast(сим*) p)[0..1], результат);

                       case КодТипа.WCHAR:
                            return Utf.fromString16 ((cast(шим*) p)[0..1], результат);

                       case КодТипа.DCHAR:
                            return Utf.fromString32 ((cast(дим*) p)[0..1], результат);
                       ...
                       case КодТипа.CLASS:
                            ...
                            if (c)
                                return Utf.fromString8 (c.вТкст, результат);
                            break;
                       ...
                       case КодТипа.STRUCT:
                            ...
                            if (s.xtoString) 
                               ...
                               return Utf.fromString8 (вТкст(), результат);
                               }
                            goto default;
                       ...
                       case КодТипа.INTERFACE:
                            ...
                            if (x)
                               ...
                               auto o = cast(Объект)(*cast(проц**)p - pi.смещение);
                               return Utf.fromString8 (o.вТкст, результат);
                               }
                            break;
        ...
        **********************************************************************/
        ...
        version (WithExtensions)
                {
                результат = Extensions!(T).run (тип, результат, p, формат);
                return (результат) ? результат : 
                       "{необрабатываемый тип аргумента: " ~ Utf.fromString8 (тип.вТкст, результат) ~ "}";
                }
             else
                version (WithDateTime)
                {
                if (тип is typeid(Время))
                   {
                   static if (is (T == сим))
                              return dateTime.форматируй(результат, *cast(Время*) p, формат);
                          else
                             {
                             // TODO: this needs в_ be cleaned up
                             сим[128] tmp0 =void;
                             сим[128] tmp1 =void;
                             return Utf.fromString8(dateTime.форматируй(tmp0, *cast(Время*) p, Utf.вТкст(формат, tmp1)), результат);
                             }
                   }
                }
                return "{необрабатываемый тип аргумента: " ~ Utf.fromString8 (тип.вТкст, результат) ~ "}";
        }
        ...
        **********************************************************************/
        ...
        protected T[] плавающее (T[] вывод, реал v, T[] формат)
        {
                ...
                return Float.форматируй (вывод, v, dec, эксп, pad);
        }
        ...
        **********************************************************************/
        ...
        private бцел пробелы (Sink сток, цел счёт)
        {
                бцел ret;
        ...
        **********************************************************************/
        ...
        /**********************************************************************
                ...
                formats a floating-point значение, and appends a хвост в_ it
...
debug (UnitTest)
{
        ...
        f[ 1.0 ] = "one".dup;
        f[ 3.14 ] = "PI".dup;
        assert( Форматировщик( "{}", f ) == "{1.00 => one, 3.14 => PI}" ||
                Форматировщик( "{}", f ) == "{3.14 => PI, 1.00 => one}");
        }
}
