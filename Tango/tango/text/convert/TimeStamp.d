/*******************************************************************************
        ...
        Converts between исконный and текст representations of HTTP время
        values. Internally, время is represented as UTC with an эпоха 
        fixed at Jan 1st 1970. The текст representation is formatted in
        accordance with RFC 1123, and the парсер will прими one of 
...
/******************************************************************************
        ...
        Parse provопрed ввод and return a UTC эпоха время. An исключение
...
/******************************************************************************
        ...
        Converts в_ the форматируй "Sun, 06 Nov 1994 08:49:37 GMT", and
        returns a populated срез of the provопрed буфер. Note that
        RFC1123 форматируй is always in абсолютный GMT время, and a thirty-
        element буфер is sufficient for the produced вывод
...
T[] форматируй(T) (T[] вывод, Время t)
{
        ...
        // преобразуй время в_ field values
        auto время = t.время;
        auto дата = Gregorian.generic.вДату (t);
...
/******************************************************************************
      ...
      Parse provопрed ввод and return a UTC эпоха время. A return значение 
...
бул разбор(T) (T[] ист, ref ВремяДня tod, ref Date дата, бцел* ate = пусто)
{
        т_мера длин;
...
/******************************************************************************
        ...
        Returns the число of elements consumed by the разбор; zero if
        the разбор неудачно
...
т_мера rfc1123(T) (T[] ист, ref Время значение)
{
        ...
        if (r)   
            значение = Gregorian.generic.воВремя(дата, tod);
        return r;
}
...
т_мера rfc1123(T) (T[] ист, ref ВремяДня tod, ref Date дата)
{
        T* p = ист.ptr;
        T* e = p + ист.length;
...
т_мера rfc850(T) (T[] ист, ref ВремяДня tod, ref Date дата)
{
        T* p = ист.ptr;
        T* e = p + ист.length;
...
т_мера asctime(T) (T[] ист, ref ВремяДня tod, ref Date дата)
{
        T* p = ист.ptr;
        T* e = p + ист.length;
...
т_мера dostime(T) (T[] ист, ref ВремяДня tod, ref Date дата)
{
        T* p = ист.ptr;
        T* e = p + ист.length;
...
/******************************************************************************
        ...
        "Decimal fractions may also be добавьed в_ any of the three время elements.
        ...
        CGPM in 2003), is used as a разделитель between the время element and
        its дробь."
...
т_мера iso8601(T) (T[] ист, ref ВремяДня tod, ref Date дата)
{
        T* p = ист.ptr;
        T* e = p + ист.length;
