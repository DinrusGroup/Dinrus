/*******************************************************************************
        ...
        Acknowledgements: 
                   ...
                   SAX as an API, and then had the foresight в_ place
 ...
 * события, it реализует this interface and registers an экземпляр with 
 ...
 * basic document-related события like the старт and конец of elements 
 * and character данные.</p>
 *
 * <p>The order of события in this interface is very important, and
 * mirrors the order of information in the document itself.  For
 * example, все of an element's контент (character данные, processing
 * instructions, and/or subelements) will appear, in order, between
 * the startElement событие and the corresponding endElement событие.</p>
 *
 ...
 * and for reporting skИПped entities (in non-validating XML
 * processors).</p>
 *
...
public class SaxHandler(Ch = сим) {
         ...
         * required) в_ supply a locator: if it does so, it must supply
         ...
         * character контент that does not match an application's
         ...
         * documentation for this метод and the documentation for {@link
         * org.xml.sax.ErrorHandler#fatalError}.  Until this ambiguity is
         * resolved in a future major release, clients should сделай no
         * assumptions about whether endDocument() will or will not be
         * invoked when the парсер есть reported a fatalError() or thrown
         * an исключение.</strong></p>
         *
         * <p>The SAX парсер will invoke this метод only once, and it will
         * be the последний метод invoked during the разбор.  The парсер shall
         * not invoke this метод until it есть either abandoned parsing
         ...
         * use prefixes in character данные or in attribute values,
         ...
         * and все {@link #endPrefixMapping endPrefixMapping}
         ...
         * "xml" префикс, since it is predeclared and immutable.</p>
         *
         * @param префикс the Namespace префикс being declared.
         * An пустой ткст is used for the default element namespace,
         * which есть no префикс.
        ...
        /*******************************************************************************
         * End the scope of a префикс-URI mapping.
         *
         ...
         * This is the пустой ткст when a default mapping scope ends.
         * @throws org.xml.sax.SAXException the клиент may throw
         *            an исключение during processing
        ...
        /*******************************************************************************
         * принять notification of the beginning of an element.
         *
         ...
         * (even when the element is пустой). все of the element's контент will be
         ...
         *
         * <ul>
         * <li>the Namespace URI and local имя are required when 
         * the namespaces property is <var>да</var> (the default), and are
         * optional when the namespaces property is <var>нет</var> (if one is
         * specified, Всё must be);</li>
         * <li>the qualified имя is required when the namespace-prefixes property
         * is <var>да</var>, and is optional when the namespace-prefixes property
         * is <var>нет</var> (the default).</li>
         * </ul>
         *
         * <p>Note that the attribute список provопрed will contain only
         * атрибуты with explicit values (specified or defaulted):
         * #IMPLIED атрибуты will be omitted.  The attribute список
         * will contain атрибуты used for Namespace declarations
         * (xmlns* атрибуты) only if the
         * <код>http://xml.org/sax/features/namespace-prefixes</код>
         * property is да (it is нет by default, and support for a 
         ...
         *        element есть no Namespace URI or if Namespace
         ...
         * @param atts the атрибуты attached в_ the element.  If
         ...
         *
         * <p>The Parser will вызов this метод в_ report each chunk of
         * character данные.  SAX parsers may return все contiguous character
         * данные in a single chunk, or they may разбей it преобр_в several
         * chunks; however, все of the characters in any single событие
         ...
         * "Astral Planes", with a код point above U+FFFF.  A сукунда case involves
         ...
         * processing instruction данные, and comments as well as in данные reported
         ...
         *
         * <p>Note that some parsers will report пробел in element
        ...
        /*******************************************************************************
         * принять notification of ignorable пробел in element контент.
         *
         * <p>Valопрating Parsers must use this метод в_ report each chunk
         * of пробел in element контент (see the W3C XML 1.0
         * recommendation, section 2.10): non-validating parsers may also
         * use this метод if they are capable of parsing and using
         * контент models.</p>
         *
         * <p>SAX parsers may return все contiguous пробел in a single
         * chunk, or they may разбей it преобр_в several chunks; however, все of
         ...
         * This is not called for сущность references within markup constructs
         * such as element старт tags or markup declarations.  (The XML
         * recommendation requires reporting skИПped external entities.
         ...
         * may пропусти external entities, depending on the values of the
         ...
         *        parameter сущность, the имя will begin with '%', and if
 ...
 * external entities (включая the external DTD поднабор and external
...
public interface EntityResolver(Ch = сим) {
         ...
         * сущность except the top-уровень document сущность.  Such entities include
         * the external DTD поднабор and external parameter entities referenced
         * within the DTD (in either case, only if the парсер reads external
         * parameter entities), and external general entities referenced
         * within the document element (if the парсер reads external general
         ...
         * system определители в_ безопасно and/or local URIs, в_ look up
         ...
         * как в_ interpret any InputSource returned by this метод, and that
 ...
 * handling, it must implement this interface and then регистрируй an
 * экземпляр with the XML читатель using the
 * {@link org.xml.sax.XMLЧитатель#setErrorHandler setErrorHandler}
 * метод.  The парсер will then report все ошибки and warnings
 ...
 * ошибки and warnings.  Note, however, that there is no requirement that 
...
public interface ErrorHandler(Ch = сим) {
         ...
         * recommendation.  The default behaviour is в_ take no
         * action.</p>
         *
         ...
         * validity constraint.  The default behaviour is в_ take no
         * action.</p>
         *
         ...
         * after the парсер есть invoked this метод, and should continue
         * (if at все) only for the sake of collecting добавьitional ошибка
         * messages: in fact, SAX parsers are free в_ stop reporting any
         * другой события once this метод есть been invoked.</p>
         *
         * @param исключение The ошибка information encapsulated in a
         *                  SAX разбор исключение.  
         * @исключение org.xml.sax.SAXException Any SAX исключение, possibly
 ...
 *
 * <p>Note that the results returned by the объект will be valid only
 ...
 *
 * <p>SAX parsers are not required в_ supply a locator, but they are
 ...
 * If no locator есть been установи by the время the application Приёмs
...
public interface Locator(Ch = сим) {
        ...
        /*******************************************************************************
         * Return the public определитель for the current document событие.
         *
        ...
        /*******************************************************************************
         * Return the system определитель for the current document событие.
         *
         ...
         * имя must always be provопрed as a <em>файл:...</em> URL, and другой
        ...
        /*******************************************************************************
         * Return the строка число where the current document событие ends.
         ...
         * In some cases, these "строка" numbers match what would be displayed
         * as columns, and in другие they may not match the источник текст
         ...
         * of the first character after the текст associated with the document 
         * событие.  The first строка is строка 1.</p>
         *
        ...
        /*******************************************************************************
         * Return the column число where the current document событие ends.
         * This is one-based число of Java <код>сим</код> values since
         ...
         * <p>If possible, the SAX driver should provопрe the строка позиция 
         * of the first character after the текст associated with the document 
         * событие.  The first column in each строка is column 1.</p>
         *
...
public class SAXException : Исключение {
        ...
        /*******************************************************************************
         * Create a new SAXException wrapping an existing исключение.
         *
         * <p>The existing исключение will be embedded in the new
         * one, and its сообщение will become the default сообщение for
         * the SAXException.</p>
         *
        ...
        /*******************************************************************************
         * Create a new SAXException из_ an existing исключение.
         *
         * <p>The existing исключение will be embedded in the new
         ...
         *
         * <p>If there is an embedded исключение, and if the SAXException
         * есть no detail сообщение of its own, this метод will return
...
class SaxParser(Ch = сим) : XMLЧитатель!(Ch), Locator!(Ch) {
        ...
        ////////////////////////////////////////////////////////////////////
        /*******************************************************************************
         * Look up the значение of a feature flag.
         *
         ...
         * and so forth.) </p>
         *
         * <p>все XMLЧитательs are required в_ recognize the
         ...
         * </pre>
         *
         * <p>Implementors are free (and encouraged) в_ invent their own features,
         * using names built on their own URIs.</p>
         *
         * @param имя The feature имя, which is a fully-qualified URI.
         * @return The current значение of the feature (да or нет).
         * @исключение org.xml.sax.SAXNotRecognizedException If the feature
         *            значение can't be assigned or retrieved.
        ...
        /*******************************************************************************
         * Набор the значение of a feature flag.
         *
         * <p>The feature имя is any fully-qualified URI.  It is
         * possible for an XMLЧитатель в_ expose a feature значение but
         * в_ be unable в_ change the current значение.
         * Some feature values may be immutable or mutable only 
         * in specific contexts, such as before, during, or after 
         * a разбор.</p>
         *
         * <p>все XMLЧитательs are required в_ support настройка
         * http://xml.org/sax/features/namespaces в_ да and
         ...
         *
         * <p>XMLЧитательs are not required в_ recognize any specific
         * property names, though an начальное core установи is documented for
         * SAX2.</p>
         *
         * <p>Implementors are free (and encouraged) в_ invent their own свойства,
         * using names built on their own URIs.</p>
         *
         * @param имя The property имя, which is a fully-qualified URI.
         * @return The current значение of the property.
         ...
         * a разбор.</p>
         *
         * <p>XMLЧитательs are not required в_ recognize настройка
         ...
         * mопрdle of a разбор, and the SAX парсер must begin using the new
        ...
        /*******************************************************************************
         * Return the current сущность resolver.
         *
         * @return The current сущность resolver, or пусто if Неук
         *         есть been registered.
        ...
        /*******************************************************************************
         * Return the current контент handler.
         *
         * @return The current контент handler, or пусто if Неук
         *         есть been registered.
         ...
         * ошибка handler в_ avoопр unexpected bugs.</p>
         *
         * <p>Applications may регистрируй a new or different handler in the
         * mопрdle of a разбор, and the SAX парсер must begin using the new
         * handler immediately.</p>
        ...
        /*******************************************************************************
         * Return the current ошибка handler.
         *
         * @return The current ошибка handler, or пусто if Неук
         *         есть been registered.
         * @see #setErrorHandler
         *******************************************************************************/
         ...
         * читатель в_ begin parsing an XML document из_ any valid ввод
         ...
         * Configuration of the XMLЧитатель объект (such as handler привязки and
         * values established for feature флаги and свойства) is unchanged
         ...
         * есть ended.  If a клиент application wants в_ терминируй 
         * parsing early, it should throw an исключение.</p>
         *
         * @param ввод The ввод источник for the top-уровень of the
         *        XML document.
         * @исключение org.xml.sax.SAXException Any SAX исключение, possibly
         *            wrapping другой исключение.
        ...
        /*******************************************************************************
         *         The meat of the class.  Turn the pull парсер's nodes преобр_в SAX
         *         события and шли out в_ the SaxHandler.
         *******************************************************************************/
        private проц doParse() {
                ...
                while (да) {
                        ...
                        default:
                                throw new SAXException("неизвестное парсер token тип");
                        }
                } 
foo:                              
...
public abstract class XMLFilter(Ch = сим) : XMLЧитатель {
         ...
         *
         * @return The предок фильтр, or пусто if Неук есть been установи.
         *******************************************************************************/
 ...
 * XMLЧитатель} and the клиент application's событие handlers.  By default, it
 * does nothing but пароль requests up в_ the читатель and события
...
public class XMLFilterImpl(Ch = сим) : SaxHandler, XMLFilter, EntityResolver, ErrorHandler {
         ...
         * this фильтр will obtain its события and в_ which it will пароль its 
         ...
         * @исключение org.xml.sax.SAXNotRecognizedException If the feature
         *            значение can't be assigned or retrieved из_ the предок.
         * @исключение org.xml.sax.SAXNotSupportedException When the
         ...
         *
         * @param имя The feature имя.
         * @return The current значение of the feature.
         * @исключение org.xml.sax.SAXNotRecognizedException If the feature
         *            значение can't be assigned or retrieved из_ the предок.
         * @исключение org.xml.sax.SAXNotSupportedException When the
         *            предок recognizes the feature имя but 
        ...
        /**
         * Get the current сущность resolver.
         *
         * @return The current сущность resolver, or пусто if Неук was установи.
         *******************************************************************************/
         ...
         *
         * @return The current контент handler, or пусто if Неук was установи.
         *******************************************************************************/
        ...
        /*******************************************************************************
         * Get the current ошибка событие handler.
         *
         * @return The current ошибка handler, or пусто if Неук was установи.
         *******************************************************************************/
         ...
         *******************************************************************************/
        public проц разбор(Ch[] контент) {
                //TODO FIXME - создай a буфер of this контент as the ввод поток, and then разбор.
                //setupParse();
                //предок.разбор(ввод);
        }
        ...
        /*******************************************************************************
         * Фильтр a старт Namespace префикс mapping событие.
         *
        ...
        /*******************************************************************************
         * Фильтр an конец Namespace префикс mapping событие.
         *
         * @param префикс The Namespace префикс.
         * @исключение org.xml.sax.SAXException The клиент may throw
         *            an исключение during processing.
        ...
        /*******************************************************************************
         * Фильтр a старт element событие.
         *
         * @param уир The element's Namespace URI, or the пустой ткст.
         * @param localName The element's local имя, or the пустой ткст.
         * @param qName The element's qualified (псеп_в_начале) имя, or the пустой
         *        ткст.
         * @param atts The element's атрибуты.
         * @исключение org.xml.sax.SAXException The клиент may throw
         *            an исключение during processing.
         *******************************************************************************/
        ...
        /*******************************************************************************
         * Фильтр an конец element событие.
         *
         * @param уир The element's Namespace URI, or the пустой ткст.
         * @param localName The element's local имя, or the пустой ткст.
         * @param qName The element's qualified (псеп_в_начале) имя, or the пустой
         ...
         *
         * <p>Before every разбор, check whether the предок is
         * non-пусто, and re-регистрируй the фильтр for все of the 
         * события.</p>
         *******************************************************************************/
 ...
 * implement.  This interface allows an application в_ установи and
 * запрос features and свойства in the парсер, в_ регистрируй
 * событие handlers for document processing, and в_ initiate
 * a document разбор.</p>
 *
 * <p>все SAX interfaces are assumed в_ be синхронно: the
 * {@link #разбор разбор} methods must not return until parsing
 * is complete, and readers must жди for an событие-handler обрвызов
 ...
 *
 * <ol>
 * <li>it добавьs a стандарт way в_ запрос and установи features and 
 *  свойства; and</li>
 * <li>it добавьs Namespace support, which is required for many
 *  higher-уровень XML standards.</li>
 * </ol>
 *
 * <p>There are adapters available в_ преобразуй a SAX1 Parser в_
 * a SAX2 XMLЧитатель and vice-versa.</p>
 *
 * @since SAX 2.0
 * @author David Megginson
 * @version 2.0.1+ (sax2r3pre1)
