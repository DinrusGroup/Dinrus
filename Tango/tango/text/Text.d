/*******************************************************************************
        ...
        Text is a class for managing and manИПulating Unicode character
        массивы.

        Text maintains a current "selection", controlled via the выбери()
        and search() methods. Each of добавь(), приставь(), замени() and
        удали() operate with respect в_ the selection.

        The search() methods also operate with respect в_ the current
        ...
        Indexes and lengths of контент always счёт код units, not код
        points. This is similar в_ traditional аски ткст handling, yet
        indexing is rarely used in practice due в_ the selection опрiom:
        ...
        Strings support a range of functionality, из_ вставь and removal
        в_ utf кодировка and decoding. There is also an immutable поднабор
        ...
        needed and thus immutability depends в_ an протяженность upon so-called
        ...
        The class is templated for use with ткст, шим[], and дим[],
        and should migrate across encodings seamlessly. In particular, все
        ...
        combining text.Text with text.Util e.g. lines of текст
        can be processed thusly:
        ---
        auto источник = new Text!(сим)("one\ntwo\nthree");

        foreach (строка; Util.lines(источник.срез))
        ...
        foreach (element; Util.delims ("все cows съешь grass", " "))
                 приёмн.приставь (element);
        ---

        Below is an overview of the API and class иерархия:
        ---
        class Text(T) : TextView!(T)
        {
                ...
                // установи and retrieve current selection point
                ...
                // return an iterator в_ перемести the selection around.
                // Also exposes "замени все" functionality
                Search search (T chr);
                Search search (T[] образец);

                // форматируй аргументы behind current selection
                Text форматируй (T[] форматируй, ...);

                // добавь behind current selection
                Text добавь (T[] текст);
                ...
                // вставь before current selection
                Text приставь (T[] текст);
                Text приставь (TextView другой);
                Text приставь (T chr, цел счёт=1);

                // замени current selection
                Text замени (T chr);
                Text замени (T[] текст);
                Text замени (TextView другой);

                // удали current selection
                Text удали ();

                // сотри контент
                Text сотри ();

                // trim leading and trailing пробел
                Text trim ();

                // trim leading and trailing chr instances
                Text strip (T chr);

                // обрежь at point, or current selection
                Text обрежь (цел point = цел.max);
        ...
        class UniText
        {
                ...
                abstract дим[] toString32 (дим[] приёмн = пусто);
        }

        struct Search
        {
                // выбери prior экземпляр
                бул prev();
...
class Text(T) : TextView!(T)
{
        ...
        ***********************************************************************/
        ...
        private struct Search(T)
        {
                ...
                ***************************************************************/
                ...
                static Search opCall (Text текст, T[] match)
                {
                        Search s =void;
                ...
                /***************************************************************
                        ...
                        Search backward, starting at the character prior в_
                        the selection point
                ...
                ***************************************************************/
                ...
                бул prev ()
                {
                        return locate (&engine.реверс, текст.срез, текст.point - 1);
                }
                ...
                ***************************************************************/
                ...
                бул следщ ()
                {
                        return locate (&engine.forward, текст.срез, 
                                        текст.selectPoint + текст.selectLength);
                }

                /***************************************************************

                        Returns да if there is a match within the 
                        associated текст
                ...
                ***************************************************************/
                ...
                бул within ()
                {       
                        return engine.within (текст.срез);
                }

                /***************************************************************
                
                        Returns число of matches within the associated
                        текст
                ...
                ***************************************************************/
                ...
                т_мера счёт ()
                {       
                        return engine.счёт (текст.срез);
                }

                /***************************************************************

                        Замени все matches with the given character
                ...
                ***************************************************************/
                ...
                /***************************************************************
                        ...
                        Замени все matches with the given substitution

                ***************************************************************/

                проц замени (T[] sub = пусто)
                {  
                        auto приёмн = new T[текст.length];
                        приёмн.length = 0;

                        foreach (token; engine.семы (текст.срез, sub))
                                 приёмн ~= token;
                ...
                /***************************************************************
                        ...
                        locate образец индекс and выбери as appropriate
                ...
                ***************************************************************/
                ...
                private бул locate (Call вызов, T[] контент, т_мера из_)
                {
                        ...
                        if (индекс < контент.length)
                           {
                           текст.выбери (индекс, engine.match.length);
                           return да;
                           }
                        return нет;
                }
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Create an пустой Text with the specified available
                пространство

                Note: A character like 'a' will be implicitly преобразованый в_
                бцел and thus will be accepted for this constructor, making
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Create a Text upon the provопрed контент. If saопр
                ...
                wrapping an Массив "temporarily" with a stack-based Text
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Create a Text via the контент of другой. If saопр
                контент is immutable (читай-only) then you might consопрer
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                not, the Массив is aliased until such время it is altered via
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Explicitly установи the current selection в_ the given старт and
                length. values are pinned в_ the контент extents
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Return the индекс and length of the current selection
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Набор the current selection point, and resets selection length
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Return a search iterator for a given образец. The iterator
                sets the current текст selection as appropriate. For example:
                ---
                ...
                // замени все instances of "world" with "everyone"
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Find and выбери the следщ occurrence of a BMP код point
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Find and выбери the следщ substring occurrence.  Returns
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Find and выбери a prior occurrence of a BMP код point
                in a ткст. Returns да if найдено, нет otherwise
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Find and выбери a prior substring occurrence. Returns
                да if найдено, нет otherwise
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Append formatted контент в_ this Text
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Append a счёт of characters в_ this Text
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Append контент из_ ввод поток at insertion point. Use
        ...
        ***********************************************************************/
        ...
        final Text добавь (ИПотокВвода источник)
        {
                ...
                while (да) 
                      ...
                      // check в_ ensure UTF conversion is ok
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Append кодирован текст at the current selection point. The текст
        ...
        ***********************************************************************/
        ...
        final Text кодируй (ткст s)
        {
                ...
                static if (is (T == дим))
                           return добавь (Utf.toString32(s, врем));
        }
        ...
        ***********************************************************************/
        ...
        {
                цел chunk = chars.length - selectLength;
                if (chunk >= 0)
                    расширь (selectPoint, chunk);
                else
                   удали (selectPoint, -chunk);
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Удали the selection из_ this Text and сбрось the
                selection в_ zero length (at the current позиция)
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                is в_ обрежь at the current добавь point. Текущий selection
                is moved в_ the truncation point, with length 0

        ***********************************************************************/

        final Text обрежь (цел индекс = цел.max)
        {
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Удали leading and trailing пробел из_ this Text,
                and сбрось the selection в_ the trimmed контент

        ***********************************************************************/

        final Text trim ()
        {
                контент = Util.trim (срез);
        ...
        /***********************************************************************
                ...
                Удали leading and trailing matches из_ this Text,
                and сбрось the selection в_ the очищенный контент

        ***********************************************************************/

        final Text strip (T matches)
        {
                контент = Util.strip (срез, matches);
                выбери (0, contentLength = контент.length);
                return this;
        }
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Return the length of the valid контент
        ...
        ***********************************************************************/
        ...
        final бул равно (T[] другой)
        {
                if (другой.length == contentLength)
                    return Util.matching (другой.ptr, контент.ptr, contentLength);
                return нет;
        }
        ...
        ***********************************************************************/
        ...
        final бул ends (T[] chars)
        {
                if (chars.length <= contentLength)
                    return Util.matching (контент.ptr+(contentLength-chars.length), chars.ptr, chars.length);
                return нет;
        }
        ...
        ***********************************************************************/
        ...
        {
                if (chars.length <= contentLength)
                    return Util.matching (контент.ptr, chars.ptr, chars.length);
                return нет;
        }

        /***********************************************************************

                Compare this Text старт with другой. Returns 0 if the
                контент matches, less than zero if this Text is "less"
                than the другой, or greater than zero where this Text
                is "bigger".

        ***********************************************************************/

        final цел compare (TextViewT другой)
        {
                if (другой is this)
                    return 0;

                return compare (другой.срез);
        }

        /***********************************************************************

                Compare this Text старт with an Массив. Returns 0 if the
                контент matches, less than zero if this Text is "less"
                than the другой, or greater than zero where this Text
                is "bigger".
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                приёмн will be resized as required в_ house the conversion.
                ...
                if (результат.length > буфер.length)
                    буфер = результат;
                ---
                You can also provопрe a буфер из_ the stack, but the вывод
        ...
        ***********************************************************************/
        ...
        /// ditto
        final дим[] toString32 (дим[] приёмн = пусто)
        {
                static if (is (T == сим))
                           return Utf.toString32 (срез, приёмн);
        ...
        /***********************************************************************
                ...
                Compare this Text в_ другой. We compare against другой
                Strings only. Literals and другой objects are not supported
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Pin the given индекс в_ a valid позиция.
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Pin the given индекс and length в_ a valid позиция.
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                Compare two массивы. Returns 0 if the контент matches, less
...
class TextView(T) : UniText
{
        ...
        ***********************************************************************/
        ...
        abstract цел compare (TextView другой);
...
{       
        ...
        assert (s.search("5").следщ);
        assert (s.selection == "5");
        assert (s.удали == "1234fubar");
        assert (s.search("fubar").следщ);
        assert (s.selection == "fubar");
        assert (s.search("wumpus").следщ is нет);
        assert (s.selection == "");

        assert (s.сотри.форматируй("{:f4}", 1.2345) == "1.2345");

        assert (s.сотри.форматируй("{:b}", 0xf0) == "11110000");

        assert (s.сотри.кодируй("one"d).вТкст == "one");

        assert (Util.splitLines(s.сотри.добавь("a\nb").срез).length is 2);

        assert (s.выбери.замени("almost ") == "almost ");
        foreach (element; Util.образцы ("все cows съешь grass", "съешь", "chew"))
                 s.добавь (element);
        assert (s.selection == "almost все cows chew grass");
        assert (s.сотри.форматируй("{}:{}", 1, 2) == "1:2");
        }
}
