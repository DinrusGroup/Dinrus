...
/*******************************************************************************
    ...
    This module is used в_ раскодируй and кодируй base64 ткст массивы. 
...
/*******************************************************************************
    ...
    encodes данные преобр_в buff and returns the число of байты кодирован.
    this will not терминируй and pad any "leftover" байты, and will instead
    ...
    bytesEncoded = inout that returns как much of the буфер was filled

*******************************************************************************/

цел encodeChunk(ббайт[] данные, ткст buff, inout цел bytesEncoded)
{
...
/*******************************************************************************
    ...
    encodes данные and returns as an ASCII base64 ткст.
...
body
{
    ...
    {
        цел bytesEncoded = 0;
        цел numBytes = encodeChunk(данные, buff, bytesEncoded);
        сим *rtnPtr = buff.ptr + bytesEncoded;
        ббайт *dataPtr = данные.ptr + numBytes;
...
/*******************************************************************************
    ...
    decodes an ASCCI base64 ткст and returns it as ббайт[] данные. Pre-allocates
...
/*******************************************************************************
    ...
    decodes an ASCCI base64 ткст and returns it as ббайт[] данные.
...
body
{
    ...
    if (данные.length > 0)
    {
        ...
        {
            pдобавьedPos++;
            ббайт current = _decodeTable[piece];
            if (current || piece == 'A')
            {
                endCount++;
        ...
        // this will try and раскодируй whatever is left, even if it isn't terminated properly (ie: missing последний one or two =)
        if (pдобавьedPos)
        {
...
version (Test)
{
    ...
    unittest    
    {
        Test.Status encodeChunktest(inout ткст[] messages)
        {
            ткст ткт = "Hello, как are you today?";
            ткст кодирован = new сим[allocateEncodeSize(cast(ббайт[])ткт)];
            цел bytesEncoded = 0;
            цел numBytesLeft = encodeChunk(cast(ббайт[])ткт, кодирован, bytesEncoded);
            ткст результат = кодирован[0..bytesEncoded] ~ кодируй(cast(ббайт[])ткт[numBytesLeft..$], кодирован[bytesEncoded..$]);
            if (результат == "SGVsbG8sIGhvdyBhcmUgeW91IHRvZGF5Pw==")
                return Test.Status.Success;
            return Test.Status.Failure;
        }
        Test.Status encodeTest(inout ткст[] messages)
        {
            ткст кодирован = new сим[allocateEncodeSize(cast(ббайт[])"Hello, как are you today?")];
        ...
        Test.Status decodeTest(inout ткст[] messages)
        {
            ббайт[1024] decoded;
        ...
        Test.Status speedTest(inout ткст[] messages)
        {
            Стдвыв("Reading...").нс;
...
//            ббайт[] данные = cast(ббайт[])FileData("blah.txt").читай;
        ...
        auto t = new Test("io.encode.Base64");
        t["Encode"] = &encodeTest;
        t["Encode Поток"] = &encodeChunktest;
        t["Decode"] = &decodeTest;
//        t["Speed"] = &speedTest;
//        t["Speed2"] = &speedTest2;
        t.run();
