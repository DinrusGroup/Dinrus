...
version (Posix)
{
    ...
    public class PollSelector: АбстрактныйСелектор
    {
         ...
         * PollSelector.
         */
        public const бцел DefaultSize = 64;

        /** Map в_ associate the провод handles with their selection ключи */
        private PollSelectionKey[ИВыбираемый.Дескр] _keys;
         ...
         * If a провод is already associated, modify the события and
         * атачмент.
         *
         * Параметры:
        ...
        in
        {
            assert(провод !is пусто && провод.ptr() >= 0);
        }
        body
        {
            debug (selector)
                Стдвыв.форматнс("--- PollSelector.регистрируй(укз={0}, события=0x{1:x})",
                              cast(цел) провод.ptr(), cast(бцел) события);

            PollSelectionKey* current = (провод.ptr() in _keys);

            if (current !is пусто)
            {
                debug (selector)
                    Стдвыв.форматнс("--- добавьing pollfd in индекс {0} (of {1})",
                                  current.индекс, _count);

                current.ключ.события = события;
                current.ключ.атачмент = атачмент;

                _pfds[current.индекс].события = cast(крат) события;
            }
            else
            {
                ...
                _pfds[_count].fd = провод.ptr();
                _pfds[_count].события = cast(крат) события;
                _pfds[_count].revents = 0;

                _keys[провод.ptr()] = new PollSelectionKey(провод, события, _count, атачмент);
                _count++;
            }
        }
        ...
        public проц отмениРег(ИВыбираемый провод)
        {
            if (провод !is пусто)
            {
                try
                {
                    ...
                    PollSelectionKey* removed = (провод.ptr() in _keys);
                    ...
                    if (removed !is пусто)
                    {
                        ...
                        // element в_ the removed slot
                        //
                        _pfds[removed.индекс] = _pfds[_count - 1];
                        _keys[cast(ИВыбираемый.Дескр)_pfds[removed.индекс].fd].индекс = removed.индекс;
                        _count--;

                        _keys.удали(провод.ptr());
                    }
                    else
                    {
                        debug (selector)
        ...
        public цел выбери(ИнтервалВремени таймаут)
        {
            ...
            // was interrupted by a signal and we need в_ restart it.
            while (да)
            {
                ...
                if (_eventCount < 0)
                {
                    ...
                    {
                        // The вызов в_ checkErrno() ends up throwing an исключение
    ...
    /**
     * Class used в_ hold the список of Conduits that have Приёмd события.
     */
    private class PollSelectionSet: ИНаборВыделений
    {
        pollfd[] fds;
        цел numSelected;
        PollSelectionKey[ИВыбираемый.Дескр] ключи;


        this(pollfd[] fds, цел numSelected, PollSelectionKey[ИВыбираемый.Дескр] ключи)
        {
            this.fds = fds;
        ...
        цел opApply(цел delegate(ref КлючВыбора) дг)
        {
            ...
            foreach (pfd; fds)
            {
                ...
                if(pfd.revents != 0)
                {
                    ...
                    auto k = (cast(ИВыбираемый.Дескр)pfd.fd) in ключи;
                    if(k !is пусто)
                    {
                        КлючВыбора current = k.ключ;
                        current.события = cast(Событие)pfd.revents;
                        if ((rc = дг(current)) != 0)
                        {
                            break;
                        }
