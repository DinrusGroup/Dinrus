...
version (linux)
{
     ...
     * options (PollSelector, СелекторВыбора). For small amounts of conduits
     ...
     *
     * selector.регистрируй(conduit1, Событие.Чтен, object1);
     * selector.регистрируй(conduit2, Событие.Write, object2);
     *
    ...
    public class EpollSelector: АбстрактныйСелектор
    {
        ...
        /** Число of события resulting из_ the вызов в_ выбери() */
        ...
        ~this()
        {
            ...
            // объект is garbage collected.
            закрой();
        }

        /**
         * Open the epoll selector, makes a вызов в_ epoll_create()
         *
         * Параметры:
         ...
         */
        public проц открой(бцел размер = DefaultSize, бцел maxEvents = DefaultMaxEvents)
        in
        {
            assert(размер > 0);
            assert(maxEvents > 0);
         ...
         * If a провод is already associated, changes the события and
         * атачмент.
         *
         * Параметры:
        ...
        body
        {
            ...
            if (ключ !is пусто)
            {
                ...
                событие.данные.ptr = cast(проц*) ключ;

                if (epoll_ctl(_epfd, EPOLL_CTL_MOD, провод.ptr(), &событие) != 0)
                {
                    checkErrno(__FILE__, __LINE__);
                }
            }
            else
            {
                ...
                // garbage collected while there is still a reference в_ it in
                // an epoll_event. This also allows в_ в_ efficiently найди the
                // ключ corresponding в_ a укз in methods where this
                // association is not provопрed automatically.
                _keys[провод.ptr()] = newkey;
                auto x = провод.ptr in _keys;
                событие.данные.ptr = cast(проц*) x;
                if (epoll_ctl(_epfd, EPOLL_CTL_добавь, провод.ptr(), &событие) != 0)
                {
        ...
        public проц отмениРег(ИВыбираемый провод)
        {
            if (провод !is пусто)
            {
                if (epoll_ctl(_epfd, EPOLL_CTL_DEL, провод.ptr(), пусто) == 0)
                {
                    _keys.удали(провод.ptr());
                }
                else
        ...
        public цел выбери(ИнтервалВремени таймаут)
        {
            ...
            while (да)
            {
                ...
                else
                {
                    ...
                    debug (selector)
                        Стдвыв("--- Restarting epoll_wait() after being interrupted by a signal\n");
                }
            }
            return _eventCount;
        }
                    ...
                    if (событие.события != 0)
                    {
                        ...
                        debug (selector)
                            Стдвыв.форматируй("---   Событие 0x{0:x} for укз {1}\n",
                                        cast(бцел) событие.события, cast(цел) ключ.провод.ptr());
