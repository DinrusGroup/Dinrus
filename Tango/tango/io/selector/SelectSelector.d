...
version (Windows)
{
    ...
    private
    {
        ...
        extern (Windows) цел выбери(цел nfds, набор_уд* readfds, набор_уд* writefds,
 ...
 * selector.регистрируй(сокет, Событие.Чтен);
 *
...
public class СелекторВыбора: АбстрактныйСелектор
{
    ...
    version (Posix)
    {
         ...
         * СелекторВыбора.
         */
        public const бцел DefaultSize = 1024;
    }
    else
    {
        /**
         ...
         * СелекторВыбора.
         */
        public const бцел DefaultSize = 63;
    }

    /**
     * Open the выбери()-based selector.
     *
     * Параметры:
     ...
     *                this значение is currently not used by this selector.
     */
    public проц открой(бцел размер = DefaultSize, бцел maxEvents = DefaultSize)
    in
    {
        assert(размер > 0);
    }
    ...
    private НаборДескр *разместиНабор(ref НаборДескр установи, ref НаборДескр наборВыд)
    {
        if(!установи.инициализован)
        {
            установи.установи(_size);
     ...
     * If a провод is already associated with the selector, the события and
     * атачмент are upated.
     *
     * Параметры:
     * провод      = провод that will be associated в_ the selector;
     *                must be a valid провод (i.e. not пусто and открой).
     * события       = bit маска of Событие values that represent the события
    ...
    in
    {
        assert(провод !is пусто && провод.ptr());
    }
    body
    {
        ИВыбираемый.Дескр укз = провод.ptr();
        ...
        if (ключ !is пусто)
        {
            if ((события & Событие.Чтен) || (события & Событие.Зависание))
            {
    ...
    public проц отмениРег(ИВыбираемый провод)
    {
        if (провод !is пусто)
        {
            ...
            if (removed !is пусто)
            {
                ...
                if (removed.события & Событие.Write)
                {
                    _writeSet.сотри(укз);
                }
                if ((removed.события & Событие.Чтен) || (removed.события & Событие.Зависание))
                {
                    _readSet.сотри(укз);
                }
                _keys.удали(укз);
     ...
     * interrupted by a signal and the 'перезапускПрерванногоСистВызова'
    ...
    public цел выбери(ИнтервалВремени таймаут)
    {
        ...
        debug (selector)
            Стдвыв.форматируй("--- СелекторВыбора.выбери(таймаут={0} msec)\n", таймаут.миллисек);
        ...
        version (Posix)
        {
            while (да)
            {
                ...
                debug (selector)
                    Стдвыв.форматируй("---   .выбери() returned {0} (максуд={1})\n",
                ...
                else
                {
                    ...
                    {
                        // checkErrno() always throws an исключение
                        checkErrno(__FILE__, __LINE__);
                    }
                    debug (selector)
                        Стдвыв.выведи("--- Restarting выбери() after being interrupted\n");
                }
            }
        }
        else
        {
            ...
            if (handlesAvailable)
            {
                ...
                _eventCount = .выбери(бцел.max, readfds, writefds, exceptfds, таймаут is ИнтервалВремени.max ? пусто : &tv);

                debug (selector)
                    Стдвыв.форматируй("---   .выбери() returned {0}\n", _eventCount);
            }
            else
            {
                Нить.сон(таймаут.интервал());
                _eventCount = 0;
            }
        }
        return _eventCount;
    ...
    {
        return (_eventCount > 0 ? new SelectSelectionSet(_keys, cast(бцел) _eventCount, _selectedReadSet,
...
private class SelectSelectionSet: ИНаборВыделений
{
    ...
    цел opApply(цел delegate(ref КлючВыбора) дг)
    {
        ...
        debug (selector)
            Стдвыв.форматируй("--- SelectSelectionSet.opApply() ({0} elements)\n", _eventCount);

        foreach (КлючВыбора current; _keys)
        {
            укз = current.провод.ptr();

            if (_readSet.набор_ли(укз))
                события = Событие.Чтен;
            else
                события = Событие.Нет;

            if (_writeSet.набор_ли(укз))
                события |= Событие.Write;
...
{
    /**
     * Helper class used by the выбери()-based Селектор в_ сохрани handles.
     * On Windows the handles are kept in an Массив of бцелs and the first
     * element of the Массив stores the Массив "length" (i.e. число of handles
    ...
    private struct НаборДескр
    {
         ...
         * in the НаборДескр.
         */
        проц установи(бцел размер = DefaultSize)
        {
        ...
        /**
         *  return да if this укз установи есть been инициализован.
         */
        бул инициализован()
        {
         ...
         */
        проц сотри(ИВыбираемый.Дескр укз)
        {
            for (бцел i = 1; i <= _buffer[0]; ++i)
            {
                if (_buffer[i] == cast(бцел) укз)
                {
                    ...
                    // copying the последний element в_ the позиция of the removed
                    // element.
                    if (i != _buffer[0])
                    {
        ...
        /**
         * Check whether the укз есть been установи.
         */
        ...
        /**
         * Cast the current объект в_ a pointer в_ an набор_уд, в_ be used with the
         * выбери() system вызов.
         */
...
else version (Posix)
{
    ...
    private struct НаборДескр
    {
        ...
        /**
         * Return да if the handleset есть been инициализован
         */
        бул инициализован()
        {
            return _buffer.length > 0;
         ...
         */
        public проц сотри(ИВыбираемый.Дескр укз)
        {
