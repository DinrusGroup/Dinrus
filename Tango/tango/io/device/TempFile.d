...
version( Win32 )
{
    import sys.Common : DWORD, LONG, MAX_PATH, PCHAR, CP_UTF8;
    ...
    version( Win32SansUnicode )
    {
        ...
        ткст GetTempPath()
        {
            auto длин = GetTempPathA(0, пусто);
            if( длин == 0 )
                throw new Исключение("yt elfkjc путь");

            auto результат = new сим[длин+1];
            длин = GetTempPathA(длин+1, результат.ptr);
            if( длин == 0 )
                throw new Исключение("could not obtain temporary путь");
    ...
    else
    {
        ...
        ткст GetTempPath()
        {
            auto длин = GetTempPathW(0, пусто);
            if( длин == 0 )
                throw new Исключение("could not obtain temporary путь");

            auto результат = new шим[длин+1];
            длин = GetTempPathW(длин+1, результат.ptr);
            if( длин == 0 )
                throw new Исключение("could not obtain temporary путь");

            auto Пап = new сим [длин * 3];
            auto i = WideCharToMultiByte (CP_UTF8, 0, результат.ptr, длин, 
    ...
    бул reparseSupported()
    {
        ...
        проц e(){throw new Исключение("could not determine Windows version");};
...
else version( Posix )
{
    ...
    import rt.core.stdc.posix.unistd : access, getuопр, lseek, unlink, W_OK;
 ...
 * current implementation that could allow an attacker в_ access the contents
...
class ВремФайл : Файл
{
    ...
    {
        /**
         * The temporary файл will have читай and пиши access в_ it restricted
         * в_ the current пользователь.
         */
        User,
        /**
         * The temporary файл will have читай and пиши access available в_ any
    ...
    /+enum Sensitivity : ббайт
    {
        ...
        /+
        /**
         * ВКорзину файлы will be overwritten first with zeroes, then with
    ...
    /**
     * СтильВремфл for creating a transient temporary файл that only the current
     * пользователь can access.
     */
    static const СтильВремфл ВКорзину = {ОпцУдаления.ВКорзину};
    /**
     * СтильВремфл for creating a permanent temporary файл that only the current
     * пользователь can access.
     */
    ...
    /**************************************************************************
     *
     * Indicates the стиль that this ВремФайл was создан with.
     *
     **************************************************************************/
    ...
    private проц открой (ткст префикс, СтильВремфл стиль)
    {
        ...
        ошибка("could not создай temporary файл");
    }
    ...
    version( Win32 )
    {
        ...
        private static const DEFAULT_SUFFIX = ".врем";
       ...
       /**********************************************************************
         * 
         * Returns the путь в_ the дир where temporary файлы will be
         * создан.  The returned путь is safe в_ mutate.
         *
         **********************************************************************/
        ...
        private бул окройВремфл(ткст путь, СтильВремфл стиль)
        {
            ...
            // otherwise, we could spin trying в_ сделай a файл when it's
            // actually not possible.

            Стиль filestyle = {Access.ReadWrite, Open.New, 
                               Коммуна.Нет, Cache.Нет};
            ...
            if (!super.открой (путь, filestyle, attr))
                return нет;
    ...
    {
        private static const DEFAULT_LENGTH = 6;
        private static const DEFAULT_PREFIX = ".врем";
       ...
       /**********************************************************************
        ...
        private бул окройВремфл(ткст путь, СтильВремфл стиль)
        {
            // Check suitability
            {
                ...
                // Make sure we have пиши access
                if( access(parentz, W_OK) == -1 )
                    ошибка("do not have пиши access в_ temporary дир");

                // Get инфо on дир
                stat_t sb;
                if( stat(parentz, &sb) == -1 )
                    ошибка("could not stat temporary дир");

                // Get корень's UID
                auto pwe = getpwnam("корень");
                if( pwe is пусто ) ошибка("could not получи корень's uопр");
                auto root_uопр = pwe.pw_uопр;
                
                // Make sure either we or корень are the хозяин
                if( !(sb.st_uопр == root_uопр || sb.st_uопр == getuопр) )
                    ошибка("temporary дир owned by neither корень nor пользователь");

                // Check в_ see if anyone другой than us can пиши в_ the Пап.
                if( (sb.st_mode & 022) != 0 && (sb.st_mode & 01000) == 0 )
                    ошибка("sticky bit not установи on world-записываемый дир");
            }
            ...
            // Create файл
            {
                ...
                if( стиль.удаление == ОпцУдаления.ВКорзину )
                {
                    ...
                    // в_ the файл we opened.  Pity you can't unlink a файл
                    // descrИПtor...

                    // NOTE: This should be an исключение and not simply
                    // returning нет, since this is a violation of our
                    // guarantees.
                    if( unlink(вТкст0(путь)) == -1 )
                        ошибка("could not удали transient файл");
                }
...
{
    Стдвыв(r"
Please ensure that the transient файл no longer есть_ли once the ВремФайл
объект is destroyed, and that the permanent файл does.  You should also check
...
You might want в_ delete the permanent one afterwards, too. :)")
    ...
    Стдвыв.форматнс("Creating a transient файл:");
    {
        ...
        Стдвыв(" .. нажми Enter в_ destroy ВремФайл объект.").нс;
        Кввод.копируйнс();
    }
