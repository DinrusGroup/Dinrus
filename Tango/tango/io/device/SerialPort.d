...
class СерийныйПорт : Устройство
{
    ...
    ***************************************************************************/
    ...
    СерийныйПорт скорость (бцел скорость)
    {
        version(Posix) {
            ...
            if(baud is пусто) {
                throw new ВВИскл("Invalid baud rate.");
            }
        ...
        version(Win32) {
            ...
            if(!SetCommState(вв.указатель, &конфиг)) ошибка();
        }
        return this;
    }
    ...
    ***************************************************************************/
    ...
    version(Win32) {
        ...
        {
            ткт = порт;
            вв.указатель = CreateFileA((`\\.\` ~ порт).вТкст0(), GENERIC_READ | GENERIC_WRITE, 0, пусто, OPEN_EXISTING, 0, пусто);
    ...
    version(Posix) {
        ...
        {
            if(файл.length == 0) throw new ВВИскл("Empty порт имя");
            if(файл[0] != '/') файл = "/dev/" ~ файл;
            
            if(файл.length > 5 && файл[0..5] == "/dev/")
                ткт = файл[5..$];
            else
                ткт = "СерийныйПорт@" ~ файл;
