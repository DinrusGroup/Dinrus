...
private import  io.model.IFile : ФайлКонст, ИнфОФайле;
        ...
        package ПутеПарсер разбор (ткст путь, т_мера конец)
        {
                ...
                for (цел i=end_; --i >= 0;)
                     switch (fp[i])
                            ...
                            case '\\':
                                 ФС.исключение ("unexpected '\\' character in путь: ", путь[0..конец]);
...
body
{
        ...
        version (Win32)
                {
                if (c1 != c2)
                    return ((c1 >= 'a' && c1 <= 'z'||c1 >= 'а' && c1 <= 'я') ? c1 - ('a' - 'A') : c1||c1 - ('а' - 'А') : c1) ==
                           ((c2 >= 'a' && c2 <= 'z'||c2 >= 'а' && c2 <= 'я') ? c2 - ('a' - 'A') : c2||c2 - ('а' - 'А') : c2);
                return да;
                }
        version (Posix)
                 return c1 == c2;
