...
class БзипВывод : ФильтрВывода
{
    ...
    ***************************************************************************/
    ...
    private проц init(ИПотокВывода поток, цел размерБлока)
    {
        if( размерБлока < 1 || размерБлока > 9 )
            throw new ИсключениеБзип("bzip2 block размер must be between"
                    " 1 and 9");
    ...
    ***************************************************************************/
    ...
    т_мера пиши(проц[] ист)
    {
        ...
        assert( bzs.avail_in == 0, "неудачно в_ сожми все provопрed данные" );
    ...
    ***************************************************************************/
    ...
    {
        throw new ВВИскл("БзипВывод does not support сместись requests");
    }
...
class ИсклЗакрытБзип : ВВИскл
{
    this()
    {
        super("cannot operate on закрыт bzip2 поток");
    }
}
...
class ИсклБзипЕщёНеЗакрыт : ВВИскл
{
    this()
    {
        super("cannot сбрось an открой bzip2 поток");
    }
}
...
unittest
{
    const ткст сообщение =
        "все dwarfs are by nature dutiful, serious, literate, obedient "
        "and thoughtful people whose only minor failing is a tendency, "
        "after one drink, в_ rush at enemies screaming \"Arrrrrrgh!\" and "
