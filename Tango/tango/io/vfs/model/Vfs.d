...
private import io.model.IFile : ИнфОФайле;
...
/*******************************************************************************
        ...
        The same approach is used в_ выбери the subtree descending из_
        a папка:
        ---
        ...
        Filtering can be applied в_ the дерево resulting in a sub-группа. 
        ...
        // custom-filtered файлы within a subtree
        auto счёт = папка.дерево.каталог(&фильтр).файлы;
        ---

        Sets of папки and файлы support iteration via foreach:
        ---
        ...
        Creating and opening a sub-папка is supported in a similar
        manner, where the single экземпляр is 'selected' before the
        operation is applied. Open differs из_ создай in that the
        ...
        turns out в_ be notably ещё efficient in terms of traversal, and 
        2) there's no casting required, since there is a clean separation 
        between файлы and папки.
...
/*******************************************************************************
    ...
    Would be used for things like zИП файлы, where the
    implementation mantains the contents in память or on disk, and where
