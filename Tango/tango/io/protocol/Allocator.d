...
/*******************************************************************************
        ...
        this is the default behaviour for Читатель instances
...
/*******************************************************************************
        ...
        Размести из_ within a private куча пространство. This supports reading
        данные as 'records', reusing the same chunk of память for each record
        загружен. The ctor takes an аргумент defining the начальное allocation
        made, and this will be increased as necessary в_ accomodate larger
        records. Use the сбрось() метод в_ indicate конец of record (reuse
        память for subsequent requests), or установи the autoreset flag в_ reuse
...
class СрезКучи : ИРазместитель
{
        ...
        ***********************************************************************/
        ...
        final проц[] размести (ИПротокол.Читатель читатель, бцел байты, ИПротокол.Тип тип)
        {
                ...
                if ((used + байты) > буфер.length)
                     буфер.length = (used + байты) * 2;
...
/*******************************************************************************
        ...
        This avoопрs Всё куча activity and copying, but requires some care
...
class СрезБуфера : ИРазместитель
{
        ...
        ***********************************************************************/
        ...
        /***********************************************************************
                ...
                No alloc or копируй: alias directly из_ буфер. While this is
                very efficient (no куча activity) it should be used only in
                scenarios where контент is known в_ fit within a буфер, and
                there is no conversion of saопр контент e.g. take care when
                using with ПротоколЭндиан since it will преобразуй within the
                буфер, potentially confusing добавьitional буфер clients.

                With СрезБуфера, it is consопрered нормаль в_ создай a Буфер
                large enough в_ contain, say, a файл and subsequently срез
                все strings/массивы directly из_ this буфер. Smaller Buffers
                can be used in a record-oriented manner similar в_ СрезКучи:
                ...
                record. БуферРоста could be used in the latter case, в_ ensure
                the largest record width is always accomodated.

                A good use of this is in handling of network traffic, where
                incoming данные is often transient and of a known протяженность. For
                другой potential use, consопрer the quantity of distinct текст
