...
class ПротоколЭндиан : ПротоколНатив
{
        ...
        ***********************************************************************/
        ...
        {
                auto ret = super.читай (приёмн, байты, тип);
        ...
        ***********************************************************************/
        ...
        override проц пиши (ук  ист, бцел байты, Тип тип)
        {
                ...
                проц пиши (цел маска, Swapper mutate)
                {
                        т_мера писатель (проц[] приёмн)
                        {
                                ...
                                mutate (приёмн.ptr, длин);
                                return длин;
                        }

                        while (байты)
                               if (байты -= buffer_.писатель (&писатель))
                                   // слей if we used все буфер пространство
...
debug (UnitTest)
{
        ...
        unittest
        {
                проц[] alloc (ПротоколЭндиан.Читатель читатель, бцел байты, ПротоколЭндиан.Тип тип)
                {
                ...
                mule = cast(ткст) протокол.читайМассив (mule.ptr, mule.length, протокол.Тип.Utf8, &alloc);
                assert (mule == тест);
