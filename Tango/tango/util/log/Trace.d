/*******************************************************************************
        ...
        Synchronized, formatted console вывод. This can be used in lieu
...
private class СинхПринт
{
        ...
        **********************************************************************/
        ...
        /**********************************************************************
                ...
                Sink for passing в_ the форматёр
...
000030:  D4 70 A7 DE  BC FB 8F 14  00 3B                     .p.......;
        ...
        **********************************************************************/
        ...
        final СинхПринт память (проц[] mem)
        {
            ...
            synchronized (стопор)
            {
                for( цел row = 0; row < данные.length; row += 16 )
                {
                    // выведи relative смещение
                    преобразуй.преобразуй (&сток, "{:X6}:  ", row );

                    // выведи данные байты
                    for( цел инд = 0; инд < 16 ; инд++ )
                    {
                        // выведи байт or stuffing пробелы
                        if ( инд + row < данные.length )
                            преобразуй (&сток, "{:X2} ", данные[ row + инд ] );
                        else
                            вывод.пиши ("   ");
                    ...
                    // другой values are printed as a dot
                    ббайт[16] аски =void;
                    цел asciiIdx;
                    for ( asciiIdx = 0;
                        (asciiIdx<16) && (asciiIdx+row < данные.length);
                        asciiIdx++ )
                    {
                        ббайт c = данные[ row + asciiIdx ];
                        if ( c < 0x20 || c > 0x7E )
                            c = '.';
                        аски[asciiIdx] = c;
                    }
