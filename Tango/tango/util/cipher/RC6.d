 ...
 * in the future, it may be wise в_ rewrite Всё RC5 and RC6 в_ use some
...
class RC6 : ШифрБлок
{
    ...
    final проц init(бул зашифруй, СимметричныйКлюч keyParams)
    {
        ...
        if (длин != 16 && длин != 24 && длин != 32)
            не_годится(имя()~": Invalid ключ length (requires 16/24/32 байты)");
    ...
    final override бцел обнови(проц[] input_, проц[] output_) {
        ...
        бцел A = БайтКонвертер.LittleEndian.в_!(бцел)(ввод[0..4]),
             B = БайтКонвертер.LittleEndian.в_!(бцел)(ввод[4..8]),
             C = БайтКонвертер.LittleEndian.в_!(бцел)(ввод[8..12]),
             D = БайтКонвертер.LittleEndian.в_!(бцел)(ввод[12..16]),
             t,
             u;
        ...
        if (_encrypt)
        {
            ...
            {
                t = Побитно.rotateLeft(B*((B<<1)+1), 5u);
                u = Побитно.rotateLeft(D*((D<<1)+1), 5u);
                A = Побитно.rotateLeft(A^t, u) + S[i<<1];
                C = Побитно.rotateLeft(C^u, t) + S[(i<<1)+1];
                t = A;
                A = B;
                B = C;
                C = D;
        ...
        else
        {
            ...
            for (цел i = ROUNDS; i >= 1; i--)
            {
                ...
                u = Побитно.rotateLeft(D*((D<<1)+1), 5u);
                t = Побитно.rotateLeft(B*((B<<1)+1), 5u);
                C = Побитно.rotateRight(C-S[(i<<1)+1], t) ^ u;
                A = Побитно.rotateRight(A-S[i<<1], u) ^ t;
            }
        ...
        вывод[0..4] = БайтКонвертер.LittleEndian.из_!(бцел)(A);
        вывод[4..8] = БайтКонвертер.LittleEndian.из_!(бцел)(B);
        вывод[8..12] = БайтКонвертер.LittleEndian.из_!(бцел)(C);
        вывод[12..16] = БайтКонвертер.LittleEndian.из_!(бцел)(D);
    ...
    private проц установи(ббайт[] ключ)
    {
        ...
        for (цел i = 0, j = 0; i < c; i++, j+=4)
            L[i] = БайтКонвертер.LittleEndian.в_!(бцел)(ключ[j..j+цел.sizeof]);
        ...
        for (цел s = 1; s <= v; s++)
        {
            A = S[i] = Побитно.rotateLeft(S[i]+A+B, 3u);
            B = L[j] = Побитно.rotateLeft(L[j]+A+B, A+B);
            i = (i + 1) % (2*ROUNDS+4);
            j = (j + 1) % c;
        }
    }
