...
class Salsa20 : ШифрПоток
{
    protected
    {
        ...
        // KeyПоток and индекс marker
        ббайт[] keyПоток;
        бцел индекс;
        
        // Internal copies of the ключ and IV for resetting the cИПher
        ббайт[] workingKey,
                workingIV;
    }
    ...
    проц init(бул зашифруй, Параметры params)
    {
        ...
        if (ключ)
        {
            if (ключ.length != 16 && ключ.length != 32)
                не_годится(имя()~": Invalid ключ length. (requires 16 or 32 байты)");
        ...
        if (!iv || iv.length != 8)
            не_годится(имя()~": 8 байт IV required.");
    ...
    protected проц keySetup()
    {
        ...
        состояние[1] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[0..4]);
        состояние[2] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[4..8]);
        состояние[3] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[8..12]);
        состояние[4] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[12..16]);
        ...
        состояние[11] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[смещение..смещение+4]);
        состояние[12] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[смещение+4..смещение+8]);
        состояние[13] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[смещение+8..смещение+12]);
        состояние[14] = БайтКонвертер.LittleEndian.в_!(бцел)(workingKey[смещение+12..смещение+16]);
        состояние[ 0] = БайтКонвертер.LittleEndian.в_!(бцел)(constants[0..4]);
        состояние[ 5] = БайтКонвертер.LittleEndian.в_!(бцел)(constants[4..8]);
        состояние[10] = БайтКонвертер.LittleEndian.в_!(бцел)(constants[8..12]);
        состояние[15] = БайтКонвертер.LittleEndian.в_!(бцел)(constants[12..16]);
    }
    
    protected проц ivSetup()
    {
        состояние[6] = БайтКонвертер.LittleEndian.в_!(бцел)(workingIV[0..4]);
        состояние[7] = БайтКонвертер.LittleEndian.в_!(бцел)(workingIV[4..8]);
    ...
    protected проц salsa20WordToByte(бцел[] ввод, ref ббайт[] вывод)
    {
        ...
        for (i = 0; i < 10; i++)
        {
            x[ 4] ^= Побитно.rotateLeft(x[ 0]+x[12],  7u);
            x[ 8] ^= Побитно.rotateLeft(x[ 4]+x[ 0],  9u);
            x[12] ^= Побитно.rotateLeft(x[ 8]+x[ 4], 13u);
            x[ 0] ^= Побитно.rotateLeft(x[12]+x[ 8], 18u);
            x[ 9] ^= Побитно.rotateLeft(x[ 5]+x[ 1],  7u);
            x[13] ^= Побитно.rotateLeft(x[ 9]+x[ 5],  9u);
            x[ 1] ^= Побитно.rotateLeft(x[13]+x[ 9], 13u);
            x[ 5] ^= Побитно.rotateLeft(x[ 1]+x[13], 18u);
            x[14] ^= Побитно.rotateLeft(x[10]+x[ 6],  7u);
            x[ 2] ^= Побитно.rotateLeft(x[14]+x[10],  9u);
            x[ 6] ^= Побитно.rotateLeft(x[ 2]+x[14], 13u);
            x[10] ^= Побитно.rotateLeft(x[ 6]+x[ 2], 18u);
            x[ 3] ^= Побитно.rotateLeft(x[15]+x[11],  7u);
            x[ 7] ^= Побитно.rotateLeft(x[ 3]+x[15],  9u);
            x[11] ^= Побитно.rotateLeft(x[ 7]+x[ 3], 13u);
            x[15] ^= Побитно.rotateLeft(x[11]+x[ 7], 18u);
            x[ 1] ^= Побитно.rotateLeft(x[ 0]+x[ 3],  7u);
            x[ 2] ^= Побитно.rotateLeft(x[ 1]+x[ 0],  9u);
            x[ 3] ^= Побитно.rotateLeft(x[ 2]+x[ 1], 13u);
            x[ 0] ^= Побитно.rotateLeft(x[ 3]+x[ 2], 18u);
            x[ 6] ^= Побитно.rotateLeft(x[ 5]+x[ 4],  7u);
            x[ 7] ^= Побитно.rotateLeft(x[ 6]+x[ 5],  9u);
            x[ 4] ^= Побитно.rotateLeft(x[ 7]+x[ 6], 13u);
            x[ 5] ^= Побитно.rotateLeft(x[ 4]+x[ 7], 18u);
            x[11] ^= Побитно.rotateLeft(x[10]+x[ 9],  7u);
            x[ 8] ^= Побитно.rotateLeft(x[11]+x[10],  9u);
            x[ 9] ^= Побитно.rotateLeft(x[ 8]+x[11], 13u);
            x[10] ^= Побитно.rotateLeft(x[ 9]+x[ 8], 18u);
            x[12] ^= Побитно.rotateLeft(x[15]+x[14],  7u);
            x[13] ^= Побитно.rotateLeft(x[12]+x[15],  9u);
            x[14] ^= Побитно.rotateLeft(x[13]+x[12], 13u);
            x[15] ^= Побитно.rotateLeft(x[14]+x[13], 18u);
        }
