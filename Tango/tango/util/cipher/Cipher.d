...
abstract class Шифр
{
    ...
    /**
     * Процесс a block of plaintext данные из_ the ввод Массив
     * and place it in the вывод Массив.
     *
     * Параметры:
    ...
    /** Возвращает: Whether or not the cИПher есть been инициализован. */
...
/** Interface for a стандарт block cИПher. */
abstract class ШифрБлок : Шифр
{
    /** Возвращает: The block размер in байты that this cИПher will operate on. */
...
{   
 ...
 /** Base паддинг class for implementing block паддинг schemes. */
 abstract class ПаддингБлокаШифра
 {
    ...
    */ 
    ...
    /**
    * Return the число of pad байты in the block.
    *
    * Параметры:
    *     input_ = Pдобавьed block of which в_ счёт the pad байты.
    *
    * Возвращает: The число of pad байты in the block.
    *
...
/** Объект representing and wrapping a symmetric ключ in байты. */
...
class ParametersWithIV : Шифр.Параметры
{
    ...
    /**
     * Параметры:
     *     params = Параметры в_ wrap.
     *     iv     = IV в_ be held.
     */
    this (Шифр.Параметры params=пусто, проц[] iv=пусто)
    {
        _params = params;
        _iv = cast(ббайт[]) iv;
    }
...
struct Побитно
{
    static бцел rotateLeft(бцел x, бцел y)
    {
        return (x << y) | (x >> (32u-y));
    }
    
    static бцел rotateRight(бцел x, бцел y)
    {
        return (x >> y) | (x << (32u-y));    
    }
    
    static бдол rotateLeft(бдол x, бцел y)
    {
        return (x << y) | (x >> (64u-y));
    }
    
    static бдол rotateRight(бдол x, бцел y)
    {
        return (x >> y) | (x << (64u-y));    
    }
}
...
/** Converts between integral типы and unsigned байт массивы */
struct БайтКонвертер
{
    ...
    /** Conversions between little эндиан integrals and байты */
    struct LittleEndian
    {
        /**
    ...
    /** Conversions between big эндиан integrals and байты */
    struct БигЭндиан
    {
