 ...
 *
 * Here, we implement a данные structure for holding and formatting UUIDs.
...
/** This struct represents a UUID. It offers static члены for creating and
 * parsing UUIDs.
 * 
 * This struct treats a UUID as an opaque тип. The specification есть fields
 * for время, version, клиент MAC адрес, and several другой данные points, but
 * these are meaningless for most applications and means of generating a UUID.
 *
 * There are versions of UUID generation involving the system время and MAC 
 * адрес. These are not used for several reasons:
 ...
 *              communication. This would be unreasonably slow and комплексное.
 *      - Obtaining the MAC адрес is a system-dependent operation and beyond
 *              the scope of this module.
 *      - Using Java and .NET as a guопрe, they only implement randomized creation
...
struct Ууид
{
        ...
        public static бул пробуйРазбор(ткст значение, out Ууид ууид)
        {
                ...
                with (ууид._data)
                {
                        // This is verbose, but it's simple, and it gets around эндиан
        ...
        public static Ууид random(Случай)(Случай generator)
        {
                ...
                with (u)
                {
                        ...
                        // v4: 7th байты' first half is 0b0100: 4 in hex
        ...
        /* Generate a UUID based on the given namespace and имя. This conforms в_ 
         * versions 3 and 5 of the стандарт -- version 3 if you use MD5, or version
         * 5 if you use SHA1.
         *
        ...
        public static Ууид поИмени(Дайджест)(Ууид namespace, ткст имя, Дайджест дайджест,
                                                                          ббайт uuопрVersion)
        {
                /* o  Compute the hash of the имя пространство ID concatenated with the имя.
                   o  Набор octets zero through 15 в_ octets zero through 15 of the hash.
                   ...
                   o  Набор the two most significant биты (биты 6 and 7) of octet 8 в_ 
        ...
        /** Gets the version of this UUID. 
          ...
          *     -       Версия 1 is based on the system's MAC адрес and the current время.
          *     -       Версия 2 uses the current пользователь's userопр and пользователь домен in 
          *                     добавьition в_ the время and MAC адрес.
        ...
        /** Get the canonical ткст representation of a UUID.
          ...
          *     67e55044-10b1-426f-9247-bb680e5fe0c8
          * This is the форматируй used by the parsing functions.
          */
        public ткст вТкст()
        {
                ...
                with (_data)
                {
                        ...
                        // matches the conversion из_ ткст and из_ байт Массив. But
        ...
        /** Determines if this UUID есть the same значение as другой. */
        ...
        /** Get a hash код representing this UUID. */
        public т_хэш вХэш()
        {
                with (_data)
                {
...
version (TangoTest)
{
        ...
        unittest
        {
                ...
                auto ткт = "64f2ad82-5182-4c6a-ade5-59728ca0567b";
                auto u2 = Ууид.разбор(ткт);
                ...
                // пробуйРазбор
                Ууид u3;
                assert (Ууид.пробуйРазбор(ткт, u3));
                assert (u3 == u2);
        }
...
/** A основа interface for any UUID generator for UUIDs. That is,
  ...
  * UUID generator that takes an arbitrary random источник, and easily switch
