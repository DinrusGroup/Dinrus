...
import io.stream.Zlib : ВводЗлиб, ВыводЗлиб;
...
import time.chrono.Gregorian : Gregorian;
...
private
{
    ...
    бкрат fromMethod(Метод метод)
    {
        switch( метод )
        {
            ...
            default:
                assert(нет, "unsupported compression метод");
        }
    }
...
class ЧитательБлокаЗип : ЧитательЗип
{
    ...
    in
    {
        assert( cast(ИПровод.ИШаг) источник.провод, "источник поток must be seekable" );
    }
    body
    {
...
private:
    ...
    body
    {
        ...
        auto file_len = шагун.сместись(0, шагун.Якорь.End);
        ...
        // Seek back and читай in the chunk.  Don't forget в_ clean up after
        // ourselves.
        шагун.сместись(-cast(дол)chunk_len, шагун.Якорь.End);
    ...
    ИПотокВвода open_file(ФайлЗаг заголовок, бул необр)
    {
        ...
        // If that's все they wanted, пароль it back.
        if( необр )
            return поток;
...
private:
    ...
    проц put_Необр(ИнфоОЗаписиЗип инфо, ЗаписьЗип Запись)
    {
        ...
        // Note: we need в_ маска off the данные descrИПtor bit because we aren't
        // going в_ пиши one.
    ...
    проц put_compressed(ИнфоОЗаписиЗип инфо, ИПотокВвода источник)
    {
        ...
        // Seek back в_ the конец of the файл, and we're готово!
    ...
    проц put_local_header(ДанныеЛокалФайлЗага данные,
            ткст имя_файла)
    {
        auto f_name = Путь.нормализуй(имя_файла);
...
private:
    ...
    static проц badsig()
    {
        thisT("corrupt сигнатура or unexpected section найдено");
    }

    static проц badsig(ткст тип)
    {
        thisT("corrupt "~тип~" сигнатура or unexpected section найдено");
    }

    static проц incons(ткст имя)
    {
        thisT("inconsistent заголовки for файл \""~имя~"\"; "
                "архив is likely corrupted");
    }

    static проц missingdir()
    {
        thisT("could not locate central архив дир; "
                "файл is corrupt or possibly not a ZИП архив");
    }

    static проц toomanyentries()
    {
        thisT("too many архив записи");
    }

    static проц toolong()
    {
        thisT("архив is too дол; limited в_ 4GB total");
    }

    static проц cdtoolong()
    {
        thisT("central дир is too дол; limited в_ 4GB total");
    }

    static проц fntoolong()
    {
        thisT("файл имя too дол; limited в_ 65,535 characters");
    }

    static проц eftoolong()
    {
        thisT("extra field too дол; limited в_ 65,535 characters");
    }
    ...
    {
        thisT("could not кодируй имяф преобр_в codepage 437");
    }
...
class ИсклКСЗип : ИсклЗип
{
    this(ткст имя)
    {
        super("checksum неудачно on zИП Запись \""~имя~"\"");
    }
...
private:
    ...
    {
        throw new thisT(сооб ~ " not supported");
    }

    static проц spanned()
    {
        thisT("разбей and multi-disk archives");
    }

    static проц zИПver(бкрат ver)
    {
        throw new thisT("zИП форматируй version "
                ~Целое.вТкст(ver / 10)
                ~"."
                ~Целое.вТкст(ver % 10)
    ...
    static проц флаги()
    {
        throw new thisT("неизвестное or unsupported файл флаги включен");
    }
    ...
    static проц метод(бкрат m)
    {
        ...
        switch( m )
        {
            ...
            default:    ms = "неизвестное";
        }

        thisT(ms ~ " compression метод");
    }
}
...
проц создайАрхив(ткст архив, Метод метод, ткст[] файлы...)
{
    ...
    foreach( файл ; файлы )
    {
        scope fp = new ФПуть(файл);
        
        ИнфоОЗаписиЗип zi;
        zi.имя = файл;
        zi.изменён = fp.изменён;

        zw.поместиФайл(zi, файл);
    }
...
проц извлекиАрхив(ткст архив, ткст приёмник)
{
    ...
    foreach( Запись ; zr )
    {
        ...
        путь = Путь.нормализуй(путь);

        // Create the предок дир if necessary.
        auto предок = Путь.разбор(путь).предок;
        if( !Путь.есть_ли(предок) )
        {
            Путь.создайПуть(предок);
        }
        ...
        // Update timestamps
        auto oldTS = Путь.штампыВремени(путь);
        Путь.штампыВремени(путь, oldTS.использовался, Запись.инфо.изменён);
    }
}
...
class СверщикЗаписиЗип : ИПотокВвода
{
    ...
    body
    {
        ...
        this.источник = new ДайджестВвод(источник, дайджест);
    }
    ...
    {
        return источник.сместись (ofs, якорь);
    }
    ...
    {
        auto байты = источник.читай(приёмн);
        if( байты == ИПровод.Кф )
            check;
        return байты;
    }
...
ткст cp437_to_utf8(ббайт[] s)
{
    foreach( i,c ; s )
    {
        if( (1 <= c && c <= 31) || c >= 127 )
        {
            ...
            т_мера k=i; // current length

            // We вставь new characters at r[i+j+k]
    ...
    foreach( i,дим c ; s )
    {
        if( !((32 <= c && c <= 126) || c == 0) )
        {
            ...
            foreach( дим d ; s[i..$] )
            {
                ...
                {
                    throw new Исключение("cannot кодируй character \""
...
проц timeToDos(Время время, out бкрат dostime, out бкрат dosdate)
{
    ...
    // *muttering happily*
    auto дата = Gregorian.generic.вДату(время);
    if( дата.год < 1980 )
        ИсклЗип.tooold;
...
class SliceSeekInputПоток : ИПотокВвода
{
    ...
    override т_мера читай(проц[] приёмн)
    {
        ...
        // Seek источник поток в_ the appropriate location.
        if( шагун.сместись(0, Якорь.Тек) != begin+_position )
            шагун.сместись(begin+_position, Якорь.Нач);

        // Do the читай
        auto читай = источник.читай(приёмн);
        if( читай == ИПровод.Кф )
            // If we got an Кф, we'll consопрer that a bug for the moment.
...
class SliceInputПоток : ИПотокВвода
{
    ...
    override т_мера читай(проц[] приёмн)
    {
        ...
        // Otherwise, сделай sure we don't try в_ читай past the конец of the срез
        if( приёмн.length > _length )
            приёмн.length = cast(т_мера) _length;
