...
/*******************************************************************************
        ...
        Lieserson, and Rivest's <EM>Introduction в_ Algorithms</EM>.
        This class was inspired by (and код cross-checked with) a 
        ...
        don't своп Cell contents or use special dummy nilnodes. 
...
struct RedBlack (V, A = AttributeDummy)
{
        ...
        **/
        ...
        /**
         * Make a new Cell with given значение, пусто линки, and BLACK color.
         * Normally only called в_ establish a new корень.
        **/

        final Ref установи (V v)
        {
                значение = v;
                left = пусто;
                right = пусто;
        ...
        /**
         * Return a new Ref with same значение and color as сам,
        ...
        **/ 
        ...
        {
                static if (is(typeof(A) == AttributeDummy))
                           auto t = alloc().установи (значение);
                       else
                          auto t = alloc().установи (значение, attribute);
        ...
        **/
        public проц checkImplementation ()
        {
                ...
                // It's too hard в_ check the property that every simple
                // путь из_ node в_ leaf есть same число of black nodes.
                // So restrict в_ the following
        ...
        /**
         * Return the minimum значение of the current (sub)дерево
        **/
        ...
        /**
         * Return the maximum значение of the current (sub)дерево
        **/
        Ref rightmost ()
        {
                auto p = this;
        ...
        /**
         * Return the корень (parentless node) of the дерево
        **/
        Ref корень ()
        {
                auto p = this;
        ...
        /**
         * Return да if node is a корень (i.e., есть a пусто предок)
        **/
        ...
        **/
        ...
        /**
         * Return the число of nodes in the subtree
        **/
        цел размер ()
        {
                auto c = 1;
        ...
        /**
         * Return node of current subtree containing значение as значение(), 
         * if it есть_ли, else пусто. 
         * Uses Comparator cmp в_ найди and в_ check equality.
        **/

        Ref найди (V значение, Compare!(V) cmp)
        {
                auto t = this;
                for (;;)
                    {
        ...
        /**
         * Return node of subtree matching "значение" 
         * or, if Неук найдено, the one just after or before  
         * if it doesn't exist, return пусто
         * Uses Comparator cmp в_ найди and в_ check equality.
        **/
        Ref findFirst (V значение, Compare!(V) cmp, бул after = да)
        {
                auto t = this;
                auto tLower = this;
                auto tGreater  = this;
        ...
        /**
         * Return число of nodes of current subtree containing значение.
         * Uses Comparator cmp в_ найди and в_ check equality.
        **/
        цел счёт (V значение, Compare!(V) cmp)
        {
                auto c = 0;
                auto t = this;
                while (t)
        ...
        цел countAttribute (A attrib, Compare!(A) cmp)
        {
                цел c = 0;
                auto t = this;
        ...
        **/
        Ref найди (V значение, A attribute, Compare!(V) cmp)
        {
                auto t = this;
        ...
        /**
         * Return a new subtree containing each значение of current subtree
        **/
        ...
        /**
         * There's no generic значение insertion. Instead найди the
         * place you want в_ добавь a node and then invoke insertLeft
         * or insertRight.
         * <P>
         * Insert Cell as the left ветвь of current node, and then
         * rebalance the дерево it is in.
         * @param Cell the Cell в_ добавь
         * @param корень, the корень of the current дерево
         * Возвращает: the new корень of the current дерево. (Rebalancing
         * can change the корень!)
        **/


        Ref insertLeft (Ref cell, Ref корень)
        {
                left = cell;
                cell.предок = this;
        ...
        /**
         * Insert Cell as the right ветвь of current node, and then
         * rebalance the дерево it is in.
         * @param Cell the Cell в_ добавь
         * @param корень, the корень of the current дерево
         * Возвращает: the new корень of the current дерево. (Rebalancing
        ...
        **/
        ...
        Ref insertRight (Ref cell, Ref корень)
        {
                right = cell;
                cell.предок = this;
                return cell.fixAfterInsertion (корень);
        ...
        /**
         * Delete the current node, and then rebalance the дерево it is in
         * @param корень the корень of the current дерево
         * Возвращает: the new корень of the current дерево. (Rebalancing
         * can change the корень!)
        **/
        ...
        Ref удали (Ref корень)
        {
                // укз case where we are only node
                if (left is пусто && right is пусто && предок is пусто)
                    return пусто;

                // if strictly internal, своп places with a successor
                if (left && right)
                   {
                   auto s = successor;

                   // To work nicely with arbitrary subclasses of Ref, we don't want в_
                   // just копируй successor's fields. since we don't know what
                   // they are.  Instead we своп positions _in the дерево.
                   корень = свопPosition (this, s, корень);
                   }

                // Start fixup at replacement node (normally a ветвь).
                ...
                if (left is пусто && right is пусто)
                   ...
                   if (replacement is пусто)
                       replacement = right;

                   // link replacement в_ предок
                   replacement.предок = предок;

                   if (предок is пусто)
                       корень = replacement;
                   else
                      if (this is предок.left)
                          предок.left = replacement;
                      else
                         предок.right = replacement;
                   ...
                   // fix replacement
                   if (color is BLACK)
                       корень = replacement.fixAfterDeletion (корень);
                   }
                return корень;
        }

        /**
         * Swap the linkages of two nodes in a дерево.
         * Return new корень, in case it изменён.
        **/

        static Ref свопPosition (Ref x, Ref y, Ref корень)
        {
        ...
        /**
         * Return color of node p, or BLACK if p is пусто
        ...
        **/
        ...
        static бул colorOf (Ref p)
        {
                return (p is пусто) ? BLACK : p.color;
        }

        /**
         * return предок of node p, or пусто if p is пусто
        **/
        ...
        /**
         * Набор the color of node p, or do nothing if p is пусто
        **/

        static проц setColor (Ref p, бул c)
        {
                if (p !is пусто)
                    p.color = c;
        }
        ...
        /**
         * return left ветвь of node p, or пусто if p is пусто
        **/
        ...
        /**
         * return right ветвь of node p, or пусто if p is пусто
        **/
        ...
        /** ОтКого CLR **/
        package Ref rotateLeft (Ref корень)
        {
                auto r = right;
                right = r.left;
        ...
        /** ОтКого CLR **/
        package Ref rotateRight (Ref корень)
        {
                auto l = left;
                left = l.right;
        ...
        /** ОтКого CLR **/
        package Ref fixAfterInsertion (Ref корень)
        {
                color = RED;
                auto x = this;
                      ...
                      if (parentOf(x) is leftOf(parentOf(parentOf(x))))
                         ...
                         if (colorOf(y) is RED)
                            ...
                            if (x is rightOf(parentOf(x)))
                               {
                               x = parentOf(x);
                               корень = x.rotateLeft(корень);
                               }

                            setColor(parentOf(x), BLACK);
                            setColor(parentOf(parentOf(x)), RED);
                            if (parentOf(parentOf(x)) !is пусто)
                                корень = parentOf(parentOf(x)).rotateRight(корень);
                            }
                         }
                      else
                         {
                         ...
                         if (colorOf(y) is RED)
                            ...
                            if (x is leftOf(parentOf(x)))
                               {
                               x = parentOf(x);
                               корень = x.rotateRight(корень);
                               }
        
                            setColor(parentOf(x), BLACK);
                            setColor(parentOf(parentOf(x)), RED);
                            if (parentOf(parentOf(x)) !is пусто)
                                корень = parentOf(parentOf(x)).rotateLeft(корень);
                            }
                         }
                      }
                корень.color = BLACK;
        ...
        /** ОтКого CLR **/
        package Ref fixAfterDeletion(Ref корень)
        {
                auto x = this;
                while (x !is корень && colorOf(x) is BLACK)
                      ...
                      if (x is leftOf(parentOf(x)))
                         ...
                         if (colorOf(sib) is RED)
                            ...
                            корень = parentOf(x).rotateLeft(корень);
                         ...
                         if (colorOf(sib) is RED)
                            ...
                            setColor(sib, BLACK);
                            setColor(parentOf(x), RED);
                            корень = parentOf(x).rotateRight(корень);
