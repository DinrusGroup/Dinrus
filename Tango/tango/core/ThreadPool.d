...
class Катушка(Арги...)
{
    ...
    this(т_мера трудяги, т_мера q_size = 0)
    {
        ...
        устФлаг(приоритетное_дело, cast(Дело*) пусто);
        устФлаг(активные_дела, cast(т_мера) 0);
        устФлаг(готово, нет);
    ...
    проц присвой(ДДела дело, Арги арги)
    {
        ...
        {
            throw new ИсклКатушки("No трудяги available!");
        }

        m.блокируй();
        scope(exit) m.разблокируй();
        auto j = Дело(дело, арги);
        устФлаг(приоритетное_дело, &j);
        активностьПула.сообщи();
        // Wait until someone есть taken the дело
        while (дайФлаг(приоритетное_дело) !is пусто)
    ...
    {
        if (дайФлаг(активные_дела) >= обойма.length)
            return нет;
        присвой(дело, арги);
        return да;
    }
    ...
    {    
        m.блокируй();
        while (q.length > 0 || дайФлаг(активные_дела) > 0)
               активностьТрудяги.жди();
        m.разблокируй();
    } 
...
private:
    ...
    проц делДело()
    {
        while (!дайФлаг(готово))
        {
            m.блокируй();
            while (q.length == 0 && дайФлаг(приоритетное_дело) is пусто && !дайФлаг(готово))
                активностьПула.жди();
            if (дайФлаг(готово)) {
            ...
            Дело дело;
            Дело* делУк = дайФлаг(приоритетное_дело);
            if (делУк !is пусто)
            {
                дело = *делУк;
                устФлаг(приоритетное_дело, cast(Дело*)пусто);
                активностьТрудяги.сообщи();
            }
            else
            {
                version (Queued) // #1896
            ...
            // Do the actual дело
            добавьФлаг!(т_мера)(активные_дела, 1);
            try {
                дело.дг(дело.арги);
            } catch (Исключение ex) { }
            добавьФлаг!(т_мера)(активные_дела, -1);
