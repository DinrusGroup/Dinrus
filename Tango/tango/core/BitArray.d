 ...
 * bit of память for storage.  Thus an Массив of 32 биты would occupy the same
...
struct МассивБит
{
    ...
    /**
     * This initializes a МассивБит of биты.length биты, where each bit значение
     ...
     * length, the new биты will be инициализован в_ zero.
     *
     * Параметры:
    ...
    /**
     * Gets the length of a бцел Массив large enough в_ hold все stored биты.
     *
     * Возвращает:
    ...
    /**
     * Resets the length of this Массив в_ биты.length and then initializes this
     *
     * Resizes this Массив в_ hold биты.length биты and initializes each bit
     ...
     */
    проц init( проц[] мишень, т_мера numbits )
    in
    {
    ...
    debug( UnitTest )
    {
      unittest
      {
        ...
        for( т_мера i = 0; i < данные.length; ++i )
        {
    ...
    цел opEquals( МассивБит rhs )
    {
        ...
        цел rest = cast(цел)(this.length & cast(т_мера)31u);
        бцел маска = ~((~0u)<<rest);
    ...
    debug( UnitTest )
    {
      unittest
      {
        ...
        assert( v.length == a.dim * бцел.sizeof );
      }
    }
     ...
     *
     * In:
     *  поз must be less than the length of this Массив.
     *
     * Возвращает:
     ...
     *
     * In:
     *  rhs.length must equal the length of this Массив.
     *
     * Возвращает:
