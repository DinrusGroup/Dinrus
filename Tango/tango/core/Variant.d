...
private import gc : СМ;
private import core.Vararg : спис_ва;
private import Traits;
private import Tuple;

private extern(C) Объект _d_toObject(проц*);
...
private
{
    ...
    struct ХранилищеВариантов
    {
        ...
        {
            /*
             * Contains куча-allocated storage for values which are too large
            ...
            // Used в_ адрес storage as an Массив.
    ...
    alias RuntimeTraits.статМасс_ли статМасс_ли;
    alias RuntimeTraits.класс_ли класс_ли;
    alias RuntimeTraits.интерфейс_ли интерфейс_ли;
}
...
class ИсклНесовпаденияВариантногоТипа : Исключение
{
    ...
    {
        super("cannot преобразуй "~ожидалось.вТкст
                    ~" значение в_ a "~got.вТкст);
    }
}
...
struct Вариант
{
    ...
    Вариант opAssign(T)(T значение)
    {
        ...
        else
        {
            ...
            {
                /*
                 * If the значение is small enough в_ fit in the storage
     ...
     *  assert(   v.является_ли!(цел) );
     *  assert( ! v.является_ли!(крат) ); // note no implicit conversion
     * -----
     */
    бул является_ли(T)()
    {
     ...
     *  auto v = Вариант(cast(цел) 42);
     *  assert( v.является_ли!(цел) );
     *  assert( v.является_ли!(крат) ); // note implicit conversion
     * -----
     */
    бул непосредственно_ли(T)()
    {
        static if( is( T == class ) || is( T == interface ) )
        {
            // Check for classes and interfaces first.
            if( cast(ИнфОТипе_Класс) тип || cast(ИнфОТипе_Интерфейс) тип )
     ...
     * is simply крат-hand for calling the является_ли member with a тип of проц.
     *
     * Возвращает:
    ...
    {
        /**
         * This is the primary mechanism for extracting a значение из_ a Вариант.
         ...
         * непосредственно_ли member with the тип S.
         *
        ...
        верниТ!(S) получи(S)()
        {
            ...
            static if( основнойТип_ли!(T) )
            {
                if( тип is typeid(T) )
                {
                    ...
                    // small, grab it out of storage; otherwise, копируй it из_
                    // the куча.
    ...
    version( РазрешитьВарарг )
    {
        ...
        static проц вВарарг(Вариант[] vars, out ИнфОТипе[] типы, out спис_ва арги)
        {
            ...
            // Create the storage, and tell the СМ whether it needs в_ be scanned
            // or not.
            auto storage = new ббайт[размер];
            СМ.setAttr(storage.ptr,
                (СМ.getAttr(storage.ptr) & ~СМ.BlkAttr.NO_SCAN)
                | (noptr ? СМ.BlkAttr.NO_SCAN : 0));

            // Dump the variants преобр_в the storage.
            арги = storage.ptr;
            auto arg_temp = арги;
...
private:
    ...
    static ук  изУк(ИнфОТипе тип, ук  ptr, out Вариант r)
    {
        ...
        if( статМасс_ли(тип) )
        {
             ...
             * in непосредственно_ли(S) and получи(S).
             */
        ...
        ук  вУк(ук  ptr)
        {
            ...
            else
            {
                if( интерфейс_ли(тип) )
                {
                     ...
                     * accordingly.
                     */
                    auto type_i = cast(ИнфОТипе_Интерфейс) тип;
                    бул найдено = нет;
     ...
     */
    цел opEqualsVariant(Вариант rhs)
    {
        if( тип != rhs.тип ) return нет;
        return cast(бул) тип.равны_ли(this.ptr, rhs.ptr);
    }

    /*
     * Same as opEqualsVariant except it does opCmp.
     */
    цел opCmpVariant(Вариант rhs)
    {
        if( тип != rhs.тип )
...
debug( UnitTest )
{
    ...
    {
        Вариант v;
        assert( v.является_ли!(проц), v.тип.вТкст );
        assert( v.пуст_ли, v.тип.вТкст );

        // Test basic целое storage and implicit casting support
        v = 42;
        assert( v.является_ли!(цел), v.тип.вТкст );
        assert( v.непосредственно_ли!(дол), v.тип.вТкст );
        assert( v.непосредственно_ли!(бдол), v.тип.вТкст );
        assert( !v.непосредственно_ли!(бцел), v.тип.вТкст );
        ...
        // Test strings
        v = "Hello, World!"c;
        assert( v.является_ли!(ткст), v.тип.вТкст );
        assert( !v.непосредственно_ли!(шим[]), v.тип.вТкст );
        ...
        v = &v;
        assert( v.является_ли!(Вариант*), v.тип.вТкст );
        assert( !v.непосредственно_ли!(цел*), v.тип.вТкст );
        assert( v.непосредственно_ли!(проц*), v.тип.вТкст );
        ...
        // Test объект storage
        {
            scope o = new Объект;
            v = o;
            assert( v.является_ли!(Объект), v.тип.вТкст );
        ...
        // Test class/interface implicit casting
        {
            ...
            Вариант v2 = a;
            assert( v2.непосредственно_ли!(Объект), v2.тип.вТкст );
            assert( v2.непосредственно_ли!(G), v2.тип.вТкст );
            assert(!v2.непосредственно_ли!(I), v2.тип.вТкст );
            ...
            assert( v2.непосредственно_ли!(Объект), v2.тип.вТкст );
            assert(!v2.непосредственно_ли!(G), v2.тип.вТкст );
            assert( v2.непосредственно_ли!(H), v2.тип.вТкст );
            assert( v2.непосредственно_ли!(J), v2.тип.вТкст );
        ...
        v = 3.1413;
        assert( v.является_ли!(дво), v.тип.вТкст );
        assert( v.непосредственно_ли!(реал), v.тип.вТкст );
        assert( !v.непосредственно_ли!(плав), v.тип.вТкст );
        assert( v.получи!(дво) == 3.1413 );

        // Test storage transitivity
        auto u = Вариант(v);
        assert( u.является_ли!(дво), u.тип.вТкст );
