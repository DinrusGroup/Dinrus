...
alias бул function(ref Исключение.ИнфОКадре fInfo,КонтекстСледа* контекст,ткстбуф) SymbolizeFrameInfoFnc;
...
extern(C) бул rt_symbolizeFrameInfo(ref Исключение.ИнфОКадре fInfo,КонтекстСледа* контекст,ткстбуф){
...
ткст имяФцииПо(ук  адр, ткст буф){
    ...
    if (rt_symbolizeFrameInfo(fInfo,пусто,буф) && fInfo.func.length){
        ...
        return "function@"~ctfe_i2a(cast(т_мера)адр);
    }
}
/// ditto
...
бул defaultSymbolizeFrameInfo(ref Исключение.ИнфОКадре fInfo,КонтекстСледа *контекст,ткстбуф){
    version(ElfSymbolification) {
...
Исключение.ИнфОСледе basicTracer( ук  ptr = пусто ){
    ...
    try{
        ...
        stdrus.дош("tracer got исключение:\n");
        stdrus.дош(e.сооб);
        e.выпиши((ткстs){ stdrus.дош(s); });
        stdrus.дош("\n");
    } catch (Объект o){
        stdrus.дош("tracer got объект исключение:\n");
        stdrus.дош(o.вТкст());
        stdrus.дош("\n");
    }
    return рез;
