...
ИнфОТипе реальныйТип (ИнфОТипе тип)
{
    ...
    auto def = cast(ИнфОТипе_Типдеф) тип;
    if (def !is пусто)
    {
        return def.основа;
    }
...
ИнфОКлассе какКласс (ИнфОТипе тип)
{
    if (интерфейс_ли (тип))
    {
        auto klass = cast(ИнфОТипе_Интерфейс) тип;
        return klass.инфо;
    }
    if (класс_ли (тип))
    {
        auto klass = cast(ИнфОТипе_Класс) тип;
        return klass.инфо;
    }
    return пусто;
}
...
{
    foreach (инфо; примениИнтерфейсы (implementor))
    {
        if (iface is инфо)
            return да;
    }
...
{
    ИнфОКлассе[] инфо;
    foreach (iface; тип.interfaces)
    {
        инфо ~= iface.classinfo;
        инфо ~= графИнтерфейсов (iface.classinfo);
    }
    return инфо;
}
...
struct примениИнтерфейсы
{
    ...
    цел opApply (цел delegate (ref ИнфОКлассе) дг)
    {
        ...
        for (; тип; тип = тип.основа)
        {
            foreach (iface; тип.interfaces)
            {
                результат = дг (iface.classinfo);
                if (результат)
                    return результат;
                результат = примениИнтерфейсы (iface.classinfo).opApply (дг);
                if (результат)
                    return результат;
            }
        }
...
{
    return (cast(ИнфОТипе_Интерфейс) тип) !is пусто;
}
...
{
    тип = реальныйТип (тип);
    return (cast(ИнфОТипе_Класс) тип) !is пусто;
}
...
ИнфОТипе типВозврат (ИнфОТипе тип)
{
    ...
    auto func = cast(TypeInfo_Function) тип;
    if (func)
        return func.следщ;
    return пусто;
}
