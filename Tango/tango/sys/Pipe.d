...
class Трубопровод : Устройство
{
    ...
    /**
     * Create a Трубопровод with the provопрed укз and access permissions.
     *
     * Параметры:
     * укз       = укз of the operating system pipe we will wrap insопрe
     *                the Трубопровод.
    ...
    version (OLD)
    {
        ...
        protected override бцел читай (проц[] приёмн)
        {
            ...
            if (!ReadFile (укз, p, приёмн.length, &читай, пусто))
            {
        ...
        protected override бцел пиши (проц[] ист)
        {
            ...
            if (!WriteFile (укз, ист.ptr, ист.length, &записано, пусто))
            {
                ошибка();
            }
            return записано;
...
class Пайп
{
    ...
    version (Windows)
    {
        ...
        package this(бцел размерБуфера, SECURITY_ATTRIBUTES *sa)
        {
            ...
            if (CreatePipe(&sourceHandle, &стокHandle, sa, cast(DWORD) размерБуфера))
            {
                _source = new Трубопровод(sourceHandle);
                _сток = new Трубопровод(стокHandle);
            }
            else
            {
                ошибка();
    ...
    {
        throw new ВВИскл("Пайп ошибка: " ~ СисОш.последнСооб);
    }
}
