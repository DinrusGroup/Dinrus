...
private import sys.Common, sys.Types;
private import sys.Pipe;
private import exception;
private import text.Util;
private import Целое = text.convert.Integer;

private import cidrus, stringz;
...
version (Posix)
{
    ...
    private import rt.core.stdc.posix.sys.жди;
...
class Процесс
{
    ...
    public struct Результат
    {
        ...
        public ткст вТкст()
        {
            ...
            {
                case Выход:
                    ткт = форматируй("Процесс завершился нормально, вернув код ", статус);
                    break;

                case Сигнал:
                    ткт = форматируй("Процесс подавлен с сигналом ", статус);
                    break;
                ...
                case Ошибка:
                    ткт = форматируй("Процесс угас с кодом ошибки ", резон) ~
                                 " : " ~ СисОш.найди(статус);
                    break;

                default:
                    ткт = форматируй("Неизвестный результат процесса ", резон);
                    break;
            }
            return ткт;
        }
    ...
    version (Windows)
    {
        private PROCESS_INFORMATION *_info = пусто;
     ...
     *
     * Параметры:
     * арги     = Массив of strings with the process' аргументы.  If there is
     ...
     * среда      = associative Массив of strings with the process' environment
     ...
     *
     * Параметры:
     * арги     = Массив of strings with the process' аргументы; the first
     ...
     *
     * Параметры:
     * среда  = associative Массив of strings containing the environment
        ...
        version (Windows)
        {
            ...
            if(_redirect != ПРедирект.Нет)
            {
                ...
                if(_redirect & ПРедирект.Ввод)
                {
                    ...
                    _stdin = pin.сток;
                    startup.hStdInput = cast(HANDLE) pin.источник.ptr();
                    // Ensure the пиши укз в_ the pipe for STDIN is not inherited.
                    SetHandleInformation(cast(HANDLE) pin.сток.ptr(), HANDLE_FLAG_INHERIT, 0);
                }
                else
                {
                ...
                if((_redirect & (ПРедирект.Вывод | ПРедирект.ВыводНаОш)) == ПРедирект.Вывод)
                {
                    ...
                    startup.hStdOutput = cast(HANDLE) pout.сток.ptr();
                    // Ensure the читай укз в_ the pipe for STDOUT is not inherited.
                    SetHandleInformation(cast(HANDLE) pout.источник.ptr(), HANDLE_FLAG_INHERIT, 0);
                }
                else
                {
                ...
                if((_redirect & (ПРедирект.Ошибка | ПРедирект.ОшНаВывод)) == ПРедирект.Ошибка)
                {
                    ...
                    startup.hStdError = cast(HANDLE) perr.сток.ptr();
                    // Ensure the читай укз в_ the pipe for STDOUT is not inherited.
                    SetHandleInformation(cast(HANDLE) perr.источник.ptr(), HANDLE_FLAG_INHERIT, 0);
                }
                else
                {
            ...
            */
            ...
            else
            {
              ...
              if (CreateProcessW(пусто, вТкст16(команда).ptr, пусто, пусто, да,
                    _gui ? CREATE_NO_WINDOW : 0,
                    (_copyEnv ? пусто : toNullEndedBuffer(_env).ptr),
                    ВТкст16н(вТкст16(_workDir)), &startup, _info))
              {
                CloseHandle(_info.hThread);
                _running = да;
              }
        ...
        else version (Posix)
        {
            ...
            if (_pid >= 0)
            {
                ...
                else
                {
                    ...
                    if(pin !is пусто)
                    {
                        dup2(pin.источник.ptr(), STDIN_FILENO);
                        pin.сток().закрой();
                        pin.источник.закрой();
                    }
                    ...
                    if(pout !is пусто)
                    {
                        dup2(pout.сток.ptr(), STDOUT_FILENO);
                    ...
                    if(perr !is пусто)
                    {
                        dup2(perr.сток.ptr(), STDERR_FILENO);
                    ...
                    // вызов в_ execv*()
                    if (fcntl(cast(цел) pexec.сток.ptr(), F_SETFD, FD_CLOEXEC) == 0)
                    {
                        ...
                        if (rc == -1)
                        {
                            Кош("Failed в_ exec ")(_args[0])(": ")(СисОш.последнСооб).нс;
                    ...
                    {
                        Кош("Failed в_ установи notification pipe в_ закрой-on-exec for ")
    ...
    {
        /**
         * Convert an associative Массив of strings в_ a буфер containing a
         * concatenation of "<имя>=<значение>" strings separated by a пусто
    ...
    {
        /**
         * Convert an Массив of strings в_ an Массив of pointers в_ сим with
         * a terminating пусто character (C strings). The resulting Массив
        ...
        /**
         * Convert an associative Массив of strings в_ an Массив of pointers в_
         * сим with a terminating пусто character (C strings). The resulting
...
class ИсклСозданияПроцесса: ProcessException
{
    ...
    {
        super("Could not создай process for " ~ команда ~ " : " ~ сообщение);
    }
}
...
class ИсклВетвленияПроцесса: ProcessException
{
    ...
    {
        super(форматируй("Could not fork process ", пид) ~ " : " ~ СисОш.последнСооб);
    }
}
...
class ИсклТушенияПроцесса: ProcessException
{
    public this(цел пид, ткст файл, бцел строка)
    {
        super(форматируй("Could not затуши process ", пид) ~ " : " ~ СисОш.последнСооб);
    }
}
...
class ИсклОжиданияПроцесса: ProcessException
{
    public this(цел пид, ткст файл, бцел строка)
    {
        super(форматируй("Could not жди on process ", пид) ~ " : " ~ СисОш.последнСооб);
    }
}
...
debug (UnitTest)
{
    unittest
    {
        ...
        try
        {
            auto p = new Процесс(команда, пусто);
        ...
        {
            Кош("Program execution неудачно: ")(e.вТкст()).нс();
        }
    }
}
