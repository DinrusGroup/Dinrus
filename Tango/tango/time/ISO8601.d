/*******************************************************************************
        ...
        This module is based on the ISO 8601:2004 стандарт, and есть functions
        for parsing (almost) every дата/время форматируй specified therein. (The
        ones not supported are intervals, durations, and recurring intervals.)

        Refer в_ the стандарт for a full descrИПtion of the formats supported.

        The functions (парсируйВремя, парсируйДату, and парсируйДатуИВремя) are
        overloaded преобр_в two different versions of each: one updates a given
        Время, and the другой updates a given ДатаДоп struct. The purpose of
        this struct is в_ support ещё detailed information which the Время данные
        тип does not (and, given its simple целое nature, cannot) support.

        Times with specified время zones are simply преобразованый преобр_в UTC: this may
        lead в_ the дата changing when only a время was разобрано: e.g. "01:00+03"
        is the same as "22:00", except that when the former is разобрано, one is
        subtracted из_ the день.
...
public struct ДатаДоп {
   ...
   out(знач) {
      ...
      auto эра = Gregorian.generic.дайЭру(знач);
      if (эра == Gregorian.AD_ERA)
         return Gregorian.generic.дайГод(знач);
      else
         return -Gregorian.generic.дайГод(знач);
   }
   ...
   private проц год(цел y) {
      if (DTyear(y)) {
         ...
         цел toдобавь = y - Gregorian.generic.дайГод(знач);
         знач = Gregorian.generic.добавьГоды(знач, toдобавь);
      } else
         year_ = y < 0 ? y-1 : y;
   }
   ...
   private проц установиВисокосным    () { маска |= 1 << 0; }
...
in {
   ...
   цел результат =
      Gregorian.generic.дайДеньНедели(
         Gregorian.generic.воВремя(год, месяц, день, 0, 0, 0, 0, эра));

   if (результат == Gregorian.ДеньНедели.Воскресенье)
      return 7;
   else
      return результат;
}
...
цел днейВМесяце(цел месяц, цел год) {
   бцел эра = erafy(год);
   return Gregorian.generic.дайДниМесяца(год, месяц, эра);
}
...
проц добавьМесяцы(ref ПолнаяДата d, цел n) { d.знач = Gregorian.generic.добавьМесяцы(d.знач, n-1); } // -1 due в_ начальное being 1
...
цел годы (ПолнаяДата d) { return Gregorian.generic.дайГод      (d.знач); }
цел месяцы(ПолнаяДата d) { return Gregorian.generic.дайМесяц     (d.знач); }
цел дни  (ПолнаяДата d) { return Gregorian.generic.дайДеньМесяца(d.знач); }
