...
public class Hijri : Календарь {
  ...
  public override Время воВремя(бцел год, бцел месяц, бцел день, бцел час, бцел минута, бцел сукунда, бцел миллисек, бцел эра) {
    return Время((daysSinceJan1(год, месяц, день) - 1) * ИнтервалВремени.ТиковВДень + дайТикиВремени(час, минута, сукунда)) + ИнтервалВремени.изМиллисек(миллисек);
  }
  ...
  private дол daysToYear(бцел год) {
    ...
    цел remaining = год - cycle - 1;
    дол дни = ((cycle * 10631L) / 30L) + 227013L;
    while (remaining > 0) {
      дни += 354 + (високосен_ли(remaining, ТЕКУЩАЯ_ЭРА) ? 1 : 0);
      remaining--;
    }
    return дни;
  }
