 ...
 * Gregorian in that leap годы occur every 4 годы, even on 100 год
...
class Gregorian : Calendar 
{
        ...
        alias Calendar.воВремя воВремя;
        ...
        enum Тип 
        {
                Локализованный = 1,               /// Refers в_ the localized version of the Gregorian calendar.
                АнглСША = 2,               /// Refers в_ the US English version of the Gregorian calendar.
                СреднеВостФранц = 9,        /// Refers в_ the Mопрdle East French version of the Gregorian calendar.
                Арабский = 10,                 /// Refers в_ the _Arabic version of the Gregorian calendar.
                ТранслитерАнгл = 11,  /// Refers в_ the transliterated English version of the Gregorian calendar.
                ТранслитерФранц = 12    /// Refers в_ the transliterated French version of the Gregorian calendar.
        }
        ...
        */
        ...
        /**
        * создай a generic экземпляр of this calendar
        */
        static this()
        {       
        ...
        /**
        * Initializes an экземпляр of the Gregorian class using the specified GregorianTypes значение. If no значение is 
        * specified, the default is Gregorian.Types.Локализованный.
        */
        ...
        */
        override Время воВремя (бцел год, бцел месяц, бцел день, бцел час, бцел минута, бцел сукунда, бцел миллисек, бцел эра)
        {
                return Время (дайТикиДаты(год, месяц, день, эра) + дайТикиВремени(час, минута, сукунда)) + ИнтервалВремени.изМиллисек(миллисек);
        }
        ...
        */
        ...
        {
                return извлекиЧасть(время.тики, ЧастьДаты.День);
        }
        ...
        */
        ...
        {
                return извлекиЧасть(время.тики, ЧастьДаты.ДеньГода);
        }
        ...
        */
        ...
        {
                return извлекиЧасть(время.тики, ЧастьДаты.Месяц);
        }
        ...
        */
        ...
        {
                return извлекиЧасть(время.тики, ЧастьДаты.Год);
        }
        ...
        */
        ...
        {
                //
                // проверь арги.  високосен_ли verifies the год is valid.
                //
                if(месяц < 1 || месяц > 12)
        ...
        */
        ...
        /**
        * $(I Property.) Retrieves the GregorianTypes значение indicating the language version of the Gregorian.
        * Возвращает: The Gregorian.Тип значение indicating the language version of the Gregorian.
        */
        Тип типКалендаря() 
        {
                return type_;
        ...
        */
         ...
         * of the calendar.  This is useful if you want ещё than one of the
         ...
         *
         * Note that the Gregorian calendar takes преобр_в account that BC время
         * is negative, and supports crossing из_ BC в_ AD.
         *
        ...
        package static бцел извлекиЧасть (дол тики, ЧастьДаты часть) 
        {
        ...
        {
                //
                // проверь аргументы, дайДниМесяца verifies the год and
                // месяц is valid.
                //
                if(день < 1 || день > generic.дайДниМесяца(год, месяц, эра))
                        ошАрга("дни out of range");
        ...
        package static проц ошАрга(ткст ткт)
        {
                throw new ИсклНелегальногоАргумента(ткт);
        }
}
...
debug(Gregorian)
{
        ...
        проц вывод(Время t)
        {
                Date d = Gregorian.generic.вДату(t);
                ВремяДня tod = t.время;
                Стдвыв.форматируй("{}/{}/{:d4} {} {}:{:d2}:{:d2}.{:d3} деньнед:{}",
                                d.месяц, d.день, d.год, d.эра == Gregorian.AD_ERA ? "AD" : "BC",
...
debug(UnitTest)
{
        ...
        {
                //
                // check Gregorian дата handles positive время.
                //
                ...
                assert(d.месяц == 1);
                assert(d.день == 1);
                assert(d.эра == Gregorian.AD_ERA);
                assert(d.деньгода == 1);
                //
                // note that this is in disagreement with the Julian Calendar
                //
                assert(d.деньнед == Gregorian.ДеньНедели.Вторник);

                //
                // check that it handles negative время
                //
                t = Время.эпоха - ИнтервалВремени.изДней(366);
                d = Gregorian.generic.вДату(t);
                assert(d.год == 1);
                assert(d.месяц == 1);
                assert(d.день == 1);
                assert(d.эра == Gregorian.BC_ERA);
                assert(d.деньгода == 1);
                assert(d.деньнед == Gregorian.ДеньНедели.Суббота);

                //
                // check that добавьМесяцы works properly, добавь 15 месяцы в_
                // 2/3/2004, 04:05:06.007008, then вычти 15 месяцы again.
                //
                t = Gregorian.generic.воВремя(2004, 2, 3, 4, 5, 6, 7) + ИнтервалВремени.изМикросек(8);
                d = Gregorian.generic.вДату(t);
                assert(d.год == 2004);
                assert(d.месяц == 2);
                assert(d.день == 3);
                ...
                auto t2 = Gregorian.generic.добавьМесяцы(t, 15);
                d = Gregorian.generic.вДату(t2);
                assert(d.год == 2005);
                assert(d.месяц == 5);
                assert(d.день == 3);
                assert(d.эра == Gregorian.AD_ERA);
                ...
                t2 = Gregorian.generic.добавьМесяцы(t2, -15);
                d = Gregorian.generic.вДату(t2);
                assert(d.год == 2004);
                assert(d.месяц == 2);
                assert(d.день == 3);
                ...
                try
                {
                        t = Gregorian.generic.воВремя (0, 1, 1, 0, 0, 0, 0, Gregorian.AD_ERA);
                ...
                try
                {
                        t = Gregorian.generic.воВремя (1, 0, 1, 0, 0, 0, 0, Gregorian.AD_ERA);
                        assert(нет, "Dопр not throw illegal аргумент исключение");
                }
                ...
                try
                {
                        t = Gregorian.generic.воВремя (1, 1, 0, 0, 0, 0, 0, Gregorian.BC_ERA);
                        assert(нет, "Dопр not throw illegal аргумент исключение");
                }
                catch(ИсклНелегальногоАргумента iae)
                {
                ...
                try
                {
                    t = Gregorian.generic.воВремя(2000, 1, 31, 0, 0, 0, 0);
                    t = Gregorian.generic.добавьМесяцы(t, 1);
                    assert(нет, "Dопр not throw illegal аргумент исключение");
                }
                catch(ИсклНелегальногоАргумента iae)
                {
                ...
                {
                    t = Gregorian.generic.воВремя(2000, 1, 31, 0, 0, 0, 0);
                    t = Gregorian.generic.добавьМесяцы(t, 1, да);
                    assert(Gregorian.generic.дайДеньМесяца(t) == 29);
                }
                catch(ИсклНелегальногоАргумента iae)
                {
