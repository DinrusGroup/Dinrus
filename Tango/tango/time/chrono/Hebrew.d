...
public class Hebrew : Calendar {
  ...
  public override Время воВремя(бцел год, бцел месяц, бцел день, бцел час, бцел минута, бцел сукунда, бцел миллисек, бцел эра) {
    checkYear(год, эра);
    return getGregorianTime(год, месяц, день, час, минута, сукунда, миллисек);
  }
  ...
  private бцел getYearType(бцел год) {
    ...
    // Satisfies -w
    throw new ИсклНелегальногоАргумента("Значение was not valid.");
  }
  ...
  private Время getGregorianTime(бцел год, бцел месяц, бцел день, бцел час, бцел минута, бцел сукунда, бцел миллисек) {
    ...
    return Время((дни * ИнтервалВремени.ТиковВДень) + дайТикиВремени(час, минута, сукунда)) + ИнтервалВремени.изМиллисек(миллисек);
  }
