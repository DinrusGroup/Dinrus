...
import math.Bracket:findRoot;
import math.Math:абс;
import math.ErrorFunction:erfc;
import Traits;

/// ziggurat метод for decreasing distributions.
/// Marsaglia, Tsang, Journal of Statistical Software, 2000
/// If есть negative is да the distribution is assumed в_ be symmetric with respect в_ 0, 
/// otherwise it is assumed в_ be из_ 0 в_ infinity.
/// Struct based в_ avoопр extra indirection when wrapped in a class (and it should be wrapped
...
struct Циггурат(СлучГ,T,alias плотностьВерФ,alias хвостГенератор,бул естьНегатив=да){
    ...
    static Циггурат создай(alias инвПлотностьВерФ, alias кумПлотностьВерФКомпл)(СлучГ рГенератор,реал xLast=-1.0L,бул check_error=да){
        ...
        реал findXLast(реал xLast){
            ...
            for (цел i=члоБлоков-2;i!=0;--i){
                ...
                assert(pAtt>=0,"инвПлотностьВерФ is supposed в_ return positive values");
            }
        ...
        with (рез){
            ...
            for (цел i=2;i<члоБлоков;++i){
                fAtt+=v/pAtt;
                assert(fAtt<=fMax,"Циггурат contruction shoot out");
                pAtt=инвПлотностьВерФ(fAtt);
                assert(pAtt>=0,"инвПлотностьВерФ is supposed в_ return positive values");
            ...
            assert((!check_error) || ошибка<реал.epsilon*10000.0,"Циггурат ошибка larger than ожидалось");
            блокПоз[0]=cast(T)(xLast*(1.0L+кумПлотностьВерФКомпл(xLast)/плотностьВерФ(xLast)));
            значФ[0]=0.0L;
            for (цел i=0;i<члоБлоков;++i){
                assert(блокПоз[i]>=блокПоз[i+1],"decresing блокПоз");
                assert(значФ[i]<=значФ[i+1],"increasing probabilty density function");
            }
        }
        return рез;
    }
    /// returns a single значение with the probability distribution of the current Циггурат
    /// and slightly worse randomness (in the нормаль case uses only 32 random биты).
    /// Cannot be 0.
    T дайБыстрСлуч() 
    {
        ...
        throw new Исключение("max nr of iterations in Циггурат, this should have probability<1.0e-1000");
    }
    /// returns a single значение with the probability distribution of the current Циггурат
    T дайСлучайный() 
    {
    ...
    /// initializes the аргумент with the probability distribution given and returns it
    ...
    /// initializes the переменная with the результат of mapping op on the random numbers (of тип T)
