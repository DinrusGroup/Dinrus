...
import math.Math:эксп,квкор,лог,PI;
...
class НормальныйИсточник(СлучГ,T){
    ...
    /// probability distribution (non normalized, should be divопрed by квкор(2*PI))
    ...
    static реал кумПлотностьВерФКомпл(реал x){ return квкор(0.5L*PI)*erfc(x/квкор(2.0L));/*normalDistributionCompl(x);*/ }
    /// хвост for нормаль distribution
    ...
    /// returns a нормаль distribued число with the given сигма (стандарт deviation) and мю (average)
    final T дайСлучайный(T сигма, T мю){
        return мю+сигма*источник.дайСлучайный();
    }
    /// initializes a переменная with нормаль distribued число and returns it
    U рандомируй(U)(ref U a){
        return источник.рандомируй(a);
    }
    /// initializes a переменная with нормаль distribued число with the given сигма and returns it
    U рандомируй(U,V)(ref U a,V сигма){
        return источник.рандомирОп((T x){ return x*cast(T)сигма; },a);
    }
    /// initializes a переменная with нормаль distribued numbers with the given сигма and мю and returns it
    ...
    /// нормаль distribution with different default сигма and мю
    ...
    struct НормальнаяДистрибуция{
        ...
        /// инициализуй a (let s and m have different типы??)
    ...
    /// returns a нормаль distribution with a non-default сигма/мю
