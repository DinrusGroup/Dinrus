...
/+ CMWC and KISS random число generators combined, for extra security wrt. plain CMWC and
+ Marisaglia, Journal of Modern Applied Statistical Methods (2003), vol.2,No.1,p 2-13
+ a simple safe and fast RNG that проходки все statistical tests, есть a large сей, and is reasonably fast
...
struct KissCmwc(бцел cmwc_r=1024U,бдол cmwc_a=987769338UL){
    ...
    бцел следщ(){
        ...
        static assert((rMask&cmwc_r)==0,"cmwc_r is supposed в_ be a power of 2"); // помести a ещё stringent тест?
        ...
        k_t = a*kiss_z+kiss_c; kiss_c = cast(бцел)(k_t>>32);
    ...
    ткст вТкст(){
        ...
        assert(i==рез.length,"unexpected размер");
        return рез;
    }
    ...
    т_мера fromString(ткст s){
        сим[16] tmpC;
        т_мера i=0;
        assert(s[i..i+11]=="CMWC+KISS99","unexpected kind, ожидалось CMWC+KISS99");
        i+=11;
        assert(s[i..i+16]==Целое.форматируй(tmpC,cmwc_a,"x16"),"unexpected cmwc_a");
        i+=16;
        assert(s[i]=='_',"unexpected форматируй, ожидалось _");
        i++;
        assert(s[i..i+8]==Целое.форматируй(tmpC,cmwc_r,"x8"),"unexpected cmwc_r");
        i+=8;
        foreach (ref знач;cmwc_q){
            ...
            assert(ate==8,"unexpected читай размер");
            i+=8;
        }
        foreach (знач;[&cmwc_i,&cmwc_c,&nBytes,&restB,&kiss_x,&kiss_y,&kiss_z,&kiss_c]){
            assert(s[i]=='_',"не найден разделитель  _ ");
...
/// some variations of the CMWC часть, the first есть a период of ~10^39461
/// the first число (r) is basically the размер of the сей and storage (and все bit образцы
