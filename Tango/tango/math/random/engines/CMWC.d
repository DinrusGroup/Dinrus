...
+ a simple and fast RNG that проходки все statistical tests, есть a large сей, and is very fast
...
struct CMWC(бцел cmwc_r=1024U,бдол cmwc_a=987769338UL){
    ...
    т_мера fromString(ткст s){
        сим[16] tmpC;
        т_мера i=0;
        assert(s[i..i+4]=="CMWC","unexpected kind, ожидалось CMWC");
        i+=4;
...
/// some variations, the first есть a период of ~10^39461, the first число (r) is basically the размер of the сей
/// (and все bit образцы of that размер are guarenteed в_ crop up in the период), the период is 2^(32*r)*a
