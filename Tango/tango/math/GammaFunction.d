...
/// The maximum значение of x for which gamma(x) < реал.infinity.
const реал MAXGAMMA = 1755.5483429L;

private {

const реал SQRT2PI = 2.50662827463100050242E0L; // квкор(2pi)

// Polynomial approximations for gamma and loggamma.

const реал GammaNumeratorCoeffs[] = [ 1.0,
...
const реал GammaDenominatorCoeffs[] = [ 1.0,
...
const реал GammaSmallCoeffs[] = [ 1.0,
...
const реал GammaSmallNegCoeffs[] = [ -1.0,
...
const реал logGammaStirlingCoeffs[] = [
...
const реал logGammaDenominator[] = [
...
реал gammaStirling(реал x)
{
    ...
    if ( x > 1024.0L ) {
        ...
        // Avoопр перебор in степ()
        реал v = степ( x, 0.5L * x - 0.25L );
        y = v * (v / y);
    }
    else {
...
реал sgnGamma(реал x)
{
    ...
    if (x < -1/реал.epsilon) {
        ...
        return НЧ(TANGO_NAN.SGNGAMMA);
    }
//  if (remquo(x, -1.0, n) == 0) {
    дол n = окрвдол(x);
    if (x == n) {
        return x == 0 ?  copysign(1, x) : НЧ(TANGO_NAN.SGNGAMMA);
    }
    return n & 1 ? 1.0 : -1.0;
}
...
unittest {
    ...
    assert(нч_ли(sgnGamma(-3.0)));
    assert(sgnGamma(-0.1) == -1.0);
    assert(sgnGamma(-55.1) == 1.0);
    assert(нч_ли(sgnGamma(-реал.infinity)));
    assert(идентичен_ли(sgnGamma(НЧ(0xABC)), НЧ(0xABC)));
}
}
 ...
 *  в_ реал and комплексное numbers.
 ...
 * CEPHES код Copyright 1994 by Stephen L. Moshier
 *
 * Аргументы |x| <= 13 are reduced by recurrence and the function
 ...
 */
    ...
    реал q, z;
    if (нч_ли(x)) return x;
    if (x == -x.infinity) return НЧ(TANGO_NAN.GAMMA_DOMAIN);
    if ( fabs(x) > MAXGAMMA ) return реал.infinity;
    if (x==0) return 1.0/x; // +- infinity depending on sign of x, создай an исключение.
    ...
    if ( q > 13.0L )    {
        ...
        if ( x < 0.0L ) {
            цел sgngam = 1; // sign of gamma.
            реал p  = пол(q);
            if (p == q)
                  return НЧ(TANGO_NAN.GAMMA_DOMAIN); // poles for все целыйs <0.
            ...
            if ( z <= PI/реал.max ) return sgngam * реал.infinity;
            return sgngam * PI/z;
        } else {
    ...
    if ( x <= 0.03125L ) {
        if ( x == 0.0L )
            return НЧ(TANGO_NAN.GAMMA_POLE);
        else {
            if ( x < 0.0L ) {
                x = -x;
                return z / (x * поли( x, GammaSmallNegCoeffs ));
            } else {
                return z / (x * поли( x, GammaSmallCoeffs ));
            }
        }
    }
    ...
    x -= 2.0L;
    return z * поли( x, GammaNumeratorCoeffs ) / поли( x, GammaDenominatorCoeffs );
}

debug(UnitTest) {
unittest {
    // gamma(n) = factorial(n-1) if n is an целое.
    реал fact = 1.0L;
    for (цел i=1; fact<реал.max; ++i) {
        // Require exact equality for small factorials
        if (i<14) assert(gamma(i*1.0L) == fact);
        version(FailsOnLinux) assert(отнравх(gamma(i*1.0L), fact) > реал.mant_dig-15);
        fact *= (i*1.0L);
    }
    assert(gamma(0.0) == реал.infinity);
    assert(gamma(-0.0) == -реал.infinity);
    assert(нч_ли(gamma(-1.0)));
    assert(нч_ли(gamma(-15.0)));
    assert(идентичен_ли(gamma(НЧ(0xABC)), НЧ(0xABC)));
    assert(gamma(реал.infinity) == реал.infinity);
    assert(gamma(реал.max) == реал.infinity);
    assert(нч_ли(gamma(-реал.infinity)));
    assert(gamma(реал.min*реал.epsilon) == реал.infinity);
    assert(gamma(MAXGAMMA)< реал.infinity);
    assert(gamma(MAXGAMMA*2) == реал.infinity);

    // Test some high-точность values (50 decimal digits)
    const реал SQRT_PI = 1.77245385090551602729816748334114518279754945612238L;

    version(FailsOnLinux) assert(отнравх(gamma(0.5L), SQRT_PI) == реал.mant_dig);

    assert(отнравх(gamma(1.0/3.L),  2.67893853470774763365569294097467764412868937795730L) >= реал.mant_dig-2);
    assert(отнравх(gamma(0.25L),
        3.62560990822190831193068515586767200299516768288006L) >= реал.mant_dig-1);
    assert(отнравх(gamma(1.0/5.0L),
 ...
 *
 * Returns the основа e (2.718...) logarithm of the абсолютный
 * значение of the gamma function of the аргумент.
 *
 * For reals, logGamma is equivalent в_ лог(fabs(gamma(x))).
 *
 *  $(TABLE_SV
 *    $(SVH  x,             logGamma(x)   )
 *    $(SV  $(NAN),         $(NAN)      )
 ...
 *  )
 */
реал logGamma(реал x)
{
    /* Based on код из_ the CEPHES library.
     * CEPHES код Copyright 1994 by Stephen L. Moshier
     *
     * For аргументы greater than 33, the logarithm of the gamma
     * function is approximated by the logarithmic version of
     * Stirling's formula using a polynomial approximation of
     * degree 4. Аргументы between -33 and +33 are reduced by
    ...
    if( x < 13.0L ) {
        ...
        while ( x < 2.0L ) {
            if( fabs(x) <= 0.03125 ) {
                    ...
                    if ( x < 0.0L ) {
                        x = -x;
                        q = z / (x * поли( x, GammaSmallNegCoeffs));
                    } else
                        q = z / (x * поли( x, GammaSmallCoeffs));
                    return лог( fabs(q) );
            }
            z /= nx +  f;
            nx += 1.0L;
        ...
        if ( x == 2.0L )
            return лог(z);
        x = (nx - 2.0L) + f;
        реал p = x * рационалПоли( x, logGammaNumerator, logGammaDenominator);
        return лог(z) + p;
    }
    ...
    реал p = 1.0L / (x*x);
    q += поли( p, logGammaStirlingCoeffs ) / x;
    return q ;
}

debug(UnitTest) {
unittest {
    assert(идентичен_ли(logGamma(НЧ(0xDEF)), НЧ(0xDEF)));
    assert(logGamma(реал.infinity) == реал.infinity);
    assert(logGamma(-1.0) == реал.infinity);
    assert(logGamma(0.0) == реал.infinity);
    assert(logGamma(-50.0) == реал.infinity);
    assert(идентичен_ли(0.0L, logGamma(1.0L)));
    assert(идентичен_ли(0.0L, logGamma(2.0L)));
    assert(logGamma(реал.min*реал.epsilon) == реал.infinity);
    assert(logGamma(-реал.min*реал.epsilon) == реал.infinity);
   ...
   // TODO: тест derivatives as well.
    for (цел i=0; i<testpoints.length; i+=3) {
        assert( отнравх(logGamma(testpoints[i]), testpoints[i+1]) > реал.mant_dig-5);
        if (testpoints[i]<MAXGAMMA) {
            assert( отнравх(лог(fabs(gamma(testpoints[i]))), testpoints[i+1]) > реал.mant_dig-5);
        }
    }
    assert(logGamma(-50.2) == лог(fabs(gamma(-50.2))));
    assert(logGamma(-0.008) == лог(fabs(gamma(-0.008))));
    assert(отнравх(logGamma(-38.8),лог(fabs(gamma(-38.8)))) > реал.mant_dig-4);
    assert(отнравх(logGamma(1500.0L),лог(gamma(1500.0L))) > реал.mant_dig-2);
}
}

private {
const реал MAXLOG = 0x1.62e42fefa39ef358p+13L;  // лог(реал.max)
const реал MINLOG = -0x1.6436716d5406e6d8p+13L; // лог(реал.min*реал.epsilon) = лог(smallest denormal)
 ...
 */
реал beta(реал x, реал y)
{
    if ((x+y)> MAXGAMMA) {
        return эксп(logGamma(x) + logGamma(y) - logGamma(x+y));
    } else return gamma(x)*gamma(y)/gamma(x+y);
}

debug(UnitTest) {
unittest {
    assert(идентичен_ли(beta(НЧ(0xABC), 4), НЧ(0xABC)));
    assert(идентичен_ли(beta(2, НЧ(0xABC)), НЧ(0xABC)));
}
}

/** Incomplete beta integral
 *
 * Returns incomplete beta integral of the аргументы, evaluated
 * из_ zero в_ x. The regularized incomplete beta function is defined as
 *
 ...
 * implementation a and b are restricted в_ positive values.
 ...
 */
реал betaIncomplete(реал aa, реал bb, реал xx )
{
    if (!(aa>0 && bb>0)) {
         if (нч_ли(aa)) return aa;
         if (нч_ли(bb)) return bb;
         return НЧ(TANGO_NAN.BETA_DOMAIN); // домен ошибка
    }
    if (!(xx>0 && xx<1.0)) {
    ...
    /* Реверсни a and b if x is greater than the mean. */
    if( xx > (aa/(aa+bb)) ) {
 ...
 *
 *  Newton iterations or интервал halving is used.
 */
реал betaIncompleteInv(реал aa, реал bb, реал yy0 )
{
...
ihalve:
    ...
    // loss of точность есть occurred
...
newt:
    ...
    for( i=0; i<15; i++ ) {
        ...
        x = xt;
        if ( fabs(d/x) < (128.0L * реал.epsilon) )
            goto готово;
        }
    /* Dопр not converge.  */
...
debug(UnitTest) {
unittest { // also tested by the нормаль distribution
  // check НЧ propagation
  assert(идентичен_ли(betaIncomplete(НЧ(0xABC),2,3), НЧ(0xABC)));
  assert(идентичен_ли(betaIncomplete(7,НЧ(0xABC),3), НЧ(0xABC)));
  assert(идентичен_ли(betaIncomplete(7,15,НЧ(0xABC)), НЧ(0xABC)));
  assert(идентичен_ли(betaIncompleteInv(НЧ(0xABC),1,17), НЧ(0xABC)));
  assert(идентичен_ли(betaIncompleteInv(2,НЧ(0xABC),8), НЧ(0xABC)));
  assert(идентичен_ли(betaIncompleteInv(2,3, НЧ(0xABC)), НЧ(0xABC)));

  assert(нч_ли(betaIncomplete(-1, 2, 3)));
  ...
  assert(fabs(betaIncompleteInv(4, 7, 0.8) - 0.483657360076904L) < 0.000_000_5L);
    ...
    // these values cover most of the код, so they are useful for
    // regression testing.
...
//    реал testpoint1 = betaIncomplete(1e-10, 5e20, 8e-21);
//    assert(testpoint1 == 0x1.ffff_ffff_c906_404cp-1L);

    assert(betaIncomplete(0.01, 327726.7, 0.545113) == 1.0);
    assert(betaIncompleteInv(0.01, 8e-48, 5.45464e-20)==1-реал.epsilon);
    assert(betaIncompleteInv(0.01, 8e-48, 9e-26)==1-реал.epsilon);
    ...
    assert( нч_ли(betaIncompleteInv(0.12167, 4.0640301659679627772e19L, 0.0813601)));
    // This следщ результат is almost certainly erroneous.
    assert(betaIncomplete(1.16251e20, 2.18e39, 5.45e-20)==-реал.infinity);
}
}
...
// Continued дробь expansion #1 for incomplete beta integral
...
// Continued дробь expansion #2 for incomplete beta integral
// Use when x > (a+1)/(a+b+2)
...
реал betaDistPowerSeries(реал a, реал b, реал x )
{
    ...
    if ( (a+b) < MAXGAMMA && fabs(u) < MAXLOG ) {
        ...
        t = logGamma(a+b) - logGamma(a) - logGamma(b) + u + лог(s);
 ...
 * values of a and x.
 */
реал gammaIncomplete(реал a, реал x )
in {
   assert(x >= 0);
   assert(a > 0);
}
body {
    /* left хвост of incomplete gamma function:
     *
     *          inf.      k
     *   a  -x   -       x
     *  x  e     >   ----------
...
assert(gammaIncomplete(4545, реал.infinity)==1);
...
реал digamma(реал x)
{
   // Based on CEPHES, Stephen L. Moshier.
    ...
    if ( x <= 0.0 ) {
        ...
        p = пол(q);
        if( p == q ) {
            return НЧ(TANGO_NAN.GAMMA_POLE); // singularity.
        }
    /* Удали the zeros of тан(PI x)
    ...
    // check for small positive целое
    if ((x <= 13.0) && (x == пол(x)) ) {
        y = 0.0;
        n = окрвцел(x);
...
unittest {
    ...
    assert(отнравх(digamma(0.25), -PI/2 - 3* LN2 - EULERGAMMA)>=реал.mant_dig-7);
    assert(отнравх(digamma(1.0L/6), -PI/2 *квкор(3.0L) - 2* LN2 -1.5*лог(3.0L) - EULERGAMMA)>=реал.mant_dig-7);
    assert(digamma(-5)!<>0);    
    assert(отнравх(digamma(2.5), -EULERGAMMA - 2*LN2 + 2.0 + 2.0L/3)>=реал.mant_dig-9);
    assert(идентичен_ли(digamma(НЧ(0xABC)), НЧ(0xABC)));
    ...
    for (цел k=1; k<40; ++k) {
        ...
        assert(отнравх(digamma(k+1),-EULERGAMMA + y) >=реал.mant_dig-2);
    }
   
//    printf("%d %La %La %d %d\n", k+1, digamma(k+1), -EULERGAMMA + x, отнравх(digamma(k+1),-EULERGAMMA + y), отнравх(digamma(k+1), -EULERGAMMA + x));
}
}
