/**
 * Ошибка Functions and Нормальный Distribution.
 *
...
import math.Math;
import math.IEEE;  // only required for unit tests
...
const реал SQRT2PI = 0x1.40d931ff62705966p+1L;    // 2.5066282746310005024
const реал EXP_2  = 0.13533528323661269189L; /* эксп(-2) */

private {
    
/* erfc(x) = эксп(-x^2) P(1/x)/Q(1/x)
   1/8 <= 1/x <= 1
...
/* erfc(x) = эксп(-x^2) 1/x R(1/x^2) / S(1/x^2)
   1/128 <= 1/x < 1/8
 ...
 *
 * erfc(x) = 1 - erf(x), and есть high relative accuracy for
 * values of x far из_ zero. (For values near zero, use erf(x)).
 *
 *  1 - erf(x) =  2/ $(SQRT)(&pi;)
 ...
 *
 * A special function expx2(x) is used в_ suppress ошибка amplification
...
unittest {
   // High resolution тест points.
    ...
    assert(отнравх(erfc(0.250L), erfc0_250 )>=реал.mant_dig-1);
    assert(отнравх(erfc(0.375L), erfc0_375 )>=реал.mant_dig-0);
    assert(отнравх(erfc(0.500L), erfc0_500 )>=реал.mant_dig-1);
    assert(отнравх(erfc(0.625L), erfc0_625 )>=реал.mant_dig-1);
    assert(отнравх(erfc(0.750L), erfc0_750 )>=реал.mant_dig-1);
    assert(отнравх(erfc(0.875L), erfc0_875 )>=реал.mant_dig-4);
    version(FailsOnLinux) assert(отнравх(erfc(1.000L), erfc1_000 )>=реал.mant_dig-0);
    assert(отнравх(erfc(1.125L), erfc1_125 )>=реал.mant_dig-2);
    assert(отнравх(erf(0.875L), erf0_875 )>=реал.mant_dig-1);
    // The DMC implementation of erfc() fails this следщ тест (just)
    assert(отнравх(erfc(4.1L),0.67000276540848983727e-8L)>=реал.mant_dig-4);

    assert(идентичен_ли(erf(0.0),0.0));
    assert(идентичен_ли(erf(-0.0),-0.0));
    assert(erf(реал.infinity) == 1.0);
    assert(erf(-реал.infinity) == -1.0);
    assert(идентичен_ли(erf(НЧ(0xDEF)),НЧ(0xDEF)));
    assert(идентичен_ли(erfc(НЧ(0xDEF)),НЧ(0xDEF)));
    assert(идентичен_ли(erfc(реал.infinity),0.0));
    assert(erfc(-реал.infinity) == 2.0);
    assert(erfc(0) == 1.0);
}
}
 ...
 *
 * If sign < 0, the результат is inverted; i.e., y = эксп(-x*x) .
 *
 * ACCURACY:
 *                      Relative ошибка:
 * arithmetic      домен        # trials      peak         rms
...
реал expx2(реал x, цел sign)
{
  ...
  /* Represent x as an exact multИПle of M plus a resопрual.
     M is a power of 2 chosen so that эксп(m * m) does not перебор
     or недобор and so that |x - m| is small.  */
...
package {
/*
Computes the нормаль distribution function.

The нормаль (or Gaussian, or bell-shaped) distribution is
defined as:

normalDist(x) = 1/$(SQRT) &pi; $(INTEGRAL -$(INFINITY), x) эксп( - $(POWER t, 2)/2) dt
    = 0.5 + 0.5 * erf(x/квкор(2))
    = 0.5 * erfc(- x/квкор(2))
...
package {
/*
 * Inverse of Нормальный distribution function
 *
...
const реал Q3[] = [ -0x1.74022dd5523e6f84p-34, -0x1.2cb60d61e29ee836p-24,
  ...
  if(p<=0.0L || p>=1.0L) {
        ...
        return НЧ(TANGO_NAN.NORMALDISTRIBUTION_INV_DOMAIN);
    }
    цел код = 1;
    реал y = p;
    if( y > (1.0L - EXP_2) ) {
...
unittest {
    ...
    // The values below are из_ Excel 2003.
...
// Excel 2003 gets все the following values wrong!
...
assert( fabs(неизвестный1 -(-33.79958617269L) ) < 0.00000005);
}
}