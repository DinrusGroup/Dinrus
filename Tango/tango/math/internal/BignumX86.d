 ...
 * The symbol [#] indicates an Массив of machine words which is в_ be
 * interpreted as a multi-байт число.
 *
 ...
 * EAX-> RAX, etc, since все these CPUs use essentially the same pИПeline, and
 * are typically limited by память access.
 * The код uses techniques described in Agner Fog's superb Pentium manuals
 * available at www.agner.org.
 * Not optimised for AMD, which can do two память loads per cycle (Intel
 ...
 *    (<< MMX)  1.7    5.3   1.5    1.2
 *  *           5.0   15.0   4.0    4.3
 *  mulдобавь      5.7   19.0   4.9    4.0
 *  div        30.0   32.0  32.0   22.4
 *  mulAcc(32)  6.5   20.0   5.4    4.9
 *
 * mulAcc(32) is multИПlyAccumulate() for a 32*32 multИПly. Thus it включает
 * function вызов overhead.
 * The timing for Div is quite unpredictable, but it's probably too slow
 * в_ be useful. On 64-bit processors, these times should
...
/*  
  ...
  (a) it frees up the EBP регистрируй
...
/* Duplicate ткст s, with n times, substituting индекс for '@'.
 *
 * Each экземпляр of '@' in s is replaced by 0,1,...n-1. This is a helper
...
alias бцел BigDigit; // A Bignum is an Массив of BigDigits. Usually the machine word размер.
    
// Limits for when в_ switch between multИПlication algorithms.
enum : цел { KARATSUBALIMIT = 18 }; // Minimum значение for which Karatsuba is worthwhile.
enum : цел { KARATSUBASQUARELIMIT=26 }; // Minimum значение for which square Karatsuba is worthwhile
...
бцел multibyteдобавьSub(сим op)(бцел[] приёмник, бцел [] src1, бцел [] src2, бцел carry)
{
    // Timing:
    // Pentium M: 2.25/цел
    // P6 семейство, Core2 have a partial флаги stall when reading the carry flag in
    // an ADC, SBB operation after an operation such as INC or DEC which
    // modifies some, but not все, флаги. We avoопр this by storing carry преобр_в
    // a resister (AL), and restoring it after the branch.
    
    enum { LASTPARAM = 4*4 } // 3* pushes + return адрес.
    asm {
        naked;
        push EDI;
        push EBX;
        ...
        mov EDI, [ESP + LASTPARAM + 5*4]; // приёмник.ptr
             // Carry is in EAX
        // Count UP в_ zero (из_ -длин) в_ minimize loop overhead.
        lea EDX, [EDX + 4*ECX]; // EDX = конец of src1.
        lea ESI, [ESI + 4*ECX]; // EBP = конец of src2.        
        lea EDI, [EDI + 4*ECX]; // EDI = конец of приёмник.

        neg ECX;
        добавь ECX, 8;
        jb L2;  // if length < 8 , bypass the unrolled loop.
L_unrolled:
        shr AL, 1; // получи carry из_ EAX
    }
        mixin(" asm {"
...
unittest
{
    ...
    c[19]=0x3333_3333;
    бцел carry = multibyteдобавьSub!('+')(c[0..18], a[0..18], b[0..18], 0);
    assert(carry==1);
...
{
    enum { LASTPARAM = 1*4 } // 0* pushes + return адрес.
    asm {
        naked;
...
бцел многобайтСдвигЛБезММХ(бцел [] приёмник, бцел [] ист, бцел numbits)
{
    ...
    asm {
        ...
        mov ESI, [ESP + LASTPARAM + 4*1]; //ист.ptr;
        mov ECX, EAX; // numbits;

        mov EAX, [-4+ESI + 4*EBX];
        mov EDX, 0;
        shld EDX, EAX, CL;
        push EDX; // Save return значение
        cmp EBX, 1;
        jz L_last;
        mov EDX, [-4+ESI + 4*EBX];
        тест EBX, 1;
...
бцел многобайтСдвигЛ(бцел [] приёмник, бцел [] ист, бцел numbits)
{
    ...
    asm {
        ...
        movd MM3, EAX; // numbits = биты в_ shift left
        xor EAX, 63;
        align   16;
        inc EAX;        
        movd MM4, EAX ; // 64-numbits = биты в_ shift right
        
        // Get the return значение преобр_в EAX
        and EAX, 31; // EAX = 32-numbits
        movd MM2, EAX; // 32-numbits
        movd MM1, [ESI+4*EBX-4];                
        psrlq MM1, MM2;
        movd EAX, MM1;  // EAX = return значение
        тест EBX, 1;
        jz L_even;
L_odd:
        cmp EBX, 1;
...
L_even: // It's either singly or doubly even
        ...
        // MAIN LOOP -- 128 байты per iteration
...
align 16;
        mov ESI, [ESP + LASTPARAM + 4*1]; //ист.ptr;
        lea EDI, [EDI + 4*EBX]; // EDI = конец of приёмник
        lea ESI, [ESI + 4*EBX]; // ESI = конец of ист
        neg EBX;                // счёт UP в_ zero.
        
        movd MM3, EAX; // numbits = биты в_ shift right
        xor EAX, 63;
        inc EAX;        
        movd MM4, EAX ; // 64-numbits = биты в_ shift left
...
бцел многобайтУмнож(бцел[] приёмник, бцел[] ист, бцел множитель, бцел carry)
{
    ...
    enum { LASTPARAM = 4*4 } // 4* pushes + return адрес.
    // We'll use p2 (загрузи unit) instead of the overworked p0 or p1 (ALU units)
    // when initializing variables в_ zero.
    version(D_PIC)
    {
        enum { zero = 0 }
    }
    ...
    asm {
        ...
        mov ESI, [ESP + LASTPARAM + 4*2];  // ист.ptr
        align 16;
        lea EDI, [EDI + 4*EBX]; // EDI = конец of приёмник
        lea ESI, [ESI + 4*EBX]; // ESI = конец of ист
        mov ECX, EAX; // [carry]; -- последний param is in EAX.
        neg EBX;                // счёт UP в_ zero.
        тест EBX, 1;
        jnz L_odd;
        добавь EBX, 1;
...
L_odd:        
        ...
        mov EAX, ECX; // получи final carry
...
// The inner multИПly-and-добавь loop, together with the Even Запись point.
// MultИПles by M_ADDRESS which should be "ESP+LASTPARAM" or "ESP". OP must be "добавь" or "sub"
// This is the most время-critical код in the BigInt library.
// It is used by Всё Mulдобавь, multИПlyAccumulate, and triangleAccumulate
сим [] asmMulдобавь_innerloop(сим [] OP, сим [] M_ADDRESS) {
    // The bottlenecks in this код are extremely complicated. The MUL, добавь, and ADC
    // need 4 cycles on each of the ALUs units p0 and p1. So we use память загрузи 
    // (unit p2) for initializing registers в_ zero.
    // There are also dependencies between the instructions, and we run up against the
    // ROB-читай предел (can only читай 2 registers per cycle).
    // We also need the число of uops in the loop в_ be a multИПle of 3.
    // The only available execution unit for this is p3 (память пиши). Unfortunately we can't do that
	// if Position-Independent Code is required.
	
	// Register usage
    // ESI = конец of ист
    // EDI = конец of приёмник
    // EBX = индекс. Counts up в_ zero (in steps of 2).
    // EDX:EAX = черновик, used in multИПly.
    // ECX = carry1.
    // EBP = carry2.
	// ESP = points в_ the множитель.
	
	// The first member of 'приёмник' which will be изменён is [EDI+4*EBX].
	// EAX must already contain the first member of 'ист', [ESI+4*EBX].

version(D_PIC) { бул using_PIC = да; } else { бул using_PIC=нет; }
return "asm {
        // Запись point for even length
        добавь EBX, 1;
        mov EBP, ECX; // carry
...
L1:
        ...
        adc ECX, EDX;
		" ~
    (using_PIC ? "" : "   mov storagenop, EDX; ") // сделай #uops in loop a multИПle of 3, can't do this in PIC режим.
	~ "
        mul int ptr [" ~ M_ADDRESS ~ "];
        " ~ OP ~ " [-4+EDI+4*EBX], EBP;
        mov EBP, zero;
...
L_done: " ~ OP ~ " [-8+EDI+4*EBX], ECX;
		...
		// final carry is сейчас in EBP
}

сим [] asmMulдобавь_enter_odd(сим [] OP, сим [] M_ADDRESS) {
return "asm {
        mul int ptr [" ~M_ADDRESS ~"];
        mov EBP, zero;   
        добавь ECX, EAX;
...
{
    // Timing: This is the most время-critical bignum function.
    ...
    //   so Запись в_ the loop is also complicated.
    ...
    const сим [] OP = (op=='+')? "добавь" : "sub";
    version(D_PIC) {
        enum { zero = 0 }
    } else {
        // use p2 (загрузи unit) instead of the overworked p0 or p1 (ALU units)
        // when initializing registers в_ zero.
        static цел zero = 0;
        // use p3/p4 units 
        static цел storagenop; // пиши-only
    }
    
    enum { LASTPARAM = 5*4 } // 4* pushes + return адрес.
    asm {
        naked;
        ...
        lea ESI, [ESI + 4*EBX]; // ESI = конец of ист
        mov EBP, 0;
        mov ECX, EAX; // ECX = ввод carry.
        neg EBX;                // счёт UP в_ zero.
        mov EAX, [ESI+4*EBX];
        тест EBX, 1;
        jnz L_enter_odd;
}
		// Main loop, with Запись point for even length
mixin(asmMulдобавь_innerloop(OP, "ESP+LASTPARAM")); 
asm {
        mov EAX, EBP; // получи final carry
        pop EBP;
        pop EBX;
        pop EDI;
        pop ESI;
...
L_enter_odd:
    mixin(asmMulдобавь_enter_odd(OP, "ESP+LASTPARAM"));
}
...
проц многобайтУмножАккум(бцел [] приёмник, бцел[] left, бцел [] right)
{
    ...
    // ECX = carry1
    // EBP = carry2
    // EDI = конец of приёмник for this пароль through the loop. Index for outer loop.
    // ESI = конец of left. never changes
    // [ESP] = M = right[i] = множитель for this пароль through the loop.
    // right.length is изменён преобр_в приёмник.ptr+приёмник.length
    version(D_PIC) {
        enum { zero = 0 }
    } else {
        // use p2 (загрузи unit) instead of the overworked p0 or p1 (ALU units)
    ...
    enum { LASTPARAM = 6*4 } // 4* pushes + local + return адрес.
    asm {
        naked;
        ...
        lea EDI, [EDI + 4*EBX]; // EDI = конец of приёмник for first пароль
        
        mov EAX, [ESP + LASTPARAM + 4*0]; // right.length
        lea EAX, [EDI + 4*EAX];
        mov [ESP + LASTPARAM + 4*0], EAX; // последний значение for EDI       

        lea ESI, [ESI + 4*EBX]; // ESI = конец of left
        mov EAX, [ESP + LASTPARAM + 4*1]; // right.ptr
        mov EAX, [EAX];
        mov [ESP], EAX; // M
outer_loop:        
        mov EBP, 0;
        mov ECX, 0; // ECX = ввод carry.
        neg EBX;                // счёт UP в_ zero.
        mov EAX, [ESI+4*EBX];
        тест EBX, 1;
        jnz L_enter_odd;
		}
        // -- Inner loop, with even Запись point
        mixin(asmMulдобавь_innerloop("добавь", "ESP"));
asm {
        mov [-4+EDI+4*EBX], EBP;
        добавь EDI, 4;
        ...
        jz outer_done;
        mov EAX, [ESP + LASTPARAM + 4*1]; // right.ptr
        mov EAX, [EAX+4];                 // получи new M
        mov [ESP], EAX;                   // save new M
        добавь int ptr [ESP + LASTPARAM + 4*1], 4; // right.ptr
        mov EBX, [ESP + LASTPARAM + 4*2]; // left.length
        jmp outer_loop;
outer_done:        
        pop EAX;
...
L_enter_odd:
    mixin(asmMulдобавь_enter_odd("добавь", "ESP"));
}
...
бцел многобайтПрисвойДеление(бцел [] приёмник, бцел divisor, бцел перебор)
{
    ...
    asm {
        ...
        // Loop из_ msb в_ lsb
        lea     EDI, [EDI + 4*EBX];        
        mov EBP, EAX; // rem is the ввод remainder, in 0..divisor-1
        // Build the pseudo-inverse of divisor k: 2^64/k
        // First determine the shift in ecx в_ получи the max число of биты in kinv
        xor     ECX, ECX;
...
kinv1:
        ...
        jnc     kinv1;
        dec     ECX;
        // Here, ecx is a left shift moving the msb of k в_ bit 32
...
L2:
        ...
        // most significant word of (rem*2^32+ввод)
        mov     EAX, [ESP+4]; //маска;
        and     EAX, [EDI - 4];
        or      EAX, EBP;
        rol     EAX, CL;
        ...
        // MultИПly by k and вычти в_ получи remainder
        // Subtraction must be готово on two words
        mov     EAX, EDX;
...
Lc:
        mov     [EDI - 4], ESI;
        lea     EDI, [EDI - 4];
        dec     int ptr [ESP + LASTPARAM + 4*1+LOCALS]; // длин
        jnz    L2;
        
        pop EAX; // discard kinv
        pop EAX; // discard маска
...
{
    /* Unlike mulдобавь, the carry is only 1 bit, 
	   since FFFF*FFFF+FFFF_FFFF = 1_0000_0000.
	   Note also that on the последний iteration, no carry can occur.
	   As for multibyteдобавь, we save & restore carry flag through the loop.
    ...
    */
    enum { LASTPARAM = 4*5 } // 4* pushes + return адрес.
 asm {
    naked;
    push ESI;
    push EDI;
	...
	push ECX;
    mov EDI, [ESP + LASTPARAM + 4*3]; //приёмник.ptr;
    mov EBX, [ESP + LASTPARAM + 4*0]; //ист.length;
    mov ESI, [ESP + LASTPARAM + 4*1]; //ист.ptr;
    lea EDI, [EDI + 8*EBX];      // EDI = конец of приёмник
    lea ESI, [ESI + 4*EBX];      // ESI = конец of ист
    neg EBX;                     // счёт UP в_ zero.
    xor ECX, ECX;             // начальное carry = 0.
L1:
    mov EAX, [ESI + 4*EBX];
    mul EAX, EAX;
    shr CL, 1;                 // получи carry
    adc [EDI + 8*EBX], EAX;
    adc [EDI + 8*EBX + 4], EDX;
    setc CL;                   // save carry
    inc EBX;
...
unittest {
	...
	aa[$-1]= 7;
    multibyteдобавьDiagonalSquares(aa[0..$-1], bb);
	assert(aa[$-1]==7);
	for (цел i=0; i<bb.length; ++i) { assert(aa[2*i]==0x8000_0000+i*i); assert(aa[2*i+1]==0x8000_0000); }
}
}
...
проц многобайтПрямоугАккумД(бцел[] приёмник, бцел[] x)
{
    for (цел i = 0; i < x.length-3; ++i) {
        приёмник[i+x.length] = multibyteMulдобавь!('+')(
             приёмник[i+i+1 .. i+x.length], x[i+1..$], x[i], 0);
    }
    бдол c = cast(бдол)(x[$-3]) * x[$-2] + приёмник[$-5];
    приёмник[$-5] = cast(бцел)c;
    c >>= 32;
    c += cast(бдол)(x[$-3]) * x[$-1] + приёмник[$-4];
...
проц многобайтПрямоугАккумАсм(бцел[] приёмник, бцел[] ист)
{
    ...
    // EBP = carry2
    // EDI = конец of приёмник for this пароль through the loop. Index for outer loop.
    // ESI = конец of ист. never changes
    // [ESP] = M = ист[i] = множитель for this пароль through the loop.
    // приёмник.length is изменён преобр_в приёмник.ptr+приёмник.length
    version(D_PIC) {
        enum { zero = 0 }
    } else {
        // use p2 (загрузи unit) instead of the overworked p0 or p1 (ALU units)
    ...
    asm {
        ...
        push EBX;
        push EBP;
        push EAX;    // local переменная M= ист[i]
        mov EDI, [ESP + LASTPARAM + 4*3]; // приёмник.ptr
        mov EBX, [ESP + LASTPARAM + 4*0]; // ист.length
        mov ESI, [ESP + LASTPARAM + 4*1];  // ист.ptr

        lea ESI, [ESI + 4*EBX]; // ESI = конец of left
        добавь int ptr [ESP + LASTPARAM + 4*1], 4; // ист.ptr, used for getting M

        // local переменная [ESP + LASTPARAM + 4*2] = последний значение for EDI
        lea EDI, [EDI + 4*EBX]; // EDI = конец of приёмник for first пароль

        lea EAX, [EDI + 4*EBX-3*4]; // up в_ ист.length - 3
        mov [ESP + LASTPARAM + 4*2], EAX; // последний значение for EDI  = &приёмник[ист.length*2 -3]     		

        cmp EBX, 3;
        jz length_is_3;
        
        // We старт at ист[1], not ист[0].		    
        dec EBX;
...
outer_loop:        
        ...
        mov EBP, 0;
        mov ECX, 0; // ECX = ввод carry.
        dec [ESP + LASTPARAM + 4*0]; // Next время, the length will be shorter by 1.
        neg EBX;                // счёт UP в_ zero.

        mov EAX, [ESI + 4*EBX - 4*1]; // получи new M
        mov [ESP], EAX;                   // save new M
...
length_is_3:
        ...
        mul EAX, [ESI - 4*2];
        mov ECX, 0;
        добавь [EDI-2*4], EAX;  // ECX:приёмник[$-5] += x[$-3] * x[$-2]        
        adc ECX, EDX;
        ...
        mov ECX, 0;
        adc EDX, 0;
        // сейчас EDX: EAX = c + x[$-3] * x[$-1]
        добавь [EDI-1*4], EAX; // ECX:приёмник[$-4] += (EDX:EAX)
        adc ECX, EDX;  //  ECX holds приёмник[$-3], it acts as carry for the последний row
// do length==2 
        mov EAX, [ESI - 4*2];
        mul EAX, [ESI - 4*1];
        добавь ECX, EAX;
...
проц многобайтПлощадь(BigDigit[] результат, BigDigit [] x)
{
    ...
    // And добавь the diagonal elements
    результат[0] = 0;
...
цел clock() { asm { push EBX; xor EAX, EAX; cpuid; pop EBX; rdtsc; } }
...
{
    // The performance results at the top of this файл were obtained using
    // a Windows устройство driver в_ access the CPU performance counters.
    // The код below is less accurate but ещё wопрely usable.
    // The значение for division is quite inconsistent.
    for (цел i=0; i<X1.length; ++i) { X1[i]=i; Y1[i]=i; Z1[i]=i; }
    цел t, t0;    
    ...
    t0 = clock();
    multibyteдобавьSub!('+')(Z1[0..1000], X1[0..1000], Y1[0..1000], 0);
    t = clock();
    multibyteдобавьSub!('+')(Z1[0..2000], X1[0..2000], Y1[0..2000], 0);
    auto добавьtime = (clock() - t) - (t-t0);
    t0 = clock();
    многобайтУмнож(Z1[0..1000], X1[0..1000], 7, 0);
    t = clock();
    многобайтУмнож(Z1[0..2000], X1[0..2000], 7, 0);
    auto multime = (clock() - t) - (t - t0);
    multibyteMulдобавь!('+')(Z1[0..2000], X1[0..2000], 217, 0);
    t0 = clock();
    multibyteMulдобавь!('+')(Z1[0..1000], X1[0..1000], 217, 0);
    t = clock();
    multibyteMulдобавь!('+')(Z1[0..2000], X1[0..2000], 217, 0);
    auto mulдобавьtime = (clock() - t) - (t - t0);        
