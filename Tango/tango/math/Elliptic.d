/**
 * EllИПtic integrals.
 * The functions are named similarly в_ the names used in Mathematica. 
 *
...
реал ellipticF(реал phi, реал m )
{
    ...
    цел d, mod, sign, npio2;

    if( m == 0.0L )
        return phi;
    a = 1.0L - m;
    if( a == 0.0L ) {
        if ( fabs(phi) >= PI_2 )  return реал.infinity;
        return  лог(  тан( 0.5L*(PI_2 + phi) )  );
    }
    npio2 = cast(цел)пол( phi/PI_2 );
    if ( npio2 & 1 )
        npio2 += 1;
    if ( npio2 ) {
    ...
    while( fabs(c/a) > реал.epsilon ) {
        temp = b/a;
        phi = phi + атан(t*temp) + mod * PI;
        mod = cast(цел)((phi + PI_2)/PI);
...
/**
 *  Incomplete elliptic integral of the сукунда kind
 *
 * Approximates the integral
 *
 * E(phi | m) = $(INTEGRATE 0, phi) квкор( 1- m $(POWER син, 2) t) dt
 *
 * of amplitude phi and modulus m, using the arithmetic -
 * geometric mean algorithm.
 */
...
{
    реал a, b, c, e, temp, t, E;
    цел d, mod, npio2, sign;

    if ( m == 0.0L ) return phi;
    реал lphi = phi;
    npio2 = cast(цел)пол( lphi/PI_2 );
    if( npio2 & 1 )
        npio2 += 1;
    ...
    if ( fabs(t) > 10.0L ) {
        ...
        if( fabs(e) < 10.0L ){
            e = атан(e);
            temp = E + m * син( lphi ) * син( e ) - ellipticE( e, m );
            goto готово;
        }
    }
    c = квкор(m);
    ...
    while( fabs(c/a) > реал.epsilon ) {
        temp = b/a;
        lphi = lphi + атан(t*temp) + mod * PI;
        mod = cast(цел)((lphi + PI_2)/PI);
        t = t * ( 1.0L + temp )/( 1.0L - temp * t * t );
        c = 0.5L*( a - b );
        temp = квкор( a * b );
        a = 0.5L*( a + b );
 ...
 *     P(x)  -  лог x Q(x).
 *
 * The аргумент x is used rather than m so that the logarithmic
 ...
 *  0 <= x <= 1
 *
 * This is equivalent в_ ellipticF(PI_2, 1-x).
 *
 * K(0) = &pi;/2.
 */
...
const реал [] P = [
   ...
   0x1.c2c43245d445добавьap-13,    // 0.00021494216542320112406
...
/**
 *  Complete elliptic integral of the сукунда kind
 *
 * Approximates the integral
 *
 * E(m) = $(INTEGRATE 0, &pi;/2) квкор( 1- m $(POWER син, 2) t) dt
 *
 * where m = 1 - x, using the approximation
 *
 *      P(x)  -  x лог x Q(x).
 *
 * Though there are no singularities, the аргумент m1 is used
...
unittest {
    ...
    assert(отнравх(ellipticKComplete(1),PI_2)>= реал.mant_dig-1);
 ...
 * Approximates the integral
 *
 * PI(n; phi | m) = $(INTEGRATE t=0, phi) dt/((1 - n $(POWER син,2)t) * квкор( 1- m $(POWER син, 2) t))
 *
