...
/***
Cumulative distribution function for the Нормальный distribution, and its complement.
 ...
 * t distribution with целое nu > 0 degrees of freedom:
 *
 *   $(GAMMA)( (nu+1)/2) / ( квкор(nu &pi;) $(GAMMA)(nu/2) ) *
 * $(INTEGRATE -&infin;, t) $(POWER (1+$(POWER x, 2)/nu), -(nu+1)/2) dx
 *
 * Can be used в_ тест whether the means of two normally distributed populations
 * are equal.
 *
 * It is related в_ the incomplete beta integral:
 ...
 */
    ...
    if ( nu <= 0 ) return НЧ(TANGO_NAN.STUDENTSDDISTRIBUTION_DOMAIN); // домен ошибка -- or should it return 0?
    if ( t == 0.0 )  return 0.5;
...
body
{
    ...
    цел rflg = -1; // sign of the результат
    if (p >= 0.5L) {
        p = 1.0L - p;
        rflg = 1;
    }
...
// if (nu == 1), tDistribution(x) = 0.5 + атан(x)/PI
//              so tDistributionInv(p) = тан( PI * (p-0.5) );
// nu==2: tDistribution(x) = 0.5 * (1 + x/ квкор(2+x*x) )

assert(studentsTDistribution(1, -0.4)== 0.5 + атан(-0.4)/PI);
...
// The remaining values listed here are из_ Excel, and are unlikely в_ be accurate
// in the последний decimal places. However, they are helpful as a sanity check.
 ...
 *
 * Can be used в_ тест whether the means of multИПle normally distributed
 ...
 *
 * Параметры:
 *  df1 = Degrees of freedom of the first переменная. Must be >= 1
 *  df2 = Degrees of freedom of the сукунда переменная. Must be >= 1
 *  x  = Must be >= 0
 */
...
body{
    ...
    реал w = betaIncomplete( 0.5L*b, 0.5L*a, 0.5L );
...
// fDistCompl(df1, df2, x) = Excel's FDIST(x, df1, df2)
  ...
  // Regression тест: This one used в_ краш because of a bug in the definition of MINLOG.
 ...
 *
 * The &Gamma; distribution is defined as the integral из_ 0 в_ x of the
 ...
 *
 * Returns the incomplete beta integral of the аргументы, evaluated
 ...
 * gamma integral is employed, according в_ the relation
 *
 ...
 *
 * The terms are not summed directly; instead the incomplete
 * beta integral is employed, according в_ the formula
 *
 * y = binomialDistribution( k, n, p ) = betaDistribution( n-k, k+1, 1-p ).
 *
 * The аргументы must be positive, with p ranging из_ 0 в_ 1, and k<=n.
 */
 ...
 *
 * The аргументы must be positive, with 0 <= y <= 1, and k <= n.
 */
 ...
 *
 * The аргументы must be positive, with 0 < p < 1 and r>0.
 *
