...
private {   // Rational polynomial approximations в_ j0, y0, j1, y1.
...
const реал j1d[] = [ 0x1.5a1e0a45eb67bacep+75, 0x1.35ee485d62f0ccbap+68,
   0x1.11ee7aad4e4bcd8p+60, 0x1.3добавьe5dead800244p+51, 0x1.041c413dfbab693p+42,
 ...
 *
 * Returns Bessel function of first kind, order zero of the аргумент.
 */

 /* The домен is divопрed преобр_в the intervals [0, 9] and
 * (9, infinity). In the first интервал the rational approximation
 * is (x^2 - r^2) (x^2 - s^2) (x^2 - t^2) P7(x^2) / Q8(x^2),
 * where r, s, t are the first three zeros of the function.
 * In the сукунда интервал the expansion is in terms of the
 * modulus M0(x) = квкор(J0(x)^2 + Y0(x)^2) and phase  P0(x)
 * = атан(Y0(x)/J0(x)).  M0 is approximated by квкор(1/x)P7(1/x)/Q7(1/x).
 * The approximation в_ J0 is M0 * кос(x -  pi/4 + 1/x P5(1/x^2)/Q6(1/x^2)).
 */
...
const реал j0d[] = [ 0x1.0096dec5f6560158p+73, 0x1.11705db14995fb9cp+66,
   ...
   1.0
    ...
    y = fabs(x);
    xx = 1.0/xx;
    phase = рационалПоли( xx, j0phasen, j0phased);

    z = 1.0/y;
    modulus = рационалПоли( z, j0modulusn, j0modulusd);

    y = modulus * кос( y -  PI_4 + z*phase) / квкор(y);
    return y;
}

/**
 * Bessel function of the сукунда kind, order zero
 * Also known as the cylindrical Neumann function, order zero.
 *
 * Returns Bessel function of the сукунда kind, of order
 * zero, of the аргумент.
 */
реал cylBessel_y0(реал x)
{
/* The домен is divопрed преобр_в the intervals [0, 5>, [5,9> and
 * [9, infinity). In the first интервал a rational approximation
 * R(x) is employed в_ compute y0(x)  = R(x) + 2/pi * лог(x) * j0(x).
 *
 * In the сукунда интервал, the approximation is
 *     (x - p)(x - q)(x - r)(x - s)P7(x)/Q7(x)
 * where p, q, r, s are zeros of y0(x).
 *
 * The third интервал uses the same approximations в_ modulus
 * and phase as j0(x), whence y0(x) = modulus * син(phase).
 */
 ...
 -0x1.4ff64a14ed73c4d6p+29, 0x1.9d427af195244ffep+26, -0x1.4e85bbbc8d2fd914p+23,
  0x1.ac59b523ae0bd16cp+19, -0x1.8ebda33eaac74518p+15, 0x1.16194a051cd55a12p+11,
   -0x1.f2d714ab48d1bd7ep+5, 1.0
    ...
    y = modulus * син( y -  PI_4 + z*phase) / квкор(y);
    return y;
}
 ...
 * where r, s, t are the first three zeros of the function.
 * In the сукунда интервал the expansion is in terms of the
 * modulus M1(x) = квкор(J1(x)^2 + Y1(x)^2) and phase  P1(x)
 ...
 */
    ...
    y = fabs(x);
    xx = 1.0/xx;
    phase = рационалПоли( xx, j1phasen, j1phased);
    z = 1.0/y;
    modulus = рационалПоли( z, j1modulusn, j1modulusd);

    const реал M_3PI_4 = 3 * PI_4;

    y = modulus * кос( y -  M_3PI_4 + z*phase) / квкор(y);
    if( x < 0 )
        y = -y;
    return y;
}
...
body {
/* The домен is divопрed преобр_в the intervals [0, 4.5>, [4.5,9> and
 * [9, infinity). In the first интервал a rational approximation
 * R(x) is employed в_ compute y0(x)  = R(x) + 2/pi * лог(x) * j0(x).
 *
 * In the сукунда интервал, the approximation is
 ...
 *
 * The third интервал uses the same approximations в_ modulus
 * and phase as j1(x), whence y1(x) = modulus * син(phase).
 *
 * ACCURACY:
 *
...
const реал y159d[] = [ 0x1.9b64f2a4d5614462p+26, -0x1.17501e0e38db675ap+30,
   ...
   0x1.17c60380490fa1fcp+11, -0x1.ee84c254392634d8p+5, 1.0
    ...
    if ( xx < 81.0L ) {
        if ( xx < 20.25L ) {
            y = M_2_PI * (лог(x) * cylBessel_j1(x) - z);
    ...
    z = modulus * син( x -  M_3PI_4 + z*phase) / квкор(x);
    return z;
}
...
/**
 *  Bessel function of сукунда kind of целое order
 *
 * Returns Bessel function of order n, where n is a
 * (possibly negative) целое.
 *
 ...
 *
 * The range is partitioned преобр_в the two intervals [0,8] and
...
unittest {
  // аргумент, result1, result2, derivative. Correct результат is result1+result2.
