...
{
    return форматируйДатуВремя (вывод, датаВремя, форматируй, ФорматДатыВремени.дайЭкземпляр(службаФормата));
}
...
ткст форматируйДатуВремя (ткст вывод, Время датаВремя, ткст форматируй, ФорматДатыВремени dtf)
{
        ...
        **********************************************************************/
        ...
        ткст разверниИзвестныйФормат(ткст форматируй, ref Время датаВремя)
        {
                ткст f;
...
version (Full)
{
        ...
        **********************************************************************/
        ...
        ткст форматируйОсобо (ref Результат результат, Время датаВремя, ткст форматируй)
        {
                ...
                if (форматируй[0] is '%')
                    {
                    // specifiers for Всё стандарт форматируй strings и custom ones
...
else
{
        ...
        if (форматируй.length is 1) // It might be one of our shortcuts.
            форматируй = разверниИзвестныйФормат (форматируй, датаВремя);

        return форматируйОсобо (результат, датаВремя, форматируй);
}
...
ткст долВТкст (ткст буфер, дол значение, цел цифры, ткст negativeSign)
{
        if (цифры < 1)
            цифры = 1;
...
ткст долВГексТкст (ткст буфер, бдол значение, цел цифры, сим форматируй)
{
        if (цифры < 1)
            цифры = 1;
        ...
        switch (specifier)
               ...
               case 'R':
                    ...
                    if (число.шкала == ФЛАГ_БЕСК)
                        return число.знак ? nf.negativeInfinitySymbol
...
{
        ткст форматируй = число.знак ? negativeNumberFormats[nf.члоОтрицатОбразцов]
...
{
        ткст форматируй = число.знак ? negativeCurrencyFormats[nf.валютнОтрицатОбразец]
                  ...
                  цел[] размерыГруппы, ткст decimalSeparator, ткст groupSeparator)
{
        ...
        if (поз > 0)
           ...
           if (размерыГруппы.length != 0)
              {
              // Calculate whether we have enough цифры в_ форматируй.
              ...
              // форматируй the разделители, и then копируй back в_ старт
...
ткст вТкст (ref Число число, ref Результат результат, сим формат, цел length, ФорматЧисла nf)
{
        switch (формат)
               ...
               default:
                     return "{не_годится FP форматируй specifier '" ~ формат ~ "'}";
               }
        return результат.получи;
}
        ...
        **********************************************************************/
        ...
        private static Число opCall (дво значение, цел точность)
        {
                ...
                if (эксп == ЭКСП)
                   ...
                   {
                   // Get the цифры, decimal point и знак.
                   сим* chars = ecvt(значение, число.точность, число.шкала, число.знак);
                   if (*chars != '\0')
                      {
                      while (*chars != '\0')
                             *p++ = *chars++;
