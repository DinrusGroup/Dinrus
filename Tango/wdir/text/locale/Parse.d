      ...
      switch (c) {
        case 'd': // день
          if (счёт == 1 || счёт == 2)
            ...
            результат.день = разбериНесколько(s, i, cast(ткст[]) dtf.abbreviatedDayNames);
          else
            результат.день = разбериНесколько(s, i, cast(ткст[]) dtf.dayNames);
          if (результат.день == -1)
            return нет;
          break;
        case 'M': // месяц
        ...
        case 't': // время метка
          if (счёт == 1)
            результат.timeMark = разбериНесколько(s, i, cast(ткст[]) stringOf(dtf.amDesignator[0]), cast(ткст[]) stringOf(dtf.pmDesignator[0]));
          else
            результат.timeMark = разбериНесколько(s, i, cast(ткст[]) dtf.amDesignator, cast(ткст[]) dtf.pmDesignator);
          break;
        case 'z':
