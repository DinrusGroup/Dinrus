  ...
  бул doParse() {
    ...
    while (поз < образец.length && i < s.length) {
      ...
      switch (c) {
        ...
        case 't': // время метка
          if (счёт == 1)
            результат.timeMark = разбериНесколько(s, i, cast(ткст[]) stringOf(dtf.определительДоПолудня[0]), cast(ткст[]) stringOf(dtf.определительПослеПолудня[0]));
          else
            результат.timeMark = разбериНесколько(s, i, cast(ткст[]) dtf.определительДоПолудня, cast(ткст[]) dtf.определительПослеПолудня);
          break;
        case 'z':
