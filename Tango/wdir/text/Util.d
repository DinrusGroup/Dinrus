/*******************************************************************************
        ...
        подставь (источник, свер, замена)     // замени/удали совпадает
...
/******************************************************************************
        ...
        Счёт все экземпляры of свер внутри источник 
...
/******************************************************************************
        ...
        Parameter 'старт' дефолты в_ источник.length
...
private struct LineFruct(T)
{
        ...
        цел opApply (цел delegate (ref T[] строка) дг)
        {
                ...
                while ((поз = locate (ист, nl, метка)) < ист.length)
                      {
                      auto конец = поз;
...
private struct DelimFruct(T)
{
        ...
        цел opApply (цел delegate (ref T[] токен) дг)
        {
                ...
                if (установи.length is 1)
                    while ((поз = locate (ист, установи[0], метка)) < ист.length)
                          {
                          токен = ист [метка .. поз];
                          if ((возвр = дг (токен)) != 0)
                               return возвр;
