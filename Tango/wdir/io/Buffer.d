...
class Буфер : ИБуфер
{
        ...
        protected т_мера        протяженность;                 // предел of действителен контент
        protected т_мера        дименсия;              // maximum протяженность of контент
        protected бул          можноСжать = да;     // сожми обходчик контент?
                       ...
                       if (бввод)
                          ...
                          if (позиция && можноСжать)
                             ...
                             if (записываемый is 0 && расширь(0) is 0)
                                 ошибка ("Токен is too large в_ fit внутри буфер");
