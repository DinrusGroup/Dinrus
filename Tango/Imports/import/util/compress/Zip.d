...
//
    ...
    struct ДанныеФайлЗага
    {
        ...
        бкрат      длина_комментариев_файла;
        бкрат      начало_номера_диска;
        бкрат      внутренние_атрибуты_файла = 0;
        бцел        внешние_атрибуты_файла = 0;
        цел         относительное_смещение_локального_заголовка;

        debug(ZIP) проц dump();

        проц изЛок(ЛокалФайлЗаг.Данные данные);
    }
...
//
    ...
    align(1)
    struct КонецДанныхЗаписиКД
    {
        бкрат      номер_диска = 0;
        бкрат      диск_с_началом_центрального_каталога = 0;
        бкрат      записи_центрального_каталога_на_данном_диске;
        бкрат      записей_центрального_каталога_всего;
        бцел        размер_центрального_каталога;
        бцел        смещение_начала_кд_от_нач_диска;
        бкрат      длина_комментариев_файла;

        debug(ZIP) проц dump();
    }

struct КонецЗаписиКД
{
    const бцел  сигнатура = 0x06054b50;

    alias КонецДанныхЗаписиКД Данные;
    Данные данные;
...
private:
     ...
     */
    КонецЗаписиКД read_eocd_record();
...
private:
     ...
     * Запись.
     */
    проц patch_local_header(бцел цпи_32, бцел сжатый_размер,
            бцел разжатый_размер);
    /*
     * Generates and outputs a local файл заголовок из_ the given инфо block and
     * compression метод.  Note that the цпи_32, сжатый_размер and
