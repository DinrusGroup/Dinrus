...
//private import  rt.console: КОНСВВОД, КОНСВЫВОД, КОНСОШ;

private import  io.device.Device,
                io.stream.Buffered, dinrus, winapi;
...
/+
								...
								{
								case 0:
								вв.указатель = cast(Дескр)   global.КОНСВВОД;					

								break;
								case 1:
								вв.указатель = cast(Дескр)  global.КОНСВЫВОД; 

								break;
								case 2:
								вв.указатель = cast(Дескр) global.КОНСОШ; 
								break;
								}
							...
							debug	эхо("redirected!");
                                ...
                                else
								 {
                                   ...
                                   for (шим* p=вывод.ptr, конец=вывод.ptr+i; p < конец; p+=i)
                                       ...
                                       if (! WriteConsoleW (cast(HANDLE) вв.указатель, p, длин, &i, пусто))
                                             ошибка();
                                       }
                                   return ист.length;
                                   }
...
/******************************************************************************
        ...
        Глобальные переменные Консоль IO
