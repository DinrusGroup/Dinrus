...
abstract class БазаДанных {
	...
	final Инструкция подготовь (ткст эскюэл) {
		return new Инструкция(this, эскюэл);
	}
	...
	final protected ткст[ткст] дайКлслова (ткст текст) {
		ткст[ткст] клслова;
			...
			foreach (ткст группа; разбей(текст, ";")) {
				if (группа == "") {
					continue;
				}
				ткст[] значч = разбей(группа, "=");
				клслова[значч[0]] = значч[1];
...
private class ТестБД : БазаДанных {
	...
	Результат запрос (ткст эскюэл) {return пусто;}
	deprecated цел дайКодОшибки () {return 0;}
	deprecated ткст дайСообОшибки () {return "";}
}
debug(DB)
{
