...
final class Ряд {
	 ...
	 */
	ткст дай (ткст имя) {
		return значПолей[дайИндексПоля(имя)];
	}
	...
	цел дайИндексПоля (ткст имя) {
		for (цел инд = 0; инд < именаПолей.length; инд++) {
			if (именаПолей[инд] == имя) {
				return инд;
			}
		}
		throw new ИсклДБИ("Имя '" ~ имя ~ "' не является действительным индексом.");
	}
	...
	проц добавьПоле (ткст имя, ткст значение, ткст объявл, цел тип) {
		именаПолей ~= имя;
		значПолей ~= значение.dup;
		объявлПолей ~= объявл.dup;
		типыПолей ~= тип;
	}
