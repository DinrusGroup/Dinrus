...
class ПгБД : БазаДанных {
	...
	override проц подключись (ткст парамы, ткст имя_пользователя = пусто, ткст пароль = пусто) {
		...
		подключение = dbi.pg.imp.PQconnectdb(toCString(парамы));
	...
	override ткст искейп (ткст текст) {
		...
		// It's okay to send текст.ptr here because текст doesnt need to be 0-term
		результат.length = PQescapeStringConn(подключение, результат.ptr, текст.ptr, текст.length, пусто);;
