...
Type *Type::merge()
{
    ...
    if (!deco)
    {
        ...
        OutBuffer buf;
        toDecoBuffer(&buf);
        StringValue *sv = stringtable.update((const wchar_t *)buf.data, buf.offset);
        if (sv->ptrvalue)
        {   t = (Type *) sv->ptrvalue;
            assert(t->deco);
        ...
        {
            sv->ptrvalue = this;
            (const wchar_t*)deco = (const wchar_t*)sv->lstring.string;
...
Type *Type::merge2()
{
    ...
    StringValue *sv = stringtable.lookup((const wchar_t *)t->deco, strlen(t->deco));
    if (sv && sv->ptrvalue)
    {   t = (Type *) sv->ptrvalue;
        assert(t->deco);
    }
