    ...
    while (1)
    {
        ...
        switch (*p)
        {
            ...
            {   unsigned char c;
                ...
                StringValue *sv = stringtable.update((wchar_t const *)t->ptr, p - t->ptr);
                Identifier *id = (Identifier *) sv->ptrvalue;
                if (!id)
                {   id = new Identifier( (const char *)sv->lstring.string,TOKidentifier);
                    sv->ptrvalue = id;
                }
                t->ident = id;
...
{
    size_t len = strlen(s);
    StringValue *sv = stringtable.update((const wchar_t *)s, len);
    Identifier *id = (Identifier *) sv->ptrvalue;
    if (!id)
    {
        id = new Identifier((const char *)sv->lstring.string, TOKidentifier);
...
{   StringValue *sv;
    ...
    {   const char *s;
        ...
        s = keywords[u].name;
        v = keywords[u].value;
        sv = stringtable.insert((const wchar_t *)s, strlen(s));
